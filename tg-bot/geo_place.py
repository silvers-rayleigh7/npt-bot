...
# Наши сюжеты считаем «рядом», если ближе этого порога. Иначе рассказываем про саму местность.
NEAR_KM = 15.0
...
def build_geo_prompt(lat: float, lon: float, content_dir: str, near_km: float = NEAR_KM) -> str:
    ...
    if close:
        ...
        return (
            ...
            f"Рядом наши научные точки: {items}. Покажи списком с расстояниями, "
            f"дай выбрать номер и расскажи про выбранную. Коротко и живо."
        )
