# ✅ Входной стенд - WP001-1, WP001-2  

# **Не надо**

# **WP001-1, WP001-2 - входные стенды**

## **Идентификация**

|  |  |
| :-: | :-: |
| \*\*Поле\*\* | \*\*Значение\*\* |
| Код объекта | WP001-1, WP001-2 |
| Название | Входной стенд научной тропы |
| Координаты | https://yandex.ru/maps/-/CPe7JU0B ; https://yandex.ru/maps/-/CPe7JNKP |
| Тип станции | Информационный, навигационный + методологический гид |

  

## **Конструкция**

Два двусторонних информационных стенда на двух стальных стойках каждый, забетонированных в грунт. Панели — HPL с УФ-стойкой печатью либо нержавеющая сталь с лазерной гравировкой. Размер каждой панели — около 120×80 см. 

Два стенда размещаются в двух точках входа на тропу, обеспечивая заход с разных направлений.

## **Смысловое назначение**

Входной стенд решает две задачи одновременно:

**Первая — навигационная.** Дать карту маршрута, время прохождения, правила поведения, обозначить основную петлю 4 км и большую петлю 12 км.

**Вторая — методологическая.** Настроить посетителя перед остальными стендами тропы.

  
  

## **Тексты стендов**

### **Сторона 1 — Навигация**

**Заголовок:** Научная тропа Иннополиса

**Основной текст:**

Это тропа построена, как серия остановок, где вы наблюдаете за природой, задаете вопросы, смотрите по сторонам и находите на них ответы.

Перед вами два маршрута длиной 4 и 12 км. На каждой остановке вас ждет маленькое наблюдение: чем-то удивиться, что-то сравнить, что-то потрогать.

Тропа устроена как книга, которую, вы можете прочесть просто прогулявшись. Цель не в том, чтобы запомнить факты, а в том, чтобы научиться по новому смотреть на мир: видеть в привычном - непривычное, а в странном - закономерность.

Сверьтесь с картой, выберите маршрут и двигайтесь по указателям. На каждом стенде задержитесь хотя бы на пару минут и сделайте то, что написано: посмотрите, сравните, шагните. 

**Визуальный блок:** карта маршрута с пиктограммами крупных станций и малых объектов; основная петля 4 км — сплошной линией; большая петля 12 км — пунктиром; обозначение точки «вы здесь».

**Правила (нижний блок):**

Не мусорить. Не повреждать объекты и стенды. Не ломать деревья и ветки. 

### **Сторона 2 — Методологический экспонат**

**Заголовок:** Что делает прогулку научной?

**Основной текст:**

Осмотритесь прямо сейчас. Что вас окружает? Светит ли солнце? Есть ли на небе облака? Поют ли птицы?

У всего есть причина. И чаще - одна причина стоит за многими событиями и наблюдениями. Задача науки не в том, чтобы собрать миллион фактов воедино, а в том, чтобы за миллионом следствий увидеть десяток причин и научить предсказывать хоть одно событие. Тогда знание само укладывается в голове.

На этой тропе каждый стенд устроен по принципу: наблюдение → вопрос → причина → применение.

Прежде чем идти дальше — остановитесь и найдите вокруг себя одно явление. Что-то, что вы раньше не замечали или принимали как должное. Запомните это и проверьте по дороге: не ответит ли на него один из следующих стендов?

**Нижний блок (подсказка к QR):**

Хотите узнать больше? — сканируйте QR-код\!.

### **Текст по QR** 

**Заголовок:** Почему наука — это не просто факты, а способ узнавать мир?

**Основной текст:**

В век, когда любой факт можно найти за десять секунд в телефоне, простое накопление фактов уже не является главной ценностью — хотя широкая эрудиция по-прежнему остаётся важной основой мышления. Но ещё важнее становится другое: умение за разными явлениями видеть общий принцип и понимать внутренние связи между ними. 

Физик Ричард Фейнман говорил: если бы всё человеческое знание пришлось уложить в одно предложение, он выбрал бы мысль о том, что всё состоит из атомов, движущихся и притягивающихся друг к другу. Из одной этой идеи выводится очень многое — от того, почему вода замерзает, до того, почему дерево стоит. Наука вообще устроена удивительным образом: фундаментальных принципов оказывается ровно столько, сколько необходимо для объяснения огромного разнообразия явлений, — и нередко они оказываются поразительно простыми по формулировке, хотя их следствия могут быть чрезвычайно сложными. 

Тропа устроена по той же логике - каждый её стенд разбирает один принцип, за которым стоит множество фактов.

Научный метод — это 4 простых шага, которые делает любой любознательный человек: заметить явление, предположить причину, придумать, как проверить, сравнить   с тем, что знали раньше. Стенды на этой тропе основаны на таком же принципе. 

В конце прогулки попробуйте вспомнить: какой из ваших «странных» вопросов так и не получил ответа на стендах? Это и есть самый ценный вопрос — тот, с которым можно дальше исследовать этот мир. Наука тем и интересна, что за каждым новым открытием начинается неизвестное.

**Комментарий**

Было бы интересно, если по окончанию прогулки, если вопрос не был отвечен, они бы задавали этот вопрос в форму и какие-нибудь крутые эксперты отвечали им на этот вопрос или учитывали его при обновлении выставки или постройки новой   

# ✅ Маятниковая роща - WP004  

## **WP004 — «Маятниковая роща»**

**Геопривязка:**[ https://yandex.ru/maps/-/CPe7F6pK](https://yandex.ru/maps/-/CPe7F6pK) **Локация:** парковая аллея с молодыми тонкими деревьями (минимум одно — в радиусе 50 м от установки, в линии взгляда от стенда; роль зафиксирована в тексте Б). **Тип станции:** средняя, двусторонний стенд при тактильной установке. **Категория опыта:** парное сравнение, телесный запуск, количественное измерение. **Круглогодичность:** да.

### **1. Замысел**

Главная мысль стенда — что формулу для периода маятника можно угадать, не выписывая уравнения движения. Достаточно посмотреть на размерности входящих в задачу величин: единственная комбинация длины нити, массы груза и ускорения свободного падения с размерностью секунды — это √(L/g), и масса в неё не входит. Этот приём — размерностный анализ — работает в широком классе задач: тем же ходом из высоты водопада оценивается скорость струи у его подножия, из температуры газа — характерная скорость молекул. Стенд показывает простой случай универсального метода, на котором держится значительная часть физики. Чтобы предсказание размерностного анализа можно было проверить руками, на балке висят четыре маятника, объединённые в две пары. Слева — две нити одинаковой длины с шарами разной массы: ожидание подсказывает, что тяжёлый шар должен качаться медленнее, но периоды совпадают, и масса в формулу действительно не входит. Справа — два шара одной массы на нитях разной длины (25 сантиметров и 1 метр): длина различается вчетверо, период — вдвое, и это проверка предсказанной размерностным анализом корневой зависимости. Те же две длины позволяют провести количественный опыт: засекая периоды на двух нитях, посетитель восстанавливает ускорение свободного падения. Возле молодого дерева в зоне видимости стенда тот же закон работает на собственной частоте ствола.

### **2. Поперечное сечение**

**ОБЩАЯ СТРУКТУРА:** возле положения устойчивого равновесия возвращающая сила пропорциональна отклонению; решение этого уравнения — гармонические колебания; период определяется отношением инерции к жёсткости системы; форма закона диктуется размерностями входящих в задачу величин и угадывается до того, как уравнение движения выписано.

**ЯВЛЕНИЯ:**

1.  Физика бытового масштаба — математический маятник: T = 2π√(L/g), масса в ответ не входит.
2.  Инженерия и акустика — пружинный осциллятор и струна музыкального инструмента: T = 2π√(m/k); собственная частота струны фиксирована натяжением и длиной.
3.  Электротехника — LC-контур: индуктивность в роли инерции, ёмкость в роли податливости, T = 2π√(LC); та же формула с другим физическим содержанием.
4.  Физика твёрдого тела — малые колебания атомов в кристаллической решётке: возвращающая сила линейна по смещению, частота определяется силовой константой; на этом приближении стоят теории теплоёмкости Эйнштейна и Дебая.
5.  Биология и гидродинамика — собственные частоты тонких деревьев и стеблей, скорость струи у основания водопада (v \~ √(gh) по тому же размерностному ходу): размерностный анализ работает не только для маятника, но для всякой задачи, где входов мало и их размерности диктуют единственную возможную комбинацию.

**НЕОЧЕВИДНАЯ СВЯЗЬ:** LC-контур и математический маятник описываются одной формулой, хотя в первом нет ни массы, ни нити, ни тяжести. Электрический заряд колеблется в катушке и конденсаторе по тому же закону, что груз на нити в поле тяготения. Одна математика — много механизмов; форма закона диктуется не природой явления, а структурой малых отклонений от положения равновесия.

### **3. Конструкция**

#### **3.1 Общая схема**

Горизонтальная балка на двух стойках; четыре маятника на нержавеющих тросах подвешены к балке в двух парах. Наклонная информационная панель с двусторонней гравировкой на одностоечном основании рядом со стендом. QR-плашка — на отдельной металлической поверхности у основания панели.

#### **3.2 Материалы**

  - Балка: стальная квадратная труба 80×80 мм, длина 2 м.
  - Стойки (две): квадратная труба 100×100 мм, забетонированы.
  - Маятники: нержавеющий трос Ø2 мм; шары стальные сферические, окрашенные порошковой эмалью.
  - Покрытие всех металлических частей — порошковая полиэфирная эмаль (антивандальная).
  - Информационная панель: HPL или металл с гравировкой, наклон 30°.

#### **3.3 Габариты и монтаж**

  - Высота балки над поверхностью: 2.0 м.
  - Пара А (масса): нити по 1.0 м, шары 0.2 кг и 0.5 кг — висят на высоте \~1.0 м, в зоне руки взрослого и подростка.
  - Пара Б (длина): нити 0.25 м и 1.0 м, шары по 0.5 кг — висят на высотах \~1.75 м и \~1.0 м соответственно; для запуска короткого маятника взрослому удобно, ребёнку достижимо в прыжке или с подсказкой.
  - Заглубление стоек: 0.8 м, бетонная подушка по локальному регламенту.
  - Массы шаров подписаны на самих шарах (гравировка по эмали).

#### **3.4 Безопасность**

Все шары без острых кромок. Длина свободного хода — не более 0.5 м в каждую сторону от вертикали; геометрия подвеса исключает столкновение шаров между собой и с людьми вне зоны запуска. Зона безопасности вокруг установки 1.5 м, обозначена покрытием или бордюром. При сильном ветре (свыше 15 м/с) маятники самоустанавливаются в вертикаль за несколько колебаний за счёт демпфирования в подвесе — внешнее возбуждение не накапливается.

### **4. Принцип работы**

1.  Посетитель подходит к установке со стороны левой пары (опыт «масса»), отклоняет оба шара одной рукой на одинаковую величину и одновременно отпускает. Видит: оба шара идут в одном ритме, проходят верхние точки одновременно, расходятся и сходятся синхронно. Масса на период не повлияла.
2.  Переходит к правой паре (опыт «длина»), тем же приёмом отпускает оба маятника вместе. Видит: за то время, пока длинный делает одно полное качание, короткий успевает два. Длина различается вчетверо, период — вдвое.
3.  Читает на панели уровни А и Б, знакомится с размерностным выводом формулы T = 2π√(L/g): видит её уже как ожидаемый результат, а не как готовую формулу из ниоткуда.
4.  По инструкции на панели засекает время десяти полных колебаний поочерёдно для двух маятников правой пары (длины 0.25 м и 1.00 м), делит каждое значение на десять — получает два значения периода.
5.  Откладывает на листе или мысленно две точки в координатах (√L, T) и проводит через них прямую (которая должна пройти через начало координат). По наклону прямой k восстанавливает g = (2π/k)². Подробное обоснование линейной регрессии и оценка погрешности — за QR-кодом.
6.  Возвращается взглядом к молодому дереву в зоне видимости стенда. Толкает ствол или ветку, замечает её собственный ритм; тот же закон работает и там.

### **5. Тексты**

### **5.1 Стенд, уровень А, Б**

Попробуйте одновременно раскачать маятники. Как вы думаете, какие будут качаться быстрее?

На первый взгляд кажется, что тяжёлый шар должен двигаться медленнее: ведь его труднее разогнать. Но опыт показывает другое— масса почти не влияет на ритм качания маятника.

Перед вами две пары маятников.

### **Опыт 1. Влияет ли масса?**

У левой пары длины нитей одинаковые, но один шар тяжелее другого примерно в 2,5 раза.

Отведите оба маятника на одинаковый угол и отпустите одновременно.в

Что происходит? Несмотря на разную массу, маятники качаются почти синхронно.

Почему так?

Тяжёлый шар действительно труднее разогнать. Но сила тяжести, возвращающая его обратно, тоже больше. Эти два эффекта компенсируют друг друга, поэтому ритм качания остаётся тем же самым.

### **Опыт 2. Влияет ли длина?**

Теперь посмотрите на правую пару. Массы шаров одинаковые, но длины нитей различаются в четыре раза: 25 сантиметров и 1 метр.

Снова отпустите маятники одновременно.

Теперь разница сразу заметна: короткий маятник качается быстро, а длинный — медленно.

Интересно, что если длина увеличивается в 4 раза, период увеличивается только в 2 раза. Маятник подчиняется закону: время одного качания растёт как квадратный корень из длины.

### **Почему маятник вообще качается?**

#### Когда шар отклоняют в сторону, сила тяжести тянет его обратно вниз. Но по инерции он проходит нижнюю точку и движется в противоположную сторону.

#### Маятник постоянно обменивает одну форму энергии на другую. У качающегося маятника есть энергия двух видов. **Кинетическая** — за счёт движения; чем быстрее летит шар, тем её больше. **Потенциальная** — за счёт высоты; в верхней точке размаха она максимальна, в нижней равна нулю.

#### Эти два вида энергии непрерывно переходят друг в друга. Сумма их остаётся постоянной — это закон сохранения энергии.

### **Где мы наблюдаем похожее?**

Маятник — простейший пример системы, которая колеблется. Качается люстра под потолком, дрожит ветка после порыва ветра, гудит струна гитары, колеблется ток в радиоприёмнике, дрожат атомы в нагретом куске металла. На первый взгляд между этими явлениями нет ничего общего. На самом деле все они подчиняются одному и тому же закону, и в основе лежит формула, очень похожая на формулу маятника.

### **Интересный факт**

#### Именно благодаря удивительно постоянному ритму маятников люди впервые научились создавать очень точные часы.

#### В XVII веке Галилей заметил, что большие и маленькие качания происходят почти за одинаковое время. Позже это свойство использовали в маятниковых часах, которые столетиями были самыми точными приборами измерения времени на Земле.

#### 

## **QR — уровень В**

Маятник — один из самых важных объектов в физике. Именно на нём впервые стало понятно, что очень разные системы могут подчиняться одному и тому же закону колебаний.

### **Формула маятника**

Для небольших отклонений период колебаний маятника задаётся формулой

T = 2π · √(L/g),

где:

  - T — период одного полного качания,
  - L — длина нити,
  - g ≈ 9.81 м/с² — ускорение свободного падения.

Из формулы сразу видно:

  - масса груза не влияет на период;
  - период растёт как корень из длины.

Именно это вы наблюдаете на стенде:

  - тяжёлые и лёгкие шары качаются одинаково;
  - маятник в 4 раза длиннее качается только в 2 раза медленнее.

### **Почему длина влияет именно так**

Маятник колеблется из-за двух эффектов:

  - **инерция** — масса груза, которая мешает ему быстро менять скорость;
  - **сила тяжести**, тянущая его обратно к положению равновесия.

Чем длиннее нить, тем меньше момент силы тяжести относительно точки подвеса при том же угле отклонения. То есть при той же массе и том же отклонении сила, возвращающая маятник в равновесие, действует слабее — и маятник возвращается медленнее.

Сама зависимость периода от длины не прямая: при увеличении длины в четыре раза период растёт только в два. Период маятника пропорционален корню квадратному из длины нити.

### **Маятник как способ измерить g**

Формулу T = 2π · √(L/g) можно использовать в обе стороны. Зная длину и измерив период, получаем

g = 4π² · L / T².

Например, для маятника длиной 1 метр период получается примерно 2 секунды. Подставив эти значения, получаем g ≈ 9.8 м/с² — именно то значение, которое используется в физике. По сути, перед вами прибор для измерения силы земного притяжения.

Одно измерение даёт оценку, но не говорит, насколько ей можно доверять. Поэтому в школьных и студенческих лабораториях измерения повторяют на нескольких длинах нити и обрабатывают их сразу все.

**Как обрабатывают.** Для каждой длины L засекают период T и наносят на график точку с координатами (√L, T). Если формула верна, точки должны лечь на прямую, проходящую через начало координат, с определённым наклоном. По этому наклону восстанавливают g.

Точки никогда не лежат на прямой идеально: каждое измерение содержит небольшую ошибку. Поэтому через них проводят прямую, которая «лучше всего соответствует» всем данным. Понять, что значит «лучше всего», помогает следующий приём.

**Метод наименьших квадратов.** Возьмём любую прямую-кандидат. Для каждой экспериментальной точки посчитаем, насколько она отклонилась от этой прямой по вертикали, и возведём это отклонение в квадрат. Сложим все квадраты. У одной из возможных прямых эта сумма окажется минимальной — её и принимают за лучшую. Для прямой y = k · x, проходящей через начало координат, оптимальный наклон даётся формулой

k = Σ(x\_i · y\_i) / Σ(x\_i²),

а отсюда g = (2π/k)².

Этот же метод первым стал применять Галилей в XVII веке, измеряя ускорение свободного падения по падению шаров с Пизанской башни: время падения t пропорционально √h, точки в координатах (√h, t) ложились на прямую, и по её наклону получалось g. Один и тот же приём работает в самых разных экспериментах.

На стенде доступны только две длины нити: через две точки прямую можно провести единственным способом, и метод наименьших квадратов здесь упрощается до простой арифметики. Полноценная регрессия начинается с пяти-десяти точек.

### **Гармонический осциллятор и общий закон**

Колебания маятника — частный случай общего закона колебаний в физике. Возьмём любой объект, спокойно находящийся в положении равновесия: шарик на дне ямки, груз на ненатянутой пружине, атом, удерживаемый в кристалле соседями. Слегка выведем его из этого положения. На него начнёт действовать возвращающая сила — тем большая, чем дальше он отклонился. Если этого объекта отпустить, он начнёт колебаться, и при небольших отклонениях движение будет похожим на качание маятника.

Когда возвращающая сила пропорциональна смещению, колебания называются **гармоническими**, а такая система — гармоническим осциллятором. Маятник при малых углах — гармонический осциллятор. Период гармонических колебаний задаётся формулой

T = 2π · √(инерция / жёсткость).

«Инерция» — то, что мешает объекту быстро менять скорость. «Жёсткость» — коэффициент, с которым возвращающая сила растёт при отклонении.

У маятника при малом угле возвращающая сила линейна по смещению, и формула даёт T = 2π · √(L/g). У груза на пружине возвращающая сила Гука линейна по смещению по определению пружины, и получается T = 2π · √(m/k). В электрической цепи с катушкой и конденсатором (LC-контуре) роль инерции играет индуктивность L (катушка сопротивляется изменению тока), роль жёсткости — обратная ёмкость 1/C, и период колебаний тока равен T = 2π · √(LC). Все эти системы устроены по-разному, но в окрестности равновесия описываются одной и той же формулой.

Это, пожалуй, главная универсальная модель в физике: с неё начинается и квантовая механика, и теория звука, и физика твёрдого тела.

В кристаллической решётке атомы тоже колеблются возле своих положений равновесия. При очень малых отклонениях каждое такое колебание — почти гармоническое. Но при больших амплитудах или при учёте взаимодействия многих атомов одновременно картина становится сложнее, и описание уже выходит за рамки одного гармонического осциллятора.

### **Связанные маятники и наблюдение Гюйгенса**

Если повесить два маятника рядом — близко, но не вплотную, — между ними возникает слабая связь: каждый чуть-чуть тянет другой через общую опору. У такой системы есть два возможных режима колебаний:

  - **синфазный** — оба маятника качаются в одну сторону одновременно;
  - **противофазный** — пока один летит вправо, другой летит влево.

Любое начальное движение раскладывается на сумму этих двух режимов. И вот что интересно: даже если сначала маятники качаются как попало, со временем они начинают двигаться согласованно. Дело в том, что в синфазном режиме маятники одновременно дёргают опору в одну сторону и тратят на её раскачивание энергию; в противофазном режиме рывки гасят друг друга, опора почти не реагирует, и колебания почти не теряют энергии. Синфазный режим затухает быстрее, и через несколько минут остаётся только противофазный: маятники качаются согласованно, в противоположных направлениях.

Это явление впервые заметил Христиан Гюйгенс в 1665 году. Он лежал больной в постели и наблюдал за двумя маятниковыми часами, висевшими на общей балке. Через какое-то время часы начали идти удивительно синхронно — он назвал это «странной симпатией часов» и описал в письме отцу. Современное количественное воспроизведение опыта с метрономами на подвижной платформе сделали Беннетт, Шац, Роквуд и Виссенфельд в 2002 году.

### **Синхронизация в живой природе**

То, что наблюдал Гюйгенс с двумя часами, — простейший пример очень общего явления, **синхронизации**. Когда есть несколько колеблющихся объектов с разными собственными ритмами и между ними есть пусть слабое, но взаимодействие, при достаточной его силе все они начинают подстраиваться друг под друга и идут в едином ритме.

В физике классическое описание синхронизации даёт **модель Курамото** (1975). В ней рассматривают N абстрактных осцилляторов с разными собственными частотами и слабой взаимной связью. Каждый осциллятор реагирует на средний ритм соседей: если все вокруг идут быстрее — он чуть ускоряется, если медленнее — замедляется. Главный результат модели в том, что когда сила связи между осцилляторами превышает определённый порог, в системе появляется коллективное поведение: часть популяции захватывается общим ритмом, и доля синхронизованных осцилляторов плавно растёт с увеличением связи. Сам переход не выглядит как мгновенный щелчок — это постепенное «втягивание» осцилляторов в общий ритм.

Модель Курамото описывает абстрактные осцилляторы, но тот же сценарий — «много слабо взаимодействующих ритмов, при достаточной связи постепенно подстраивающихся друг под друга» — встречается в очень разных системах живой природы:

  - **Светлячки рода Pteroptyx** в болотах Юго-Восточной Азии. Тысячи самцов собираются на одном дереве и сначала вспыхивают вразнобой. Каждый светлячок чуть-чуть подстраивает свой ритм мигания под тех соседей, которых видит. Через несколько минут вся колония вспыхивает и гаснет как одна гигантская лампа.
  - **Синусовый узел сердца** — участок мышечной ткани, который задаёт ритм сокращений. Клетки этого узла — крошечные биологические осцилляторы, каждый со своей собственной частотой
  - **Нейроны гиппокампа.** В момент воспоминания группы нейронов начинают активироваться согласованно, в общем ритме; 

  
  
  
  
  

### **6. Карта прохождения фильтров манифеста**

|  |  |  |  |
| :-: | :-: | :-: | :-: |
| \*\*\\\#\*\* | \*\*Фильтр\*\* | \*\*Прошёл\*\* | \*\*Замечание\*\* |
| 1 | Честность конструкции | да | Сталь, бетон, антивандальная окраска; четыре маятника без подвижных узлов |
| 2 | Воспроизводимость | да | Балка, две стойки, четыре троса, четыре шара — типовой набор |
| 3 | Поперечное сечение | да | Гармонический осциллятор и линейный режим; ось — размерностный анализ |
| 4 | Тактильность | да | Запуск руками, парное сравнение, измерение времени, оценка g по двум точкам |
| 5 | Метод вместо энциклопедии | да, усилен | Размерностный анализ — главный методологический урок; в Б показан перенос на водопад и скорость молекул |
| 6 | Локальный ландшафт | да (при выполнении правки) | Привязка к молодому дереву или ветке в зоне видимости стенда; если поблизости есть водопад или ручей с перепадом — пример с √(gh) дополнительно цепляется за конкретное место |
| 7 | Уровни А/Б/В | да | На А и Б нет упоминания угла; разложение sin θ ≈ θ и нелинейная поправка полностью в В |
| 8 | Тиражируемость | да | Конструктивное ядро универсально, локальный смысл — только в текстах |

### **7. Что требует уточнения до производства**

1.  Выбрать ландшафтный пример осциллятора в радиусе 50 м от стенда. Подходит молодое дерево (берёза, рябина) высотой 3–5 м с тонким стволом; альтернатива — куст ивняка у воды, тростник в прибрежной зоне. Зафиксировать в проекте, отразить в финальной строке текста уровня Б. Если в зоне видимости есть водопад, ручей с перепадом или сток с заметной высотой — это второй ландшафтный пример (для размерностного приёма с √(gh)), его тоже стоит упомянуть в Б.
2.  Подтвердить пару длин 0.25 м / 1.00 м у пары Б. Если короткий шар на высоте 1.75 м оказывается недостижим для ребёнка 8 лет, перейти на пропорциональное масштабирование: 0.4 м / 1.6 м с балкой высотой 2.2 м (соотношение длин четыре, периоды 1.27 с и 2.54 с).
3.  Натурный тест периодов на собранном узле: длинные маятники пары А должны показать 2.00 ± 0.05 с (одинаково для обоих шаров в пределах разброса); короткий маятник пары Б — 1.00 ± 0.03 с. При значительных расхождениях проверить вертикальность стоек и крепления тросов.
4.  Согласовать материал шаров (стальные литые, шарикоподшипниковые, с эмалью): окончательный выбор по бюджету и весовому коридору. Веса 0.2 кг и 0.5 кг в паре «масса» выдержать с точностью ±5 г.
5.  На балку у каждой точки подвеса нанести длину нити (гравировка по эмали) — посетителю эта цифра нужна для построения графика.
6.  Разместить QR-плашку на отдельной металлической поверхности у основания информационной панели — антивандальный режим и читаемость с телефона на высоте 1.2–1.4 м.
7.  Учесть в регламенте обслуживания ежегодную проверку нержавеющих тросов и креплений; окраску шаров — раз в три года.

  
  
  

# ✅ Услышь расстояние - WP003  

### **DONE**

### **Экспонат WP003 — «Услышь расстояние»**

**Геопривязка:**[ https://yandex.ru/maps/-/CPe7F8JN](https://yandex.ru/maps/-/CPe7F8JN) **Локация:** край склона оврага, окраина Введенской слободы **Тип станции:** крупная станция — акустическая «точка эха» с информационной панелью **Категория опыта:** телесный эксперимент (хлопок → слух → измерение) **Круглогодичность:** да

#### **1. Замысел**

Посетитель на открытой площадке у оврага хлопает в ладоши — и через долю секунды эхо «хлопает» в ответ. Это не иллюзия: к нему вернулся его же собственный хлопок, отражённый плотным грунтом склона. Засекая задержку, посетитель сам измеряет расстояние до склона и до леса — и обнаруживает, что тот же принцип работает у летучих мышей, дельфинов, подводных сонаров и медицинского УЗИ. Везде одна и та же схема: послать волну, поймать отражение, измерить время, узнать расстояние.

Экспонат показывает универсальный принцип отражения волны от границы двух сред — тот же, что у света на зеркале и у морской волны у берега.

#### **2. Конструкция**

**Общая схема**

Статичный объект без движущихся и настраиваемых узлов. Три элемента:

Помост-платформа («точка слушателя») — 2×2 м, деревянный настил на металлическом каркасе, высота над землёй 100–150 мм. Покрытие нескользящее. Точка установки помоста подбирается эмпирически при разметке — по максимальной чёткости эха от тестового хлопка и минимуму паразитных отражений от застройки. Рядом с помостом — вертикальный стенд-экран из звукопоглощающего материала (пенопласт и т. п.).

Столбик-указатель рядом с помостом — металлическая труба высотой 1,2 м, заглубление 0,6–0,8 м, бетонирование. На столбике — пиктограмма хлопка и стрелка ориентации на склон, когда стоишь за поглощающим экраном, и когда выходишь из-за него.

Информационная панель на отдельной стойке — HPL или металл с гравировкой, высота центра панели \~1,4 м.

**При необходимости — направленный конус-раструб**

По результатам натурного акустического теста может быть установлен стационарный (без поворотных узлов) металлический конус-раструб для усиления направленного хлопка:

  - диаметр раструба 50–80 см, длина 40–60 см (уточнить);
  - ось жёстко направлена на склон, высота оси 1,3 м;
  - опора — стальная труба, забетонирована на 0,8–1,0 м;
  - у основания — бетонный приступок \~20 см для доступа детей.

Решение об установке конуса принимается после теста: если хлопок без раструба даёт достаточно чёткое эхо — конус избыточен.

**Материалы**

Помост: лиственница, термодревесина или ДПК на металлическом каркасе. Столбик и стойка панели: металлическая труба с порошковой окраской либо кортеновская сталь. Панель: HPL или металл с лазерной гравировкой. Конус-раструб (при наличии): нержавеющая сталь или кортен, сварной шов по образующей, кромки завальцованы или обварены. Крепёж: нержавеющий, под спецключ.

**Монтаж**

Точка установки всего комплекса выбирается совместно с проектировщиками на этапе разметки по двум критериям:

  - максимальная чёткость эха от склона оврага;
  - расстояние до ближайших жилых строений окраины Введенской слободы (недопустимо создавать источник периодического шума у жилья).

#### **3. Принцип работы**

Посетитель встаёт на помост, ориентируется по стрелкам на столбике и громко хлопает в ладоши один раз. Звуковая волна расходится во все стороны, доходит до склона оврага, отражается от плотного грунта, и часть её возвращается. Посетитель считает задержку в уме («раз-и…»), сверяется с таблицей на панели — и получает расстояние до склона. На первом хлопке путь звука к ближайшему лесу заглушён экраном. Когда посетитель выходит из-за экрана и хлопает снова, эхо от деревьев возвращается намного быстрее: лес ближе склона.

Движущихся и настраиваемых узлов нет. Объект не требует калибровки, синхронизации или периодической регулировки. Работа в штатном режиме — в любую погоду и любой сезон.

#### **4. Тексты Уровень А**

##### **4.1 Стенд (информационная панель)** 

Знаете ли вы, как измерить расстояние с помощью звука?

**Эксперимент.**

1.  Встаньте на помост лицом к противоположному склону оврага. За спиной — звукопоглощающий экран: он гасит звук, идущий в сторону леса, и в ответ возвращается только эхо от склона.
2.  Громко хлопните в ладоши один раз (вместо хлопка можно резко крикнуть — короткое «эй» или «оп»). Отсчитайте про себя время до прихода эха и подставьте задержку в формулу ниже: получится расстояние до склона.
3.  Сойдите с помоста, обогните экран и встаньте лицом к лесу. Хлопните или крикните снова и сравните задержку с первой.

Скорость звука в воздухе у поверхности земли при обычной температуре близка к 340 м/с и от направления распространения не зависит. За время t, прошедшее между хлопком и эхом, звуковая волна успевает пройти путь до препятствия и вернуться обратно, то есть удвоенное расстояние s. Отсюда

  - s — расстояние до препятствия,
  - v — скорость звука,
  - t — время между хлопком и эхом.

Если эхо вернулось через 1 секунду, склон находится примерно в 170 метрах. Если через полсекунды — примерно в 85 метрах.

|  |  |
| :-: | :-: |
| \*\*Задержка t\*\* | \*\*Расстояние до склона s\*\* |
| 0,5 с | ≈ 85 м |
| 1 с | ≈ 170 м |
| 2 с | ≈ 340 м |

Отдельное эхо можно услышать только от достаточно далёких препятствий. В комнате стены слишком близко: отражения накладываются друг на друга, и вместо эха мы слышим лишь изменение тембра или гулкость помещения. А вот на открытой местности склон оврага находится далеко, поэтому отражённый звук приходит с заметной задержкой.

Рядом с помостом установлен звукопоглощающий экран. Он ослабляет звук, уходящий в сторону леса, чтобы первым возвращалось именно эхо от склона оврага. Если выйти из-за экрана и хлопнуть снова, можно заметить, что эхо от деревьев приходит быстрее, так как лес расположен ближе.

Тот же принцип люди используют в технологиях и природе. Корабельный эхолот измеряет глубину моря по времени возвращения звукового сигнала. Ультразвуковой сканер в медицине строит изображение внутренних органов по отражению волн внутри тела. А летучие мыши и дельфины буквально «видят» мир эхом: испускают звуки и определяют расстояние до объектов по задержке отражения.

Б**ЛОК 2 - ПАРАБОЛИКИ (параболические антенны)**

## **Голос в фокусе** 

На двух удалённых краях площадки стоят большие чашеобразные отражатели, обращённые друг к другу.

**Эксперимент.**

1.  Подойдите к отражателям с разных сторон, по одному к каждой чаше, лицом к ней.
2.  Один говорит спокойным голосом, не повышая его. Второй слушает: на таком расстоянии без отражателей пришлось бы кричать, а здесь обычная речь слышна отчётливо.
3.  Поменяйтесь ролями.
4.  Сделайте шаг в сторону. Эффект сразу пропадёт.

Поверхность каждой чаши — параболоид, и у неё есть особая точка, называемая фокусом. Её положение для каждой чаши строго одно.

Звук, идущий от говорящего, попадает на первую чашу и отражается от неё. Если звук от говорящего находится в фокусе, отражённые волны складываются в направленный пучок, который не расходится в стороны, а идёт ровно к другой чаше. Там пучок отражается во второй раз и собирается в её фокус — туда, где находится ухо слушающего. Энергия звука, совершая путь, почти не рассеивается, и шёпот слышен так же, как если бы говорящий стоял рядом. Стоит отойти от чаши, и обе отражённые волны теряют общее направление: голос рассеивается во все стороны как обычно.

То же свойство используют не только со звуком. Спутниковая тарелка собирает в свой фокус радиоволны от спутника, там же стоит приёмник. Радиотелескоп так ловит излучение далёких галактик. Автомобильная фара устроена наоборот: лампа в фокусе зеркала превращается в дальний параллельный пучок света.

### **Уровень Б.**

### **Почему мы не слышим эхо в комнате?**

Человеческий слух обладает конечным временным разрешением. Если два звука приходят слишком близко по времени, мозг воспринимает их как единый сигнал.

Порог разделения составляет примерно:

Δt≈0.05–0.07 сΔt≈0.05–0.07 с

Это называется порогом Хааса.

За 0,05 секунды звук успевает пройти:

То есть отдельное эхо различимо только от препятствий, находящихся дальше примерно 8–10 метров. В комнате стены ближе этого предела, поэтому отражения сливаются с исходным звуком и образуют реверберацию.

**Как впервые измерили скорость звука**

Скорость звука в воздухе одним из первых измерил французский математик Марен Мерсенн в 1630 году — сразу двумя способами. В первом он засекал секундным маятником время между вспышкой пушечного выстрела и приходом её звука на известном расстоянии: свет от вспышки доходит практически мгновенно, поэтому весь интервал — это время пробега звука. Получилось около 450 м/с. Во втором методе Мерсенн становился на известном расстоянии от стены и засекал время между выстрелом и приходом эха — тем же приёмом, который повторяет посетитель этого стенда. Здесь вышло около 315 м/с.

Разница между двумя цифрами была так велика, что современники подозревали: отражённый звук движется медленнее прямого. Дело было в неточности отсчёта времени без хороших часов. Истинное значение лежит между измерениями Мерсенна и при умеренной температуре близко к 340 м/с; к концу XVIII века оно было определено достаточно точно.

  
  

### **QR-секции**

#### Когда вы слышите эхо, на самом деле вы сталкиваетесь с одной из самых фундаментальных задач физики: определением расстояния по времени прохождения волны

### **Волновая модель эха**

Звук — это продольная механическая волна: в воздухе распространяются не сами молекулы, а области повышенного и пониженного давления. Скорость распространения определяется тем, насколько быстро среда успевает передавать упругие деформации.

Для идеального газа скорость звука задаётся формулой:

#### Главное следствие: скорость звука зависит от температуры.

#### 

### **Геометрия измерения**

#### Когда вы хлопаете в ладоши, волна проходит путь до препятствия и обратно. Поэтому расстояние вычисляется так:

#### 

#### Именно поэтому эхолокация — это по сути измерение времени с очень высокой точностью.

**Интересный факт**

Особенно впечатляет эхолокация летучих мышей. Они испускают ультразвуковые импульсы частотой до 100–200 кГц и анализируют отражённый сигнал за доли секунды.

Современная техника умеет посылать и принимать ультразвук не хуже мыши. Но мозг животного делает нечто гораздо более сложное: по одному отражению он одновременно восстанавливает расстояние, размер, форму, фактуру поверхности и даже характер движения объекта.

Как именно нейронные сети мозга выполняют такую обработку в реальном времени — одна из открытых задач современной нейрофизиологии.

#### **Интересный факт**

Летучая мышь в полной темноте за доли секунды восстанавливает по эху трёхмерную картину окружающего пространства и отличает птицу от ветки, на которой та сидит. Технически мы умеем посылать и принимать ультразвук не хуже мыши, и алгоритмы обработки сигнала постоянно совершенствуются. Но как именно мозг мыши извлекает из эха одновременно расстояние, размер, форму, фактуру и подвижность цели, остается одной из открытых задач нейрофизиологии.

#### **5. Безопасность**

Подвижных узлов нет — нет риска защемления и травмы при взаимодействии. Помост: высота настила 100–150 мм, нескользящее покрытие; кромки скруглены. Зона безопасности вокруг помоста — 1 м с открытых сторон, без препятствий. При наличии конуса-раструба: кромки раструба завальцевать или обварить, исключить острую кромку на уровне лица ребёнка; бетонный приступок у опоры — нескользящее покрытие, скруглённые края.

#### **6. Что требует уточнения до производства**

Натурный акустический тест на месте: выбор точки помоста по чёткости эха, замер фактической задержки (ожидаемый диапазон — подлежит измерению). Решение по конусу-раструбу: устанавливать ли, по итогам натурного теста. Если да — диаметр, длина, профиль (конус/парабола), материал (нержавейка vs кортен). Материал настила помоста: лиственница, термодревесина или ДПК — выбор по бюджету и регламенту обслуживания. Тип нескользящего покрытия помоста — конкретная модель/состав. Содержание и формат разметки на столбике-указателе (пиктограммы, язык подписей) — в едином стиле проекта тропы. Размещение и тип QR-носителя: отдельная табличка или интеграция в информационную панель.

  
  
  

# ✅ Водораздел - WP016  

# **DONE**

# **WP016 — «Водораздел»**

|  |  |
| :-: | :-: |
| \*\*Поле\*\* | \*\*Значение\*\* |
| Геопривязка | \<https://yandex.ru/maps/-/CPe7ZOzT\> |
| Локация | Точка на гребне водораздела с видом на ручей и противоположный склон оврага. Если конкретная вершина гребня по логистическим причинам недоступна — точка обзора с прямой видимостью гребня и стрелкой-указателем «гребень в N метрах». |
| Тип станции | Малая станция: информационный стенд (двусторонняя точка обзора) + телесное действие через локацию |
| Категория опыта | Наблюдение рельефа + одиночный осмысленный шаг через водораздел |
| Круглогодичность | Да |

## **1. Замысел**

Посетитель стоит на гребне холма. Под левой ногой капля уйдёт в один ручей, под правой — в другой; между ними нет ничего, кроме воображаемой линии. Эту линию никто не чертил — она нарисована природой и идёт по всем гребням Земли. Один шаг — и адрес капли меняется.

После этого простого действия рельеф вокруг посетителя становится **читаемой картой**: каждый склон — это направление, каждый овраг — приёмник, каждая вершина — узел разделения. Универсальный принцип «вниз по самому крутому склону» рождает всю речную сеть планеты, и тот же принцип работает в кровеносной системе, в траектории обучения нейросети, в любом ландшафте с локальными минимумами.

Главная мысль, которую посетитель уносит: **на любой неровной поверхности — будь то Земля, ткань организма или ландшафт функции потерь — каждая точка имеет свой бассейн притяжения, и границы между бассейнами проходят по гребням**.

## **2. Конструкция**

### **3.1 Общая схема**

Один узел: наклонный стенд.

  - **Сторона 1 (от тропы)**: заголовок-вопрос, тексты А и Б, схема локального водосбора, поле под числовые значения, QR-код. Эта сторона видна посетителю на подходе.
  - **Доп секция** локальная карта в масштабе, отметка «вы здесь», граница водораздела красной линией, стрелки направления стока, синяя линия русла ручья и его дальнейший путь (Свияга → Волга), легенда. Эта сторона видна, когда посетитель стоит на гребне и смотрит на пейзаж.
  - **Стрелки-указатели от гребня водораздела к двум стокам** 

  

### **3.2 Материалы3.3 Габариты и монтаж**

  - Высота стенда: 1,2 м.
  - Панель: \~40×30 см, центр на высоте \~1,1 м (видимость над снегом).
  - Заглубление пятки: ≥ 40 см, защита от опрокидывания и проворота.

## **4. Принцип работы (сценарий действия посетителя)**

1.  Посетитель подходит к стенду по тропе, видит сторону 1: заголовок «Куда уходит вся вода?» и тексты А и Б.
2.  Читает уровень А. Поднимает глаза, смотрит вокруг — видит склоны оврага, гребни.
3.  Делает шаг в одну сторону — представляет, как капля под ногой уходит в один ручей. Шагает в другую — капля идёт в другой ручей. **Линия водораздела впервые становится физическим, а не картографическим объектом.**
4.  Читает уровень Б — получает универсальную формулировку принципа.
5.  По желанию сканирует QR — уходит в уровень В: водный баланс, перенос принципа на кровь и нейросети, открытый вопрос про перехват рек

  
  
  

### **Тексты стендов**

## **Финальный текст стенда WP016 «Водораздел»**

### **Уровень А — верхний блок**

**Куда уходит вода?**

Конёк двускатной крыши делит дождь между двумя жёлобами: капли с одного ската уходят в один водосток, со второго — в другой, а граница пробегает ровно по верхней линии. Площадка под ногами устроена так же, только сделана не строителем, а самой природой.

Присмотритесь: от узкой полосы под ногами поверхность мягко понижается в две стороны, к вытянутым ложбинам. В сухую погоду их легко не заметить, но во время дождя и снеготаяния по ним сбегает вода. Капля, упавшая чуть в одну сторону от линии под ногами, уйдёт в один сток, упавшая чуть в другую — в соседний.

Эту линию называют водоразделом. Площадь, с которой вода собирается в один сток, — её водосбором. Водораздел нигде не размечен: его рисует сам рельеф, и проходит он по гребням — там, где склон меняет направление.

### **Уровень Б — основной блок**

**Откуда берётся речная сеть**

**Как вода выбирает путь?** Каждая капля движется вниз по самому крутому из доступных ей склонов: гравитация тянет её к земле, а рельеф направляет туда, где спуск круче всего. Маленькие потоки сливаются в ручьи, ручьи — в малые реки, малые реки — в большие. Вода, ушедшая с этой площадки, в конце концов попадает в Свиягу, оттуда в Волгу и в Каспий. 

**Сколько воды приносит обычный дождь?** Метеорологи измеряют дождь толщиной слоя воды, который остался бы на ровной поверхности, если бы ничего не впитывалось и не стекало. Слой в десять миллиметров — примерно с кончик мизинца — даёт на квадратный километр 10 000 тонн воды.

**Куда она уходит?** По поверхности течёт не вся эта масса. Часть впитывается в почву и пополняет грунтовые воды. Этот процесс называется инфильтрацией. Часть испаряется с поверхности. Часть деревья втягивают корнями и испаряют через листья. Этот процесс называется транспирацией. Остаток движется по поверхности к понижениям и образует поверхностный сток — именно его собирают ручьи и реки.

Та же закономерность встречается не только в течении воды, но и в движении самых разных потоков: кровотоком в тканях, распространением тепла, электрическими цепями и даже обучением нейронных сетей. Во всех этих системах под действием разницы давлений, температур, напряжений или вероятностей что-то переносится из области «избытка» в область «недостатка». Подробнее по QR-коду. 

### **Уровень В — QR, расширение**

**Водоразделы за пределами гидрологии**

**1. Что общего у дождя, крови и нейронной сети?** На первый взгляд ничего. Но если смотреть на них как на потоки, устроены все три одинаково: что-то течёт по «склону» некоторого поля, и пространство при этом разделяется на области сбора.

В гидрологии градиентом служит высота, потоком — стекающая вода, бассейном — речной водосбор, гребнем — водораздел.

В тканях организма градиент задаёт давление, потоком служат кровь и межклеточная жидкость, бассейнами — зоны, которые «обслуживает» каждый капилляр или вена. И речная сеть, и сеть капилляров устроены ветвисто, по сходным геометрическим правилам — это одно из проявлений общего закона переноса по градиенту. 

В машинном обучении параметры обучающейся модели — это точка в многомерном пространстве, на котором задана функция потерь. Оптимизатор сдвигает эту точку по антиградиенту, и она приходит в один из локальных минимумов. Множество начальных положений, из которых траектория сходится к одному и тому же минимуму, называется его бассейном притяжения, а граница между двумя соседними бассейнами — гребнем функции потерь. Слова «бассейн» и «граница бассейна» используются здесь не как метафора: гидрология и обучение нейросетей попадают в один и тот же класс задач — спуск по градиенту скалярного поля. Конкретные уравнения у воды и у нейросети, конечно, разные, но геометрия «куда сходятся траектории и где их граница» работает одинаково. 

**2. Куда на самом деле уходит дождь?** Из 10 000 тонн воды, выпадающих на квадратный километр в виде дождя, до стока доходит лишь часть. Остальное расходится по нескольким путям: инфильтрация переводит воду в почвенные горизонты и пополняет грунтовые воды; с открытой поверхности — почвы, листьев, временных луж — часть влаги испаряется; деревья извлекают воду корнями и испаряют её через устьица листьев (транспирация). То, что не ушло ни одним из этих путей, образует поверхностный сток.

Лес меняет соотношение этих долей. Кроны смягчают удар капель, подстилка и корни делают почву рыхлой и проницаемой, и под лесом доля инфильтрации возрастает за счёт стока. Поэтому облесённый водосбор даёт сглаженный паводок, а распаханное поле — резкий и высокий.

**3. Как измеряют водосборы?** Площадь и геометрию водосбора сегодня вычисляют по цифровой модели рельефа. Для каждой клетки расчётной сетки алгоритм определяет направление наибольшего уклона и трассирует путь воды до точки слива; объединение клеток, чьи траектории сходятся в одном устье, и составляет водосбор. Метод был реализован в 1980-х годах и сегодня лежит в основе любой гидрологической карты, нарисованной по спутниковым данным.

Доли инфильтрации, испарения, транспирации и поверхностного стока измеряют экспериментально — годами сравнивают соседние водосборы с разным растительным покровом, например облесённый и распаханный. Такие парные наблюдения, начатые в Швейцарии и США в начале XX века, и показали, что лес меняет не столько объём задержанной воды, сколько режим её движения. В машинном обучении ландшафт функции потерь обследуют тысячами численных запусков с разных начальных точек и составляют карту бассейнов притяжения — она играет ту же роль, что карта водосборов в гидрологии.

**4. Стоят ли водоразделы на месте?** Нет, речная сеть медленно перестраивается. Один из ручьёв подмывает свой берег быстрее соседа и постепенно отступает вверх по течению; в какой-то момент он подходит к водоразделу настолько близко, что захватывает часть водосбора соседа. Это явление называется перехватом рек, или речным пиратством; геоморфологи описали его ещё в XIX веке по характерным изломам в плане речных долин. Водораздел при перехвате смещается на десятки и сотни метров за тысячелетия.

Где и когда произойдёт следующий перехват, заранее предсказать нельзя: процесс зависит от прочности пород, режима осадков, растительного покрова и редких событий вроде катастрофических наводнений. Не исключено, что гребень под этим стендом тоже медленно ползёт в одну из сторон — тогда капля, уходящая сейчас в один сток, через несколько тысяч лет попадёт в другой.

  
  
  
  
  

# ✅ Мост Леонардо - WP014  

# Цепная линия - каждый провис является цепной линией (добавлена)

# **Done**

# **WP014 — «Мост Леонардо»**

## **Общие данные**

|  |  |
| :-: | :-: |
| \*\*Поле\*\* | \*\*Значение\*\* |
| ID | 24 |
| Код | WP014 |
| Название | «Мост Леонардо» |
| Геолокация | https://yandex.ru/maps/-/CPe7ZZJS |
| Тип станции | Пешеходный мостик + информационный столбик + демонстрационная модель |
| Формат взаимодействия | Телесный опыт (переход) + тактильный эксперимент (модель) + наблюдение + чтение |

## **Состав объекта**

Объект состоит из трёх связанных элементов, работающих как единая педагогическая станция:

1.  **Основной мост** — транспортное сооружение, по которому посетитель физически переходит через ручей.
2.  **Демонстрационная модель** — тактильный эксперимент рядом с мостом, на котором принцип можно собрать и проверить руками.
3.  **Информационный столбик** с двусторонней табличкой.

Разделение на три элемента — принципиальное. Основной мост даёт телесный опыт («я прошёл по конструкции без гвоздей»), модель даёт эпистемологическую честность («я могу проверить принцип сам»), табличка даёт объяснение («я понимаю, почему это работает»).

## **1. Основной мост**

### **Конструкция**

Пешеходный мост через ручей, собранный по самонесущей схеме Леонардо да Винчи.

  - **Материал:** лиственница или дуб (морозостойкие породы, устойчивые к циклам замерзания).
  - **Количество элементов:** 10–12 брёвен.
  - **Длина пролёта:** 10 м (по ширине ручья в конкретной точке).
  - **Ширина прохода:** \~1,2 м.
  - **Перила:** стальной трос в оплётке из нержавеющей стали.
  - **Опоры:** лежни на бетонных подушках (защита от контакта дерева с грунтом и водой).
  - **Скрытое усиление:** металлический крепёж внутри конструкции, невидимый для посетителя.

### **Инженерный принцип**

Исторический принцип взаимного расклинивания элементов сохранён — брёвна действительно уложены по схеме Леонардо и действительно передают нагрузку друг другу. Однако конструкция дополнительно усилена скрытым металлическим крепежом ради соответствия современным нормам безопасности пешеходных сооружений.

Справочные материалы по схеме:

  - https://etudes.ru/mathgrounds/leonardos-bridge/
  - https://4brain.ru/blog/%D0%BC%D0%BE%D1%81%D1%82-%D0%B4%D0%B0-%D0%B2%D0%B8%D0%BD%D1%87%D0%B8/

### **Расчётная нагрузка**

Обычная пешеходная нагрузка с коэффициентом запаса, принятым для парковых мостов (в проектной документации уточняется конструктором).

## **2. Демонстрационная модель (обязательный элемент)**

### **Назначение**

Модель закрывает методологический разрыв между тем, что утверждает стенд («мост держится без гвоздей»), и тем, что посетитель видит («большой мост всё-таки усилен»). На маленькой модели принцип можно собрать, разобрать, нагрузить и сломать — и убедиться, что геометрия действительно работает.

### **Конструкция**

  - **Размер:** \~60 × 80 см (настольного масштаба, устанавливается на столбике-подставке рядом с мостом).
  - **Элементы:** 7 реек из той же лиственницы, что и основной мост.
  - **Антивандальная мера:** рейки попарно скреплены короткими страховочными цепочками на свободных шарнирах — мост можно собрать и разобрать, но рейки нельзя унести.
  - **Подставка:** столбик высотой 85 см с плоской верхней площадкой и бортиком, удерживающим рейки от падения на землю.

### **Инструкция на подставке («Собери сам»)**

Короткая схема сборки в 4 шага, выгравированная на металлической пластине (не бумажная — выцветет, не наклейка — отклеится).

1.  Уложи три первые рейки крестом на бортике подставки.
2.  Перепле­ти четвёртую рейку в нахлёст так, чтобы она прижимала две предыдущие.
3.  Добавь рейки 5–7 по той же схеме — каждая следующая работает клином для двух предыдущих.
4.  Надави ладонью сверху по центру. Если конструкция плавно оседает и держит вес — собрано верно.

*Финальные формулировки уточняются с конструктором по итогам прототипа.*

## **3. Информационный столбик**

### **Параметры**

  - **Высота:** 1,2 м.
  - **Двусторонняя табличка:** сторона 1 — основной текст стенда; сторона 2 — решение зависит от геологической проработки точки (потенциально — локальный контекст про конкретный ручей, окружение, сезонные изменения).
  - **Материал таблички:** металл с фрезерованным или глубоко окрашенным текстом (не печать, не плёнка).
  - **QR-код:** на нижней части стороны 1, размер \~6 × 6 см.

### **Сторона 1 — Уровень А**

## **Брёвна, которые держат друг друга**

*Мост Леонардо да Винчи и принцип самонесущей конструкции*

Посмотрите на мост перед вами. Он сложен из десяти брёвен, и ни одно из них не скреплено с другими, и тем не менее конструкция надёжно держит вес взрослого человека. Разберём, как это устроено.

Представьте каменную арку над дверным проёмом старого дома. Камни арки вытесаны клиньями: вес сверху давит на верхний клин, тот передаёт усилие не вниз, а в стороны — соседним клиньям, и так далее до самих опор. Чем больше нагрузка, тем плотнее клинья прижаты друг к другу.

В мосту Леонардо роль клиньев играют сами брёвна — благодаря тому, как они уложены. Каждое бревно прижато сразу с нескольких сторон: снизу его подпирают два соседа, сверху давят ещё два, и каждый из этих четырёх, в свою очередь, удерживается такими же соседями дальше по конструкции. Любая попытка вытащить отдельное бревно упирается в разнонаправленные прижимы со всех сторон; пока соседи на месте, бревно остаётся зажатым. Между брёвнами действует только сжатие и сила трения, и крепёж становится не нужен. Такие конструкции называют **самонесущими**.

Схему предложил Леонардо да Винчи в начале XVI века — как переправу, которую армия могла собрать без инструментов и разобрать после прохождения. Современный мост построен по той же геометрии, но дополнительно укреплён скрытым металлическим крепежом: этого требуют нормы безопасности парковых сооружений. Стоящая рядом модель собирается без крепежа — на ней принцип можно увидеть в чистом виде.

  
  

# **QR-код — Уровень В** 

Вспомните ряд книг на полке. Пока книги плотно прижаты друг к другу, ряд держится; стоит вытащить одну — соседние начинают валиться. Самонесущие конструкции работают по той же логике, и отсюда же — их главное ограничение. Они хорошо держат нагрузку, направленную сверху вниз, то есть сжимающую, и заметно хуже справляются с любой другой. За счёт собственного веса и трения мост сопротивляется боковому ветру и небольшим рывкам, но если приложить силу снизу вверх, попытавшись оторвать его от опор, конструкция распадётся: связей, способных работать на растяжение, между брёвнами нет.

**Каков предел длины пролёта?** Сама геометрия конструкции диктует первое ограничение: чем длиннее мост, тем меньше угол наклона брёвен к горизонту и тем слабее боковой прижим между ними при том же весе сверху. С некоторой длины этого прижима перестаёт хватать, чтобы трение удерживало брёвна от проскальзывания. Для пешеходных мостов схема Леонардо работает до пролётов в несколько десятков метров; на больших длинах в дело вступают другие инженерные ограничения — на прочность материала, на изгиб, на ветровые нагрузки. У висячих мостов, например, главным становится сопротивление троса разрыву под собственным весом конструкции. По той же геометрической причине существенна шероховатость материала: отполированные брусья соскальзывают друг с друга, какой бы точной ни оказалась геометрия укладки.

**Как такие конструкции рассчитывают?** До середины XX века — почти исключительно прямым экспериментом: из реек или карандашей собирали модель в малом масштабе и нагружали её до разрушения, а параллельно раскладывали силы аналитически на каждом узле. Сегодня основная часть работы делается на компьютере методом конечных элементов: конструкция разбивается на тысячи мелких ячеек, для каждой решаются уравнения равновесия, и программа предсказывает, где и при какой нагрузке появятся опасные напряжения. Натурные модели и физические испытания при этом не исчезли — они используются для проверки расчёта и для тех случаев, когда поведение материала плохо описывается формулами: например, для усталости древесины при многолетних циклах увлажнения. По тому же сочетанию методов рассчитывают и большие арочные сооружения — от античного акведука Пон-дю-Гар до купола Санта-Мария-дель-Фьоре во Флоренции.

**Другая дорога к арке без растяжений.** Обязательно ли строить арку из прямых элементов? Нет, есть и другой способ: подобрать саму форму свода так, чтобы линия действия силы тяжести шла строго вдоль оси арки. А какая это форма? Возьмите цепочку и подвесьте её за концы — под собственным весом она примет вполне определённую кривую. Эту кривую называют **цепной линией**, или **катенарией** (от лат. *catena* — цепь). Натяжение в любой её точке направлено по касательной к самой цепи, и каждое звено испытывает только растяжение вдоль линии.

Если такую кривую мысленно перевернуть, растяжение во всех точках сменится сжатием — получится арка, в которой каждый элемент работает только на сжатие, без изгиба. Для камня и бетона, плохо переносящих растяжение, это оптимальная форма свода. По катенарной кривой Антонио Гауди проектировал своды Саграда Фамилии — он буквально подвешивал в мастерской верёвочные модели и фотографировал их вверх ногами. По ней же построены Арка Гейтуэй в Сент-Луисе и купол собора Святого Павла в Лондоне.

У самонесущей конструкции, таким образом, есть два независимых способа избавиться от растяжений. Мост Леонардо оставляет элементы прямыми и подбирает геометрию укладки. Катенарная арка укладывает элементы подряд и подбирает кривую свода. Решения разные, инженерная задача одна.

**Открытый вопрос.** Все самонесущие конструкции, созданные человеком, — арки, своды, купола, иглу, мост Леонардо — превращают вертикальный вес в направленное сжатие между соседними элементами. А как обстоит дело с живой природой? Если присмотреться, чистое сжатие используется и здесь — но избирательно. Кости позвоночных хорошо приспособлены именно к нему: предельная прочность кортикальной кости на сжатие почти в полтора раза выше, чем на растяжение, а позвоночник несёт практически чистую сжимающую нагрузку. Раковины моллюсков работают как арочно-купольные сжатые конструкции — особенно у глубоководных видов, удерживающих внешнее давление. Ствол дерева большую часть жизни сжат собственной кроной. Однако длинные кости конечностей при ходьбе работают преимущественно на изгиб, травянистые стебли — на изгиб и внутреннее давление, и в целом полностью «арочных» конструкций в живом мире немного. Возможная причина в том, что чистая арка плохо масштабируется: при росте организма её пришлось бы перестраивать каждый раз, когда меняются размеры. Растущая кость и растущее дерево справляются с этой задачей, постоянно подстраивая внутреннюю структуру под текущую нагрузку, — арке такая перестройка недоступна.

## **6. Эксплуатация**

### **Климат и деградация**

  - Лиственница/дуб под лаком или маслом для наружных работ: ресурс 15–25 лет при ежегодном осмотре.
  - Обновление покрытия: раз в 2–3 сезона.
  - Бетонные подушки: бессрочно, защищают от гниения в точках контакта.
  - Стальной трос в оплётке: ресурс 10+ лет, замена по износу оплётки.

### **Вандалоустойчивость**

  - **Основной мост:** высокая. Повиснуть, прыгать, раскачиваться — конструкция рассчитана.
  - **Модель:** средняя. Рейки скреплены цепочками от кражи, но грубым применением могут быть выведены из строя. Заложить в эксплуатацию как расходник: ресурс реек \~2 сезона, склад комплектов на парк.
  - **Табличка:** стандартные для уличных стендов риски (граффити, царапины). Металл с фрезерованным текстом — оптимум.

  
  

# ✅ Валун-термометр - WP001-3  

**DONE**

WP001-3 — «Валун-термометр»

Геопривязка: конкретный валун на тропе в Иннополисе 

Локация: открытое место с выраженной ориентацией С–Ю; валун без густой полуденной тени.

Тип станции: малая, точечная, привязанная к одному природному объекту.

Категория опыта: телесный эксперимент — сравнение температур ладонью, попытка ориентирования.

Круглогодичность: частично. Конструкция работает в любую погоду; сам опыт надёжно воспроизводится в солнечный день около полудня. В пасмурную погоду и в тени эффект размывается — опыт превращается из навигационного в учебный.

  

1\. Замысел

Посетитель подходит к валуну и прикладывает ладонь сначала к одной стороне, потом к другой. В солнечный день в умеренных широтах разница чувствуется сразу: южная сторона ощутимо теплее северной. С этого жеста начинается мысль о том, что нагрев поверхности определяется углом, под которым луч встречается с поверхностью: чем круче — тем больше энергии на каждом квадратном сантиметре. Тот же принцип объясняет, почему снег на северных склонах лежит дольше, почему виноградники в Европе разбивают на южных склонах и почему тропики жарче полюсов. Один камень оказывается моделью планетарного процесса в человеческий рост. И тут же — оговорка: эффект надёжно ощущается только в ясный день около полудня; в пасмурную погоду и в тени он размывается, и опыт превращается из навигационного в учебный.

  

2\. Поперечное сечение

ОБЩАЯ СТРУКТУРА (одна фраза, без объектов):

Чем круче поток энергии падает на поверхность, тем меньше площадь, по которой он распределён, и тем больше энергии получает каждая единица площади.

ЯВЛЕНИЯ:

  

Физика быта — стороны валуна, ствола, стены здания: одинаковое солнце, разный угол, разная температура.

География / геология — северные и южные склоны: на северных снег держится дольше, виноградники в Европе разбивают на южных.

Биология — мох на стволах часто гуще с северной стороны как следствие меньшего нагрева и большей влажности.

Климатология (планетарный масштаб) — тропики жарче полюсов не из-за расстояния до Солнца, а из-за крутизны падения лучей.

Инженерия — ориентация зданий, выбор места под сад, расчёт солнечных панелей.

  

НЕОЧЕВИДНАЯ СВЯЗЬ:

Пункт 4. Тропики жарче не потому, что ближе к Солнцу, а потому, что там лучи падают круче. Эта мысль и делает камень моделью континента: разница расстояний от тропика до полюса ничтожна по сравнению с разницей углов. Когда посетитель чувствует ладонью перепад на одном валуне, он держит в руке миниатюру того же закона, который кроит климатические зоны Земли.

  

3\. Конструкция

3.1 Общая схема

Столбик-указатель из стальной трубы с двусторонней табличкой, забетонированный рядом с валуном.

Табличка установлена с небольшим выносом так, чтобы посетитель сначала прочитал задание, потом подошёл к камню и приложил ладонь к двум сторонам.

Сам валун — действующая часть объекта.

  

3.2 Материалы

  

Столбик: стальная труба ⌀60 мм, порошковая покраска в тоне типовой парковой инфраструктуры.

Табличка: нержавеющая сталь 2–3 мм, \~40×30 см, двусторонняя гравировка.

Основание: бетон, заглубление по типовому узлу парка.

  

3.3 Габариты и монтаж

  

Высота столбика: 1,2 м от уровня земли.

Заглубление: по типовому узлу с учётом глубины промерзания грунта в Иннополисе.

Расстояние от столбика до валуна: достаточное для свободного обхода камня с двух сторон, ориентировочно 60–80 см (уточняется на месте).

  

4\. Принцип работы

Делает шаг к валуну, прикладывает ладонь к одной из сторон, держит несколько секунд.

Переходит к противоположной стороне, прикладывает ладонь, держит несколько секунд.

Замечает разницу температур — в солнечный день она ощутима даже за 3–5 секунд.

Возвращается к стенду, читает объяснение: разница берётся из угла падения лучей; южная сторона смотрит на полуденное солнце почти в лицо, на северную свет ложится вскользь.

Пробует угадать, где юг, по теплу в ладони, и встаёт лицом туда, где было теплее.

Обходит стенд, читает «Но это всё-таки не компас» — узнаёт условия, при которых способ надёжен.

По желанию переходит к QR и узнаёт, как тот же принцип определяет климат от камня до континента.

**5. Тексты**

### **5.1 Стенд, уровень А**

#### **Почему одна сторона валуна теплее другой?**

Потрогайте валун с разных сторон. В солнечный день вы заметите, что одна сторона ощутимо теплее остальных. Эта сторона — южная. Откуда берётся разница?

Если фонарик светит на стол прямо сверху, получается маленькое яркое пятно. Если его наклонить, пятно становится крупнее и тусклее: тот же свет растекается по большей площади. С солнечным светом происходит то же самое.

В среднем за день, и особенно в полдень, больше всего света достаётся южной стороне. К северной стороне полуденное солнце вообще не поворачивается: на наших широтах оно в полдень стоит над южной частью горизонта. 

**Где работает этот же принцип**

Снег весной дольше лежит на северных склонах оврагов и холмов. Мох на стволах деревьев чаще гуще с северной стороны. Виноградники разбивают на южных склонах: каждый дополнительный градус наклона к солнцу даёт прибавку к урожаю.

По разнице температур на валуне можно приблизительно найти юг — достаточно повернуться лицом к самой тёплой стороне. Способ работает в ясную погоду, на открытом месте, около полудня. Почему именно в этих условиях — на обороте.

### **5.2 Стенд, уровень Б**

#### **Заменяет ли камень компас?**

Камень не может полностью заменить компас. Разница температур на сторонах валуна происходит вследствие угла, под которым солнечные лучи падают на поверхность, и она легко стирается погодой и окружением.

**Что мешает наблюдать эффект**

В пасмурный день облака рассеивают свет, и он приходит со всего небосвода почти равномерно: южная сторона перестаёт отличаться от северной. Тень от соседнего дерева выравнивает температуру сторон за несколько минут. Ветер охлаждает наветренную сторону сильнее остальных и может либо усилить разницу, либо перевернуть её: при северном ветре сильнее всего остывает северная сторона, при южном — как раз южная. Дождь оставляет камень мокрым, и вода, испаряясь, забирает тепло со всех сторон.

Поэтому у способа три обязательных условия: ясное небо, открытое место, полуденные часы. Если хотя бы одно нарушено, по разнице температур нельзя определить стороны света — она расскажет лишь о случайной истории конкретного камня: где он стоял на солнце в последний час, не было ли над ним тени, не намочил ли его дождь, с какой стороны дует ветер.

**Где работает тот же закон**

Этот принцип также работает в строительстве и в природе. На южных скатах крыш быстрее тает снег и устанавливают солнечные коллекторы. На южных склонах холмов длиннее и теплее вегетационный период, поэтому туда садоводы сажают теплолюбивые культуры. У отдельно стоящего дерева кора с южной стороны суше, грубее и часто темнее, чем с северной.

### **5.3 QR — расширенный текст**

#### **Угол падения лучей и климат планеты**

Если на одном валуне разница в нагреве сторон составляет всего несколько градусов, насколько большую разницу тот же закон создаёт в масштабе планеты?

**Инсоляция: сколько энергии приходит от Солнца**

Количество солнечной энергии, падающей на единицу горизонтальной поверхности за единицу времени, называется инсоляцией и измеряется в ваттах на квадратный метр. Эта величина зависит от того, под каким углом луч встречает поверхность: чем ближе угол к прямому, тем меньшую площадь занимает световое пятно и тем больше энергии достаётся каждому её квадратному сантиметру. При угле падения 30° мощность распределяется по вдвое большей площади, чем при 90°, и поверхность получает вдвое меньше тепла.

**Иннополис: лето и зима**

Иннополис лежит на широте около 56°. В полдень в день летнего солнцестояния угол падения лучей на горизонтальную землю составляет здесь около 57°, а в день зимнего всего 11°. Поэтому зимой даже в безоблачный день поверхность получает в несколько раз меньше тепла, чем летом. Для вертикальной южной стороны валуна летом угол падения лучей около 33°, и она заметно нагревается. Северная сторона в этот же момент прямого солнечного света практически не получает.

**Климатические зоны Земли**

В масштабе планеты тот же закон формирует климатические пояса. Тропики жарче полярных широт, потому что лучи там падают под бо́льшим углом к поверхности. Среднегодовая инсоляция на экваторе примерно в два с половиной раза выше, чем за Северным полярным кругом.

**Где это измеряют и применяют**

Инсоляцию регистрируют приборами, которые называются пиранометрами; они фиксируют поток солнечной энергии при разных углах и ориентациях поверхности. Эти данные используют в агрономии при выборе участков под посевы, в строительстве при ориентации зданий и солнечных коллекторов, в климатологии при расчёте теплового баланса планеты.

  
  

# ✅ Краеведческий стенд WP013  

# **DONE**

# **WP013** **Краеведческий стенд**

## **1. Карточка объекта**

|  |  |
| :-: | :-: |
| \*\*Код\*\* | 23WP013 |
| \*\*Название\*\* | Краеведческий стенд «Ландшафт с памятью» (Введенская Слобода, бывшая Басурманская) |
| \*\*Тип\*\* | Двусторонний информационный стенд с зоной отдыха; якорный объект тропы |
| \*\*Геопривязка\*\* | https://yandex.ru/maps/-/CPe7ZINX |
| \*\*Зимняя расчистка\*\* | Да, приоритет |
| \*\*Питание\*\* | Не требуется |

 

## **2. Замысел**

Стенд работает как точка остановки и якорь тропы. Посетитель садится на скамью напротив панели и читает не очередную справку «здесь была такая-то слобода», а маленькое исследование. Главная мысль, которую он уносит: всё, что мы знаем о прошлом этого места, приходит из трёх разных источников — царских указов и церковных описей, археологических находок, устного предания. Они говорят о разном и врут по-разному.

На стенде каждое утверждение помечено значком источника:  документ,  находка,  рассказ. Посетитель видит, откуда мы это знаем — и тем самым получает не набор сведений, а инструмент, переносимый на любой исторический текст, который встретит дальше: в музее, в учебнике, в новостях. На обратной стороне — карта двух петель тропы и блок «Вы здесь», без всякой исторической нагрузки.

## **3. Структура объекта (методологическое сечение)**

**Общая структура.** Любое утверждение о прошлом приходит из определённого источника. Источник определяет, насколько утверждению можно доверять и какие у него слепые пятна.

Три способа знания о прошлом (это и есть «явления» сечения, разнесённые по типам источников, а не по доменам природы):

**1.  Документ.** Царские указы, церковные описи, ревизские сказки. Дают точные даты, имена, числа; молчат обо всём, что не записали.

**2.  Артефакт.** Стоянки, керамика, могильники. Говорят о вещах, занятиях, технологиях; молчат о мотивах и чувствах.

**3.  Предание.** Устный рассказ, передаваемый через поколения. Сохраняет живую память, в том числе о том, чего не было в бумагах; за века обрастает деталями и теряет точность.

**Неочевидная связь.** Все три источника говорят об одном месте, но никогда не накрывают полностью одно и то же. Документы знают про 1556 год, но молчат о городище марийцев. Археология знает про городище эпохи бронзы, но молчит о людях XVI века. Предание помнит черемисов, но не знает указа Грозного. Полная картина места собирается только из трёх источников вместе — и каждый закрывает чужие слепые пятна.

## **4. Конструкция**

### **4.1. Общая схема**

Двусторонняя панель 120×80 см на двух вертикальных стойках; рядом — скамья на 3–4 человек на расстоянии 1,5–2 м от стенда (сидящему видна вся панель без наклона головы). Площадка \~4–5 м² с дренажом.

### **4.2. Материалы**

• Панель: HPL (High Pressure Laminate) с УФ-стойкой печатью. Ресурс 7–10 лет без заметной деградации.

• Стойки: металл с антикоррозионным покрытием. Крепёж — нержавеющая сталь (оцинковка через два сезона даёт ржавые потёки).

• Скамья: бетонно-деревянный модуль или лиственница без лака. Крашеный металл и лакированное дерево требуют ежегодного обновления — отказываемся.

• Покрытие площадки: щебёночная отсыпка или плотный грунт.

### **4.3. Габариты и монтаж**

Нижний край панели — \~90 см от грунта (комфортно взрослому стоя, доступно ребёнку и человеку в коляске). Заглубление стоек по местному регламенту.

### **4.4. Безопасность**

Кромки панели — со скруглением; стойки без выступающих болтов. Зимой — приоритет расчистки до скамьи.

### **4.5. QR-код**

На стороне 2 (навигационной), в правом нижнем углу, 8×8 см с контрастной рамкой и явной подписью о содержимом.

## **5. Принцип работы**

Посетитель подходит к стенду, садится на скамью. На стороне 1 — заголовок и хук («Три имени одного места»). Под хуком — уровень А: краткая историческая канва. Дальше — уровень Б: объяснение, почему у этого места три голоса памяти и зачем рядом с каждым фактом стоит значок. Справа — три блока, по одному на каждый источник. Внизу — блок «Читайте ландшафт»: указание на конкретную видимую отсюда деталь, которая и есть зримый след прошлого.

Прочитав, посетитель встаёт, обходит стенд (или поворачивает голову — если стенд стоит к нему боком) и видит карту тропы: где он сейчас, какие петли, какие расстояния. По QR — углублённый материал об источниках истории места и приёмах краеведческой работы.

## **6. Тексты (после литературной вычитки)**

### **6.1. Сторона 1 — История этого места**

#### **Заголовок**

Три имени одного места

#### **Хук**

У села три имени и три эпохи. До 1556 года здесь стояло городище марийцев-язычников — имя не сохранилось. С 1556 года — Басурманская слобода, по указу Ивана Грозного. С 1648 года, после освящения церкви, — Введенская Слобода. Вы стоите там, где эти три эпохи сменяли друг друга.

#### **Уровень А. Основная мысль**

Место не менялось — менялись люди и то, как они его называли. Сначала здесь жили марийцы-язычники. После постройки Свияжска (города-острова за горой, основанного Иваном Грозным в 1551 году) сюда переселили стрельцов из Московского княжества и освободили их от податей. При Борисе Годунове добавили земледельцев из тверских и новгородских земель — на участки, отнятые у татарских мурз, не принявших православия. В 1648 году здесь освятили деревянную церковь Введения во храм Пресвятой Богородицы — и слобода получила третье имя.

#### **Уровень Б. Три голоса памяти**

У истории этого места три разных источника, и каждый говорит о своём. Документы — царские указы, церковные описи, ревизские сказки — дают точные даты, имена, цифры. Но сохраняют только то, что когда-то записали на бумагу. Археология рассказывает о вещах: какие строили дома, чем кормились, что клали в могилы. О мотивах и чувствах она молчит. Предание — самое живое: память передаётся из уст в уста; зато за четыре века неминуемо обрастает деталями. Поэтому рядом с каждым утверждением — значок. Он подсказывает, откуда мы это знаем и насколько этому можно доверять.

 

|  |
| :-: |
|  \*\*По письменным источникам\*\*\*\*1556.\*\* По указу Ивана Грозного на правом берегу Свияги основана Басурманская слобода. Сюда переведены стрельцы из Московского княжества с семьями и освобождены от податей.\*\*К 1565 году\*\* в слободе уже 130 дворов. Для России того времени это крупное село: во внутренних, давно освоенных уездах крестьяне жили деревнями по 3–5 дворов, а село в 20 дворов считалось большим. В Казанском и Свияжском уездах, впрочем, крупные сёла скоро становятся обычным делом.\*\*После 1584,\*\* при Борисе Годунове, слободу пополнили земледельцами из тверских и новгородских земель. Их поселили на участки, отнятые у татарских мурз, отказавшихся принять православие.\*\*1648.\*\* Освящена деревянная церковь Введения во храм Пресвятой Богородицы — слобода получает имя Введенской.\*\*1872.\*\* На пожертвования помещицы Назимовой деревянную церковь заменили на каменную. Она стоит и поныне.\*\*До революции.\*\* Село — в составе Юматовской волости Свияжского уезда. Работает церковно-приходская школа. |

 

|  |
| :-: |
| \*\*Археологически подтверждено\*\*В окрестностях найдены \*\*две стоянки приказанской культуры\*\* — это XIV–IX века до нашей эры, конец бронзового века. Люди жили в полуземлянках, пасли скот и сеяли хлеб, плавили бронзу. От той эпохи остались плоскодонные горшки с сетчатым узором и грунтовые могильники — археолог в этих местах находит именно это. О том, на каком языке говорили и каких богов чтили, керамика молчит. |

 

|  |
| :-: |
|  \*\*По преданию\*\*Старики рассказывали: до прихода стрельцов на этом холме стояло городище марийцев-язычников (русские называли их черемисами). Где именно стояло городище — бумаги не сохранили; помнит об этом только устный рассказ, передаваемый из поколения в поколение. |

 

|  |
| :-: |
|  \*\*Читайте ландшафт\*\*\*\*Посмотрите на дома перед вами. Это часть Введенской слободы\*\* — того самого поселения, у которого с XVI века менялись имена и население, а само место оставалось прежним. Многое из этой истории не попало ни в документы, ни в раскопки, но сохранилось в планировке улиц, в очертаниях дворов, в одиноких старых деревьях. По мере движения по тропе подобные детали встречаются регулярно, и за каждой из них стоит своя история.   |

 

### **6.2. Сторона 2 — Карта тропы и место, где вы стоите**

#### **Заголовок**

Вы здесь. Две петли: 4 и 12 километров

#### **Вступление**

От этого стенда расходятся две кольцевые петли тропы разной длины. Короткая петля длиной 4 километра проходится прогулочным шагом примерно за час; она рассчитана на прогулку с детьми и удобна в зимнее время. Длинная петля длиной 12 километров занимает около трёх часов, выходит за пределы села и доходит до Лысой горы.

  

#### **Таблица расстояний**

*(заполняется после трассировки)*

|  |  |  |
| :-: | :-: | :-: |
| \*\*До чего\*\* | \*\*Расстояние\*\* | \*\*Время пешком\*\* |
| До ближайшей станции по короткой петле | \\\[N\\\] км | \\\~\\\[M\\\] мин |
| До ближайшей станции по длинной петле | \\\[N\\\] км | \\\~\\\[M\\\] мин |
| Полная короткая петля (возврат сюда) | 4 км | \\\~60 мин |
| Полная длинная петля (возврат сюда) | 12 км | \\\~3 ч |

 

  
  
  

# ✅ Квадрат жизни - WP010  

## **Done**

## **WP010 — «Квадрат жизни»**

**Локация:**[ https://yandex.ru/maps/-/CPe7VKKm](https://yandex.ru/maps/-/CPe7VKKm)

#### **Тип станции**

Рамка + столбик с двусторонней табличкой (сезонное переключение без сменных элементов).

#### **Конструкция**

Заметная металлическая рамка 1×1 м из стального уголка с антикоррозийным покрытием, вкопанная заподлицо с поверхностью грунта. Допуск по высоте кромки: −5 / 0 мм относительно уровня дёрна — рамка не должна выступать (риск спотыкания, зацеп газонокосилкой) и не должна сильно проваливаться (теряется визуальная граница площадки). Рамка должна считываться как граница исследовательской площадки, но не создавать опасную кромку.

Рядом — столбик высотой 1,2 м с двусторонней табличкой \~40×30 см. Материал таблички — нержавеющая сталь, текст и схема нанесены гравировкой (не краской — гравировка практически не стирается и не выцветает). Столбик ставится так, чтобы человек, читая табличку, мог сразу перевести взгляд внутрь рамки под ногами.

Сезонное переключение смысла обеспечивается двумя сторонами таблички: летней и зимней. Сменных элементов нет — это обнуляет сезонное обслуживание. Летом рамка видна полностью и работает как площадка наблюдения за жизнью почвы; зимой рамка скрыта под снегом, но столбик и табличка остаются видны, и зимняя сторона работает как напоминание о невидимом слое под снегом.

Обслуживание: ежегодная проверка столбика на устойчивость и состояния кромки рамки (заглублена ли она, нет ли эрозии внутри рамки, не сформировалась ли вытоптанная плешь).

#### **Смысл экспоната**

Объект переключает масштаб — и заодно категории. Перед посетителем один квадратный метр земли, и на нём не удаётся провести границу между живым и неживым. Не справка про почвенных обитателей, а демонстрация принципа: в природе есть участки, где живое и неживое больше не существуют по отдельности и работают как единый механизм. Для таких участков есть слово — биогеоценоз. Почва под ногами — его эталон.

Две стороны таблички показывают этот принцип с двух разных направлений. Летняя сторона: живое и неживое строят друг друга — органика непрерывно становится почвой, почва — живым. Зимняя сторона: неживое оберегает живое — снег и пойманный в нём воздух удерживают температуру почвы около нуля, и сообщество переживает морозы. Летом это видно через разложение, зимой — через теплоизоляцию. Но экспонат один, и биогеоценоз один

  

## **Сторона 1 (летняя)**

**Что видно под ногами**

Загляните взгляд внутрь рамки. Сверху лежат свежие листья, их еще можно узнать  по форме и цвету. Ниже —старые и прошлогодние, они потемневшие, ломкие. Под ними слой, в котором уже не отличить листа от коры или веточки. Глубже — рыхлая тёмная масса без отдельных частей. Это и есть почва.

**Куда уходит листва**

Листву под ногами разбирают на молекулы грибы, бактерии и мелкие почвенные беспозвоночные. Освободившиеся минеральные вещества тут же перехватывают корни и отправляют в новые побеги и листья.

**Как отличить живое от неживого?**

Здесь нет четкой границы. В лесной подстилке живые организмы и среда обмениваются веществами постоянно. Для таких систем советский геоботаник Владимир Сукачёв в 1940-х годах ввёл термин *биогеоценоз* — сообщество живых организмов вместе со средой, неотделимой от них.

**Что удерживает жизнь под снегом?**

Зимой эта рамка скрыта под снегом. Кажется, что под полуметровым сугробом должен быть такой же мороз, как над ним. На самом деле температура на границе снега и почвы редко опускается ниже нуля даже в самые сильные морозы.

**Почему**

Свежий рыхлый снег — это лёд и воздух, причём воздух занимает до 90% объёма. Воздух плохо проводит тепло, и пока он неподвижен — заперт между снежинками, — тепло идёт через него очень медленно. То тепло, которое почва успела накопить за лето, за зиму просто не успевает уйти. Чем толще и рыхлее сугроб, тем сильнее эффект: плотный наст работает хуже, а земля без снега промерзает на десятки сантиметров.

**Тот же принцип в быту**

По такому же принципу работают стеклопакет и пуховая куртка. В стеклопакете тепло удерживает не толщина стёкол, а воздушный зазор между ними. В куртке — не сам пух, а воздух, который пух удерживает между перьями. Снежный покров устроен по тому же принципу, только «перьями» здесь служат снежинки.

**Что это значит для жизни в почве**

Под покровом снега грибы, бактерии, дождевые черви и корни все равно продолжают работать. 

Летом живое и неживое строят друг друга: опавшие листья перерабатываются и уходят в почву, из почвы минеральные вещества поступают — в растения. Зимой неживое защищает живое: вода, ставшая снегом, и пойманный в ней воздух не дают системе промёрзнуть. Биогеоценоз один и тот же, просто в разные сезоны главную роль играют разные его части

  

## **QR — углубление**

**Как отличить биогеоценоз от случайного соседства живого и неживого?**

Не любая точка на земле, где встречаются живое и среда, образует биогеоценоз. Например, на одиноком гранитном валуне посреди степи живёт мох, но он почти не влияет на сам валун и не меняет климат вокруг — это просто соседство. Биогеоценоз появляется там, где живое и среда обмениваются веществом и энергией достаточно долго и продолжительно, чтобы изменение одного сразу сказывалось на другом.

Лесная подстилка — наглядный случай такой системы. Если убрать дождевых червей, через несколько лет изменится структура почвы и пропадёт часть растений, которым нужен рыхлый верхний слой. Если зимой нет снега, на следующий год не вернутся виды, чьи личинки не пережили мороз. Если изменить влажность, изменится скорость разложения и состав почвенных грибов. Все эти связи можно наблюдать на одном участке.

Похожим образом устроены и многие другие сообщества. Коралловый риф — это известняковая постройка, созданная живыми организмами, на которой живут эти же организмы. Болото удерживает влагу с помощью мха, а сам мох может существовать только в этой воде. Лишайник — симбиоз гриба и водоросли (или цианобактерии), сросшихся так плотно, что вне партнёрства ни один из них не выживает. 

*Как это изучают.* Биогеоценозы исследуют долгим мониторингом: на ключевых участках годами измеряют температуру и влажность почвы на разной глубине, взвешивают опадающую и разложившуюся листву, разбирают пробы грунта под микроскопом. На одном квадратном метре умеренного леса в среднем оказывается несколько десятков тысяч мелких животных и сотни видов почвенных микроорганизмов. Сравнение соседних участков, например ненарушенного леса и вытоптанной тропы, показывает, что вместе с верхним слоем почвы исчезает примерно половина всего сообщества.

*Открытый вопрос.* Где проходит граница биогеоценоза? Считать ли лесную поляну и окружающий её лес одной системой или двумя разными? Однозначного ответа нет: экологи задают границы в зависимости от задачи. Это ещё одно напоминание, что в природе нет резко очерченных «объектов» — есть градиенты, в которых линии проводим мы сами.

  
  
  

# ✅ Террасы времени - WP012  

## **DONE**

## **WP012 — «Террасы времени»**

**Геопривязка:**[ https://yandex.ru/maps/-/CPe7VP21  
](https://yandex.ru/maps/-/CPe7VP21) **Локация:** склон речной долины, где сохранились ровные площадки на нескольких высотных уровнях, с обзором на современное русло и соседний овраг.  
 **Тип станции:** распределённая наблюдательная — несколько столбиков на разных уровнях рельефа.  
 **Категория опыта:** телесное прохождение временно́й шкалы + чтение ландшафта.  
 **Круглогодичность:** да.

### **1. Замысел**

Посетитель спускается по склону речной долины и обнаруживает, что ровные площадки под ногами — это бывшие дны́ той самой реки, что течёт сейчас глубоко внизу. Каждая ступень — отметка прежнего уровня воды. Между уровнями — десятки тысяч, иногда сотни тысяч лет.

Главная мысль уносится не как факт «вот тут была река», а как способ читать рельеф: ступенчатый склон — это память о ритме, в котором менялись условия. Тот же принцип, но в сжатом масштабе времени, посетитель тут же видит в соседнем овраге: распахали поле — изменился базис для ручейка — он принялся врезаться заново. Один механизм, два масштаба времени; разница в три порядка.

Редкая сила экспоната — в том, что **временну́ю шкалу проходишь ногами**. Это превращает абстракцию «эпохи долины» в физическое ощущение спуска.

### **2. Поперечное сечение**

**ОБЩАЯ СТРУКТУРА (одна фраза, без объектов):****  
** Меняется внешнее условие → прежнее равновесие нарушается → поток врезается заново к новому уровню → бывшая поверхность остаётся выше как ступень-память о переходе.

**ЯВЛЕНИЯ:**

1.  *Геоморфология, мезомасштаб* — речные террасы. Долина-река, тысячелетия. Базис: уровень моря/озера, в которое впадает река, плюс тектоника, плюс климат. Ступени — заброшенные поймы.
2.  *Антропогенная эрозия, микромасштаб* — овраг. Ручей-распаханное поле, десятилетия. Базис: уровень эрозии после распашки и потери дёрна. Ступенька в верховье оврага — та же терраса в миниатюре.
3.  *Морская геоморфология, макромасштаб* — морские террасы. Берег-океан, сотни тысяч лет. Базис: уровень моря, менявшийся в ледниковые циклы. Ступени — древние береговые линии на возвышенных побережьях (Крым, Белое море, Тихоокеанское побережье).

**НЕОЧЕВИДНАЯ СВЯЗЬ:****  
** Овраг и древняя речная терраса — один и тот же процесс на разных скоростях. То, что большой реке нужны тысячелетия, овраг проделывает за время человеческой жизни. Это не аналогия, а буквально один механизм; разница только в энергии потока и устойчивости пород.

**ОГРАНИЧЕНИЕ СЕЧЕНИЯ.** Все три явления — внутри одного домена (геоморфология). Кросс-доменных аналогов с тем же механизмом «эрозия → новое равновесие» не существует. Сечение работает не через смену доменов, а через **смену масштабов времени** — десятилетия / тысячелетия / сотни тысяч лет. Это честно и заявлено явно.

### **3. Конструкция**

**3.1 Общая схема.****  
** 3–4 столбика высотой 1,2–1,5 м, расставленные на разных высотных уровнях склона долины. Главный столбик — на верхней обзорной площадке, с расширенной табличкой \~50×40 см: заголовок, тексты А и Б, схема профиля, QR. Остальные столбики — на нижележащих ступенях, с малыми табличками \~20×15 см: номер уровня, высота над современным руслом, ориентировочный возраст (после геодезической верификации).

**3.2 Материалы.****  
** Столбики — нержавеющая сталь, толстостенная труба или квадрат. Таблички — стальная пластина с лазерной гравировкой (заполнение чёрной эмалью). Крепление — сварка к столбику. Антивандальное исполнение: никаких сменных табличек на болтах.

**3.3 Габариты и монтаж.****  
** Заглубление в грунт ≥ 0,6 м на бетонную пробку; верхний обрез столбика 1,2–1,5 м от земли (читается стоящему взрослому и подростку без нагибания). Малые таблички — на верхушке столбика, наклон 15° от вертикали для удобства чтения.

**3.4 Безопасность.****  
** Закруглённые кромки таблички (радиус ≥ 3 мм). Зона безопасности 1 м вокруг столбика без камней и обнажённых корней. Ограничений по возрасту нет; в гололёд верхняя площадка может требовать предупреждения, если уклон спуска к следующему столбику \> 15°.

### **4. Принцип работы**

1.  Посетитель подходит к главному столбику на верхней площадке.
2.  Читает уровень А — короткий хук с указанием спуститься. Смотрит вниз и видит современное русло.
3.  Читает уровень Б — рассуждение про базис равновесия и про то, почему склон ступенчатый, а не наклонный.
4.  Спускается на следующую ступень. На малой табличке — номер уровня и (если есть) высота с возрастом. Прикидывает разницу высот глазом.
5.  Доходит до самой нижней ступени, смотрит снизу вверх: видит всю лестницу долины и проделанный путь во времени.
6.  По указанию из уровня Б смотрит в сторону оврага — узнаёт тот же механизм в миниатюре.
7.  По желанию — сканирует QR и читает уровень В: как датируют террасы, что общего с морскими берегами и что наука пока не понимает.

Ключевое: посетитель **ногами проходит** временну́ю шкалу. Текст и пространство работают как один объект.

#### **5.1 Стенд, уровень А**

**Заголовок:** Ступенчатый склон долины

Когда уровень в водохранилище сбрасывают рывками, на бетонной стенке остаются горизонтальные полосы — отметки уровней, на которых вода держалась дольше всего перед очередным сбросом.

На склонах больших речных долин видны такие же следы, только намного крупнее: ровные площадки на разных высотах, вытянутые вдоль реки. Современное русло проходит ниже всех площадок.

Каждая площадка когда-то была дном реки или её поймой. Потом река опустилась ниже, а прежняя поверхность осталась на старой высоте. Чем выше площадка, тем она древнее. Эти ровные ступени называются речными террасами.

#### **5.2 Стенд, уровень Б**

**Заголовок:** Почему склон долины ступенчат

**Почему река держится на одной высоте?**

Главная характеристика реки на длинном участке — её **продольный уклон**: на сколько метров вода опускается на каждый километр пути. У равнинной реки это сантиметры на километр, у горной — метры.

Уклон не случаен. Если он слишком крут, поток несётся быстро и подмывает дно — река углубляет русло, и уклон уменьшается. Если слишком пологий, скорость падает, наносы выпадают на дно — русло поднимается, и уклон растёт. Со временем уклон выходит на ту величину, при которой эрозия и накопление наносов уравновешивают друг друга. Дальше река переносит наносы и не меняет своей высоты — она лишь медленно расширяет пойму вбок. Такое состояние называют **продольным профилем равновесия**.

У этой равновесной кривой есть нижняя точка — уровень, ниже которого река углубляться уже не может. Этот уровень называется **базисом эрозии**. Обычно базис — это уровень моря или озера, в которое впадает река. Базис смещается при тектоническом поднятии суши и при изменении климата; когда он сдвигается, прежний профиль перестаёт быть равновесным, и река начинает перестраивать русло.

**Что происходит, когда базис понижается?**

Прежний уклон оказывается избыточным. Река начинает врезаться в собственное дно и постепенно опускает русло к новому положению равновесия. Прежняя пойма остаётся на старой высоте, уже отделённая от потока.

Такие приподнятые остатки бывших пойм называются **речными террасами**. Серия террас на склоне — запись нескольких понижений базиса подряд: ровная площадка отвечает периоду покоя, уступ между площадками — этапу врезания.

**Тот же механизм в овраге**

В овраге всё разыгрывается по той же схеме, только в сжатом времени. После распашки и разрушения дернового покрова базис эрозии для местного ручья резко падает. За несколько десятилетий в верховье оврага вырастают маленькие ступеньки — та же лестница террас, в миниатюре. Большой реке на перестройку русла требуются тысячи лет; оврагу — десятилетия.

**Где этот рельеф работает**

На высоких речных террасах часто стоят старые города: Саратов, Нижний Новгород, Ярославль. Терраса выше паводкового уровня и при этом близка к реке — главной транспортной артерии прошлого. Опоры крупных мостов через большие реки уходят сквозь рыхлые аллювиальные слои террас вниз, к коренным породам: это самое надёжное основание.

*Схема рядом со столбиком: профиль долины в разрезе. Современное русло внизу, выше — две-три ступени; пунктиром обозначены прежние уровни реки.*

#### **5.4 QR — уровень В**

**Заголовок:** Террасы: происхождение, датировка, открытые вопросы

**Почему ступени, а не наклон?**

Ступенчатый профиль возникает не при любом понижении базиса, а только при таком, которое идёт неравномерно во времени. Если бы базис понижался плавно, река отслеживала бы его непрерывным углублением русла, и склон оказался бы просто наклонным, без площадок.

Ступени появляются тогда, когда длинные периоды покоя сменяются короткими этапами врезания. В период покоя река успевает выработать на достигнутом уровне широкую плоскую пойму. При очередном понижении базиса вода быстро прорезает себе путь вниз и оставляет прежнюю пойму нетронутой выше. Поэтому число террас отвечает числу крупных перестроек базиса, а их высоты и ширина — амплитуде каждой перестройки и длительности периодов покоя.

**Какие получаются скорости**

У Волги в районе Саратова сохранилось несколько террас; высота верхней — около ста метров, её возраст оценивают приблизительно в 500 тысяч лет. Средняя скорость врезания получается порядка 0,2 мм/год — тот же порядок, что и скорость тектонического поднятия древних платформ. Это согласие — косвенный довод в пользу того, что главная причина врезания на Русской равнине именно тектоническая.

**Как определяют возраст террас**

Возраст устанавливают несколькими независимыми методами; согласие результатов считается главным критерием надёжности.

*Радиоуглеродный анализ.* Работает по органическим остаткам — древесине, костям, углю — в речных отложениях. Применим для возрастов до 50 тысяч лет.

*Оптически стимулированная люминесценция.* Минералы в отложениях под действием природной радиоактивности постепенно накапливают сигнал, а солнечный свет полностью обнуляет его. Лабораторное измерение даёт время, прошедшее с момента, когда минерал в последний раз был на поверхности под солнцем и затем оказался похороненным под наносами. Та же физика лежит в основе термолюминесцентного датирования керамики в археологии: там сигнал обнуляется обжигом, а измеряется тысячелетия спустя.

*Корреляция с океаном.* Местные террасы сопоставляются с глобальной шкалой колебаний уровня океана, восстановленной по морским осадкам.

**Морские террасы**

На побережьях работает тот же механизм. В ледниковые эпохи уровень океана опускался на 100–120 метров, в межледниковья поднимался обратно. Эти циклы повторяются с периодом около 100 тысяч лет — следствие медленных изменений орбиты и наклона земной оси (циклы Миланковича). Каждое крупное изменение оставляло на побережье ровную полосу — морскую террасу. Серии таких террас сохранились в Крыму, на Белом море, на Тихоокеанском побережье Северной Америки. Принцип подстройки рельефа к меняющемуся базису одинаков для речной террасы, морской террасы и верховья оврага; различаются только источник изменений и характерное время процесса.

**Что остаётся неясным**

В каждой долине история террас складывается из двух наложенных причин. Первая — климат: он меняет объём стока и количество наносов и действует синхронно на больших территориях. Вторая — тектоника: медленные движения коры поднимают или опускают участки суши локально.

На масштабе континента эти вклады разделимы — климатический сигнал общий, тектонический локально-региональный. Но в одной долине, особенно для древних верхних террас, чьи возрасты лежат за пределом радиоуглеродного метода, расцепить вклады бывает крайне трудно. Одну и ту же лестницу можно реконструировать и как след смен климата, и как след тектонического поднятия. Поэтому реконструкции древнейших террас обычно даются с диапазоном возможных интерпретаций.

  
  

# ✅ Солнечные часы - WP020  

**DONE**

**WP020 — «Солнечные часы»**

Геопривязка: координаты точки — уточнить (см. п. 7). Локация: открытая площадка с гарантированным освещением Солнцем с 9 до 17 часов круглый год; локальный ориентир для полуденной тени выбирается обходом точки на этапе монтажа. Тип станции: малая наблюдательная (астрономия повседневного времени). Категория опыта: измерение по природному прибору с табличной поправкой к шкале. Круглогодичность: да, в светлое время суток в ясную или малооблачную погоду.

### **1. Замысел**

Посетитель находит час по тени гномона и сравнивает с телефоном. Расхождение — несколько минут, и оно заметно сразу. На табличке у часов — поправка на сегодняшнюю дату: прибавишь её к показаниям тени, и разница исчезнет в пределах минуты.

Из этого уносится одна мысль. Гномон измеряет истинное положение Солнца над этой точкой земного шара — он адресный прибор, собранный под широту места. Телефон показывает усреднённое поясное время — общее соглашение, в котором у всех в полосе шириной полторы тысячи километров часы идут синхронно. Между «истинно здесь» и «удобно для всех» — известная заранее, табличная разница.

Та же структура работает в любой паре «прибор — общая шкала». Стрелка компаса показывает магнитный полюс, карта ориентирована на полюс географический — разница называется магнитным склонением и записана в таблицах. Астрономические часы идут по реальному вращению Земли, атомные — равномерно; раз в несколько лет в гражданское время добавляют координационную секунду. Приливы в порту приходят в свой час по расписанию, фактический максимум смещается на десятки минут от ветра и давления — поправка тоже табличная.

Один принцип, разные домены. Прибор честен. Шкала — соглашение. Между ними — заранее посчитанная величина.

### **2. Поперечное сечение**

**Общая структура.** Прибор регистрирует истинное явление в этой точке → жизнь использует усреднённое значение для всех точек сразу → между ними остаётся известная заранее, табличная разница.

**Явления.**

1.  **Астрономия времени.** Солнечные часы показывают истинное местное солнечное время; гражданские часы — среднее поясное. Разница — уравнение времени плюс долготная поправка к меридиану пояса. Амплитуда до 16 минут плюс до получаса по долготе.
2.  **Навигация.** Стрелка магнитного компаса показывает направление на магнитный полюс; карта ориентирована на полюс географический. Разница — магнитное склонение: для Москвы около +11° к востоку, для Казани около +13°, для Иркутска около −2°. Значения медленно сдвигаются, потому что магнитный полюс дрейфует, и поправки приходится переиздавать.
3.  **Точное время.** Шкала UT1 определяется по реальному вращению Земли (с пассажными инструментами и квазарной интерферометрией); шкала UTC — атомными часами, идущими равномерно. Земля чуть замедляется, и раз в несколько лет в UTC добавляют «координационную секунду», чтобы две шкалы не разошлись больше чем на 0,9 с.
4.  **Геофизика приливов.** Расписание приливов в порту считается заранее по гармоническому анализу солнечной и лунной составляющих. Фактическое время максимума смещается от расписания на минуты или десятки минут из-за ветра, атмосферного давления, направления течения — для каждого порта известна типовая местная поправка.

**Неочевидная связь.** Уравнение времени для солнечных часов и магнитное склонение для компаса — родственные величины. В обоих случаях прибор измеряет физически честное направление, а карта или часы построены на «удобной» шкале; в обоих случаях разница известна заранее, табулирована и медленно меняется со временем. Школьный опыт с компасом и магнитным склонением закрывает уравнение времени без новых усилий — это и есть мост между доменами.

### **3. Конструкция**

#### **3.1 Общая схема**

  - Бетонный или каменный куб-основание стороной 60–80 см; верхняя грань строго горизонтальная — несущая база.
  - Стальной гномон-стержень или труба диаметром 25–40 мм, длина около 60 см. Наклон к горизонту равен широте места; ориентация — на небесный полюс (по Полярной звезде).
  - Стальной полукруг или полное кольцо циферблата диаметром 40–60 см; плоскость перпендикулярна гномону, то есть параллельна плоскости земного экватора. Часовые метки нанесены через равные 15° — это ключевое преимущество экваториальной конструкции: без формул, по транспортиру.
  - Информационная панель рядом, верх около 160 см над землёй: текст уровней А и Б, QR-код к уровню В, график уравнения времени, схема аналеммы, таблица поправок на шесть дат года.
  - Площадка из бетонной плитки размером около 2×2 м для подхода.

#### **3.2 Материалы**

  - Основание: бетон М300 или гранит/габбро.
  - Гномон: нержавеющая сталь AISI 304; кончик — шар-наконечник для безопасности (см. 3.4).
  - Циферблат: нержавеющая сталь, лазерная резка; метки — гравировка или пескоструй; обработка с обеих сторон, если кольцо двустороннее.
  - Информационная панель: металл с гравировкой или керамика с обжигом.
  - Крепёж гномона: сквозное анкерное, с возможностью юстировки ±2° при монтаже.

#### **3.3 Габариты и монтаж**

  - Заглубление фундамента — ниже глубины промерзания (для средней полосы не менее 1,5 м). Без этого гномон уйдёт за две-три зимы от пучения грунта, и часы начнут систематически врать. Это критический пункт.
  - Высота верха информационной панели около 160 см.
  - Угол гномона к горизонту — широта места, точность ±0,1°.
  - Ориентация гномона выставляется при монтаже двумя независимыми способами для контроля: ночью — по Полярной звезде, днём — по компасу с поправкой на магнитное склонение для этой точки (взять из IGRF-таблицы на год монтажа).
  - Сторона циферблата: с весеннего по осеннее равноденствие тень ложится на верхнюю плоскость, с осеннего по весеннее — на нижнюю. В дни равноденствий тень идёт по ребру и почти исчезает — это сам по себе наблюдаемый эффект, стоящий отдельной строки в тексте стенда.

#### **3.4 Безопасность**

  - Кончик гномона — шар-наконечник радиусом не менее 8 мм; иначе вылет острого конца должен быть гарантированно выше 180 см от уровня площадки. Уточнить на месте.
  - Кромки циферблата: радиус скругления не менее 3 мм.
  - Покрытие площадки нескользкое, рассчитанное на зимнюю эксплуатацию.
  - Снег с верхней плоскости циферблата зимой счищается вручную (общая практика для всей сети, отдельного инвентаря не требует).

### **4. Принцип работы**

Сценарий действий посетителя в порядке прохождения:

1.  Подходит к часам, видит наклонный стержень-гномон и стальной диск с метками через равные промежутки.
2.  Находит метку, к которой ближе всего ложится тень гномона; читает время.
3.  Сверяется с телефоном. Видит расхождение — обычно от 5 до 30 минут.
4.  Читает уровень А на стенде: телефон показывает усреднённое для всей широкой полосы страны, гномон — Солнце над этой точкой именно сейчас.
5.  Смотрит таблицу поправок на сегодняшнюю дату. Например: «15 ноября: тень опережает гражданское время на 16 минут плюс долготная поправка для этой точки».
6.  Прибавляет (или вычитает) поправку к показаниям тени. Получает время, совпадающее с телефоном в пределах минуты.
7.  Переходит к уровню Б: узнаёт, откуда взялась поправка — две причины складываются, орбита эллиптична, ось наклонена.
8.  По QR — уровень В: те же таблицы поправок есть у моряка с компасом, у астрономов с атомными часами, у диспетчера приливного порта.
9.  Дополнительно, в дни равноденствий: видит, что тень не ложится на циферблат, а идёт по ребру — заметный наблюдаемый эффект, который не требует отдельной конструкции.

### **5. Тексты**

Адаптировал тексты А и Б под стиль «Кванта»; уровень В причесал по тем же правилам, чтобы все три уровня звучали в одном регистре (если уровень В на QR трогать не нужно, опираться можно на исходную версию — он уже близко к запрошенному тону).

## **Стенд, уровень А**

### **Тень знает три ответа**

**Посмотрите на тень от гномона** — наклонного стержня в центре циферблата. Одна и та же тень умеет показывать сразу три вещи: который сейчас час, где север, и на какой широте мы стоим.

**Час.** Найдите метку, к которой ближе всего край тени. Это истинное местное солнечное время — момент, когда Солнце стоит над этой точкой земного шара. Прибавьте сегодняшнюю поправку из **таблицы внизу (НУЖНЫ ТОЧНЫЕ КООРДИНАТЫ МЕСТА УСТАНОВКИ, ТАБЛИЦА ЗАВИСИТ ОТ НИХ)** — получите время на телефоне. Расхождение обычно в несколько минут: Земля идёт по орбите неравномерно.

**Север.** Дождитесь местного полудня — момента, когда тень самая короткая за день. В этот момент она указывает строго на север. Без компаса, без поправок на магнитное склонение.

**Широта.** Длина полуденной тени в день летнего солнцестояния (около 22 июня) связана с географической широтой места. Двадцать три века назад этим воспользовался Эратосфен: палка, колодец и расстояние между двумя городами — большего для измерения окружности Земли ему не понадобилось.

## **Стенд, уровень Б**

### **Как палкой измерить планету**

#### **Опыт Эратосфена**

В III веке до н. э. директор Александрийской библиотеки Эратосфен Киренский измерил окружность Земли. Без телескопа и астрономических приборов — только палка, её тень и сообщение из соседнего города.

Эратосфен знал, что в Сиене (нынешний Асуан) в день летнего солнцестояния в полдень Солнце стоит точно над головой: вертикальные предметы не отбрасывают тени, а свет доходит до дна глубокого колодца. В тот же момент в Александрии, лежащей к северу, гномон отбрасывал короткую тень под углом 7,2° к вертикали.

#### **Что из этого следует**

Возьмите апельсин и воткните в него две спички — одну строго по радиусу в одну точку, вторую так же — в другую. Поднесите конструкцию к далёкой лампе. Спичка, направленная прямо на лампу, тени не даёт. Вторая, повёрнутая относительно первой, наклонена к лучам света и отбрасывает короткую тень. Угол между двумя спичками равен углу между двумя радиусами апельсина в этих точках.

Сиена и Александрия — две такие точки на «апельсине» Земли. Спичка в Сиене — без тени. Спичка в Александрии — с тенью под 7,2°. Значит, угол между радиусами Земли в этих точках тоже 7,2°.

Дальше — арифметика. Полная окружность — 360°. Дуга между двумя городами — 7,2°, то есть 1/50 окружности. Расстояние между Сиеной и Александрией, известное по караванным путям, — около 5000 стадий, примерно 800 километров. Вся окружность Земли — около 40 000 километров. Современное значение — 40 075 километров. Ошибка эксперимента меньше 1%.

#### **Откуда берутся три ответа тени**

Эратосфен использовал длину тени, чтобы вычислить разницу широт. Но у тени есть ещё два независимых свойства, и на них держится остальная история измерения мира.

**Длина тени → широта.** Высота Солнца над горизонтом в полдень зависит от даты и широты места. Зная одно и измерив другое, можно вычислить третье. По этому принципу до конца XX века моряки определяли своё положение в открытом океане секстантом.

**Направление тени → стороны света.** В полдень Солнце оказывается ровно на юге, и тень указывает строго на север. У такой стрелки есть преимущество перед магнитной: она показывает истинный географический север, без поправки на магнитное склонение (в Москве оно сейчас около 11°). Этим способом египтяне выравнивали основания пирамид с точностью до десятой доли градуса.

**Скорость поворота тени → время.** Земля поворачивается на 15° в час. Если плоскость циферблата параллельна экватору, а гномон — оси вращения Земли, тень на циферблате поворачивается с той же равномерной скоростью. Поэтому метки можно нанести через равные углы — как сделано на этих часах.

#### **Где принцип работает сегодня**

Археологи восстанавливают возраст древних сооружений по точности их астрономической ориентации: Стоунхендж в Англии указывает на точку восхода Солнца в день летнего солнцестояния, пирамида Кукулькана в Чичен-Ице построена так, что в дни равноденствий тень на её ступенях складывается в фигуру ползущей змеи. Спутниковая навигация GPS опирается на ту же геометрическую идею, что и Эратосфен: измеряется время прихода сигнала от спутников с известным положением, и по геометрии вычисляется собственная координата. Палку заменили атомные часы, тень — радиоволна, но принцип остался прежним: время плюс геометрия равно место.

## **QR — уровень В**

### **Палка, тень и измерение мира**

#### **Тень как часы: геометрия экваториального циферблата**

**Почему метки идут через равные углы?** Земля поворачивается равномерно — 15° в час. Если плоскость циферблата параллельна плоскости земного экватора, а гномон параллелен оси вращения, тень на циферблате поворачивается ровно с той же скоростью, что и Земля.

Понять это проще мысленным переносом. Представьте, что вы стоите на Северном полюсе. Ось Земли направлена вертикально, а горизонтальная плоскость под ногами параллельна плоскости экватора. Палка, воткнутая в полюс, отбрасывает тень, которая поворачивается абсолютно равномерно: Земля делает 15° в час, и тень делает то же самое. Из любой другой точки шара эту равномерность можно сохранить, наклонив весь чертёж так, чтобы циферблат остался параллельным экватору, а гномон — оси Земли. Экваториальный циферблат и есть такой наклонённый чертёж — он повторяет геометрию вращения Земли в уменьшенном масштабе.

**Горизонтальные часы.** У них циферблат лежит на земле. Тень в экваториальной плоскости по-прежнему поворачивается равномерно — на 15° в час. Но её проекция на горизонт уже неравномерна: у полудня тень за час сдвигается на меньший угол, к утру и вечеру — на больший. На циферблате это видно так: метки чаще к полудню и реже к краям. Угол метки α от полуденной линии даётся формулой

tg α = sin φ · tg H,

где φ — широта места, H — часовой угол Солнца (по 15° за каждый час от полудня). На экваторе горизонтальный циферблат не работает: ось Земли лежит в плоскости циферблата, и часовые линии соединяются в одну . По той же геометрии мастер строит вертикальный циферблат на стене дома — меняются только углы.

#### **Тень как компас: ориентирование по полуденной тени**

Этот способ ориентирования старше магнитного компаса на тысячи лет. Им пользовались египетские землемеры, выравнивая основания пирамид по линии «север–юг»: отклонение пирамиды Хеопса от истинного направления — меньше десятой доли градуса. Финикийские мореплаватели определяли курс по полуденной тени задолго до того, как магнитный компас дошёл из Китая в Средиземноморье.

Магнитная стрелка указывает на магнитный полюс, не совпадающий с географическим: угол между ними называется магнитным склонением. В Москве оно сейчас около +11° к востоку, в Казани около +13°, в Иркутске около −2°. Магнитный полюс к тому же дрейфует — за последнее столетие сместился из канадской Арктики в сторону Сибири, и Международный геомагнитный эталон IGRF переиздаётся раз в пять лет. Полуденная тень от поправок свободна.

Как поймать момент местного полудня без часов? Замечайте длину тени в течение дня и фиксируйте кратчайшую — это и есть истинный полдень в данной точке.

#### **Тень как координата: подробнее об опыте Эратосфена**

Высота Солнца над горизонтом в полдень рассчитывается по формуле

h = 90° − φ + δ,

где φ — географическая широта места, δ — склонение Солнца (угол его подъёма над небесным экватором, меняющийся от −23,5° зимой до +23,5° летом).

В день летнего солнцестояния δ = +23,5°. На широте Тропика Рака (φ = 23,5°) высота получается ровно 90°: Солнце в зените, вертикальные предметы тени не отбрасывают. Севернее тень появляется и удлиняется. Сиена лежала почти точно на Тропике, Александрия — севернее на 7,2°.

Главная сила опыта — в правильности допущений. Эратосфен принял, что Земля — шар, что Солнце далеко (и его лучи в обеих точках параллельны) и что разница углов Солнца равна разнице широт. Все три допущения оказались верны с высокой точностью; ошибка в окружности Земли — меньше одного процента.

#### **Современная навигация**

**Астрономическая навигация.** До появления GPS моряки и геодезисты определяли координаты по высоте Солнца и звёзд. Высоту измеряли секстантом — портативным прибором, дающим угловое расстояние от светила до горизонта. По высоте Солнца в полдень или Полярной звезды ночью определяли широту. Для долготы нужен был ещё один прибор — точный морской хронометр, идущий по гринвичскому времени: долгота вычислялась по разнице между местным временем (моментом полудня на корабле) и показанием хронометра. Точность хорошего секстанта — до угловой минуты, что в паре с хронометром даёт ошибку места около километра. Задачу долготы на море впервые решил английский часовщик Джон Гаррисон в середине XVIII века; до его хронометра долгота оставалась нерешаемой инструментально, и корабли регулярно теряли курс на сотни миль. В морском флоте астрономическая навигация оставалась резервным методом до конца XX века.

  

**GPS.** Спутниковая навигация работает по той же геометрической идее, что и Эратосфен, только обратным ходом. Спутники с атомными часами на орбите посылают сигналы; приёмник измеряет время прихода сигналов от нескольких спутников и вычисляет собственное положение. Принцип прежний: время плюс геометрия равно координата. Точность гражданского GPS — около 3 метров, геодезического — до миллиметров.

**Археоастрономия.** Многие древние сооружения ориентированы по точкам восхода Солнца в дни солнцестояний и равноденствий. По этой ориентации археологи восстанавливают возраст и культурный контекст сооружений — от европейских мегалитов до сибирских степных календарей.

#### **Открытый вопрос**

Палка с тенью даёт три измерения — направление, координату, время. Можно ли получить четвёртое?

В природе такой пример уже есть. Пчёлы ориентируются по поляризации света неба даже в пасмурную погоду, когда самого Солнца не видно. Свет, рассеянный молекулами атмосферы, поляризуется характерным образом относительно направления на Солнце; у пчелы в верхней части глаза есть поляризационный фильтр, и она «считывает» направление, как мы считываем время по тени. Эту способность открыл австрийский этолог Карл фон Фриш в середине XX века.

Современные навигационные приборы для облачной погоды и движения под водой используют тот же принцип — поляризационные компасы. Палка не нужна, нужен фильтр в глазу или на сенсоре. Логика остаётся прежней: измерь геометрический параметр света — узнай место.

Какое ещё свойство солнечного света можно превратить в прибор?

  
  
  

# ✅ Обсерватория - WP011  

**Done**

**WP011 — «Обсерватория без крыши»**

Геопривязка: Лысая Гора у Приюта №5; широта ≈ 55° с. ш.; абсолютная высота 170 м, 117 м над окружающей равниной  
 Тип станции: видовая, малая  
 Категория опыта: измерение, парное сравнение (день / ночь)  
 Круглогодичность: да, ночной режим работает в безоблачную погоду

### **1. Замысел**

Здесь у мира действительно виден край: с открытой вершины горизонт лежит на конкретном расстоянии, которое можно сосчитать в уме. Посетитель встаёт за топоскоп, поворачивается вокруг него и сличает гравированный контур с реальной панорамой — так становится видно, что граница видимости в каждую сторону одна и та же и определяется только высотой глаз над равниной. Вечером тот же холм превращается в обсерваторию: посетитель находит Полярную над северным горизонтом, прикладывает кулак за кулаком от горизонта вверх — и считывает свою широту прямо со звёздного неба.

Главная мысль уносится одна. Положение наблюдателя на земном шаре — и линия горизонта, и его широта — определяется одной геометрией: касательной к шару. Днём она проведена к поверхности и даёт дальность; ночью — вдоль земной оси и даёт широту. Один треугольник, повёрнутый дважды. Тот же чертёж, что лежит в начале и геодезии, и навигации, и кругосветных плаваний.

### **2. Поперечное сечение**

**ОБЩАЯ СТРУКТУРА:** положение наблюдателя на шаре определяется касательной к этому шару — её длина задаёт расстояние, её угол задаёт координату.

**ЯВЛЕНИЯ:**

1.  Геометрия — расстояние до горизонта d ≈ √(2Rh): касательная от глаза к поверхности, прямоугольный треугольник с радиусом.
2.  Астрономия — высота Полярной над горизонтом = географическая широта: та же касательная, проведённая к оси мира, а не к поверхности.
3.  Физика атмосферы — реальный коэффициент видимости 3,86 вместо геометрического 3,57: добавка к чистой геометрии из-за рефракции в плотных нижних слоях воздуха.
4.  История науки — измерение окружности Земли Эратосфеном: тот же треугольник, измеренный в двух точках одновременно, даёт радиус.
5.  История инженерии — задача долготы и морской хронометр Гаррисона (1761): широта берётся за час по звёздам, а долгота — век дожидалась часов, которые шли точно на качающейся палубе.

**НЕОЧЕВИДНАЯ СВЯЗЬ:** морской хронометр. Это сюжет не о звёздах, а о точности времени — но он входит в то же сечение, потому что вторая координата на шаре строится через тот же чертёж, только время в нём заменяет одну из сторон.

### **3. Конструкция**

#### **3.1 Общая схема**

  - **Панорамный топоскоп** — горизонтальный диск Ø 80 см на бетонном постаменте высотой 1,1 м.
  - **Двусторонний информационный стенд** — две стороны формата ≈ A1, размещён вплотную к топоскопу.
  - **QR-табличка** в нижней части стенда — лазерная гравировка по нержавеющей пластине.
  - **Флюгер** — вынесен на 5–7 м от станции, оформляется как самостоятельный микрообъект и в сюжет обсерватории не включается (см. правку 3 в аудите).

#### **3.2 Материалы**

|  |  |  |  |
| :-: | :-: | :-: | :-: |
| \*\*Узел\*\* | \*\*Материал\*\* | \*\*Покрытие\*\* | \*\*Крепёж\*\* |
| Диск топоскопа | Нержавейка AISI 304, 8 мм, или чугун | Травление + порошковая краска для контрастности линий | Скрытое анкерное крепление к закладной плите |
| Постамент | Армированный бетон | Гидрофобная пропитка | Заглубление на 0,8 м |
| Стенд (полотно) | HPL компакт 10 мм или сталь 3 мм | УФ-печать (HPL) / порошок + лазер (сталь) | Скрытые антивандальные винты Torx с пином |
| Стойки стенда | Профиль 80×80×4 мм | Горячее цинкование + порошок | Заглубление 0,8 м |
| QR-табличка | Нержавейка 2 мм | Глубокая лазерная гравировка | Сварка к нижней раме стенда |

#### **3.3 Габариты и монтаж**

  - Топоскоп: верх диска на высоте 1,1 м (доступно ребёнку с 8 лет и взрослому).
  - Стенд: общая высота 2,0 м, нижний край полотна на 0,9 м над землёй.
  - Ориентация диска по сторонам света — допуск не хуже ±1° (контроль буссолью и GPS-привязкой при монтаже).
  - Площадка вокруг — гравий или мощение в радиусе 1,5 м.

#### **3.4 Безопасность**

  - Все кромки диска и стенда — снятие фасок ≥ 2 мм.
  - Постамент без острых углов, верхний срез скруглён.
  - Зона безопасности 1,5 м — без посадок и временных конструкций.
  - Расчётная нагрузка на стенд — 80 кг бокового толчка без сдвига.

#### **3.5 Дополнения на топоскоп (по правке 4)**

  - **Концентрические сектора** на диске с подписями дальности горизонта для разных высот глаз над равниной: «h = 1,5 м → 4 км», «h = 117 м (стоя на этой площадке) → 38 км», «с самолёта на 10 км → 360 км». Это переводит диск из таблички в инструмент.
  - **Северный сектор** с гравированной шкалой высоты Полярной: «на этой широте — 55°» и пунктиром указанием направления «лесенки кулаков».

### **4. Принцип работы**

1.  Посетитель подходит к топоскопу со стороны западного сектора.
2.  Смотрит на гравированный контур и переводит взгляд на реальный горизонт; находит соответствие.
3.  Поворачивается вокруг топоскопа на 360°, сверяя контур с панорамой и читая подписи направлений (расстояний до знакомых ориентиров).
4.  Замечает на диске сектор «h → дальность» и понимает, что граница видимости — не свойство глаза, а функция высоты.
5.  Читает уровень А стороны 1 (хук + образ), затем уровень Б (вывод формулы) — если интересно.
6.  Вечером возвращается; находит Полярную над северной частью горизонта по гравированной отметке на северном секторе диска.
7.  Вытягивает руку, ставит кулак нижним ребром на горизонт, поднимает второй кулак, третий — считает «шаги» до Полярной.
8.  Получает ≈ 5,5 кулаков и сличает с числом 55° на стенде. Понимает, что только что измерил свою географическую широту.
9.  Читает уровень Б стороны 2 — узнаёт, что это та же геометрия, повёрнутая.
10. Сканирует QR; уносит сюжет про Эратосфена, хронометр и форму Земли.

## **5.1 Сторона 1 — «Почему у мира есть край»**

### **Уровень А**

Почему с балкона девятого этажа видно дальше, чем со двора, а с самолёта — дальше, чем с балкона? Земля —  шарообразная, и её поверхность изгибается. Чем выше глаз наблюдателя, тем больше удалённых её участков остаётся в пределах прямой видимости; всё, что лежит дальше, скрыто за выпуклостью шара.

Эта граница видимости и называется *горизонтом*. Со ста семнадцати метров над окружающей равниной она проходит примерно в 38 километрах от точки наблюдения; это расстояние можно сверить с гравированной шкалой топоскопа.

### **Уровень Б — «Откуда берётся 38 километров»**

Сначала представим Землю как идеальный шар, а наблюдение — как геометрически «чистую» задачу: никакие препятствия, атмосфера или искажения света не мешают, а видимость определяется только формой поверхности. 

**Геометрия задачи.** Наблюдатель находится на высоте h над поверхностью. Самая дальняя видимая точка лежит там, где поверхность Земли «уходит из-под взгляда» — то есть в точке касания. Соединим три точки: центр Земли, точку касания и точку наблюдателя. Получается треугольник со сторонами R, d и R + h. Радиус всегда перпендикулярен касательной — значит, треугольник прямоугольный, и прямой угол приходится на точку касания. 

**Расчёт.** По теореме Пифагора

    d² = (R + h)² − R² = 2Rh + h².

При h порядка сотен метров и R = 6371 км слагаемое h² на много порядков меньше произведения 2Rh, и им можно пренебречь. После подстановки радиуса и приведения единиц задача сводится к удобной формуле

    d ≈ 3,57·√h   (d в километрах, h в метрах).

При h = 117 м (высота точки наблюдения над окружающей равниной, на которой расположен Приют №5 ) она даёт d ≈ 38,6 км.

**Поправка на воздух.** В реальности видимость заметно превышает геометрическую: эмпирический коэффициент ближе к 3,86. Воздух у поверхности плотнее, чем в верхних слоях; показатель преломления плавно убывает с высотой, и горизонтальный световой луч в таком градиенте слабо искривляется вниз. Геометрически это означает, что луч огибает не реальную Землю, а воображаемый шар большего радиуса. Эффект называется *атмосферной рефракцией* и меняется с температурой и давлением воздуха.

**Где работает та же формула.** Через √(2Rh) рассчитывают радиогоризонт антенн связи и радиолокаторов, дальность видимости огней маяков с моря, расстояние, на котором с борта самолёта на крейсерской высоте десять километров земная поверхность ещё различима — около 360 километров.

## **5.2 Сторона 2 — «Высота звезды равна вашей широте»**

### **Уровень А**

Можно ли измерить угол на небе, не имея при себе никакого прибора? можно. Кулак вытянутой руки закрывает примерно десять градусов небесной сферы; пропорции тела устроены так, что это соотношение у разных людей почти одинаковое.

**Что делать.** Найдите Полярную звезду над северной частью горизонта. Поставьте основание кулака на линию горизонта; поднимите второй кулак над первым, третий — над вторым, и так далее, пока не дойдёте до самой звезды. На этой широте в столбце поместится ровно пять с половиной кулаков, то есть высота Полярной над горизонтом составит около 55°.

**Что это число означает.** Оно совпадает с географической широтой места наблюдения, и совпадение не случайно: высота Полярной над горизонтом и широта наблюдателя — один и тот же угол, измеренный двумя разными способами. Подробности — ниже.

Метод позволяет определить широту любой точки в северном полушарии без приборов и карт; им пользовались мореплаватели за столетия до появления секстанта.

### **Уровень Б — «Тот же треугольник, повёрнутый»**

Чертёж со стороны 1 — прямоугольный треугольник с радиусом Земли и касательной к её поверхности — описывал расстояние до горизонта. Тот же чертёж, повёрнутый и наложенный на ночное небо, объясняет, почему высота Полярной звезды над горизонтом совпадает с географической широтой места.

**Что такое широта.** Географическая широта φ — это угол между плоскостью земного экватора и направлением из центра Земли в точку стояния наблюдателя.

**Что такое направление на Полярную.** Ось вращения Земли указывает почти точно на Полярную звезду. Расстояние до неё больше радиуса Земли в сотни миллиардов раз, поэтому направление на Полярную из любой точки поверхности можно считать параллельным земной оси.

**Геометрия совпадения.** Местная вертикаль в точке наблюдения совпадает с радиусом, проведённым из центра Земли в эту точку. Угол между местной вертикалью и направлением на Полярную равен, следовательно, углу между земной осью и тем же радиусом, то есть 90° − φ. Высота Полярной над горизонтом по определению равна 90° за вычетом её углового отклонения от вертикали, и в итоге даёт ровно φ.

**Одна геометрия — две задачи навигации.** Прямоугольный треугольник с радиусом Земли и касательной к её поверхности решает обе задачи определения положения по широте: днём по высоте наблюдателя над равниной он даёт расстояние до горизонта, ночью по высоте Полярной над линией горизонта — географическую широту.

Так определяется только одна из двух координат. Вторая, долгота, требует одновременного знания местного и опорного времени и потому веками оставалась нерешённой задачей мореплавания. Её решили в XVIII веке морским хронометром Гаррисона — об этом за QR.

### **5.3 Уровень В (за QR)**

#### **1. Почему коэффициент 3,57, а на практике 3,86**

**Почему получается больше геометрического.** Видимость в спокойной атмосфере примерно на восемь процентов превышает чисто геометрическую оценку. Причина — атмосферная рефракция. Воздух у поверхности Земли плотнее, чем в верхних слоях; показатель преломления с высотой плавно убывает, и горизонтальный световой луч в таком градиенте слабо искривляется вниз. Геометрически это эквивалентно тому, что свет распространяется вдоль не реальной Земли, а воображаемого шара большего радиуса. В стандартной модели атмосферы этот эффективный радиус оценивают как (7/6)·R, и предел видимости увеличивается в √(7/6) ≈ 1,08 раза — отсюда коэффициент около 3,86 вместо геометрических 3,57.

**Когда поправка перестаёт быть малой.** В обычных условиях, когда температура воздуха плавно убывает с высотой, рефракция близка к стандартной — те самые восемь процентов добавки к геометрической дальности. Картина меняется при температурной инверсии: если у поверхности лежит слой холодного воздуха, а над ним — более тёплый, градиент плотности усиливается и сильнее загибает горизонтальный луч вниз. Иногда настолько, что наблюдатель видит над горизонтом предметы, уже скрытые за ним геометрически. В навигационной литературе это явление называют *верхним миражом*, или «поднятыми кораблями».

На Луне, у которой атмосферы практически нет, рефракционной поправки тоже нет; видимое расстояние до горизонта совпадает с чисто геометрическим. Но сам этот коэффициент уже другой: радиус Луны около 1737 км, и формула √(2Rh) даёт d ≈ 1,86·√h. С высоты человеческого роста горизонт на Луне лежит примерно в 2,5 километра, тогда как на Земле — около 5 километров. Геометрия та же, радиус меньше.

#### **2. Одна геометрия — три задачи**

Во всех трёх классических задачах работают одни и те же геометрические элементы: шар с радиусом R, точка наблюдателя на его поверхности (или над ней), радиус, проведённый в эту точку, и касательная плоскость к шару в этой точке — она же местный горизонт.

**Расстояние до горизонта.** Наблюдатель находится на высоте h над поверхностью. Прямая, проходящая через точку наблюдателя и касающаяся шара в одной точке, отсекает на поверхности самый дальний видимый участок. Длина касательного отрезка от наблюдателя до точки касания и есть искомая дальность горизонта: d ≈ √(2Rh).

**Широта по звёздам.** Полярная звезда лежит почти точно на оси вращения Земли и удалена настолько, что направление на неё из любой точки поверхности можно считать параллельным самой оси. Высота Полярной над горизонтом — это угол между направлением на неё и плоскостью местного горизонта. Геометрически он совпадает с углом между радиусом, проведённым в точку наблюдения, и плоскостью земного экватора, — то есть с географической широтой.

**Окружность Земли.** Если в двух точках, лежащих на одном меридиане, одновременно измерить угол между местной вертикалью и направлением на одну и ту же звезду (или на Солнце в полдень), разность измеренных углов окажется равна угловому расстоянию между точками, видимому из центра планеты. Зная линейное расстояние между точками, можно вычислить радиус Земли. Этим способом её радиус впервые измерил Эратосфен

  

### **3. Как это узнаёт наука**

**Эратосфен и радиус Земли.** Около 240 года до нашей эры Эратосфен Киренский заметил, что в день летнего солнцестояния в Сиене (современный Асуан) полуденное Солнце проходит через зенит — его лучи освещают дно глубоких колодцев. В Александрии, лежащей примерно на том же меридиане севернее Сиены, в тот же момент Солнце отклоняется от вертикали на угол, который Эратосфен измерил по тени гномона. Угол составил около 7°12′, или 1/50 полной окружности. Зная расстояние между городами по дорожным измерениям, Эратосфен получил длину земного меридиана; современные оценки показывают, что точность его результата была около одного процента.

**Задача долготы.** Долгота численно равна разности местного времени и времени в опорной точке, переведённой в угловое расстояние из расчёта 15° на час. Местное время определяется по Солнцу или звёздам непосредственно; время опорной точки приходится знать заранее и сохранять с высокой точностью на протяжении всего рейса. До XVIII века часов, способных надёжно работать в условиях морской качки, перепадов температуры и влажности, не существовало. Их создал английский часовщик Джон Гаррисон: его модель H4, прошедшая испытания в трансатлантическом плавании 1761–1762 годов, накопила ошибку менее одной минуты за 81 день пути.

**Современная геодезия.** Положение точки на поверхности Земли определяют по сигналам спутниковых группировок (GPS, ГЛОНАСС, Galileo, BeiDou) и по лазерной дальнометрии до орбитальных мишеней. Типичная точность — несколько метров для подвижного приёмника и единицы миллиметров для геодезических опорных пунктов мирового каталога.

### **4. Что наука уточняет до сих пор**

**Геоид вместо шара.** Реальная фигура Земли отличается и от математического шара, и от эллипсоида вращения. Усреднённое представление о её форме называют *геоидом*: это поверхность постоянного потенциала силы тяжести, совпадающая со средним уровнем океанов в отсутствие приливов, ветров и течений. Геоид отклоняется от опорного эллипсоида на десятки метров — вверх над избытками массы (горные системы, плотные блоки коры) и вниз над дефицитами (океанические впадины, разрежённые области мантии). Карту этих отклонений строят по измерениям спутниковых миссий GRACE и GOCE, регистрирующих микроскопические возмущения собственного орбитального движения. Обратная задача — однозначная реконструкция распределения масс в недрах Земли по гравитационному полю на её поверхности — математически некорректна и остаётся одним из активных направлений геофизики.

**Прецессия и смена полярных звёзд.** Полярная звезда занимает положение вблизи северного полюса мира лишь временно. Земная ось совершает *прецессию* — медленное движение по конусу с углом раствора 23,4° и периодом около 25 800 лет, вызванное лунно-солнечным влиянием на экваториальное вздутие планеты. В эпоху строительства пирамид Гизы (около 2700 года до нашей эры) полярной звездой была Тубан в созвездии Дракона. К началу VI тысячелетия нашей эры на её место выйдет Альраи (γ Цефея); через несколько тысячелетий её сменят Денеб и Вега. Совпадение оси вращения с яркой звездой северного неба, удобное для астрономической навигации, в исторических масштабах оказывается скорее исключением, чем правилом.

  
  
  

# ✅ Белая берёза - WP017  

# **Done**

# **WP017 «Белая берёза»** 

### **Базовые данные**

|  |  |
| :-: | :-: |
|   |   |
| ID | 26 |
| Код объекта | WP017 |
| Рабочее название | Белая берёза |
| Тип станции | Столбик с табличкой + тактильный модуль сравнения |
| Геометка | \<https://yandex.ru/maps/-/CPe7ZHYg\> |
| Тип научного рассуждения | Повторяющийся закон (альбедо) |
| Связь с маршрутом | Парный объект к WP016 (водосбор); принципиальная разведёнка сюжетов |

### **Замысел**

Берёза в средней полосе настолько привычна, что её цвет перестают замечать. Объект возвращает белизне коры статус загадки и тут же отвечает на неё одним общим законом, который посетитель проверяет собственной рукой за пять секунд. После этого тот же закон узнаётся в крышах греческих домов, в полярных льдах и в любой светлой поверхности под солнцем. Сечение — альбедо. Парный объект WP016 разводит сюжеты: там — про воду в ландшафте, здесь — про свет и тепло на поверхности.

### **Принцип работы**

Посетитель кладёт ладонь на белую пластину рядом со стволом, потом на тёмную пластину того же размера и материала. Разница температур ощутима за несколько секунд. Дальше он переносит это наблюдение на саму кору и на рельефный срез ствола, где видно, какой именно слой берёза защищает белизной. Текст уровня Б связывает этот частный случай с общим законом и тремя его проявлениями за пределами биологии; QR ведёт в открытый вопрос — какой будет роль белой коры в более тёплом климате.

### **Конструкция**

Основной модуль. Стальная стойка высотой 1,1–1,2 м, забетонированная в основание. Двусторонняя табличка на нержавеющей стали с гравировкой (или HPL с защитным покрытием). Антивандальный крепёж.

Три встроенных тактильных элемента:

1.  Белая пластина «как кора берёзы». Фрагмент стабилизированной берёзовой коры под прозрачной защитой или реалистичная литая копия с фактурой. Матовая, светлая, без бликов.
2.  Тёмная парная пластина — тот же материал и толщина, но тёмного цвета (имитация коры дуба или окрашенный металл). Размещена рядом с белой, на том же уровне, под тем же углом к солнцу. Расстояние между ними — около ладони, чтобы человек сравнивал одновременно.
3.  Рельефный срез ствола с ясно читаемыми слоями: внешняя кора, внутренняя кора, камбий, древесина. Литой металл или композит с разной фактурой слоёв, чтобы их можно было различить на ощупь. На срезе — стрелка от солнца к внешнему слою коры и короткая надпись «здесь свет отражается».

Привязка к месту. Живая берёза рядом — плюс: посетитель переносит наблюдение на реальное дерево. Но экспонат полноценен и без неё: зимой без листвы, при смене дерева или если конкретная берёза со временем погибнет.

### **Сторона 1 — основная**

**Заголовок:** Почему берёза белая, а дуб — нет?

**Основной текст (уровень А):**

В жаркий день в чёрной футболке человек быстрее перегревается и сильнее ощущает жар, чем в белой. То же самое можно почувствовать прямо здесь. 

Приложи ладонь к белой пластине, потом к тёмной рядом — за несколько секунд разница температур становится отчётливой.

Светлая поверхность отражает большую часть падающих лучей, тёмная поглощает их почти полностью и быстро нагревается. Кора берёзы светлее, чем у большинства деревьев средней полосы, и под прямыми лучами она остаётся заметно прохладнее дубовой или сосновой.

Тонкий светлый слой на наружной стороне рельефного среза — это внешняя кора. Она и работает экраном: отражает свет, защищая живые ткани под собой.

### **Уровень Б**

**Заголовок:** Один закон — от коры до ледника

**Почему светлая кора выгодна именно зимой?**

Казалось бы, в холодное время перегрев дереву не грозит. Но именно зимой светлый цвет приносит главную пользу. В морозный солнечный день солнце стоит низко и светит почти горизонтально, а лежащий под деревом снег добавляет отражённый поток снизу. Тёмный ствол за такой день успевает заметно нагреться, а к ночи быстро остывает. От резких перепадов температуры кора растрескивается, и вместе с ней повреждается камбий — тонкий слой живых клеток, за счёт которого нарастает древесина. Светлая кора отражает большую часть лучей и нагревается слабее, поэтому суточные перепады температуры внутри ствола сглажены.

Долю света, которую поверхность отражает обратно, называют альбедо.

**Где работает тот же закон**

Жители Средиземноморья белят крыши и стены, чтобы дома не перегревались летом. Высокогорные ледники с высоким альбедо тают медленнее тёмных скал и держатся дольше. Принцип одинаков для коры, крыши и ледника: отражённый свет поверхность не нагревает.

А сам белый цвет коре даёт бетулин — вещество её внешнего слоя. Кроме окраски, он работает как антисептик, поэтому грибки и насекомые осваивают берёзовую кору с трудом.

### **Текст QR-кода**

**Заголовок:** Где белизна перестаёт быть преимуществом?

**Углубление (уровень В):**

Альбедо — это коэффициент отражения поверхности, значение от 0 до 1. У свежего снега оно достигает 0,9: поверхность возвращает обратно почти весь падающий свет. У сухого асфальта — около 0,1. Белая берёзовая кора по этому показателю стоит ближе к снегу, чем к коре других деревьев.

Поглощённая поверхностью мощность равна (1 − α) · P, где α — альбедо, а P — падающий поток. Чем α ближе к единице, тем меньше остаётся на нагрев. На этом простом соотношении держатся современные климатические модели: спутники постоянно измеряют альбедо льдов, пустынь и лесов, чтобы оценить, сколько солнечной энергии Земля поглощает и сколько возвращает в космос.

**Почему белая кора появилась именно здесь** 

Почти все виды берёзы растут в умеренной и северной зонах, где разом действуют три фактора: яркое солнце, отражающий свет снег и большие суточные перепады температуры. Несколько видов добрались до субтропиков и тропических гор Азии. В широтах, где этих условий нет, белая кора преимуществ не даёт, и эволюция её не закрепила.

Дубы, ели и клёны живут в тех же северных широтах, но защищаются иначе: у них толстая кора и значительный запас живых тканей внутри ствола. Закон отражения работает для любой поверхности; разные растения просто выбрали разные стратегии защиты.

**Как это измеряют**

Альбедо определяют фотометром: прибор сравнивает падающий и отражённый световые потоки. Температуру коры на солнце и в тени фиксируют термопарами, заведёнными под её внешний слой.

**Открытый вопрос**

Если белизна — это защита от перегрева, почему не все северные деревья светлые? И сохранится ли её значение в климате, где зимы становятся теплее, а суточные перепады мягче?

  
  
  

# ✅ Спутниковая навигация WP026  

## **DONE**

## **WP026. Спутниковая навигация без магии**

### **Замысел**

Точка, в которой у посетителя оказываются сразу две координаты одного и того же места: «земная», выгравированная на латуни прямо у него под ногами, и «небесная», горящая в этот момент на экране его смартфона. Эти координаты практически совпадают, и за их совпадением скрывается работа целого созвездия спутников, атомных часов на орбите и одной фундаментальной размерной связи, из которой и выводится вся точность системы: свет проходит 30 сантиметров за одну наносекунду.

Объект устанавливается на границе открытого неба и кроны дерева или навеса с таким расчётом, чтобы посетитель мог сделать несколько шагов в сторону и оказаться в тени для радиосигнала. Получившийся натурный эксперимент позволяет ему собственными глазами увидеть на экране, как число видимых спутников уменьшается, а заявленная точность определения координат, наоборот, растёт в метрах.

### **Принцип работы**

В основе работы спутниковой навигации лежит универсальный приём, который встречается всюду, где требуется измерить расстояние, не имея возможности дотянуться до объекта непосредственно. Суть приёма заключается в том, чтобы послать сигнал, засечь время его распространения и, умножив это время на известную скорость, получить искомое расстояние. Тот же приём используется летучей мышью, которая посылает ультразвук и по времени возвращения эха определяет положение мотылька. Им же пользуются сейсмологи, устанавливающие эпицентр землетрясения по разнице времён прихода волны на три удалённые станции. К нему прибегает и любой человек, который после вспышки молнии считает секунды до раската грома, чтобы прикинуть расстояние до грозы.

Спутниковая навигация работает по той же логике, однако со скоростью света вместо скорости звука и с атомными часами вместо счёта секунд про себя. Именно этот скачок в точности измерения времени и доводит универсальный приём до его физического предела.

### **Конструктив**

Объект состоит из вертикального столбика и металлического кружка в мощении. Столбик высотой 1,2 м выполнен из оцинкованной стали и устанавливается у точки с открытым небом и хорошим приёмом GPS-сигнала, причём с таким расчётом, чтобы в нескольких шагах от него находилась плотная крона дерева или навес. Это позволяет посетителю выполнить натурное сравнение приёма в открытом и закрытом пространстве.

В верхней части столбика закреплена наклонная панель формата A2, изготовленная из листа HPL толщиной 6 мм. По нижнему краю панели расположена металлическая плашка с QR-кодом, который ведёт на интерактивную веб-страницу. Рядом со столбиком в мощение утоплен металлический кружок диаметром 10 см, выполненный из латуни или нержавеющей стали, на котором глубоко выгравированы координаты этой точки в системе WGS-84. Эти координаты получены геодезической съёмкой с точностью до сантиметров, а глубина гравировки составляет не менее 1,5 мм, что предохраняет надпись от стирания подошвами обуви за несколько сезонов эксплуатации.

### **Графика панели**

Поверхность панели разделена на три смысловые зоны, последовательно проводящие посетителя от одного спутника к полной геометрической картине.

Первая зона озаглавлена «Один спутник — одна сфера» и показывает спутник над поверхностью Земли, расходящийся от него сигнал и приёмник в произвольной точке. Из схемы становится ясно, что одного измеренного расстояния для определения координаты недостаточно, поскольку приёмник может находиться в любой точке сферы, описанной вокруг спутника. Подпись поясняет: «Спутник сообщает только две вещи — точное время и своё положение в космосе».

Вторая зона называется «Три сферы — одна точка» и содержит геометрическую схему, на которой три сферы, описанные вокруг трёх различных спутников, пересекаются в одной точке у поверхности Земли. Стрелки от каждого спутника показывают измеренные расстояния. В подписи разъясняется: «Пересечение трёх сфер даёт координату, однако на практике для расчёта требуется ещё один, четвёртый спутник, поскольку часы в смартфоне не атомные, и поправку к ним приёмник вычисляет заодно с координатами».

Третья зона посвящена размерной связи и носит название «Размер имеет значение». Она содержит наглядную инфографику соответствия временных интервалов пространственным расстояниям: одна наносекунда отвечает 30 сантиметрам, одна микросекунда — 300 метрам, одна миллисекунда — 300 километрам. Рядом помещены силуэты, соразмерные этим масштабам: человек на самом кружке, городской квартал и целый регион. Подпись подводит итог: «Вся точность системы упирается в точность часов».

## **Тексты** 

#### **Уровень А**

Посмотрите вниз. На металлическом кружке у вас под ногами выбиты координаты этой точки — широта и долгота, измеренные геодезистами с точностью до сантиметров. А теперь откройте на смартфоне любое навигационное приложение. На экране появятся почти те же самые цифры.

**Как спутники определяют наше положение** 

Координаты местоположения на Земле можно определить с помощью спутниковых навигационных систем, таких как GPS или ГЛОНАСС. Эти системы работают благодаря сети спутников, которые постоянно передают сигналы с информацией о своём местоположении и времени отправки сигнала. Ваше устройство, будь то смартфон или автомобильный навигатор, принимает эти сигналы и рассчитывает расстояние до каждого спутника.

**Почему нужны сразу несколько спутников** 

Представьте, что каждый спутник — это маяк, который сообщает, где он находится в определённый момент времени. Ваше устройство знает, как далеко оно от каждого маяка, но чтобы точно определить своё местоположение, ему нужно знать расстояние до трёх спутников. Когда это расстояние известно, устройство может определить, где вы находитесь, решая систему из трёх уравнений. Это похоже на пересечение трёх сфер, где каждая сфера описывает возможное положение в пространстве относительно одного спутника. Точка пересечения всех трёх сфер и будет вашим местоположением.

### **Почему точность координат меняется**

Найдите в настройках навигационного приложения список видимых спутников. Над открытой площадкой их обычно видно от восьми до пятнадцати. Перейдите под навес или под плотную крону дерева и посмотрите снова: спутников станет меньше, а указанная погрешность в метрах вырастет.

Причина в том, что навигация зависит не только от самих спутников, но и от качества принимаемого сигнала. Чем больше спутников одновременно «видит» устройство и чем меньше сигнал искажается препятствиями, тем точнее определяется положение. Поэтому в городе среди высоких зданий или в густом лесу координаты работают заметно хуже, чем на открытом пространстве.

#### **Уровень Б**

**Как находят точку по расстояниям**

Представьте, что вам известно расстояние от вас до колокольни в центре города — допустим, три километра. Где вы находитесь? Где-то на окружности радиуса три километра вокруг колокольни. Добавим вторую известную точку, скажем водонапорную башню, и второе расстояние до неё. Две окружности пересекаются в двух точках — остаётся выбрать из них одну. С тремя известными точками выбор становится однозначным.

Спутниковая навигация устроена точно так же, только в трёхмерном пространстве и с движущимися «колокольнями». Их роль выполняют навигационные спутники, обращающиеся вокруг Земли на высоте около 20 000 километров. Каждый спутник непрерывно передаёт радиосигнал, в котором содержатся показания его бортовых часов и параметры орбиты, по которым приёмник вычисляет положение спутника в данный момент времени. Всю остальную работу выполняет приёмник в смартфоне.

**Расстояние из задержки**

Радиосигнал распространяется со скоростью света и доходит до приёмника за доли секунды. Приёмник сравнивает время, записанное в сигнале, с показаниями собственных часов и получает задержку. Умножив её на скорость света, он узнаёт расстояние до спутника.

Одного расстояния мало: приёмник может находиться в любой точке сферы такого радиуса вокруг спутника. Два расстояния задают окружность пересечения двух сфер, а три — оставляют единственную точку вблизи поверхности Земли. Эта точка и есть искомые координаты. Способ измерения расстояния по задержке сигнала называется в науке дальнометрией по времени распространения.

На практике спутников нужно не три, а четыре: дополнительный сигнал позволяет приёмнику учесть неточность собственных часов — подробнее об этом за QR-кодом

  

**Откуда берётся точность: 30 сантиметров за наносекунду**

Что определяет точность всей системы? Точность измерения времени. Здесь работает одно простое соотношение: за наносекунду — миллиардную долю секунды — свет проходит 30 сантиметров. Значит, ошибка часов в одну наносекунду даёт ошибку координаты в 30 сантиметров, а ошибка в одну микросекунду — уже 300 метров, то есть смещение на соседнюю улицу.

Поэтому на спутниках стоят атомные часы. Обычные кварцевые часы, какими снабжены бытовые приборы, начинают отклоняться от эталона на микросекунды уже за секунды или минуты работы — а для спутниковой навигации даже такая ошибка слишком велика. Никакие технические ухищрения этот предел не обходят: инженеры могут лишь максимально приблизить измерения к физическим границам точности. 

**Почему точность падает под навесом**

Метод корректен, пока сигнал доходит до приёмника по прямой. Под навесом, крышей или плотной кроной часть излучения отражается от препятствия и приходит окольным путём с дополнительной задержкой. Приёмник, не различая прямой и отражённый сигналы, воспринимает увеличение задержки как увеличение расстояния, и координаты смещаются. В тоннеле, в помещении и под водой навигация перестаёт работать вовсе: прямой радиосигнал до приёмника просто не доходит.

Это не недостаток аппаратуры, а ограничение самого метода. Когда расстояние вычисляется по времени прихода сигнала, любое отражение искажает результат — и в эхолоте, и в радаре, и в спутниковом приёмнике.

**Где ещё работает тот же принцип**

Дальнометрия по времени распространения встречается в самых разных областях. Летучая мышь посылает ультразвуковой импульс и по времени возврата эха определяет, где находится мотылёк. Сейсмологи устанавливают эпицентр землетрясения по разнице во времени прихода волны на удалённые станции. Тем же приёмом пользуется любой наблюдатель грозы: задержка в три секунды между молнией и громом соответствует примерно одному километру при скорости звука 340 м/с.

Спутниковая навигация работает по той же схеме, но с радиосигналом вместо звука. Замена скорости звука на скорость света позволяет применить метод к спутникам на высоте 20 000 километров — однако той же заменой увеличивается и цена ошибки во времени: ошибка в одну микросекунду переводится теперь не в сантиметры, а в сотни метров. Поэтому работоспособность системы держится не на скорости сигнала, а на точности атомных часов на борту спутника.

#### **Уровень В (за QR-кодом)**

**Почему спутников должно быть четыре, а не три**

Из приведённого выше рассуждения может показаться, что трёх спутников достаточно: у приёмника три неизвестные координаты — широта, долгота и высота, — и три измеренных расстояния задают три уравнения.

Каждое уравнение представляет собой теорему Пифагора в трёхмерном пространстве: квадрат расстояния от приёмника до спутника равен сумме квадратов разностей их координат по трём осям. Известны координаты спутника и измеренное расстояние, неизвестны три координаты приёмника. Одно уравнение задаёт сферу; два — окружность их пересечения; три — две точки, из которых лишь одна лежит вблизи поверхности Земли.

В этом рассуждении неявно предполагается, что приёмник измеряет время прихода сигнала абсолютно точно. На деле часы приёмника не атомные, а кварцевые, и их показания расходятся с истинным временем на неизвестную величину порядка микросекунд. Эта величина искажает измеренную задержку одинаково для всех спутников сразу, и в каждое из трёх уравнений входит одна и та же дополнительная неизвестная — поправка часов приёмника. Неизвестных становится четыре, а для однозначного решения системы из четырёх неизвестных нужно четыре уравнения. Четвёртое поставляет сигнал от четвёртого спутника.

В результате приёмник одновременно вычисляет своё положение и поправку к собственным часам. Благодаря этому даже бытовой смартфон знает точное время лучше, чем позволяет его собственный кварцевый механизм.

**Релятивистские поправки**

Часы на навигационном спутнике идут не с той же скоростью, что аналогичные часы на поверхности Земли. Здесь одновременно действуют два релятивистских эффекта, направленных в противоположные стороны.

Согласно специальной теории относительности, движущиеся часы идут медленнее неподвижных, и быстро летящий по орбите спутник замедляется. Согласно общей теории относительности, часы в более слабом гравитационном поле идут быстрее, а гравитация на высоте 20 000 километров заметно слабее, чем у поверхности Земли. Второй эффект превосходит первый, и в сумме бортовые часы уходят вперёд относительно земных примерно на 38 микросекунд в сутки.

Если бы при проектировании системы эти поправки не учитывались, ошибка определения координат нарастала бы со скоростью около 11 километров в сутки — и через несколько часов система стала бы бесполезной. Спутниковая навигация — один из редких примеров техники, где теория относительности выступает не как теоретическое построение, а как обязательный рабочий элемент инженерного расчёта.

**Другие навигационные системы**

Помимо американской GPS в мире одновременно работают российская ГЛОНАСС, европейская Galileo и китайская BeiDou. Физический принцип у всех четырёх систем один и тот же; различаются они частотами сигналов, числом спутников в группировке и конфигурацией орбит. Современный смартфон, как правило, принимает сигналы нескольких систем сразу — и общее число доступных спутников оказывается заметно больше, чем у любой отдельной из них, что повышает точность.

**Предел точности гражданского сигнала**

Почему в обычном режиме спутниковая навигация даёт точность лишь 2–5 метров? Точность ограничивают одновременно три фактора.

Первый — ионосфера. При прохождении через верхние слои атмосферы скорость радиосигнала изменяется, и приёмник учитывает эту задержку только приближённо.

Второй — многолучёвость. В городской застройке сигнал многократно отражается от стен и крыш зданий, и приёмнику трудно выделить прямой луч среди отражённых.

Третий — конечное временное разрешение самого кодового сигнала. У передаваемой со спутника последовательности есть наименьший различимый временной интервал, ниже которого измерение задержки в принципе невозможно.

Существуют, впрочем, геодезические приёмники, обрабатывающие не код, а фазу несущей радиоволны. Они достигают миллиметровой точности, но требуют принципиально иной аппаратуры и многочасовых сеансов наблюдений на каждой точке. Именно такими приёмниками выбивался кружок у ваших ног.

**Интерактивная карта спутников**

Веб-страница, на которую ведёт этот QR-код, в реальном времени показывает все навигационные спутники, активные сейчас над этой точкой. Сравните полное число спутников на карте с тем, которое видит ваш смартфон: приёмник в кармане принимает заметно меньше, потому что часть сигналов перекрыта зданиями, рельефом и кронами деревьев.

  
  

### **Сезонность и устойчивость**

Навигационные системы работают круглый год и круглосуточно, в любых погодных условиях и в любое время суток. Панель из HPL, оцинкованный столбик и металлический кружок в мощении рассчитаны на срок уличной эксплуатации, превышающий десять лет, и не нуждаются в регулярном обслуживании. QR-код ведёт на внешний сервер, благодаря чему в составе самого объекта отсутствует локальная электроника, которая могла бы выйти из строя или потребовать ремонта.

### **Безопасность**

Никаких специальных мер безопасности для эксплуатации объекта не требуется. Столбик не имеет острых краёв, а металлический кружок утоплен в мощение заподлицо с его поверхностью и потому не создаёт препятствия для прохода посетителей.

  
  
  

# ✅ Солнечная система · краткое описание - WP008  

## **Done**

## **WP008. «Солнечная система»**

Шишкин парк, Иннополис

### **1. Идентификация**

|  |  |
| :-: | :-: |
|   |   |
| ID | 1 |
| Код объекта | WP008 |
| Название | «Солнечная система» в Шишкин парке |
| Геопривязка | \<https://yandex.ru/maps/-/CPen7I02\> |
| Тип объекта | Масштабная прогулочная модель + арт-объект + наблюдение + телесное соотнесение |
| Формат | Линейный маршрут 165 м: сфера-Солнце + 8 маркеров-планет + двусторонний стенд (+ опц. внутренний стенд) |
| Электричество | Не требуется |
| Сезонность | Всесезонный; шар и стенд видны над снеговым покровом |

### **2. Образовательный смысл**

#### **2.1 Главная идея**

Солнечная система — почти сплошная пустота с редкими крошками вещества. Посетитель проходит 165 метров пешком и телесно ощущает масштаб: первые четыре планеты укладываются в восемь метров, дальше начинаются десятки метров пустоты до каждой следующей.

#### **2.2 Педагогическая лестница**

|  |  |  |
| :-: | :-: | :-: |
| \*\*Ступень\*\* | \*\*Вопрос\*\* | \*\*Ответ на стенде\*\* |
| Наблюдение | Что вижу? | Шар, столбики, нарастающие промежутки при ходьбе |
| Вопрос | Почему так пусто? | Стенд ставит этот вопрос явно |
| Причина | Откуда взялась пустота? | Планеты «собрали» вещество вокруг своих орбит при формировании |
| Механизм | Как работает этот процесс? | Газопылевой диск → слипание → планеты «собрали» вещество; Юпитер помешал образоваться планете, оставив пояс астероидов |
| Наука | Как мы это знаем? | Радарная локация, телескоп «Кеплер», сравнение с другими планетными системами |

#### **2.3 Два масштаба — честный компромисс**

Расстояния на маршруте честные: 1 м = 27,2 млн км. Диаметры планет увеличены в 50 раз: в честном масштабе Земля была бы точкой 0,47 мм — тоньше швейной иглы. Стенд объясняет этот компромисс явно, делая посетителя соучастником.

### **3. Тексты стендов**

Адаптация под стиль журнала «Квант»: убраны экскурсионные обращения («пройдите», «дойдите», «вы стоите»), поэтические метафоры («нагулял оболочки», «лепят облик», «выгребли пыль»), парцелляция и длинные тире как драматический приём. Структура педагогической лестницы сохранена, но переименована: вместо «Хука» — «Введение», блок «Действие посетителя» удалён как противоречащий выбранному регистру. В каждом блоке Б термин вводится после описания явления, а не до него.

### **3. Тексты стендов**

Уровень А

#### **3.1 Внешний стенд — Сторона 1**

**Заголовок** От Солнца до Нептуна — 165 метров

**Что перед вами**

Этот шар диаметром 2,5 метра — модель Солнца. Отсюда до Нептуна, самой дальней планеты, 165 метров пешком. Один шаг по маршруту — это примерно 20 миллионов километров в настоящем космосе.

**Где какие планеты**

На картинках в учебниках планеты стоят почти впритык. В жизни всё иначе.

Первые четыре планеты — Меркурий, Венера, Земля, Марс — теснятся возле Солнца: все они помещаются в восьми метрах от него. А дальше начинается пустота. От Марса до Юпитера — ещё двадцать метров. От Сатурна до Урана — около пятидесяти. На большей части пути по маршруту вы не встретите ничего. Солнечная система — это в основном пустое пространство, и редкие планеты в нём — скорее исключение.

**Откуда столько пустого пространства**

4,5 миллиарда лет назад на месте Солнечной системы крутилось облако газа и пыли. Под собственной тяжестью оно сжалось в плоский диск. Внутри этого диска пылинки сталкивались и слипались — сначала в камни, потом в глыбы размером с гору, и в конце концов в планеты.

Возле молодого горячего Солнца лёгкие вещества — газ и лёд — быстро испарились. Остались только тугоплавкие материалы, и из них собрались небольшие каменные планеты. Дальше от Солнца было холодно, газ и лёд никуда не делись — там выросли огромные Юпитер и Сатурн, а ещё дальше — ледяные Уран и Нептун.

Между Марсом и Юпитером планета так и не получилась. Юпитер был слишком тяжёлым: своей гравитацией он разгонял соседние обломки и не давал им слипнуться. Этот рассыпанный материал так и остался кружить вокруг Солнца — мы называем его поясом астероидов.

#### **3.2 Уровень Б**

**Заголовок** Два масштаба в одной модели

**Почему здесь два масштаба**

Расстояния и размеры планет показаны по-разному. Один метр маршрута — это примерно 27 миллионов километров реального пространства, а сами планеты увеличены ещё в 50 раз. Зачем понадобилась такая хитрость?

**Футбольное поле и горошина**

Диаметр Земли — около 13 тысяч километров, а до Солнца — 150 миллионов: разница в 12 тысяч раз. Если Солнце сделать размером с мандарин, Земля в честном масштабе будет точкой меньше полумиллиметра — её просто не увидеть. Поэтому расстояния на маршруте мы оставили настоящими, а планеты пришлось увеличить.

**Это свойство самой системы**

Солнечная система устроена так, что показать её в точности на одной картинке нельзя: тела в ней крошечные по сравнению с расстояниями между ними. Такие системы называют разреженными. У любой картинки есть выбор: либо размеры, либо расстояния — что-то одно.

То же самое — внутри атома. Увеличьте ядро атома водорода до размера теннисного мяча, и ближайший электрон окажется примерно в двух километрах от него. 

#### **3.3 Внутренний стенд (опционально — при реализации входа в сферу)**

⚠ Реализуется только при принятии решения о входном отверстии в сфере. До принятия этого решения — на согласовании.

**Заголовок** Что происходит внутри Солнца

**Введение** Зажжённая спичка горит несколько секунд, полено в печи — несколько часов, а Солнце светит уже 4,5 миллиарда лет и будет светить ещё столько же. Откуда столько энергии?

Внутри сферы вы находитесь там, где в реальном масштабе располагается центр Солнца. Температура здесь — около 15 миллионов кельвинов, давление — около 250 миллиардов атмосфер.

**Уровень A. Что происходит в этих условиях** Ядра атомов водорода в обычных условиях отталкиваются друг от друга: они одноимённо заряжены. Но при температуре в миллионы градусов они движутся настолько быстро, что вступают во взаимодействие. Так каждую секунду Солнце перерабатывает около 600 миллионов тонн водорода. Часть исходной массы при этом исчезает: примерно 4 миллиона тонн в секунду переходят в энергию излучения по формуле E = mc².

**Уровень B. Механизм** Описанный процесс называется термоядерным синтезом. Он даёт практически всю энергию Солнца, но образующийся в ядре свет не достигает поверхности сразу. Вещество в недрах настолько плотное, что фотон без конца поглощается и переизлучается атомами; от рождения в ядре до выхода в фотосферу проходят от сотен тысяч до нескольких миллионов лет. После этого дошедшие до поверхности Солнца фотоны долетают до поверхности Земли за 8 минут.

Тот же термоядерный синтез человек пытается воспроизвести на Земле — в установках типа токамак, где из небольшого количества тяжёлых изотопов водорода можно было бы получить много энергии. Однако на практике такие установки пока не достигали устойчивого положительного энергетического баланса (с учётом всей затраченной энергии на работу системы). Для этого строится ITER — международный экспериментальный реактор, предназначенный для демонстрации возможности получения энергии из термоядерного синтеза. Солнце удерживает плазму собственной гравитацией, а на Земле для удержания приходится строить сверхпроводящие магниты огромных размеров.

#### **3.4 QR — расширенный материал**

**Заголовок** Солнечная система: масштабы, плотности, закономерности

**Блок 1. Масштабы модели**

Один метр маршрута соответствует 27,2 миллиона километров реального пространства. В этом масштабе Земля имела бы диаметр 0,47 миллиметра — меньше толщины швейной иглы; Юпитер был бы горошиной 5,2 миллиметра, Солнце — мандарином 5 сантиметров. Чтобы маркеры планет можно было увидеть на маршруте, их размеры увеличены ещё в 50 раз.

Для сравнения: самая протяжённая в мире масштабная модель Солнечной системы, Sweden Solar System, растянута вдоль шведской автотрассы Е4 на 950 километров; в её масштабе Солнце — это арена городского стадиона в Стокгольме.

**Блок 2. Откуда пустота между орбитами**

Изначально вещество протопланетного диска было распределено более или менее равномерно. Современная картина с пустыми промежутками между орбитами сложилась за сотни миллионов лет под действием гравитации крупных планет.

Юпитер набрал массу первым. Своим тяготением он возмущал движение планетезималей в соседних областях диска: между его орбитой и орбитой Марса частицы выбрасывались на вытянутые траектории и в конечном счёте либо рассеивались, либо притягивались другими телами. Поэтому здесь образовался пояс астероидов вместо отдельной планеты. Нептун, постепенно смещаясь наружу, тем же образом расчистил область за своей орбитой; то, что осталось, сегодня называется поясом Койпера — это сильно разрежённое скопление ледяных тел.

**Блок 3. Закон обратных квадратов**

Уличный фонарь освещает землю ярче всего прямо под собой. Шаг в сторону — становится темнее. По какой именно зависимости?

Свет от точечного источника распределяется по поверхности воображаемой сферы, в центре которой стоит сам источник. Площадь сферы пропорциональна квадрату её радиуса: при удвоении расстояния та же энергия распределяется по площади, в четыре раза большей. Поэтому интенсивность света падает как 1/r² — это и есть закон обратных квадратов.

Нептун находится примерно в 30 раз дальше от Солнца, чем Земля, поэтому получает примерно в 900 раз меньше солнечного излучения. Это одна из ключевых причин его крайне низкой средней температуры (около −215 °C), однако не единственная: итоговая температура планеты зависит также от её альбедо и теплового баланса атмосферы. Тому же закону подчиняются гравитационное притяжение точечных масс, громкость звука от точечного источника в открытом пространстве и мощность принимаемого радиосигнала. Одна и та же геометрическая причина связывает четыре разных по природе явления.

**Блок 4. Та же геометрия в атоме**

Если бы ядро атома водорода имело размер теннисного мяча, ближайший электрон находился бы от него примерно в двух километрах. Атом, как и Солнечная система, в основном пуст: редкие массивные частицы разделены огромными по сравнению с их собственными размерами расстояниями. В обоих случаях частицы удерживаются на месте центральным силовым полем — гравитационным у планет, электромагнитным у электронов, — а движение по орбите не даёт им упасть к центру.

**Блок 5. Орбитальный резонанс**

Чтобы качели раскачались, толкать их нужно в такт собственному периоду качаний; вне такта толчки гасят разгон, а не усиливают его. Похожее правило работает в небесной механике, только в роли толчков выступают гравитационные взаимодействия одного тела на другое.

Три крупных спутника Юпитера — Ио, Европа и Ганимед — обращаются вокруг него с периодами, относящимися как 1 : 2 : 4. Пока Ганимед делает один оборот, Европа — два, Ио — четыре, и все трое регулярно оказываются примерно в одном и том же взаимном положении. Такое соотношение периодов называется орбитальным резонансом.

Маленькие гравитационные толчки, которые тела передают друг другу в этих повторяющихся точках, складываются и удерживают орбиты в строгой пропорции. Резонансы во многом определяют, какие конфигурации планетных орбит остаются устойчивыми миллиарды лет, а какие распадаются. Резонанс 2 : 3 с Нептуном удерживает на устойчивой орбите Плутон; в поясе астероидов, напротив, тяготение Юпитера выметает определённые орбиты, и в этих местах образуются пустые полосы — щели Кирквуда.

**Блок 6. Как это узнаёт наука**

Расстояния в Солнечной системе измеряют радиолокацией: к планете посылают радиосигнал, замеряют время его возвращения и умножают на скорость света, после чего делят пополам. Первое такое измерение расстояния до Венеры провели в 1961 году; именно эти данные позволили заменить приблизительные «150 миллионов километров» от Земли до Солнца на точное число.

Историю формирования и миграции планет восстанавливают по компьютерному моделированию протопланетных дисков и по сравнению с другими звёздными системами. Основные наблюдательные данные сюда поставляют космические телескопы «Кеплер» и TESS. Оказалось, что планеты вокруг других звёзд расставлены гораздо разнообразнее, чем считалось до начала этих наблюдений; сравнение с такими системами помогает понять, какие особенности нашей Солнечной системы типичны для планетных систем вообще, а какие специфичны.

**Блок 7. Открытый вопрос**

Маршрут модели заканчивается на Нептуне, в 165 метрах от Солнца. В том же масштабе ближайшая к Солнцу звезда, Проксима Центавра, оказалась бы примерно в 1470 километрах отсюда — это сравнимо с расстоянием от Москвы до Сочи. В 2016 году у этой звезды нашли планету земного типа на таком расстоянии от неё, при котором на её поверхности может существовать жидкая вода. Эта полоса вокруг звезды называется зоной обитаемости.

Существующие космические аппараты добираются до краёв Солнечной системы за десятилетия и оттуда уже не возвращаются. При их скоростях полёт до Проксимы Центавра занял бы десятки тысяч лет. Проект Breakthrough Starshot предлагает иной путь: разгонять миниатюрные аппараты массой около грамма мощным наземным лазером до заметной доли скорости света. При такой скорости долёт занял бы около двадцати лет — за время одного человеческого поколения. Технология пока в стадии разработки.

### **4. Масштабные параметры**

Масштаб расстояний: 1 м тропы = 27 200 000 км (1 : 27 200 000 000). Диаметры объектов увеличены отдельно — в 50 раз относительно масштаба расстояний.

|  |  |  |  |  |
| :-: | :-: | :-: | :-: | :-: |
| \*\*Объект\*\* | \*\*Расст. от Солнца, м\*\* | \*\*Реальный диаметр\*\* | \*\*В модели (×50)\*\* | \*\*В честном масштабе\*\* |
| Солнце | 0 | 1 392 000 км | 2 550 мм | 51 мм |
| Меркурий | 2,12 | 4 880 км | 8,9 мм | 0,18 мм |
| Венера | 3,97 | 12 104 км | 22,2 мм | 0,44 мм |
| Земля | 5,49 | 12 756 км | 23,4 мм | 0,47 мм ≈ игла |
| Марс | 8,36 | 6 792 км | 12,5 мм | 0,25 мм |
| Юпитер | 28,55 | 142 984 км | 262 мм | 5,2 мм ≈ горошина |
| Сатурн | 52,33 | 120 536 км | 221 мм | 4,4 мм |
| Уран | 105,30 | 51 118 км | 94 мм | 1,9 мм |
| Нептун | 165,00 | 49 528 км | 91 мм | 1,8 мм |

Дополнительный ориентир: пояс астероидов — между Марсом (8,36 м) и Юпитером (28,55 м). Рекомендуется обозначить на схеме стенда.

### **5. Техническая карточка (Build Card)**

#### **5.1 Описание конструкции**

Объект состоит из четырёх независимых компонентов, размещённых вдоль тропы: (1) полая сфера «Солнце» на бетонном постаменте; (2) восемь столбиков-маркеров — планеты — с табличками; (3) двусторонний информационный стенд на двух стойках; (4) опциональный внутренний стенд внутри сферы. Электричество не требуется. Все компоненты устанавливаются независимо.

#### **5.2 Основные элементы**

**Компонент 1 — сфера «Солнце»**

  - Полая сфера ⌀ 2 550–2 560 мм
  - Бетонный постамент высотой 500 мм
  - Общая высота вершины: \~3 050–3 060 мм
  - Круговая площадка вокруг постамента: радиус 3 м от основания, бетон или мощение
  - Концентрические кольца орбит на площадке: гравировка или металлические вставки
  - (Опционально) входное отверстие в сфере для прохода посетителей

**Компонент 2 — маркеры-планеты (8 шт.)**

  - 8 столбиков вдоль тропы, материал и высота — на уточнении
  - На каждом столбике: табличка с названием планеты, диаметром в модели, расстоянием от Солнца

**Компонент 3 — информационный стенд**

  - Двусторонний, на двух стальных стойках, забетонированных в грунт
  - Две информационные панели (Сторона 1, Сторона 2)
  - Металлическая плашка с QR-кодом: гравировка + дублирование ссылки текстом

**Компонент 4 — внутренний стенд (опционально)**

  - Малый стенд или табличка внутри сферы
  - Активируется только при реализации входного отверстия

#### **5.3 Материалы и покрытия**

|  |  |  |
| :-: | :-: | :-: |
| \*\*Элемент\*\* | \*\*Вариант А\*\* | \*\*Вариант Б / примечание\*\* |
| Сфера | Кортеновская сталь | Стеклопластик на металлическом каркасе |
| Постамент | Монолитный бетон | — |
| Панели стенда | HPL + УФ-печать | Нержавеющая сталь + гравировка |
| QR-плашка | Металл, гравировка | — |
| Стойки стенда | Сталь (профиль — уточнить) | — |
| Столбики-планеты | Не указано — требует уточнения | — |
| Таблички на столбиках | Не указано — требует уточнения | — |

#### **5.4 Габариты и монтаж**

**Сфера**

  - ⌀ 2 550–2 560 мм
  - Постамент: высота 500 мм, сечение — уточнить
  - Вершина объекта: \~3 050–3 060 мм
  - Тип фундамента постамента: не указан — требует уточнения
  - Способ крепления сферы к постаменту: не указан — требует уточнения

**Площадка**

  - Радиус: 3 м от основания постамента
  - Покрытие: бетон или мощение (тип — уточнить)

**Стенд**

  - Две стойки, забетонированные в грунт; глубина заделки — уточнить
  - Габарит панелей: не указан — требует уточнения
  - Расстояние от сферы: не указано — требует уточнения
  - Ориентация по сторонам света: не указана (влияет на долговечность УФ-печати)

**Маркеры-планеты**

  - Высота столбиков: не указана — требует уточнения
  - Способ установки: не указан — требует уточнения
  - Линейный охват маршрута: 165 м

#### **5.5 Принцип работы**

Объект работает без механики и электроники. Взаимодействие пассивное: ничего не движется, не вращается, не нажимается. Настройка и калибровка не требуются.

Штатный сценарий: посетитель подходит к сфере, читает стенд → проходит 165 м, останавливаясь у столбиков-планет → (опционально) входит внутрь сферы → сканирует QR.

#### **5.6 Безопасность и ограничения**

⚠ Безопасность — требование, не факт. Механизмы защиты требуют проработки на этапе проектирования.

  - Высота сферы \~3,05 м: риск попытки залезть; механизм защиты не описан — требует проектного решения
  - Входное отверстие (если реализуется): риск застревания, мусора, нецелевого использования; форма и размер проёма — на согласовании
  - Кортеновская сталь: острые кромки при патинировании — требуется обработка торцов
  - Площадка 3 м: покрытие должно исключать скольжение при намокании и обледенении
  - Столбики-планеты: высота и форма должны исключать травму при столкновении (особенно для детей)

#### **5.7 Уличная устойчивость**

|  |  |
| :-: | :-: |
| \*\*Элемент\*\* | \*\*Оценка устойчивости\*\* |
| Кортен | Самозащищающийся материал, патина = защита. Десятилетия без покраски. |
| Стеклопластик | Уязвим к ударам, граффити. Требует проверки гелькоута 2×/год. |
| Бетон | Всесезонный, устойчив к нагрузкам. Десятилетия. |
| HPL-панели | УФ-выцветание за 5–7 лет. Ориентация по сторонам света — критична. |
| Нерж. сталь + гравировка | Долговечнее HPL, не выцветает. Дороже в производстве. |
| QR-плашка (гравировка) | Высокая долговечность. Уязвима к царапинам. |
| Столбики | Устойчивость зависит от материала — не указан. |

#### **5.8 Обслуживание**

  - Сфера кортен: ежегодный визуальный осмотр равномерности патины
  - Сфера стеклопластик: осмотр 2×/год, устранение сколов и трещин гелькоута
  - Площадка: уборка мусора, листвы, снега; контроль орбитальных вставок после зимы
  - Стенд: ежегодная проверка затяжки болтов; контроль читаемости QR-кода
  - QR-ссылка: проверка доступности не реже 1×/год (смена хостинга)
  - Панели HPL: замена при выцветании (ориентировочно через 5–7 лет)
  - Столбики-планеты: периодичность зависит от материала — уточнить при выборе

#### **5.9 Слабые места**

Наиболее уязвимые элементы — панели с УФ-печатью, столбики дальних планет (Уран, Нептун) и входное отверстие сферы (если реализуется).

  - Панели с УФ-печатью: выцветание за 5–7 лет
  - Столбики Урана и Нептуна (105–165 м от входа): сложнее контролировать, выше риск вандализма
  - Кольца орбит на площадке: металлические вставки могут смещаться при пучении грунта
  - Стеклопластиковая сфера: уязвима к граффити и ударам больше, чем кортен
  - Входное отверстие: высокий риск нецелевого использования
  - Позиции столбиков: при повреждении и переустановке необходима фиксация координат в исполнительной документации

  
  

# ➕ WP008-а «Меркурий»  

### **DONE**

#### **WP008-а «Меркурий»**

**1. Код и тип** ID: 6. Код объекта: WP008-а. Название: Меркурий (столбик-планета). Принадлежность к станции: «Солнечная система», вторая точка на маршруте — сразу после шара «Солнце». Тип объекта: столбик-указатель с масштабным шариком и информационной табличкой. Масштаб модели: 1 метр тропы = 28 миллионов километров реального пространства. Диаметры планет дополнительно увеличены в 50 раз относительно масштаба расстояний; в едином масштабе планеты были бы невидимы. Тип научного рассуждения: ограничение (пропорции Солнечной системы таковы, что модель в одном масштабе для расстояний и диаметров на человеческой площадке невозможна).

**2. Конструкция** Металлический столбик высотой 1,5 м из стальной трубы ⌀60 мм с порошковой антивандальной покраской, забетонированный в основание. На вершине — подставка-«ладонь» из стальной пластины с закреплённой металлической сферой диаметром 8,9 мм. Диаметр шарика соответствует принятому для станции масштабу: расстояния от Солнца до Нептуна укладываются примерно в 160 м, диаметры увеличены в 50 раз относительно масштаба расстояний.

Шарик крепится к пластине приваркой или заклёпкой (не резьбой), чтобы исключить откручивание. У основания шарика на пластине — контрастное цветное кольцо диаметром 3–4 см: зимний ориентир, когда сам шарик теряется в снегу.

Расстояние от центра шара «Солнце» до центра шарика «Меркурий» по маршруту — 2,1 м. Если подрядчику удобнее привязываться к краю входной площадки, точную монтажную отметку считать от финального положения постамента и внешнего стенда станции.

Табличка — нержавеющая сталь с гравировкой, крепление антивандальными болтами. Размер и высота подобраны так, чтобы текст читался и взрослым, и ребёнку (около 1,2 м от земли). На табличке предусмотрено место под миниатюрную схему-линейку.

Круглогодичность: да. Зимой ключевую визуальную роль играют столбик, табличка и контрастное кольцо у основания шарика.

**3. Педагогическая роль в маршруте** Меркурий — первая планета на тропе и первое место, где посетитель телесно ощущает масштаб Солнечной системы. Два шага от Солнца — и вот уже вторая «планета», почти невидимая. Это и есть главный экспонат: не сама планета как астрономический объект, а пропорция системы, которую нельзя почувствовать ни по книге, ни по схеме.

Объект должен работать как короткая, очень ясная остановка. Минимум текста, один ключевой факт о масштабе, один крючок к следующей планете. Биография Меркурия — в QR общей станции.

**4. Текст таблички**

**Меркурий. 8,9 мм — и 2 метра назад**

Шарик перед вами - и есть планета. В том же масштабе, в котором Солнце позади — мяч.

Настоящий Меркурий — 4 879 км в поперечнике. Здесь он уменьшен в 550 миллионов раз. От Солнца до Меркурия в космосе — 58 миллионов километров; эти 58 миллионов превратились в 2,1 метра, которые вы только что прошли. На всей тропе 1 метр = 28 миллионов километров.

Уменьшить Солнечную систему «честно» — в одном масштабе и для размеров, и для расстояний — на человеческой площадке нельзя. Чтобы шарик Меркурия дотянул хотя бы до размера горошины, тропа должна была бы тянуться километрами — до Нептуна было бы целых девять. Поэтому на этой модели диаметры увеличены в 50 раз относительно расстояний — и Меркурий всё равно остался меньше ногтя. Так и устроена Солнечная система: планеты — крошечные точки в пустоте.

Оглянитесь на Солнце, потом снова на шарик. Дальше планеты будут расходиться всё быстрее: до Нептуна по этой тропе ещё около 158 метров.

Меркурий ближе всех к Солнцу, но самая горячая планета — не он. Следующий столбик — Венера.

*Схема на табличке.* Горизонтальная линейка во всю ширину таблички, на ней засечки: Солнце → Меркурий → Венера → Земля → Марс → Юпитер → Сатурн → Уран → Нептун. Меркурий — первая засечка почти вплотную к Солнцу. Подпись под линейкой: «1 метр = 28 миллионов километров».

**5. Текст для QR (общий QR станции «Солнечная система», блок «Меркурий»)**

**Почему Солнечную систему нельзя уложить в один честный масштаб**

Диаметр Меркурия (4 879 км) примерно в 12 000 раз меньше расстояния от него до Солнца (58 млн км). Чтобы сохранить это соотношение и при этом сделать Меркурий видимым — скажем, шариком в один сантиметр, — от Солнца до него пришлось бы отмерить 120 метров, а до Нептуна — около 9 километров.

На станционной тропе выбран компромисс: честный масштаб расстояний (1 м = 28 млн км) и увеличение диаметров в 50 раз, чтобы планеты хотя бы угадывались. Даже с этой поправкой Меркурий остался меньше сантиметра — это и есть реальная пропорция системы, в которой мы живём.

**Про сам Меркурий**

Солнечные сутки на Меркурии длятся 176 земных суток — дольше, чем меркурианский год (88 суток). Парадокс решается так: вокруг оси Меркурий оборачивается за 59 земных дней, вокруг Солнца — за 88. Пока на планете проходит один солнечный день, от полудня до полудня, Меркурий успевает почти дважды обогнуть Солнце.

Сила тяжести на Меркурии в 2,6 раза меньше земной. Если здесь вы подпрыгиваете на 40 см, на Меркурии та же сила толчка подняла бы вас выше метра. На Марсе — примерно столько же: гравитация у них с Меркурием почти одинаковая. На Луне — выше двух метров. А Юпитер прижал бы вас обратно: сила тяжести там в два с половиной раза сильнее земной, прыжок не дотянул бы и до пятнадцати сантиметров. У самого Юпитера, впрочем, и опоры для прыжка нет — он газовый.

**Почему Меркурий не самая горячая планета**

Температура на поверхности Меркурия меняется от –180 °C ночью до +430 °C днём. А средняя температура Венеры, которая дальше от Солнца, выше — около +460 °C, и она почти не падает ночью. Причина — плотная атмосфера Венеры из углекислого газа: она держит тепло, как стеклянная крышка над горячей тарелкой. У Меркурия атмосферы практически нет, и ночная сторона быстро остывает.

**Как это узнаёт наука**

Расстояния до близких планет измеряют радаром: посылают импульс к планете и засекают время возврата сигнала. До дальних планет добираются автоматическими станциями. Диаметр планеты получают, умножая её угловой размер в телескопе на уже известное расстояние. Температуры Меркурия сначала измерили дистанционно по тепловому излучению, потом проверили на месте — аппаратами Mariner 10 (1974) и MESSENGER (2011–2015).

  
  
  

# ➕ 7WP008-б «Венера»  

**DONE**

**7WP008-б «Венера»**

### **Конструкция**

Металлический столбик высотой 1,5 м из стальной трубы ⌀60 мм с порошковой антивандальной покраской, забетонированный в основание. На вершине — подставка-«ладонь» из стальной пластины с закреплённой сферой диаметром 22,2 мм, окрашенной в желтовато-дымчатый оттенок Венеры. Диаметр скорректирован под единый масштаб станции: 165 м до Нептуна и увеличение диаметров в 50 раз. Расстояние от Солнца по маршруту — \~4,0 м от центра Солнца. Табличка — нержавеющая сталь с гравировкой, крепление — антивандальные болты.

**Круглогодичность:** да. Столбик и табличка видимы зимой; окраска сферы устойчива к УФ и перепадам температур.

**QR-код на объекте:** не устанавливается. Углубление уводится через общий QR станции «Солнечная система».

### **Параметры**

|  |  |
| :-: | :-: |
| \*\*Параметр\*\* | \*\*Значение\*\* |
| Диаметр сферы | 22,2 мм (1:50 от 12 104 км) |
| Расстояние от «Солнца» | \\\~4,0 м от центра |
| Высота столбика | 1,5 м |
| Цвет сферы | желтовато-дымчатый |
| Материал столбика | стальная труба ⌀60 мм, порошковая покраска |
| Материал таблички | нержавеющая сталь, гравировка |
| Крепление | антивандальные болты, бетонное основание |

### **Тип научного рассуждения**

Общая причина. Нагрев поверхности Венеры, работа стеклянной теплицы и удержание тепла под одеялом сводятся к одной интуиции — преграда для отдачи поднимает равновесную температуру. Физические механизмы при этом разные: переизлучение длинных волн, подавление конвекции, задержка теплоотдачи через ткань. Экспонат использует парадокс «ближе к Солнцу Меркурий, а горячее Венера», чтобы показать: расстояние — не главное, атмосфера решает судьбу планеты.

### **Текст таблички (стенд)**

#### **Почему Венера горячее Меркурия**

Меркурий ближе к Солнцу. Но самая горячая поверхность в Солнечной системе — у Венеры: около +462 °C. Расстояние до Солнца — не главное.

Перед вами Венера в масштабе 1:50 по диаметру. Земля — такого же размера, на соседнем столбике, в 1,6 м дальше от Солнца. Планеты-близнецы по размеру и составу, но на Венере плавится свинец, а на Земле — умеренный климат. Отличает их одно: атмосфера.

Планета получает энергию от Солнца и отдаёт её обратно тепловым излучением. Когда приход равен отдаче — температура стабильна. Атмосфера Венеры — это 96% углекислого газа, давление у поверхности — 92 атмосферы. Облака Венеры отражают около трёх четвертей солнечного света обратно в космос; оставшаяся четверть достигает поверхности и нагревает её. Дальше работает CO₂: для коротких волн солнечного света он прозрачен, а для длинных волн теплового излучения поверхности — нет. CO₂ поглощает их и переизлучает во все стороны, в том числе обратно вниз. Отдача в космос замедлена, приход — нет. Температура растёт, пока приход и отдача не сравняются, — на +462 °C.

Бытовая интуиция тут работает: теплица в солнечный день нагревается выше уличного воздуха, под одеялом теплее, чем без него. Физика во всех трёх случаях разная: у Венеры важно переизлучение длинных волн обратно к поверхности; у теплицы — что стекло мешает горячему воздуху уйти конвекцией; одеяло просто тормозит передачу тепла через ткань. Общее одно: преграда для отдачи поднимает равновесную температуру.

Попробуйте. Приложите ладонь к сфере и к металлу столбика. На солнце они разной температуры — это тот же баланс прихода и отдачи, только для двух предметов на тропе.

Дальше по маршруту — Земля. Парниковый эффект работает и там, но мягче: он поднимает среднюю температуру поверхности примерно на 33 °C выше расчётной. Без него — вечная мерзлота.

### **Углубление (общий QR станции, раздел «Венера»)**

#### **Откуда берутся +462 °C и почему Марс не греется так же**

Температуру можно оценить из баланса излучения: мощность поглощённого солнечного света равна мощности уходящего теплового. Облака Венеры отражают около трёх четвертей солнечного света — поэтому «эффективная» температура планеты, с которой она светит в космос, всего около −40 °C. У Меркурия облаков и атмосферы нет, тёмная поверхность поглощает почти весь приходящий свет, и днём она нагревается до +430 °C. Но плотная атмосфера Венеры из CO₂ перехватывает длинноволновое излучение поверхности — и равновесие на самой поверхности сдвигается к +462 °C, выше, чем на Меркурии даже в полдень.

На Марсе атмосфера тоже из CO₂, но её почти нет: давление у поверхности в 160 раз меньше земного. Задерживать тепловое излучение нечему — поверхность промёрзла до −60 °C в среднем. Тот же газ, тот же принцип, но рабочим оказывается не состав, а масса атмосферного столба.

**Как это узнаёт наука.** Состав и давление атмосферы Венеры измерены советскими посадочными аппаратами серии «Венера» в 1970–80-х: «Венера-7» села в 1970-м, «Венера-13» в 1982 году передавала с поверхности 127 минут, пока температура и давление не выводили приборы из строя. Температуру поверхности также выводят из радиоизлучения планеты — волны сантиметрового диапазона проходят сквозь облака.

**Ещё о Венере.** Венера вращается в обратную сторону — если бы вы стояли на её поверхности, Солнце вставало бы на западе. Период оборота вокруг оси — 243 земных дня; это длиннее венерианского года в 225 дней. А вот солнечные сутки, день и ночь, на Венере короче — около 117 земных дней. Давление у поверхности — 92 атмосферы, как на глубине 900 м в земном океане.

**Открытый вопрос.** Земля и Венера стартовали почти одинаковыми — одна масса, одинаковый набор летучих веществ, одна орбитальная зона. Венеру так и называют — «злой сестрой Земли». Но судьбы разошлись: одна стала океаном, другая — духовкой. Гипотезы связывают расхождение с потерей воды на Венере в раннюю эпоху и запуском необратимого парникового эффекта, но какой фактор оказался решающим — неизвестно.

### **Схема на табличке**

Две стрелки у поверхности планеты: входящий солнечный свет (короткие волны, после отражения большей части облаками доходит остаток) и исходящее тепловое излучение (длинные волны, часть возвращается обратно). Подпись: «Часть прихода отражают облака. Из того, что доходит, в космос почти ничего не уходит. Температура растёт, пока потоки не сравняются».

### **Ключевые факты (для справки при монтаже)**

  - Диаметр Венеры: 12 104 км (Земля — 12 742 км, разница ≈5%)
  - Температура поверхности: \~+462 °C (горячее Меркурия)
  - Атмосфера: 96% CO₂, давление у поверхности — 92 атмосферы
  - Альбедо Бонда: \~0,75 (облака отражают около трёх четвертей солнечного света)
  - Эффективная температура: \~−40 °C (то, с чем планета светит в космос)
  - Вращение: ретроградное, сидерический период оборота длиннее венерианского года
  - Сравнение с Землёй: диаметры различаются на 5%

### **Взаимодействие посетителя**

**Тип:** наблюдение, чтение, тактильное сравнение температур сферы и столбика. **Время у экспоната:** 1–2 минуты на табличку, +3–5 минут при чтении QR. **Связь с соседними объектами:** естественный переход к столбику «Земля» — сравнение двух планет-близнецов и разницы их атмосфер.

  
  
  

# ➕ ЭК-008 «Земля» (столбик-планета) Конструкция  

## **DONE**

## **ЭК-008 «Земля» (столбик-планета)**

### **Замысел**

Третья точка станции «Солнечная система» — и первая, где абстрактный масштаб тропы превращается в личное переживание. Между Меркурием с Венерой и более далёкими планетами посетитель встречает свой дом: шарик размером с горошину, в пяти с половиной метрах от Солнца. Этот столбик не объясняет Землю — он переключает рассказ с расстояний на причины и говорит о том, как одно гравитационное взаимодействие может проявляться в природе по-разному.

### **Принцип работы (что уносит посетитель)**

На уровне переживания — масштаб становится телесным: «вот мой дом, и он размером с горошину».

На уровне метода — способ научного рассуждения, который называется «общая причина». Два внешне разных явления — приливы в океанах и устойчивый коридор наклона земной оси — сводятся к одному соседу: Луне. Но сводятся через разные механизмы. Приливы рождаются из разности притяжения по разные стороны Земли; стабилизация оси — из крутящего момента, который Луна создаёт на сплюснутой у экватора Земле. Причина одна, проявления разные.

Этот способ мышления переносится на другие системы — двойные звёзды, устойчивость орбит, условия на экзопланетах. Энциклопедические факты о Земле (магнитное поле, атмосфера, тектоника) сознательно не включены: они утяжелили бы фокус и заменили метод набором сведений.

### **Сечение реальности**

Структура одной фразой: **одна и та же причина, действуя на разные особенности тела, проявляется через разные механизмы и даёт разные эффекты в одной системе.**

Три явления из разных доменов с разными механизмами:

  - **Приливы в океанах** (механика жидкостей, разность сил притяжения по разные стороны Земли).
  - **Стабилизация наклона оси на геологических временах** (динамика твёрдого тела, момент сил на экваториальной выпуклости).
  - **Поведение оси Марса** (та же небесная механика без массивного спутника) — контрольный случай.

Один уровень глубже, в материалах QR — четвёртое явление: расхождение Земли и Венеры. Здесь сечение работает «от противного»: близость по массе и радиусу не предопределяет одинаковую судьбу, потому что эволюцию планеты определяет не один параметр.

### **Привязка к маршруту**

Земля — третий объект после Солнца и пары Меркурий–Венера. Расстояние от центра столбика-Солнца — около 5,5 м. Посетитель доходит до этой точки уже через несколько шагов: масштаб расстояний начинает чувствоваться телесно, и именно здесь он впервые становится личным. После «горошины-Земли» остальные планеты воспринимаются как соседи по одной системе, а не как пункты списка.

### **Конструктив**

Типовой для серии столбик станции «Солнечная система»:

  - Стальная труба ⌀ 60 мм, высота 1,5 м, порошковая антивандальная покраска, забетонированный фундамент.
  - Подставка-«ладонь» из стальной пластины на вершине; сфера ⌀ 23,4 мм — стальная, окрашена в узнаваемый сине-зелёно-белый образ Земли. Покраска уличная, выцветание за 2–4 года под УФ допустимо: узнаваемость сохраняется.
  - **Крепление сферы — сварное, а не винтовое.** Винтовое на малом шарике уязвимо к раскручиванию и срыву; сварка даёт живучесть на уровне всей конструкции.
  - Табличка — нержавеющая сталь, гравировка, антивандальные болты крепления. Замены не требует десятилетиями.
  - Отдельный QR на столбике не размещается. Все материалы уровня В идут через общий QR станции, раздел «Земля».

Диаметр сферы — 23,4 мм, расстояние 5,5 м от Солнца. Это компромисс двух масштабов: диаметры на тропе увеличены в 50 раз относительно масштаба расстояний (165 м до Нептуна). В «честном» масштабе расстояний планеты были бы невидимы; в «честном» масштабе диаметров тропа растянулась бы на километры.

### **Текст на табличке**

**Земля.** Диаметр: 12 742 км.

#### **Уровень А**

Сине-зелёно-белый шарик на этом столбике — модель Земли в том же масштабе, в котором в начале тропы установлен столбик-Солнце. В этом масштабе диаметр Земли равен 2,5 сантиметра, а расстояние до Солнца — 5,5 метра. В действительности это около 150 миллионов километров: расстояние, которое свет проходит за 8 минут 20 секунд. Земля — единственное место в Солнечной системе, где известна жизнь.

#### **Уровень Б**

В этой модели Луны нет, хотя в реальном масштабе расстояний она оказалась бы примерно в 9 сантиметрах от земного шарика — около тридцати его диаметров. Тем не менее именно гравитация Луны определяет работу двух разных процессов на Земле.

Луна притягивает разные точки Земли с разной силой: чем точка ближе к Луне, тем сила больше. Из-за этой разности Земля под действием Луны слегка вытягивается вдоль линии Земля–Луна, и сильнее всего деформируется её водная оболочка. Поскольку Земля при этом вращается вокруг своей оси, под вытянутостью один за другим проходят берега, и в каждой прибрежной точке уровень океана дважды в сутки поднимается и опускается. Возникающая из разности сил называется приливной силой.

Та же гравитация удерживает ось вращения Земли в узком коридоре наклонов. Земля не идеальный шар: на экваторе она слегка сплюснута, и на эту экваториальную выпуклость гравитация Луны действует с моментом сил. Этот момент возвращает ось к равновесному положению всякий раз, когда возмущения от Солнца и других планет пытаются её сместить, и на геологических временах наклон земной оси удерживается в пределах 22–24,5°.

У Марса крупных спутников нет, и расчёты небесной механики показывают, что его ось за миллионы лет может отклоняться в значительно более широких пределах. Стабильность наклона земной оси — одно из условий, при которых климат меняется достаточно медленно, чтобы биосфера успевала к нему приспосабливаться; условие важное, но не единственное.

### **Материалы для QR (раздел «Земля», уровень В)**

#### **Приливные силы и стабилизация оси: два механизма одной гравитации**

Гравитация убывает с квадратом расстояния, поэтому относительно близкое тело может действовать сильнее, чем далёкое тело гораздо большей массы. Луна — характерный пример: она почти в восемьдесят раз легче Земли и примерно в четыреста раз ближе к ней, чем Солнце, и в результате её гравитационное влияние на Землю в ряде эффектов превосходит солнечное.

**Первый эффект — приливные силы.** Луна притягивает разные точки Земли с разной силой: ближняя сторона притягивается сильнее, дальняя — слабее. Разность этих сил называют приливной силой; под её действием Земля слегка вытягивается вдоль линии Земля–Луна. Жёсткая кора при этом смещается на десятки сантиметров, а подвижная вода океанов — заметно больше. Поскольку Земля вращается вокруг своей оси, под приливным вздутием постоянно проходят новые участки поверхности, и в каждой прибрежной точке уровень воды дважды в сутки поднимается и опускается. Физически здесь работает градиент гравитационной силы — её различие в разных точках одного тела.

**Второй эффект — стабилизация наклона оси вращения.** Земля слегка сплюснута: её экваториальный радиус примерно на 21 километр больше полярного. Гравитация Луны действует на эту экваториальную выпуклость и создаёт момент сил, который стремится развернуть ось вращения Земли перпендикулярно к плоскости лунной орбиты. Поскольку Земля сама вращается, момент не разворачивает ось целиком, а заставляет её прецессировать — медленно описывать конус с периодом около 26 тысяч лет. Одновременно этот момент возвращает ось к равновесному наклону всякий раз, когда возмущения от Солнца и других планет пытаются её сместить. Физически здесь работает уже не градиент силы, а её момент, приложенный к несферичной фигуре Земли.

У обоих эффектов одна и та же причина — гравитационное притяжение Луны, но проявляется она через разные физические механизмы. Это типичная ситуация в небесной механике: одна сила, приложенная к неоднородному и несимметричному телу, способна одновременно вызывать его деформацию и менять его вращательное движение.

#### **Как это посчитали**

Устойчивость наклона земной оси неочевидна теоретически. Даже задача трёх тел в небесной механике не решается в замкнутом виде, поэтому поведение Солнечной системы на больших временах изучают численно.

Жак Ласкар в 1993 году проинтегрировал уравнения движения планет Солнечной системы на интервалы в миллионы лет вперёд и назад. В одной из серий расчётов он удалял Луну из модели и наблюдал поведение земной оси. Без Луны наклон оси выходил из узкого коридора и колебался в значительно более широких пределах; с Луной — оставался около 22–24,5°. Конкретные границы «безлунного» диапазона в более поздних работах разные авторы оценивали по-разному, в зависимости от учитываемых возмущений и начальных условий, но качественный вывод воспроизводится устойчиво: без массивного спутника ось Земли вела бы себя заметно хаотичнее, и наклон менялся бы на десятки градусов за миллионы лет.

Марс служит естественной контрольной точкой для этих расчётов. Крупных спутников у него нет, и численные модели предсказывают, что его наклон оси за миллионы лет действительно колеблется в широком диапазоне; геологические данные о климатических циклах Марса с этими предсказаниями согласуются.

#### **О двух масштабах в модели Солнечной системы**

Любая модель Солнечной системы — школьная, парковая или книжная — сталкивается с одним и тем же противоречием. Если выбрать единый масштаб и для размеров планет, и для расстояний между ними, то либо планеты становятся неразличимы (при модели длиной в сотни метров Землю пришлось бы делать меньше миллиметра), либо тропа растягивается на километры (при сантиметровой Земле расстояние до Нептуна оказывается около десяти километров). Поэтому в большинстве моделей принимают два разных масштаба — один для диаметров, другой для расстояний.

На этой тропе диаметры планет увеличены примерно в 50 раз относительно масштаба расстояний. В масштабе, использованном для диаметров, Луна оказалась бы примерно в 9 сантиметрах от земного шарика. В масштабе, использованном для расстояний между планетами, — всего в 4,5 миллиметра, почти вплотную. В действительности среднее расстояние от Земли до Луны составляет около 384 400 километров, или примерно 30 земных диаметров.

#### **Сравнение Земли и Венеры**

По массе Венера составляет около 81% массы Земли, по радиусу — около 95%. По этим параметрам две планеты можно считать почти близнецами. По условиям на поверхности они расходятся радикально: средняя температура у поверхности Венеры около 465 °C, атмосферное давление около 90 атмосфер, атмосфера состоит почти полностью из углекислого газа, ни жидкой воды, ни океанов на Венере нет.

Современная сравнительная планетология не сводит это расхождение к одной причине. Венера расположена ближе к Солнцу, и поток солнечной энергии на её орбите примерно вдвое больше, чем у Земли, что смещает порог устойчивости жидкой воды на поверхности и меняет режим водяного пара в атмосфере. По одной из общепринятых гипотез, в ранней истории Венеры вода поднялась в верхние слои атмосферы, разложилась под действием солнечного ультрафиолета на водород и кислород, и водород был утрачен в космос. Геологическая эволюция Венеры тоже отличается от земной: на ней нет тектоники литосферных плит, основной механизм разгрузки внутреннего тепла — крупные эпизоды вулканизма, а состав атмосферы поддерживается долговременной дегазацией недр. Все эти процессы взаимосвязаны и могут усиливать друг друга.

Окончательного ответа на вопрос, почему две планеты, близкие по массе и радиусу, пришли к столь разным состояниям, у современной науки пока нет.

#### **Гравитация, космические скорости, навигация**

Гравитация Земли определяет две характерные скорости, важные для космической техники. Первая космическая скорость, около 7,9 км/с — минимальная скорость, при которой тело, движущееся горизонтально у поверхности Земли, удерживается на круговой орбите и не падает обратно. Вторая космическая скорость, около 11,2 км/с — минимальная скорость, при которой тело покидает гравитационное поле Земли. Обе скорости зависят только от массы и радиуса притягивающего тела и для других планет и звёзд вычисляются по тем же формулам.

Иногда стабилизирующее действие Луны на ось Земли связывают с возможностью астрономической навигации, но эта связь не вполне корректна. Практическая навигация по светилам опирается на регулярность видимого суточного и годового движения звёзд и планет, на астрономические таблицы и на точное измерение времени. Долгопериодические изменения наклона земной оси, которые Луна сглаживает на временных шкалах в миллионы лет, для практической навигации несущественны: эти изменения происходят слишком медленно, чтобы сказаться на жизни даже многих поколений мореходов.

#### **Открытый вопрос**

Насколько строго стабильный наклон оси необходим для существования сложной жизни, остаётся открытым вопросом. В геологическом прошлом Земля проходила через периоды значительных колебаний наклона, вызванных резонансами с другими планетами, и тогдашняя биосфера была устроена проще современной. Где находится порог допустимой нестабильности для современной сложности жизни и насколько в этом вопросе работают другие факторы — атмосфера, океан, орбитальные циклы, углеродный цикл, тектоника, — пока определить не удаётся.

  

### **Эксплуатация**

  - **Крепление сферы — сварное.** На малом шарике винтовое крепление уязвимо для откручивания и срыва; сварка даёт живучесть на уровне всей конструкции.
  - **Покраска шарика** выцветает за 2–4 года под УФ. Это допустимо: узнаваемость в сине-зелёно-белой гамме сохраняется. В график обслуживания станции — обновление покраски шариков раз в 3–4 года.
  - **Табличка** — гравировка по нержавеющей стали, замены не требует.
  - **QR на столбике не размещается.** Общий QR станции ведёт на страницу с разделом «Земля», там лежит всё содержимое уровня В. Отдельный QR на малом объекте утяжелил бы композицию и продублировал навигацию.

### **Эмоциональный эффект**

Из девяти столбиков серии этот — точка личной привязки. После «горошины-Земли в пяти с половиной метрах от Солнца» масштаб тропы перестаёт быть абстрактным числом и становится телесным переживанием: остальные планеты воспринимаются уже не как список, а как соседи по одной системе.

  
  
  

# ➕ 9WP008-г «Марс» (столбик-планета)  

## **9WP008-г «Марс» — итоговое описание экспоната**

### **Конструкция**

Стандартный столбик станции «Солнечная система»: стальная труба Ø 60 мм высотой 1,5 м с порошковой антивандальной покраской, забетонирована в основание. На вершине — стальная подставка-«ладонь» с закреплённой сферой Ø 12,5 мм, окрашенной в красно-рыжий. Диаметр выдержан в масштабе станции (диаметры планет увеличены в 50 раз, от Солнца до Нептуна 165 м). Расстояние от Солнца по маршруту около 8,4 м. Табличка из нержавеющей стали с гравировкой, антивандальные болты. QR общий со станцией, отдельного кода на объекте нет.

### **Место на маршруте**

После Земли посетитель делает несколько шагов и подходит к Марсу — первой планете, заметно отделённой от тесной группы внутренних миров. Маленькая табличка отрабатывает одну идею: облик Марса во многом объясняется его малой массой, но не сводится к ней. Дальше по маршруту газовые гиганты покажут обратную сторону того же приёма: что даёт избыток массы.

### **Тип научного рассуждения**

Один параметр объясняет многое, но не всё. Малая масса Марса позволяет понять несколько разнородных свойств: тонкую атмосферу, маленькое ядро, отсутствие тектоники плит, существование Олимпа. Однако как только речь заходит о том, куда делась густая ранняя атмосфера, одной массы уже недостаточно, и в объяснение приходится включать магнитное поле и солнечный ветер. В этом и состоит приём сравнительной планетологии: попытка вывести облик планеты из немногих параметров и фиксация границ, в которых такой вывод работает.

### **Текст таблички (уровень А, стиль «Квант»)**

#### **Марс. Почему самая высокая гора Солнечной системы находится на маленькой планете?**

Сфера на столбике изображает Марс в едином масштабе со всей станцией: расстояние от модели Солнца здесь 8,4 метра. По диаметру Марс почти вдвое меньше Земли, по массе примерно в десять раз. Многие свойства планеты связаны с этой малой массой, хотя и по-разному.

Диаметр Марса 6 779 километров. Атмосфера почти целиком состоит из углекислого газа, давление у поверхности в сотни раз ниже земного. При этом самая высокая гора Солнечной системы (потухший вулкан Олимп высотой около 22 километров) находится именно на Марсе. Сочетание выглядит странно: маленькая планета с тонкой атмосферой и при этом крупнейший вулкан Солнечной системы. Объясняются эти свойства одной общей причиной — малой массой, но через разные физические механизмы.

**Гора.** Малое ядро Марса остыло относительно быстро, и тектоника плит, привычная для Земли, на этой планете не сформировалась: марсианская кора практически неподвижна. На Земле вулкан, расположенный над «горячей точкой» в мантии, постепенно сползает с неё вместе с литосферной плитой, и за миллионы лет образуется цепочка островов, как, например, Гавайские. На неподвижной коре Марса один и тот же вулкан может расти над одной точкой миллиарды лет, что и привело к появлению Олимпа.

Высоту этой горы определяет и второй механизм, чисто механический. Сила тяжести на поверхности Марса составляет примерно 38% земной. Эта величина непропорциональна диаметру: она зависит от массы планеты и её радиуса, а через них — от средней плотности. Меньшая плотность Марса дополнительно ослабляет гравитацию по сравнению с тем, что давала бы одна разница в размере. При меньшей силе тяжести основание горы выдерживает значительно больший столб породы, прежде чем материал начнёт пластически течь. На Земле гора высотой 22 километра не удержалась бы и расплылась бы под собственным весом, а на Марсе такая высота возможна.

**Атмосфера.** Здесь история устроена сложнее. Лёгкие газы (водород, гелий, водяной пар) слабая гравитация удерживает плохо: их молекулы при тепловом движении часто достигают второй космической скорости и постепенно уходят в космос. Однако это лишь часть истории. Основную массу ранней плотной атмосферы Марс потерял по другой причине. Когда жидкое ядро планеты остыло, в нём прекратилась конвекция и исчезло глобальное магнитное поле. С этого момента атмосфера Марса оказалась беззащитной перед солнечным ветром, потоком заряженных частиц от Солнца, который у Земли отклоняется магнитосферой. Солнечный ветер ионизирует молекулы верхней атмосферы и уносит ионы в космос. Орбитальный аппарат MAVEN измерил скорость этого процесса в наши дни: около ста граммов газа в секунду.

**Цвет.** Красная окраска поверхности обусловлена окисленным железом в пыли. По существу это та же химия, что у обычной ржавчины: железо в марсианских породах когда-то прореагировало с водой и кислородом, оксиды железа со временем измельчились, и ветер за миллиарды лет разнёс рыжую пыль по всей планете.

Сфера на столбике и расстояние от неё до модели Солнца выдержаны в едином масштабе со всей станцией, что позволяет напрямую сравнить Марс с соседней моделью Земли. Разница в диаметре заметна с первого взгляда, тогда как разница в силе тяжести на поверхности заметно больше: эти два соотношения связаны с массой планеты, но через разные физические зависимости, и отождествлять их не следует.

### **Что идёт в общий QR станции, раздел «Марс» (уровень Б, стиль «Квант»)**

Этот раздел углубляет одну линию таблички: как малая масса планеты связана с её обликом и где границы такого объяснения.

**1. Что удерживает атмосферу.**

Для любой планеты можно определить вторую космическую скорость — минимальную скорость, при которой пробное тело покидает её гравитационное поле и больше не возвращается. Молекулы газа в атмосфере движутся с разными скоростями, и существует небольшой «хвост» распределения, в котором молекулы движутся значительно быстрее средней. Чем легче молекула и чем выше температура газа, тем больше доля молекул, у которых скорость превышает вторую космическую. Эти молекулы и покидают планету.

Вторая космическая скорость составляет около 5 км/с для Марса, 11,2 км/с для Земли и около 60 км/с для Юпитера. Этим объясняется, почему Марс не удержал водород и гелий, тогда как Юпитер удерживает даже водород. Однако для тяжёлых молекул, и в первую очередь для углекислого газа, тепловой механизм работает плохо, и его одного недостаточно, чтобы объяснить, куда делась плотная ранняя атмосфера Марса.

Основной механизм здесь иной и связан не с гравитацией, а с магнитным полем. Глобальное магнитное поле Земли создаётся динамо-механизмом в её жидком внешнем ядре: конвективные течения проводящего вещества в присутствии затравочного поля поддерживают его существование. На молодом Марсе подобное поле тоже было, но ядро у этой планеты относительно небольшое и быстро остыло; примерно четыре миллиарда лет назад конвекция остановилась и поле исчезло. С этого времени атмосфера Марса оказалась открытой потоку солнечного ветра. Заряженные частицы солнечного ветра ионизируют молекулы верхней атмосферы и уносят их в космос. Этот процесс называют ионным выносом, и миссия MAVEN с 2014 года измеряет его темп: Марс продолжает терять атмосферу и сегодня, со скоростью около ста граммов в секунду.

Иначе говоря, малая масса — один из ключевых факторов в истории марсианской атмосферы, но не единственный. К нему добавляются потеря магнитного поля, действие солнечного ветра, поглощение углекислого газа карбонатами в горных породах и последствия ранних столкновений с крупными телами.

**2. Предельная высота горы.**

Грубую оценку максимальной высоты горы можно получить из условия, что давление столба породы на основание не превышает её предела прочности на сжатие:

h≈σ/(ρg),h \\approx \\sigma / (\\rho g),h≈σ/(ρg),

где σ — прочность породы на сжатие, ρ — её плотность, g — ускорение свободного падения. Для Земли эта оценка даёт около 10 километров, что близко к высоте Эвереста. Для Марса при g ≈ 3,7 м/с² (примерно в 2,6 раза меньше земного) предельная высота поднимается до 25–26 километров, и Олимп с его 22 километрами в этот диапазон укладывается. Существование такой горы — прямое следствие малой массы планеты.

**3. Сравнение четырёх планет: Меркурий, Марс, Земля, Венера.**

Один параметр (масса) при прочих равных давал бы четыре сценария: отсутствие атмосферы у Меркурия, тонкая атмосфера у Марса, умеренная с водой у Земли, плотная и горячая у Венеры. Венера, однако, показывает, что прочих равных не бывает. Её масса близка к земной, но сценарий радикально отличается, и ключевой фактор здесь уже не масса, а расстояние до Солнца и развившийся парниковый эффект.

**4. Красный цвет.**

Марсианская пыль состоит главным образом из оксидов и оксогидроксидов железа. Долгое время считалось, что основная её часть — безводный гематит, образовавшийся в сухую эпоху планеты через медленное окисление железосодержащих минералов атмосферой. Работа 2025 года, в которой данные орбитальных аппаратов сопоставлены со спектрами лабораторно воспроизведённой марсианской пыли, показала, что основной компонент окраски — это ферригидрит, минерал, образующийся в холодной воде. Это означает, что окисление железа произошло ещё во влажный период ранней истории Марса, а позднее ветер за миллиарды лет разнёс пыль по всей поверхности. Не все детали схемы устоялись, но в главном согласие есть: красная окраска Марса — это ржавчина из ранней эпохи.

**5. Как это узнала наука.**

Состав атмосферы Марса измеряют спектрометры орбитальных и посадочных аппаратов: Viking в 1970-х годах, Curiosity с 2012 года, Perseverance с 2021 года. Скорость атмосферного убегания и роль солнечного ветра изучает миссия MAVEN с 2014 года. Гравитационное поле и внутреннее строение восстанавливают по отклонениям траекторий искусственных спутников, а в 2018–2022 годах сейсмометр аппарата InSight зарегистрировал марсотрясения и позволил оценить параметры коры, мантии и ядра. Высоту Олимпа измерил лазерный альтиметр MOLA на орбитальном аппарате Mars Global Surveyor с точностью до метров.

**6. Открытый вопрос.**

Какой плотности достигала атмосфера молодого Марса и когда именно она ушла? От этого зависит, как долго на поверхности существовала жидкая вода и сколько времени сохранялись условия, при которых могла появиться жизнь. Ответ ищут несколькими путями: измерением изотопных отношений в современной атмосфере (тяжёлые изотопы накапливаются, потому что лёгкие убегают первыми), поиском карбонатов и других следов ранней атмосферы в породах, моделированием ранней магнитосферы и динамо-механизма в остывающем ядре.

### **Что вынесено с этой точки маршрута**

Темы, которые могли бы попасть в QR Марса, но шире и относятся к другим точкам станции:

  - история открытия закона тяготения, законы Ньютона, расчёт траекторий — в отдельную точку «Движение и тяготение» или в QR Солнца;
  - теории образования планетной системы, эклиптика — в QR Солнца;
  - демонстрация с ведром воды на вращении — в отдельный демонстрационный модуль рядом с темой тяготения.

В QR Марса остаётся только то, что раскрывает основную линию: один параметр объясняет многое, но не всё.

### **Ограничения**

Уличный формат. Антивандальная порошковая покраска столбика, нержавеющая табличка, антивандальные болты. Красно-рыжий оттенок сферы со временем выгорает на солнце, и одну сферу планируется перекрашивать раз в 3–5 лет. Единый QR станции снимает необходимость обслуживать отдельный код на малом объекте.

### **Способ взаимодействия**

Чтение и сравнение масштабов: сфера на столбике соотносится с расстоянием до модели Солнца и с соседней сферой Земли.

### **Параметры**

|  |  |
| :-: | :-: |
| \*\*Параметр\*\* | \*\*Значение\*\* |
| QR отдельный | нет (через общий QR станции) |
| Подсветка | нет |
| Перекраска сферы | раз в 3–5 лет |

  
  

# ➕ 10WP008-д «Пояс астероидов» (маркер)  

  

# **10WP008-д «Пояс астероидов» (маркер)**

**Тип экспоната**

Маркер области. Ставится между Марсом и Юпитером как условная отметка протяжённой зоны — не планеты, а пояса из множества мелких тел.

**Конструкция**

Невысокий металлический столбик высотой 1,0 м, забетонированный в основание. На вершине — горизонтальная рамка размером \~20×15 см, в которую залиты 15 разнокалиберных камней (гравий, галька) в прозрачной эпоксидной смоле с UV-стабилизатором. Табличка — нержавеющая сталь с гравировкой, размещена так, чтобы рамка с камнями попадала в поле зрения сразу под текстом. Крепление — антивандальные болты.

**Размещение в масштабной схеме**

При масштабе станции 1 а.е. ≈ 5,5 м главный пояс (2,06–3,28 а.е., от внутренней резонансной границы 4:1 с Юпитером до внешней 2:1) занимает зону примерно от 11,3 м до 18 м от Солнца. Маркер ставится в визуальный центр этой зоны — около 15 м. Рамка с камнями работает как тактильный объект: посетитель видит горстку камней на пятачке 20×15 см, и это становится опорой для объяснения на табличке (в реальности такая же горстка разбросана по объёму в сотни миллионов километров).

**Круглогодичность**

Частичная. При высоком снежном покрове нижняя часть столбика может уходить под снег — поэтому табличка размещается так, чтобы читаться и зимой, даже если основание утоплено в снег на 30–40 см.

**Смысловая задача**

После Марса маршрут делает переход: дальше уже не одна планета, а целая область, заполненная множеством небольших тел. Маркер развенчивает киношный образ «каменной стены», через которую корабль с трудом прорывается. Главная физика экспоната — ограничение со стороны массивного соседа: близкий Юпитер мешает веществу на этих орбитах собраться в обычную планету и продолжает выбивать тела из устойчивых орбит до сих пор. Из этого стержня вытекают и остальные особенности пояса.

**Тип научного рассуждения**

Ограничение. Близость массивного соседа подавляет планетную сборку — и из этого одного механизма вытекают все наблюдаемые особенности: миллионы мелких тел вместо одной планеты, малая суммарная масса, высокая разреженность, беспрепятственный пролёт аппаратов.

**Текст таблички**

**Пояс астероидов**

В прозрачную смолу на верхушке столбика залиты пятнадцать разнокалиберных камней — компактный образ того, что заполняет кольцевую область между орбитами Марса и Юпитера. Это главный пояс астероидов: миллионы твёрдых тел самого разного размера, от пылинок до Цереры диаметром около 940 километров. Суммарная масса всего пояса при этом в тридцать с лишним раз меньше массы Луны.

Почему вещество, распределённое в широком кольце, не собралось ещё в одну планету, как это произошло с Землёй или Марсом? Главная причина — близость Юпитера. Своим тяготением он постоянно возмущает движение астероидов: их орбиты вытягиваются (растёт эксцентриситет), а относительные скорости при сближениях возрастают. При таких скоростях столкновения уже не приводят к слипанию — взаимного тяготения сталкивающихся тел не хватает, чтобы удержать их вместе, и при ударе они дробятся. Этот эффект называют подавлением аккреции: близкая массивная планета не даёт мелким телам сливаться в крупные.

Внутри пояса есть полосы, которые почти пусты. Это резонансные орбиты — те, на которых период обращения тела составляет с периодом Юпитера простое отношение, например 3:1 или 5:2. На такой орбите Юпитер регулярно проходит мимо астероида в одной и той же точке его пути; одинаково направленные толчки накапливаются, и за миллионы лет тело уходит из резонансной полосы. Эти провалы называют щелями Кирквуда; самый внешний из них, резонанс 2:1, очерчивает внешнюю границу пояса.

Пояс настолько разрежён, что межпланетные аппараты пересекают его без специальных манёвров: с 1972 года через него прошли «Пионер-10», оба «Вояджера», «Галилео», «Кассини», «Новые горизонты», и ни один не встретил астероид случайно. Сближение с конкретным телом требует отдельной прицельной траектории, рассчитанной заранее.

**Содержание QR**

*Щели Кирквуда.* На резонансной орбите тело раз за разом получает гравитационный толчок от Юпитера в одной и той же фазе своего движения. Однонаправленные возмущения накапливаются: эксцентриситет орбиты постепенно растёт, и в итоге тело покидает резонансную полосу. Сильнее всего эффект проявляется при простых отношениях периодов — 4:1, 3:1, 5:2 и 2:1; именно этим четырём резонансам соответствуют наиболее заметные провалы в распределении астероидов. Резонанс 2:1 (около 3,28 астрономической единицы от Солнца) задаёт внешнюю границу пояса.

*Тот же механизм в других системах.* Троянские астероиды Юпитера движутся по той же орбите, что и сам Юпитер, в точках Лагранжа L4 и L5, расположенных на 60° впереди и позади планеты. Эти точки — устойчивые положения равновесия в гравитационном поле системы «Солнце — Юпитер»: попавшее в них тело удерживается совместным действием двух притяжений и центробежной силы. Кольца Сатурна остаются тонкими и не сливаются в спутник по близкой причине: они находятся внутри предела Роша — зоны, где приливные силы планеты превышают взаимное тяготение частиц, и крупное тело не может удержаться от приливного разрушения. За одним принципом — гравитационным доминированием более массивного соседа — стоит целый набор разных по виду структур.

*Церера.* Среди всех тел пояса только Церера обладает достаточно сильным собственным тяготением, чтобы сжать себя в почти шарообразную форму; её диаметр приближается к 940 километрам. По этому признаку — достигнутому гидростатическому равновесию — её относят к карликовым планетам. От полноценной планеты Цереру отличает то, что она не расчистила свою орбиту от других тел: она движется внутри пояса, среди множества меньших соседей.

*Открытый вопрос.* Сколько вещества было в этой зоне изначально, какую его часть унесли мигрирующие гиганты Юпитер и Сатурн и куда оно делось, до сих пор остаётся предметом моделирования. Современные сценарии (Grand Tack, jumping-Jupiter и их варианты) сходятся в том, что нынешний пояс — небольшой остаток первоначальной популяции тел, сильно перемешанной по составу за счёт миграции гигантов.

  

**Примечания для монтажа**

Высота таблички рассчитана на чтение с учётом возможного снежного покрова до 30–40 см.

Эпоксидная смола — обязательно с UV-стабилизатором. Без него смола пожелтеет и помутнеет за 2–3 сезона, и камни перестанут читаться.

Камни в рамке — разнокалиберные, от 3–5 мм до 2–3 см, визуально разнообразные по цвету и форме. Это важно: разнообразие поддерживает тезис «миллионы тел разного размера».

Гравировка таблички глубокая, контрастная заливка (чёрная по нержавейке) — чтобы текст читался и при косом освещении, и при снеге, налипшем на поверхность.

Расположение маркера на тропе: точка \~15 м от объекта «Солнце» по основной линии масштаба. Если по геометрии тропы 15 м недостижимы (поворот, перепад высоты, корни) и приходится ставить ближе к 18 м, в табличке тогда стоит мягко оговорить: «маркер у внешней границы пояса, ближе к Юпитеру».

  
  
  

# ➕ WP008-е «Юпитер» (столбик-планета)  

## **WP008-е «Юпитер» (столбик-планета) — финальная редакция**

**Тип станции**

Точка-планета на маршруте «Солнечная система». Одиннадцатая по счёту от Солнца — после Меркурия, Венеры, Земли, Луны, Марса и пояса астероидов. Первый объект в ряду газовых гигантов.

**Конструкция**

Планетарный столбик, унифицированный с другими планетами станции.

  - Столбик — стальная труба ⌀60 мм, высота 1,5 м, порошковая антивандальная покраска.
  - Основание — забетонированное, заподлицо с покрытием тропы.
  - Подставка — стальная пластина-«ладонь», приваренная к вершине столбика.
  - Сфера-Юпитер — диаметр 26,2 см. Окраска полосатая, бежево-рыжая гамма с намёком на атмосферные пояса. Полосы размытые, без точной калибровки — полосы самого Юпитера так же размыты и текучи.
  - Табличка — нержавеющая сталь с гравировкой, крепление антивандальными болтами к столбику под сферой.
  - Углублённый материал — через единый QR станции «Солнечная система», размещённый на стартовом стенде у Солнца. Отдельного QR на Юпитере нет.

**Масштаб и расстояние**

Диаметр сферы получен из реального диаметра Юпитера (139 820 км) увеличением диаметров в 50 раз относительно исходного масштаба модели. Расстояние от центра Солнца по маршруту — \~28,5 м. Полная длина станции — 165 м до Нептуна.

В модели диаметры и расстояния сжаты в разных масштабах: будь они в одном, пришлось бы выбирать между невидимыми точками-планетами на 165 метрах и реалистичными сферами на тропе в несколько километров. Это само по себе небольшой урок — о том, как несопоставимы в Солнечной системе размеры тел и промежутки между ними.

**Роль точки на маршруте**

Юпитер — первый объект, который выглядит как тело, а не как точка. До него посетитель видел четыре горошины (Меркурий и Марс — меньше сантиметра, Земля и Венера — по 1,3 см) на первых 20 метрах тропы. Переход от последней горошины к 26-сантиметровой сфере — физически ощутимый контраст, который несёт основной образовательный груз точки. Всё остальное — текст, пояснение к этому контрасту.

Расстояние до Юпитера по тропе (\~28,5 м от Солнца) почти втрое больше суммарного расстояния до всех четырёх внутренних планет. И это тоже не случайность: между Марсом и Юпитером в ранней Солнечной системе проходила снеговая линия — граница, за которой менялся сам тип собираемых планет.

### **Текст таблички**

**Юпитер: между планетой и звездой**

Перед стендом — сфера диаметром 26 сантиметров. Позади, на первых двадцати метрах тропы, четыре горошины: Меркурий, Венера, Земля и Марс. Юпитер на этой модели — первое тело, форму которого можно разглядеть невооружённым глазом.

Его диаметр — около 140 000 километров, масса — в 318 раз больше массы Земли; в Юпитере уместились бы все остальные планеты Солнечной системы, и ещё осталось бы место. Но дело не только в размере. Внутренние планеты сложены из камня и металла; Юпитер почти целиком состоит из водорода и гелия, как Солнце. По составу он ближе к звезде, чем к Земле.

Откуда такое различие? В молодой Солнечной системе вещество в протопланетном диске было распределено неравномерно. Близко к Солнцу было жарко, и лёд испарялся; собираться в планеты могли только тугоплавкие соединения — камень и металл. Дальше от Солнца температура опускалась ниже точки замерзания воды, аммиака и метана, и эти вещества переходили в твёрдую фазу. Границу, за которой это происходит, называют снеговой линией. Льда за ней оказалось в несколько раз больше, чем камня внутри: зародыш Юпитера быстро набрал массу и стал притягивать ещё и газ, которым диск был богат.

Мог ли Юпитер стать звездой? Чтобы в центре газового шара начался термоядерный синтез водорода, температура там должна достичь примерно десяти миллионов градусов. Эта температура задаётся массой: чем тяжелее шар, тем сильнее внешние слои сдавливают центр, тем выше температура в его глубине. Расчёт и наблюдения дают для порога зажигания водорода массу около 0,08 массы Солнца — примерно в 80 раз больше массы Юпитера. До этой границы Юпитеру не хватило вещества.

На модели снеговая линия приходится на участок тропы между Марсом и Юпитером: десять метров отделяют последнюю каменную планету от первого гиганта.

За снеговой линией собрались и три другие планеты-гиганта: Сатурн, Уран и Нептун. Сатурн похож на Юпитер по составу — тоже газовый шар из водорода и гелия. Уран и Нептун выделяют в отдельный класс: их недра состоят в основном из воды, аммиака и метана под огромным давлением, и поэтому их называют ледяными гигантами. Из четырёх гигантов ни один не подошёл к порогу зажигания водорода ближе, чем Юпитер.

**Диаметр:** 139 820 км **Масса:** 318 масс Земли; более чем вдвое больше суммарной массы остальных планет **Расстояние от Солнца:** 5,2 а. е. (778 млн км) **Расстояние на тропе:** 28,5 м от Солнца **Масштаб сферы:** диаметр увеличен в 50 раз от исходного масштаба модели

  

### **Содержание для единого QR станции (раздел «Юпитер»)**

**Почему 80 юпитеров зажглись бы, а 79 — нет**

Масса определяет температуру в центре газового шара через простую логику: внешние слои давят на ядро, ядро сопротивляется давлением разогретого газа. Чем больше масса, тем сильнее давление, тем выше температура равновесия. Термоядерный синтез водорода включается примерно при 10⁷ K — этот порог соответствует массе около 0,08 массы Солнца, то есть \~80 масс Юпитера.

Между планетами и звёздами есть промежуточный класс — коричневые карлики. Их массы — от 13 до 80 юпитерианских. Им хватает давления, чтобы запустить синтез дейтерия (он идёт при более низких температурах), но не хватает для синтеза обычного водорода. Светят они тускло и быстро остывают.

Массы коричневых карликов и планет-гигантов у других звёзд измеряют по слабым колебаниям родительской звезды под действием их тяготения и по затмениям при транзитах. Температуру и состав атмосфер — по спектрам. Пороговая масса в 80 юпитеров взята не с потолка: это результат моделей внутреннего строения, проверенных наблюдениями сотен объектов.

**Почему Большое красное пятно живёт столетиями**

Большое красное пятно — вихрь шириной больше Земли. Непрерывно его наблюдают с конца XIX века: первое надёжно зафиксированное наблюдение относится к 1831 году, регулярные — с 1878-го. Есть данные, что нечто похожее видели и астрономы XVII века (так называемое «постоянное пятно» Кассини), но непрерывной цепочки наблюдений между XVII и XIX веками нет, и доказать, что это тот же самый вихрь, пока не удалось. В любом случае один и тот же атмосферный вихрь живёт на одной планете не меньше полутора столетий.

На планете с твёрдой поверхностью такой вихрь рассеялся бы за недели: трение о сушу и горы тормозит вращение воздушных масс. У Юпитера поверхности нет — атмосфера плавно переходит в жидкий водород, без границы, о которую можно было бы «разбиться».

Это общая черта газовых гигантов: у Сатурна тоже живут долгоживущие вихри (например, «Большой белый овал»), и полосы на его диске — те же атмосферные пояса, что у Юпитера. У ледяных гигантов вихри тоже бывают (на Нептуне его наблюдал «Вояджер-2»), но менее устойчивые.

С середины XX века пятно постепенно уменьшается. В 1880-х его длина оценивалась в \~40 000 км, сейчас — около 12 000 км, форма из вытянутого овала приближается к кругу. Механизм уменьшения понят не полностью — это открытый вопрос.

**Что ещё**

*Спутники Юпитера.* Четыре крупнейших — Ио, Европа, Ганимед, Каллисто — открыл Галилей в 1610 году в самодельный телескоп. Это было первое прямое наблюдение тел, обращающихся вокруг другой планеты, а не вокруг Земли, — сильный аргумент в пользу гелиоцентрической системы Коперника. Ганимед — крупнейший спутник в Солнечной системе, по размеру больше Меркурия (по массе — вдвое легче). Под ледяной корой Европы с высокой вероятностью есть жидкий океан.

*Где граница «большой планеты» и «маленькой звезды».* У других звёзд находят планеты массивнее Юпитера; коричневые карлики обнаружены и как одиночные объекты, и в системах со звёздами. Провести чёткую линию между этими классами — по массе, по способу образования или по тому, идёт ли в ядре хоть какой-то синтез, — до сих пор предмет спора.

**Рекомендуемая иллюстрация на QR-странице**

Одномерная шкала масс в единицах массы Юпитера, три порога:

|  |  |  |
| :-: | :-: | :-: |
| \*\*Объект\*\* | \*\*Масса (в юпитерах)\*\* | \*\*Что происходит\*\* |
| Земля | 0,003 | Каменная планета |
| Юпитер | 1 | Газовая планета |
| Коричневые карлики | 13–80 | Синтез дейтерия, синтеза водорода нет |
| Звезда (красный карлик) | от 80 | Синтез водорода, светит |

Одна ось, три порога — визуальная сводка ключевого рассуждения.

**Взаимодействие посетителя**

Визуальное и сравнительное. Посетитель не трогает сферу (хотя она это выдержит — шар приварен, столбик забетонирован). Смысловое действие — оглянуться назад на горошины-планеты и вперёд на следующих гигантов, ощутить переход. Дополнительное действие — вернуться к Марсу и взглянуть на Юпитер оттуда: десяток метров маршрута между ними и есть «снеговая линия» модели.

**Обслуживание**

  - Порошковая покраска столбика — осмотр ежегодно, полное обновление каждые 5–7 лет.
  - Окраска сферы (УФ-выцветание полос) — обновление каждые 3–5 сезонов; функционально на экспонат не влияет, косметика.
  - Гравированная табличка из нержавеющей стали — 15+ лет без замены.
  - Бетонное основание — десятилетия.
  - Тонкой настройки нет, ломать нечего, вандалоустойчивость максимальная для формата.

<!-- end list -->

  -   

  
  

# ➕ WP008-ж «Сатурн» (столбик-планета)  

## **WP008-ж «Сатурн» — финальная версия**

### **Конструкция**

Типовой столбик планетарной серии станции: стальная труба Ø 60 мм высотой 1,5 м, порошковая антивандальная покраска, бетонный фундамент. На вершине — сфера Ø 22,1 см в светло-песочной гамме. Вокруг сферы — кольцо Ø \~45 см в плоскости горизонта, стилизующее кольца Сатурна.

### **Требования к кольцу**

Кольцо заявлено как тактильный элемент — единственная точка маршрута, где посетитель может прикоснуться к модели. Это требует пересмотра исходного ТЗ («стальная полоса на тонких спицах»): тонкие спицы не выдерживают нагрузки взрослого человека, а полоса с острой кромкой на уровне лица опасна. Два варианта исполнения:

**Кольцо-трубка.** Замкнутое кольцо из стальной трубки Ø \~10 мм, приваренное к сфере через 3–4 короткие массивные стойки. Закруглённый профиль исключает опасные кромки, трубчатое сечение держит нагрузку на боковое касание и опору рукой.

**Перфорированный диск.** Плоский металлический диск в плоскости колец, перфорированный или сетчатый для облегчения веса. Ближе по физическому смыслу к настоящим кольцам (это сплошной диск, а не обод), прочнее на распределённую нагрузку.

В любом случае — закруглённые кромки, расчёт на нагрузку взрослого человека при опоре рукой (не на «лечь на кольцо»), порошковая антивандальная покраска, коррозионная защита в точках крепления.

Расстояние от Солнца по маршруту — 52,3 м от центра модели Солнца. Табличка — нержавеющая сталь с гравировкой, антивандальные болты. Углублённые материалы — через общий QR станции «Солнечная система», без отдельного кода на малом объекте.

### **Место на маршруте**

После Юпитера маршрут снова растягивается. Сатурн встречает посетителя как второй газовый гигант, но с другим визуальным образом: Юпитер берёт массой, Сатурн — геометрией колец и неожиданно низкой плотностью. Это единственная точка маршрута с тактильным контактом — посетитель может коснуться модели кольца. После Сатурна маршрут уходит к ледяным гигантам, Урану и Нептуну, где работает уже иная химия.

### **Тип научного рассуждения**

Общая причина. Два бросающихся в глаза факта — плотность Сатурна ниже плотности воды и крайне тонкие кольца — сводятся к одной логике: Сатурн собран из лёгкого вещества, а вращающиеся системы частиц со временем неизбежно уплощаются. На заднем плане — повторяющийся механизм: уплощение работает одинаково для колец Сатурна, протопланетного диска Солнечной системы, дисковых галактик и аккреционных дисков вокруг чёрных дыр.

### **Текст таблички**

# **Сатурн: почему планета легче воды и почему её кольца такие тонкие**

Сфера диаметром около 22 сантиметров на столбике — модель Сатурна. Кольцо вокруг сферы — модель его колец; это единственный элемент на маршруте, до которого можно дотронуться рукой. Расстояния между объектами на маршруте даны в масштабе 1 : 50 миллиардов, и в нём же модель Солнца находится в 52 метрах позади. Размеры самих планет показаны крупнее: в масштабе расстояний Сатурн был бы здесь сферой не больше двух миллиметров.

Сатурн — вторая по размеру планета Солнечной системы со средним диаметром около 116 000 километров. При этом его средняя плотность составляет 0,69 г/см³, что меньше плотности воды. Причина в составе: Сатурн почти целиком состоит из водорода и гелия, двух самых лёгких элементов, а на тяжёлые вещества в ядре приходится сравнительно небольшая часть массы. Средняя плотность планеты близка к плотности водородно-гелиевой смеси при тех давлениях, которые создаёт её собственная гравитация. Распространённое утверждение «Сатурн плавал бы в воде» представляет собой корректное сравнение плотностей, но не описание возможной сцены: подходящего по размеру океана во Вселенной не существует, а собственная гравитация не позволила бы планете спокойно существовать внутри чужой жидкости.

Главные кольца Сатурна имеют почти 280 000 километров в поперечнике, что составляет около трёх четвертей расстояния от Земли до Луны. Типичная же толщина колец — около 10 метров, так что отношение поперечника к толщине оказывается примерно 1 : 28 000 000. Такая геометрия не случайна. Кольца сложены из множества кусков водяного льда (от пылинок до глыб размером с дом), которые движутся вокруг планеты по близким орбитам. Если какая-то частица оказывается выше или ниже средней плоскости, рано или поздно она сталкивается с соседями; в каждом столкновении горизонтальная составляющая скорости в среднем сохраняется, а вертикальная частично гасится и переходит в тепло. За миллионы лет вертикальные движения постепенно затухают, и облако частиц вытягивается в тонкий диск.

Этот процесс называют диссипативным уплощением вращающейся системы. Он действует всюду, где вокруг общего центра движется множество частиц, способных сталкиваться. Тот же механизм 4,5 миллиарда лет назад превратил протопланетное облако вокруг молодого Солнца в плоский диск, и именно поэтому орбиты всех планет лежат сегодня почти в одной плоскости. Он же формирует звёздные диски спиральных галактик и аккреционные диски, в которые сворачивается вещество, падающее на чёрные дыры. Низкая плотность Сатурна и крайняя тонкость его колец оказываются двумя независимыми проявлениями физики, общей для систем самого разного масштаба — от роя ледяных частиц вокруг планеты до окрестностей чёрной дыры.

  

### **Что идёт в общий QR станции (раздел «Сатурн»)**

Одна линия, без ответвлений. Всё углубляет принцип «лёгкое вещество и уплощение дисков — одна история».

**1. Как на самом деле выстраиваются плотности планет.** Близко к Солнцу в протопланетном диске было жарко, и конденсировались только тяжёлые вещества — силикаты, металлы, окислы. Каменные планеты получились плотные: Меркурий 5,43 г/см³, Венера 5,24, Земля 5,51, Марс 3,93. За «снеговой линией» стало достаточно холодно, чтобы замерзали лёгкие соединения — вода, аммиак, метан; а у самых массивных зародышей хватило гравитации, чтобы собрать сам водород и гелий. Так выросли газовые гиганты с плотностью в несколько раз ниже: Юпитер 1,33, Сатурн 0,69, Уран 1,27, Нептун 1,64. Среди них Сатурн оказался самым лёгким: при его массе сжатие недр меньше, чем у Юпитера, а доля тяжёлых элементов в общем составе небольшая. Заметь: строгого падения «по порядку планет» нет. Нептун плотнее Урана, Сатурна и даже Юпитера; среди каменных Меркурий чуть плотнее Венеры и Марса, но уступает Земле. Общая картина другая — большой разрыв между группой каменных планет и группой газовых; точный порядок внутри каждой группы определяется массой, составом и историей конкретного тела.

**2. Почему вращающиеся системы становятся плоскими.** Частицы, движущиеся вокруг общего центра, имеют близкие друг к другу горизонтальные скорости и случайные вертикальные. При столкновении горизонтальная составляющая в среднем сохраняется, вертикальная гасится. За много столкновений диск становится тонким. Этот же механизм когда-то превратил протопланетное облако вокруг молодого Солнца в плоский диск — отсюда плоскость, в которой сегодня лежат орбиты всех планет. Тот же — формирует дисковые галактики и аккреционные диски вокруг чёрных дыр. Масштабы разные, физика одна.

**3. «Сатурн плавал бы» — приём, а не сцена.** Сравнение плотностей корректно само по себе: 0,69 меньше 1,00. Но буквальной картинки быть не может — никакого океана подходящего размера во Вселенной нет, и под собственной гравитацией планета в чужой жидкости спокойно существовать не сможет. Сравнение плотностей универсально как приём: им оценивают и кусок дерева в реке, и состав далёкой планеты по её массе и радиусу. Это самый короткий способ сказать про Сатурн главное.

**4. Как это узнала наука.** Среднюю плотность планеты вычисляют из массы (по влиянию на траектории спутников и космических аппаратов) и радиуса (по видимому диску и прохождению света через атмосферу). Толщину и структуру колец Сатурна напрямую измерил «Кассини» — аппарат, проработавший в системе Сатурна с 2004 по 2017 год. Он наблюдал, как звёзды «моргают», просвечивая сквозь кольца, а в финале миссии прошёл внутри самих колец.

**5. Четыре шага наблюдений.** Галилей в 1610 году увидел у Сатурна «уши» и не понял, что это. Гюйгенс в 1655-м описал их как плоский диск. Кассини в 1675-м различил щель между двумя кольцами — её и назвали в его честь, щелью Кассини. Аппарат «Кассини», работавший с 2004 по 2017 год, прошёл внутри колец и измерил их структуру напрямую.

**6. Открытый вопрос.** По данным «Кассини», кольца Сатурна относительно молоды — возможно, им всего 100–400 миллионов лет. Если это так, мы наблюдаем их на короткой по космическим меркам стадии. Откуда они взялись — точного ответа пока нет: рассматриваются столкновение ледяного спутника, разрушение захваченной кометы, остатки материала, так и не собравшегося в луну.

### **Ограничения**

Уличный формат: антивандальная порошковая покраска столбика и кольца, нержавеющая табличка, антивандальные болты. Кольцо — доработанное по одному из двух вариантов (трубчатое или перфорированный диск), с запасом прочности под нагрузку взрослого человека при боковом контакте и опоре рукой. Единый QR станции снимает необходимость обслуживать отдельный код на малом объекте.

### **Способ взаимодействия**

Наблюдение, чтение, тактильный контакт с кольцом. Единственная точка маршрута с тактильным элементом.

|  |  |
| :-: | :-: |
| \*\*Параметр\*\* | \*\*Значение\*\* |
| QR отдельный | Нет (через общий QR станции) |
| Подсветка | Нет |

  
  

# ➕ WP008-з «Уран» (столбик-планета)  

#### **WP008-з «Уран» (столбик-планета)**

##### **Тип конструкции**

Столбик-планета. Аналогичная другим планетарным столбикам маршрута «Солнечная система» конструкция.

##### **Описание конструкции**

На вершине столбика закреплена сфера диаметром 9,4 см, окрашенная в бледно-голубой оттенок. На сфере тонкой линией прочерчена ось вращения, проходящая через два условных «полюса»; сфера установлена так, что эта ось наклонена почти к горизонту (около 98° к вертикали столбика), — в той же ориентации, что у реального Урана относительно плоскости орбиты.

Столбик — стальная труба ⌀60 мм с порошковой антивандальной покраской, забетонированная в основание. Расстояние от Солнца по маршруту — \~105,3 м от центра Солнца. Табличка — нержавеющая сталь с гравировкой, крепление — антивандальные болты. Дополнительные материалы — через общий QR станции «Солнечная система».

##### **Педагогическая логика**

К Урану посетитель приходит уже после длинного участка маршрута. Уран здесь — не курьёз («странная планета, лежит на боку»), а крайний случай общего закона: сезоны на любой планете определяются наклоном её оси к плоскости орбиты. У Земли — 23°, у Марса — 25°, у Сатурна — 27°, у Урана — 98°. Один и тот же геометрический принцип, доведённый почти до предела.

Тип научного рассуждения, который отрабатывается на этом объекте: повторяющийся закон в ряду значений одного параметра. Дополнительно в QR — рамка знания: наблюдаемый сегодня факт (наклон оси) и гипотеза о прошлом (как этот наклон возник) — разные типы знания.

Ключевое физическое наблюдение на стенде — прочерченная ось сферы. Именно она превращает абстрактное «98°» в то, что видно глазом. Без этой линии текст на табличке теряет опору.

##### **Сечение**

**Принцип:** угол между осью вращения тела и плоскостью его орбиты определяет амплитуду сезонного цикла.

**Ряд проявлений (через QR доступен полный):**

  - Меркурий, \~0° — наклон почти отсутствует, сезонов от наклона нет.
  - Земля, 23° — умеренный наклон, плавная смена сезонов.
  - Марс, 25° — почти как у Земли, чуть резче.
  - Сатурн, 27° — тот же характер, год длиннее (≈29 земных лет).
  - Нептун, 28° — обычный наклон, сезон \~40 лет.
  - **Уран, 98°** — крайняя точка: годы непрерывного света и годы непрерывной тьмы.
  - Венера, 177° — вращение «опрокинуто», но наклон оси к орбите малый.

Видно: 98° — не выпадение из ряда, а его край.

##### **Текст на табличке**

**Уран. Наклон 98°**

На сфере прочерчена линия: это ось вращения Урана. У моделей других планет маршрута такая линия близка к вертикали; здесь она почти лежит в горизонтальной плоскости.

У каждой планеты есть две выделенные оси: ось её собственного вращения и перпендикуляр к плоскости её орбиты. Угол между ними называют наклоном оси. У Земли он равен 23°, у Марса 25°, у Сатурна 27°, у Урана 98°: ось вращения Урана почти лежит в той же плоскости, в которой планета движется вокруг Солнца.

Именно наклон оси и порождает времена года. Когда одно из полушарий планеты наклонено к Солнцу, солнечные лучи падают на него почти отвесно, поверхность нагревается сильнее, и на этом полушарии наступает лето. Через полоборота вокруг Солнца то же полушарие оказывается наклонённым в обратную сторону, лучи скользят по поверхности, и наступает зима. Чем больше наклон оси, тем резче разница между этими положениями. На Земле она умеренная, и сезоны сменяются плавно. На Уране, при наклоне 98°, разница становится предельной: одно полушарие непрерывно освещено в течение многих лет, тогда как второе всё это время остаётся в тени. Полный оборот Урана вокруг Солнца длится 84 земных года, и каждый из четырёх сезонов тянется около 21 года.

Один и тот же геометрический фактор, наклон оси вращения, задаёт смену сезонов на любой планете. Уран показывает его в крайней точке: ось наклонена на 98° и почти лежит в плоскости орбиты.

*QR-код станции «Солнечная система»: как измерили наклон оси Урана, какие есть гипотезы о его происхождении и чем Уран отличается от Нептуна.*

### **Материалы для QR (раздел «Уран»)**

#### **Как измеряют наклон оси Урана**

Прямо «увидеть» ось вращения планеты нельзя: ось — геометрическая прямая, у неё нет ни цвета, ни фактуры. Но её положение в пространстве можно восстановить по двум независимым типам наблюдений.

Первый из них — движение деталей в атмосфере. Уран относится к газовым гигантам, и «поверхностью» с точки зрения наблюдателя является верхний слой облаков. Снимки, сделанные с интервалами в часы и сутки, позволяют отследить смещение облачных образований; по этим смещениям восстанавливают ось вращения и его период (для Урана — около 17 часов 14 минут).

Второй тип — геометрия системы колец и крупных спутников. У всех планет-гигантов их регулярные кольца и крупные спутники формируются и затем удерживаются в плоскости, близкой к экваториальной плоскости планеты. У Урана кольца и орбиты пяти крупных спутников (Миранда, Ариэль, Умбриэль, Титания, Оберон) лежат в плоскости, почти перпендикулярной плоскости движения Урана вокруг Солнца. Это значит, что и экваториальная плоскость Урана почти перпендикулярна плоскости его орбиты, а ось вращения, перпендикулярная экватору, оказывается почти в самой плоскости орбиты.

Два независимых способа измерения дают согласованный результат: наклон оси Урана составляет примерно 98°. Основной массив данных был получен зондом «Вояджер-2» при пролёте мимо планеты в январе 1986 года; впоследствии эти данные уточнили наблюдения телескопа «Хаббл» и крупных наземных обсерваторий.

#### **Почему наклон оси задаёт смену сезонов**

Количество солнечной энергии, попадающей на единицу площади поверхности планеты, зависит от двух факторов: расстояния до Солнца и угла падения лучей. Расстояние меняется в течение года незначительно (большинство планетных орбит близки к окружностям), поэтому основной причиной сезонных колебаний оказывается именно угол падения.

Если ось вращения планеты строго перпендикулярна плоскости орбиты, то угол падения солнечных лучей в каждой точке поверхности в течение года не изменяется. День и ночь зависят только от широты, но никаких сезонных колебаний нет. Близка к такой ситуации Венера, наклон оси которой составляет всего около 2,6°.

Стоит оси наклониться, и картина меняется. По мере движения планеты вокруг Солнца её полушария поочерёдно оказываются обращёнными к Солнцу сильнее или слабее. На «летнем» полушарии лучи падают круче, поверхность получает больше энергии; на «зимнем» полушарии лучи падают полого, и поверхность освещается слабее. Через полоборота положения полушарий меняются на обратное.

Количественно эффект тем сильнее, чем больше угол наклона. На Земле, при наклоне 23°, разница угла падения лучей в средних широтах между серединой лета и серединой зимы составляет около 47° — отсюда и привычные нам колебания температуры. На полярных широтах за счёт того же наклона возникают полярные дни и ночи, когда Солнце в течение нескольких месяцев не поднимается или, наоборот, не опускается над горизонтом.

При наклоне 98° эффект достигает другой границы. На большом участке орбиты одно из полушарий Урана полностью повёрнуто к Солнцу: для всех его широт, кроме узкой полосы вблизи экватора, Солнце не садится в течение многих лет подряд. Второе полушарие в это же время полностью отвёрнуто от Солнца, и на нём наступает многолетняя ночь. Поскольку период обращения Урана вокруг Солнца равен 84 земным годам, длительность каждого из четырёх сезонов составляет примерно 21 год. Очередное равноденствие на Уране (момент, когда Солнце пересекает плоскость экватора планеты) произошло в 2007 году; следующее ожидается в 2049-м.

#### **Откуда у Урана такой наклон**

Этот вопрос относится уже не к текущему состоянию Урана, а к его истории. Прямых наблюдений далёкого прошлого Солнечной системы у нас нет, поэтому единственный путь — построить модель, которая согласовала бы между собой имеющиеся косвенные данные.

Самая распространённая модель предполагает катастрофическое столкновение. Согласно ей, в первые сотни миллионов лет после формирования Солнечной системы Уран столкнулся с крупным телом протопланетного типа, по массе сравнимым с Землёй или более тяжёлым. Удар сильно изменил направление оси вращения и одновременно объяснил бы несколько других известных особенностей планеты: слабый внутренний тепловой поток и сильное смещение оси магнитного поля относительно оси вращения (у Урана это смещение достигает 60°). Эта модель согласуется с общей картиной эпохи формирования Солнечной системы, в которой столкновения крупных тел действительно происходили часто: подобный механизм считается основной причиной и наклона земной оси.

Существуют и альтернативные сценарии. Один из них — медленное гравитационное накопление углового момента за счёт резонансного взаимодействия с крупным несостоявшимся спутником или с другими планетами на ранних этапах. Численное моделирование показывает, что и такой процесс за миллионы лет мог бы повернуть ось Урана на большие углы.

Прямых доказательств ни одна из моделей не имеет; речь пока идёт о согласовании косвенных данных. Существенный прогресс ожидается после следующей космической миссии к ледяным гигантам: она обсуждается в планах NASA и ESA, но ещё не запущена.

#### **Чем Уран отличается от Нептуна**

В небольшой телескоп Уран и Нептун выглядят очень похоже: голубоватые диски почти одинакового диаметра. Различия между ними становятся заметны только при детальном исследовании.

Главное физическое различие связано с внутренним тепловым потоком. Все четыре планеты-гиганта Солнечной системы ещё не остыли после формирования и продолжают излучать наружу больше энергии, чем получают от Солнца. У Юпитера и Сатурна эта внутренняя энергия превышает солнечную примерно в полтора-два раза, у Нептуна — приблизительно в 2,5 раза. Уран здесь оказывается исключением: его внутренний тепловой поток крайне слаб, и наружу он отдаёт почти столько же энергии, сколько поглощает от Солнца. Причины этой аномалии до конца не выяснены; одна из версий, как и в случае с наклоном оси, связана с гипотетическим столкновением, которое могло нарушить конвекцию во внутренних слоях планеты.

Разница во внутреннем потоке приводит к существенно разному поведению атмосфер. Атмосфера Урана спокойная, без выраженных бурь и долгоживущих структур; общий оттенок — бледно-голубой. На Нептуне внутренняя энергия питает мощную атмосферную динамику: на нём зарегистрированы самые сильные ветра в Солнечной системе (до 600 м/с), а на снимках регулярно появляются яркие облачные пятна, которые формируются и распадаются в течение нескольких недель. Цвет Нептуна также более насыщенно-синий, что связано не только с присутствием метана (он есть и у Урана), но и с особенностями верхних слоёв атмосферы.

Наклон оси Нептуна — 28°, то есть обычный для крупной планеты. Сезоны на нём устроены так же, как на Земле и Марсе: разница между полушариями умеренная, а длительность каждого сезона составляет около 40 земных лет, так как период обращения Нептуна — 165 лет.

Внешнее сходство Урана и Нептуна — совпадение цвета и размера; по внутреннему устройству, тепловому режиму и геометрии вращения это две разные планеты.

  

##### **Параметры**

|  |  |
| :-: | :-: |
| \*\*Параметр\*\* | \*\*Значение\*\* |
| Диаметр сферы | 9,4 см |
| Цвет сферы | бледно-голубой |
| Ось вращения на сфере | тонкая прочерченная линия, наклон к вертикали ≈98° |
| Диаметр планеты (реальный) | 50 724 км |
| Наклон оси к плоскости орбиты | 98° |
| Период обращения вокруг Солнца | 84 земных года |
| Длительность одного сезона | \\\~21 земной год |
| Расстояние от Солнца по маршруту | \\\~105,3 м от центра Солнца |
| Столбик | стальная труба ⌀60 мм, порошковая антивандальная покраска, забетонированное основание |
| Табличка | нержавеющая сталь с гравировкой, антивандальные болты |
| Дополнительные материалы | общий QR станции «Солнечная система» |

  
  

# ➕ WP008-и «Нептун» (столбик-планета)  

## **WP008-и «Нептун» (столбик-планета)**

**Тип конструкции** Столбик. Аналогичная планетарная конструкция, завершающая линейный участок модели «Солнечная система».

**Описание** На вершине столбика закреплена сфера диаметром 9,1 см, окрашенная в насыщенный синий цвет. Столбик — стальная труба ⌀60 мм с порошковой антивандальной покраской, забетонированная в основание. Расстояние от Солнца по маршруту — 165 м от центра Солнца; это конец линейного участка модели. Табличка — нержавеющая сталь с гравировкой, крепление антивандальными болтами. Дополнительные материалы — через общий QR станции «Солнечная система».

**Смысл остановки** Нептун завершает основную линию маршрута. До этой точки человек уже прошёл всю внутреннюю систему, пояса, гигантов — и доходит до последней большой остановки. Это финал линейной модели, и он отведён под один из самых сильных сюжетов в истории астрономии: Нептун — первая планета, которую сначала вычислили, а уже потом увидели. Задача таблички — показать не факт открытия, а сам метод: невидимое находят по тому, как оно действует на видимое. Этот приём работает и сегодня — от поиска экзопланет до оценки массы чёрных дыр и гипотез о тёмной материи.

**Тип научного рассуждения** Повторяющийся закон / общая причина. Один метод — восстановление невидимой массы по возмущению видимого движения — проявляется в разных масштабах и эпохах: от Нептуна 1846 года до современной астрофизики. Вторичная линия — «рамка»: ньютоновская механика настолько точна, что даже доли угловой секунды расхождения становятся сигналом к открытию, а не списываются на погрешность.

Перевёл оба текста в регистр научно-популярной статьи: начинаю с того, что человек видит на месте, через обратную задачу теории возмущений выхожу на механизм, термины («лучевая скорость», «перигелий», «итерационная схема», «Sgr A\*») ввожу после соответствующих явлений. Парцелляция, ритмические тире и обращения к читателю убраны; фразы цельные, сложноподчинённые, с явной причинно-следственной сцепкой.

## **Текст на табличке**

**НЕПТУН** Диаметр: 49 244 км · Расстояние от Солнца в этой модели: 165 м

Эта синяя сфера диаметром 9,1 см изображает Нептун, восьмую и самую дальнюю из больших планет Солнечной системы. Расстояние в 165 метров от Солнца, отмеренное вдоль тропы, принадлежит той же масштабной модели; дальше линейная часть маршрута заканчивается.

Нептун интересен не только своим положением: эту планету математики предсказали раньше, чем астрономы её увидели. В 1820-х годах наблюдатели заметили, что Уран движется по орбите не совсем так, как предписывает закон всемирного тяготения. Расхождение между рассчитанным и наблюдаемым положениями Урана составляло доли угловой секунды, но со временем накапливалось и не объяснялось ошибками измерений. Возможных объяснений оставалось три: ошибка наблюдений, неточность ньютоновской механики или присутствие ещё одной, ненаблюдавшейся планеты, возмущающей орбиту Урана.

Чтобы проверить последнюю гипотезу, нужно было решить обратную задачу теории возмущений: по известному отклонению восстановить параметры тела, которое его вызывает. Французский астроном Урбен Леверье и независимо от него английский математик Джон Адамс провели такой расчёт и указали, где именно на небесной сфере должна находиться искомая планета. 23 сентября 1846 года Иоганн Галле в Берлинской обсерватории направил телескоп на координаты, присланные Леверье, и в первую же ночь обнаружил неизвестную планету меньше чем в градусе от предсказанной точки. Её назвали Нептуном.

Метод, опробованный на Нептуне, оказался общим. По периодическому изменению лучевой скорости звезды, движущейся вокруг общего с невидимой планетой центра масс, сегодня обнаруживают экзопланеты; по движению звёзд вблизи центра Галактики вычисляют массу скрытой там сверхмассивной чёрной дыры; по слишком высоким скоростям вращения галактик заключают, что для удержания звёзд на их орбитах одной только видимой материи недостаточно. Везде работает один и тот же принцип: если наблюдаемое движение видимого тела не согласуется с расчётом, в системе действует ещё одна, пока не видимая масса, и характер расхождения позволяет её вычислить.

Подробнее — по QR на общей станции «Солнечная система».

## **Текст за QR**

**Метод Леверье: от Нептуна до тёмной материи**

К началу 1840-х годов несоответствие между расчётной и наблюдаемой орбитой Урана накопилось уже до нескольких угловых минут. Уран был открыт Уильямом Гершелем в 1781 году, и за прошедшие десятилетия астрономы накопили достаточно точные данные, чтобы убедиться: ньютоновская теория тяготения, превосходно описывающая движение всех остальных планет, для Урана даёт сбой. Расхождение было малым по абсолютной величине, но систематическим, и списать его на случайные ошибки наблюдений становилось всё труднее.

Объяснений могло быть три: систематическая ошибка опорных каталогов или самих наблюдений; неточность ньютоновского закона тяготения на больших расстояниях от Солнца; наличие ещё одной планеты за орбитой Урана, гравитация которой возмущает его движение. К середине 1840-х годов первое объяснение было исчерпано тщательной проверкой, а второе считалось маловероятным, поскольку закон Ньютона уже два столетия безотказно работал во всех остальных задачах небесной механики. Самой экономной гипотезой оставалась неизвестная планета.

Прямая задача небесной механики формулируется так: даны массы, положения и скорости всех тел системы, нужно рассчитать их движение в будущем. Обратная задача гораздо сложнее: по наблюдаемому отклонению орбиты одного тела нужно восстановить возмущающее его тело, его массу и параметры орбиты. На современном языке такие задачи решают итерационно: задаются положением и массой гипотетического возмутителя, рассчитывают создаваемое им возмущение, сравнивают с наблюдаемым отклонением и подбирают параметры до совпадения.

Эту работу независимо друг от друга проделали двое. Английский математик Джон Куч Адамс закончил расчёты к осени 1845 года и передал результаты в Кембридж и в Гринвичскую обсерваторию, однако наблюдательная программа поиска планеты в Англии двигалась медленно. Урбен Леверье опубликовал свои расчёты в 1846 году и отправил координаты в Берлин, где у Иоганна Галле имелись свежие карты соответствующего участка неба. Уже в первую ночь, 23 сентября 1846 года, Галле заметил неизвестную звездоподобную точку, которой на карте не было; на следующий день её положение заметно сместилось относительно неподвижных звёзд, что и доказывало её планетную природу. Реальное положение Нептуна оказалось в 55 угловых минутах от точки, рассчитанной Леверье. Приоритет открытия Нептуна сегодня делят между Леверье и Адамсом.

Метод Леверье пережил XIX век и был применён ещё раз с неожиданным результатом. К началу XX века с большой точностью установили, что перигелий орбиты Меркурия, то есть точка её наибольшего сближения с Солнцем, смещается на 43 угловые секунды в столетие больше, чем предсказывает классическая теория с учётом возмущений от всех известных планет. По аналогии с Нептуном предположили существование внутримеркуриевой планеты, заранее названной Вулканом. Поиски Вулкана велись несколько десятилетий и не дали результата. Объяснение появилось в 1915 году в общей теории относительности Эйнштейна: вблизи массивного Солнца ньютоновского закона тяготения уже недостаточно, и точная релятивистская поправка к нему даёт как раз требуемые 43 угловые секунды смещения перигелия Меркурия в столетие. Диагноз во многом совпадал с нептуновским: наблюдение расходилось с теорией. Однако причина оказалась не в новой массе, а в пределах применимости самой теории.

Подход, основанный на поиске невидимой массы по её гравитационному влиянию на видимое движение, лежит сегодня в основе нескольких ключевых направлений астрофизики.

Метод лучевых скоростей применяется для поиска экзопланет. Звезда, вокруг которой обращается планета, движется не вокруг собственного центра, а вокруг общего центра масс системы; для далёкого наблюдателя это выглядит как небольшое периодическое колебание звезды вдоль луча зрения. Соответствующее доплеровское смещение спектральных линий звезды отвечает амплитудам скорости всего в несколько метров в секунду, однако современные спектрографы способны их регистрировать. Этим методом было открыто большинство первых экзопланет около звёзд солнечного типа.

Звёзды вблизи центра Млечного Пути движутся по плотно сжатым кеплеровским орбитам вокруг невидимого центрального объекта. Многолетние наблюдения за их орбитами позволили вычислить массу этого тела: около четырёх миллионов солнечных масс, сосредоточенных в области размером порядка нескольких астрономических единиц. Такая концентрация массы при столь малом размере допускает единственное истолкование: в центре Галактики находится сверхмассивная чёрная дыра, обозначаемая Sgr A\*. За эту работу Райнхард Генцель и Андреа Гез получили в 2020 году Нобелевскую премию по физике.

Наконец, измеренные скорости вращения вещества в спиральных галактиках на больших расстояниях от их центров оказались значительно выше тех, которые допускает наблюдаемая видимая масса. Если бы дело было только в видимом веществе, внешние области галактик не удерживались бы на своих орбитах. Чтобы согласовать наблюдения с законами тяготения, в галактиках должна присутствовать дополнительная невидимая масса, в несколько раз превосходящая видимую. Так была сформулирована гипотеза тёмной материи; вопрос о её природе остаётся одним из главных открытых вопросов современной физики.

Что касается самой планеты, на Нептуне дуют самые быстрые ветры в Солнечной системе: их скорости достигают 2100 километров в час. Источник такой энергии для далёкой планеты не вполне ясен, поскольку поток солнечного излучения там слаб, а внутреннее тепло ледяных гигантов невелико. Существенную роль играет малое трение: холодная разреженная атмосфера слабо рассеивает крупномасштабные течения, и однажды возникшие ветры долго сохраняются.

Метод Леверье продолжает работать и внутри самой Солнечной системы. После того как «Вояджер-2» в 1989 году пролетел мимо Нептуна, масса планеты была существенно уточнена, и старые расхождения в движении самого Нептуна, дававшие в XX веке повод говорить о существовании Планеты X за его орбитой, исчезли. Однако в 2010-х годах астрономы обнаружили, что орбиты группы далёких транснептуновых объектов сгруппированы вокруг определённого направления необычным образом — так, как если бы их формировала гравитация ещё одной, неизвестной массивной планеты на сильно вытянутой орбите за Нептуном. Существует ли в действительности эта гипотетическая Девятая планета, прямые наблюдения пока не показывают. Если она когда-нибудь будет найдена, метод её открытия окажется тем же, каким был открыт Нептун в 1846 году.

  

**Основная тема взаимодействия посетителя** Наблюдение, чтение, история науки — как демонстрация научного метода.

**Наличие таблички** Да.

**Наличие QR** Нет индивидуального — используется общий QR станции «Солнечная система».

  
  
  

# ➕ WP009-к «Плутон» (столбик-сюрприз)  

# **WP009-к «Плутон» — итоговое описание экспоната**

**Тип станции:** малая, столбик-сюрприз (входит в комплект станции «Солнечная система»). **Категория опыта:** обнаружение + наблюдение + чтение. **Круглогодичность:** да. **Локация:** за пределами благоустроенной зоны, в поле за комплексом общежитий, в 216,7 м от центра «Солнца» по линии маршрута станции (51,7 м за столбиком Нептуна в принятом на станции масштабе расстояний).

## **1. Замысел**

После 165 метров от «Солнца» посетитель доходит до Нептуна и считает, что станция закончилась — карта станции «Солнечная система» именно так и устроена, Плутон в ней не упомянут. Через полсотни метров от Нептуна, уже на нерасчищенной территории, человек неожиданно натыкается на ещё один столбик — внешне такой же, как остальные планетарные. На вершине, в подставке-«ладони», закреплён крошечный металлический шарик диаметром 4,4 мм. Это Плутон.

Сюрприз работает на трёх уровнях сразу. Первый — шутка масштаба: оказывается, Солнечная система ещё не кончилась, и в ней «спрятался» объект размером с большую горошину после полутора сотен метров «настоящих» планет. Второй — эмоциональная находка: Плутон обнаруживается ровно так же, как его обнаружили в 1930 году, — за пределами того, что было известно. Третий — содержательный: на табличке посетитель читает не «Плутону не повезло», а описание того, почему статус планеты — это **физическое описание роли тела в Солнечной системе**, а не присвоенный ярлык.

Главный когнитивный сдвиг: посетитель уносит способ думать про классификации в естественных науках. Категория описывает поведение объекта (здесь — гравитационную роль), а не назначается ему сверху.

## **2. Поперечное сечение**

**Общая структура:** объект на границе категории показывает, чем эта категория определяется, — через свой невозможный случай.

**Явления:**

1.  *Астрономия* — Плутон: формальный критерий планеты, не выполненный одним из трёх условий.
2.  *Биология* — вирусы: на границе категории «живое» (нет собственного обмена веществ — но есть наследственность и эволюция).
3.  *Химия* — металлоиды (бор, кремний, германий): на границе категории «металл» (часть свойств есть, часть нет).
4.  *Языкознание* — наречия в русском, образованные от деепричастий («молча», «нехотя»): на границе категории «наречие vs глагольная форма».

**Неочевидная связь:** во всех четырёх случаях категория задаётся не списком объектов и не «решением сверху», а **набором требуемых свойств**. Объект, который удовлетворяет части требований, но не всем, показывает, какие именно свойства считаются определяющими. Граничный случай работает как линза для определения.

## **3. Конструкция**

**3.1 Общая схема.** Стальной планетарный столбик, **визуально идентичный** остальным столбикам станции «Солнечная система» (это критическое требование: сюрприз работает только при полной узнаваемости формы). На вершине — подставка-«ладонь», в ней закреплён шарик ⌀ 4,4 мм. Сбоку — табличка из нержавеющей стали с гравировкой и QR-кодом.

**3.2 Материалы.**

  - Столбик: стальная труба ⌀ 60 мм, порошковая антивандальная покраска, забетонирована в основание.
  - Подставка-«ладонь»: листовой металл, та же покраска.
  - Шарик: металлическая сфера ⌀ 4,4 мм; крепление — точечная сварка ножки шарика к ладони, **эпоксидная** заливка стыка. **Резьбового соединения нет** (резьбу легко открутить).
  - Запас: 5–10 шариков на складе на весь срок службы. Замена — минуты, деталь копеечная.
  - Табличка: нержавеющая сталь с гравировкой, антивандальные болты.

**3.3 Габариты и монтаж.** По высоте и габаритам столбик — типовой для станции. Заглубление и фундамент — по проекту станции «Солнечная система».

**3.4 Безопасность.** Шарик 4,4 мм — без острых кромок, фиксация на сварке, потеря возможна (для этого и склад запасных). Подъезд к точке монтажа на нерасчищенной территории — единственная заметная сложность.

## **4. Принцип работы**

1.  Посетитель проходит маршрут станции, последовательно встречая планеты от Меркурия до Нептуна.
2.  У Нептуна (165 м от Солнца) маршрут по карте заканчивается.
3.  Двигаясь дальше — по тропинке или интуитивно вдоль линии станции, — посетитель через \~50 м замечает ещё один столбик в поле, типовой для станции.
4.  Подходит, обнаруживает шарик 4,4 мм. На этом этапе уже работает первый эффект — удивление масштабом и тем, что маршрут не кончился.
5.  Читает табличку (уровень А + Б). Здесь работают эмоциональный (Плутон как находка) и содержательный (критерий планеты как физическое требование) уровни.
6.  Если заинтересован — открывает QR (уровень В): количественная сторона критерия, история открытий пояса Койпера и Эриды, миссия New Horizons, открытый вопрос вокруг определения.

## **5. Тексты**

## **Текст А — стенд, заголовок и вход**

**Плутон**

На вершине столбика закреплён металлический шарик диаметром 4,4 мм. В масштабе станции это Плутон — самое известное тело за орбитой Нептуна. Реальный диаметр Плутона составляет 2377 км, чуть меньше диаметра Луны (3474 км). Среднее расстояние от Солнца — около 5,9 млрд км, и свет проходит этот путь за 5 часов 30 минут. На станции этому расстоянию соответствуют 216,7 метра, то есть на 51,7 метра дальше столбика Нептуна.

Расстояния между телами на станции выдержаны в едином масштабе, а размеры планет — нет: в том же масштабе диаметр Плутона составил бы около 0,1 мм, и шарик было бы невозможно разглядеть. Поэтому диаметры планет показаны в другом, более крупном масштабе.

## **Текст Б — стенд, объяснение**

**Почему Плутон — карликовая планета**

В распределении тел Солнечной системы заметна закономерность: вблизи орбит крупных планет других тел сравнимой массы нет. Земля, Юпитер, Нептун — единственные крупные объекты в своих орбитальных зонах. За орбитой Нептуна картина меняется: там расположено множество тел сопоставимого с Плутоном масштаба, и сам Плутон оказывается лишь одним из них.

Эта особенность не случайна. По мере формирования планетной системы массивное тело гравитационно взаимодействует с окружающими малыми: одни захватываются на устойчивую орбиту, другие отбрасываются на дальние орбиты, третьи поглощаются. В итоге крупное тело со временем расчищает свою орбитальную зону и становится в ней динамически доминирующим. У Плутона этого не произошло: его массы недостаточно, чтобы расчистить пространство вокруг, и он продолжает делить орбитальную зону со множеством подобных объектов. Они образуют пояс Койпера — область за орбитой Нептуна, в которой расположено большое число малых и средних тел.

Именно эту разницу Международный астрономический союз использовал в 2006 году, когда уточнил определение планеты. По новой формулировке планета должна (1) обращаться вокруг Солнца, (2) быть достаточно массивной, чтобы под собственной тяжестью принять форму, близкую к шару, и (3) динамически доминировать в своей орбитальной зоне. Первые два условия Плутон выполняет, третье — нет, и поэтому его отнесли к карликовым планетам. В ту же категорию вошли Эрида, Хаумеа и Макемаке — другие крупные тела пояса Койпера.

## **QR — уровень В, расширение**

**Подробнее: критерий планеты, открытия пояса Койпера, миссия New Horizons**

**Что значит «расчистить орбитальную зону»**

В резолюции IAU 2006 года численного порога для третьего критерия не задано, и динамическое доминирование принято оценивать косвенно. Одна из количественных оценок, предложенная астрономом Стивеном Сотером, — отношение массы тела к суммарной массе всех остальных объектов в его орбитальной зоне. У больших планет это отношение очень велико: для Земли оно близко к 1,7 миллиона, для Юпитера — около 620 тысяч, для Нептуна — около 24 тысяч. У карликовых планет картина обратная: у Плутона это отношение составляет примерно 0,08, у Эриды — около 0,10, у Цереры — около 0,33. То есть в орбитальной зоне Плутона «не-Плутона» по массе более чем в десять раз больше, чем самого Плутона. Между планетами и карликовыми планетами лежит разрыв в несколько порядков, и именно за ним стоит классификация IAU.

**Откуда взялись «соседи» Плутона**

С момента открытия Плутона Клайдом Томбо в 1930 году и до начала 1990-х других тел за орбитой Нептуна обнаружить не удавалось. Существование такой популяции ещё в 1951 году предположил Джерард Койпер; отсюда и название области, закрепившееся позже. Первое подтверждение пришло в 1992 году, когда Дэвид Джевитт и Джейн Луу открыли объект 1992 QB1. К концу 2003 года было каталогизировано около 800 транснептуновых объектов, к середине 2000-х их число превысило тысячу. В 2005 году группа Майкла Брауна обнаружила Эриду — тело, сопоставимое с Плутоном по размеру и превосходящее его по массе примерно на 27%. Это поставило перед астрономическим сообществом выбор: либо признать планетой и Эриду, и, вероятно, ещё несколько подобных тел в будущем, либо уточнить само определение. IAU выбрал второй путь.

**Геология Плутона и миссия New Horizons**

Космический аппарат New Horizons отправился к Плутону в январе 2006 года, за восемь месяцев до резолюции IAU. Пролёт состоялся 14 июля 2015 года; за несколько часов сближения аппарат передал тысячи снимков с разрешением до сотни метров на пиксель. На них видна крупная светлая область в форме сердца — Tombaugh Regio, названная в честь первооткрывателя Плутона. Её западная часть — бассейн Sputnik Planitia глубиной несколько километров, заполненный льдами азота, оксида углерода и метана. На поверхности бассейна почти нет ударных кратеров; по моделям, её возраст не превышает 10 миллионов лет — для тела возрастом более четырёх миллиардов лет это совсем недавно. Основной механизм обновления — конвекция азотного льда: тонкий слой замёрзшего азота получает достаточно тепла из недр Плутона, чтобы медленно подниматься и опускаться, перемешивая поверхность. У такого небольшого и удалённого тела наличие активной геологии оказалось неожиданным; полностью её источник пока не объяснён, и в качестве дополнительных факторов обсуждаются приливные взаимодействия со спутником Хароном, остаточное тепло первичной аккреции и экзотермические фазовые переходы во льдах.

**Открытый вопрос**

Критерий «расчистки окрестности» в его нынешней формулировке вызывает критику и внутри астрономического сообщества — прежде всего со стороны Алана Стерна, главного исследователя миссии New Horizons. Стерн и его коллеги указывают, что в самой резолюции IAU количественный порог не задан, что орбиты крупных планет не идеально расчищены (например, орбита Нептуна пересекается с орбитой Плутона, находящегося с ним в резонансе 2 : 3), и что критерий зависит от расстояния тела до Солнца: на дальних орбитах при формальном применении такой оценки любое тело будет «недостаточно очищающим». В качестве альтернативы предлагаются геофизические определения, по которым планетой считается любое тело, достаточно массивное для гидростатического равновесия (то есть имеющее почти сферическую форму), независимо от его орбитального окружения. При таком подходе планет в Солнечной системе оказалось бы более ста, включая Луну и крупные спутники Юпитера и Сатурна. Современное определение IAU остаётся в силе, но дискуссия о его уточнении продолжается. Плутон, остававшийся планетой ровно 76 лет, и сегодня остаётся самым изученным и самым известным телом за орбитой Нептуна.

  
  

## **6. Педагогическая задача**

Тип научного рассуждения, который несёт экспонат, — **ограничение**. Природа не присваивает телам статус планеты произвольно; «планета» — это описание того, какую гравитационную роль тело играет в своей зоне. Плутон показывает границу этого понятия с обратной стороны: какие именно свойства тело должно иметь, чтобы попасть в категорию.

Экспонат работает одновременно в трёх режимах:

1.  **Шутка масштаба.** После 165 метров «настоящих» планет посетитель обнаруживает, что Солнечная система не закончилась, и тянется ещё на полсотни метров — до объекта размером 4 мм.
2.  **Эмоциональная находка.** Плутон оказывается неожиданным постскриптумом, который маршрут изначально не обещал. История находки повторяет историю самого Плутона: маленький, далёкий, обнаруженный за пределами уже известного.
3.  **Демонстрация физического принципа.** Определение планеты описывает гравитационную роль тела, а не служит бюрократическим ярлыком. Плутон, не выполнивший одно из трёх условий, делает это требование наглядным.

## **7. Размещение и подача**

Столбик **не упоминается на карте станции «Солнечная система»**. Это обязательное требование: сюрприз работает только при отсутствии анонса. Посетитель должен сначала решить, что маршрут закончился, и только потом наткнуться на продолжение.

QR-код размещается на табличке Плутона **отдельно от общего QR станции**. Это поддерживает статус Плутона как самостоятельной находки и не «склеивает» его с информационным слоем основной станции.

## **8. Что требует уточнения до производства**

1.  **Натурная привязка точки.** Полевые координаты столбика за общежитиями, согласование локации с эксплуатацией территории, выбор маршрута подвоза материалов (нет подготовленного подъезда).
2.  **Технология крепления шарика.** Эпоксидная заливка + точечная сварка ножки шарика к «ладони» — описать на уровне технологической карты, выбрать марку эпоксидного состава для уличных условий, протестировать на образце.
3.  **Видимость шарика 4,4 мм в полевых условиях.** Натурный тест: при каком освещении и фоне «ладони» шарик считывается с расстояния 1–1,5 м, чтобы посетитель его не пропускал.
4.  **Защита от пропусков на маршруте.** Тропинка от Нептуна к Плутону: оставлять ли её естественной (часть эффекта «маршрут уже закончился») или дать минимальную набивку, чтобы зимой и в высокой траве объект находился. Решение — натурно, после первой зимы.
5.  **Запасные шарики и точка обслуживания.** Регламент проверки наличия шарика (раз в N месяцев), назначенный ответственный, место хранения запаса.
6.  **Финальная сверка дат и численных значений на табличке** перед гравировкой: 5,5 ч (время света от Солнца), 2 377 км (диаметр), 165 м и 216,7 м (расстояния на маршруте), 800 и 1000+ (число известных KBO к концу 2003 / к середине 2000-х), 1930 (открытие), 1992 (1992 QB1), 2006 (резолюция IAU и старт New Horizons), июль 2015 (пролёт), 76 лет (как долго Плутон был планетой).

  
  
  

# WP002Компания птиц  

ВОЗМОЖНЫ 2 варианта объекта 

  

WP002-A

**«Голос леса» — зачем и как звучат птицы**

  

# **1. Суть экспоната**

|  |  |
| :-: | :-: |
| \*\*Главная идея\*\* | Звук у птиц — это сигнал с функцией, а среда отбирает, какие сигналы работают лучше. |
| \*\*Что наблюдает посетитель\*\* | Звуки вокруг: разницу между короткими, повторяющимися, высокими и низкими сигналами. |
| \*\*Почему здесь\*\* | В густом лесу акустическая среда реально отличается от открытого пространства: листья рассеивают высокие частоты, фильтрация происходит |

 

# **2. Техническое описание**

|  |  |
| :-: | :-: |
| \*\*ID\*\* | WP002-A |
| \*\*Название\*\* | «Голос леса» — зачем и как звучат птицы |
| \*\*Тип объекта\*\* | Информационный стенд с QR-модулем, без подвижных частей, без электроники |
| \*\*Рекомендуемая локация\*\* | Лесной участок тропы, густой смешанный лес с выраженным подлеском |

 

## **2.1. Конструкция**

Одиночный стенд-панель на стальной стойке. Несущая схема — вертикальная труба, заглублённая в бетонное основание, с наклонной информационной панелью в верхней части. Информация наносится гравировкой и цветной УФ-печатью. Отдельный элемент — выгравированный QR-код на металлической табличке в нижней части панели. Экспонат полностью пассивный: без электроники, без подвижных частей, без питания.

 

**Основные элементы**

• несущая стойка

• информационная панель с наклоном к зрителю

• бетонное основание

• гравированная QR-табличка

 

**Материалы и покрытия**

• стойка: стальная труба Ø76–89 мм, горячее цинкование и порошковая окраска

• панель: нержавеющая сталь с гравировкой и цветной УФ-печатью под защитным лаком, либо анодированный алюминий

• основная схема (кривые затухания): наносится гравировкой — не только УФ-печатью, чтобы выцветание не уничтожило смысловой центр стенда

• QR-табличка: нержавеющая сталь, лазерная гравировка

• крепёж: нержавеющий, антивандальный

• основание: бетон М200

 

**Габариты и монтаж**

|  |  |
| :-: | :-: |
| \*\*Общая высота\*\* | ориентировочно 1600–1800 мм от уровня земли |
| \*\*Размер панели\*\* | ориентировочно 600×900 мм |
| \*\*Угол наклона панели\*\* | 15–20° от вертикали |
| \*\*Заглубление стойки\*\* | не менее 800 мм |
| \*\*Установка\*\* | бетонирование стойки в пробурённое отверстие |

 

 

## **2.3. Обслуживание**

• ежегодный визуальный осмотр

• очистка мягкой щёткой и водой

• ежегодная проверка QR и актуальности страницы

• обновление содержимого страницы по QR без выезда на объект

• подкрашивание стойки и обновление защитного слоя раз в 5–7 лет

 

## **2.4. Риски и слабые места**

|  |  |
| :-: | :-: |
| \*\*Риск\*\* | \*\*Митигация\*\* |
| Выцветание УФ-печати через несколько сезонов | Основная схема (кривые затухания) — гравировкой, печать только для цвета |
| Царапины на защитном лаке | Регулярный осмотр, обновление лака раз в 5–7 лет |
| Коррозия сварного шва при повреждении покрытия | Горячее цинкование + осмотр стыков |
| QR ведёт на несуществующую страницу | Поддержка страницы на хостинге; ключевые тезисы дублированы на стенде |

 

## **2.5. Открытые вопросы (требуют уточнения)**

• точная геопривязка

• итоговые размеры панели после макета

• глубина промерзания грунта

• цвет стойки

• выбор материала панели (нержавейка vs анодированный алюминий)

• адрес страницы по QR

• тип антивандального крепежа

 

 

# **3. Тексты стенда**

 

## **3.1. Заголовок**

**Почему в лесу голоса звучат вот так**

 

## **3.2. Хук**

|  |  |
| :-: | :-: |
| \*\*ХУК\*\* | Прислушайтесь: птицы вокруг не просто поют — они работают. Они повторяют свои сигналы снова и снова.  |

 

## **3.3. Слой A — наблюдение и базовая причина**

|  |  |
| :-: | :-: |
| \*\*A\*\* | Лес не пустой коридор для звука — это густой фильтр из листьев, ветвей и стволов. Высокие, тонкие звуки теряются в нём быстрее, чем низкие. Длинный сигнал рискует исказиться. Короткий можно повторять вновь и внось. Не дошёл один — дойдёт следующий. |

 

## **3.4. Слой B — механизм**

|  |  |
| :-: | :-: |
| \*\*B\*\* | При общении с сородичами, а иногда и с представителями других видов, многие животные используют звуковые сигналы. Их характеристики, в том числе частота, заметно отличаются у разных видов. Согласно гипотезе акустической адаптации, частота издаваемых животными звуков зависит от среды, в которой они живут. Так, в лесу высокочастотные звуки быстро затухают, поэтому здесь выгоднее издавать низкочастотные сигналы — они распространятся дальше. Впрочем, наблюдения в дикой природе далеко не всегда подтверждают данную идею — возможно, из-за того, что на особенности издаваемых животными звуков также оказывают влияние размер их тела, эволюционная история и половой отбор. |

 

## **3.5. Действие посетителя**

|  |  |
| :-: | :-: |
| \*\*ДЕЙСТВИЕ\*\* | Послушайте несколько секунд и попробуйте заметить: звуки вокруг короткие или длинные? Повторяются ли? Слышите ли что-то, что похоже на один и тот же сигнал, посланный несколько раз подряд? |

 

  

 

 

# **4. Текст QR-страницы**

  

## **4.1. Заголовок QR**

**Почему формула голоса вытекает из размера?**

 

## **4.2. Основной текст**

|  |  |
| :-: | :-: |
| \*\*C\*\* | Частота звука и размер источника связаны простой логикой размерности: частота измеряется в 1/секунду, скорость звука — в метрах/секунду. Разделите скорость на длину — и получите частоту. Отсюда f ∼ v/L: меньше источник (L) → выше частота (f). Именно поэтому маленькие птицы в среднем звучат выше крупных — это не случайность и не выбор природы, а неизбежность, продиктованная единицами измерения. Природа не могла выбрать другой закон: единицы просто не сошлись бы.Усложняет картину сиринкс — особый голосовой аппарат птиц, позволяющий некоторым видам производить одновременно два независимых звука. Среда добавляет второй слой отбора поверх анатомии: один и тот же вид в плотном лесу и на опушке звучит немного по-разному. |

 

## **4.3. Открытый вопрос**

|  |  |
| :-: | :-: |
| \*\*?\*\* | Меняется ли голос птицы в течение жизни, если она переезжает из одного типа леса в другой? Есть ли у птиц что-то вроде «акцента среды» — и насколько быстро он формируется? |

 

 

# **5. Указания для макета панели**

Визуальная логика: не список птиц, а схема «сигнал + среда + наблюдение».

 

## **5.1. Элементы макета**

|  |  |
| :-: | :-: |
| \*\*Элемент\*\* | \*\*Описание\*\* |
| \*\*Заголовок\*\* | «Почему в лесу голоса звучат вот так» |
| \*\*Иконки сигналов\*\* | 4 типа: короткий позыв / тревожный крик / повторяющийся сигнал / дробь дятла |
| \*\*Блок действия\*\* | «Слушай → замечай тип сигнала → сканируй QR» |
| \*\*QR-блок\*\* | Merlin Sound ID + страница тропы с углублением |

 

 

  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  
  

**WP002-B** 

**Крыло в небе**

*Как читать силуэт и узнавать хищника*

 

# **1. Суть экспоната**

Главная научная идея: форма крыла — это решение аэродинамической задачи; по силуэту можно зачастую понять образ жизни птицы. Ни одно крыло не может быть универсальным — оптимизация под одну задачу всегда ухудшает другую. Это принцип компромисса, который работает в природе и технике по одной физике.

 

|  |  |
| :-: | :-: |
| \*\*Наблюдение на месте\*\* | Реальные силуэты птиц в небе над открытым пространством; |
| \*\*Локация\*\* | Поле, пойма, опушка, обзорная точка с открытым небом |
| \*\*Сезонность\*\* | Работает круглый год; эффект усилен в сезоны миграции |

 

# **2. Техническое описание**

## **2.1. Конструкция**

Одиночный стенд-панель на стальной стойке с бетонным основанием. Панель увеличенного формата: в верхней части — четыре силуэта крыльев и четыре пиктограммы сред без ответов (зона задачи), в нижней — зона расшифровки. Посетитель сначала сопоставляет самостоятельно, затем проверяет ответ. Нижний блок содержит три пары силуэтов «птица — самолёт». QR ведёт на углубление.

## **2.2. Основные элементы**

• несущая стойка

• увеличенная информационная панель

• зона «задача» — четыре силуэта крыльев + четыре пиктограммы сред

• зона «ответ» — крыло → среда → птица → тип охоты

• нижний блок «птица — самолёт» — три пары силуэтов с подписями

• гравированная QR-табличка

• бетонное основание

 

## **2.3. Материалы и покрытия**

|  |  |
| :-: | :-: |
| \*\*Стойка\*\* | Стальная труба, горячее цинкование и порошковая окраска |
| \*\*Панель\*\* | Нержавеющая сталь с гравировкой и УФ-печатью либо анодированный алюминий |
| \*\*Силуэты\*\* | Гравировка с заливкой или контрастная печать; цветные зоны — слабое место по деградации |
| \*\*Крепёж\*\* | Антивандальный |
| \*\*Основание\*\* | Бетон |

 

## **2.4. Габариты и монтаж**

|  |  |
| :-: | :-: |
| \*\*Общая высота\*\* | 1700–1900 мм (ориентировочно) |
| \*\*Размер панели\*\* | 900×1200 мм (ориентировочно) |
| \*\*Угол наклона панели\*\* | 15–20° |
| \*\*Заглубление стойки\*\* | Не менее 1000 мм; уточняется по грунту и ветровой зоне |
| \*\*Установка\*\* | Бетонирование стойки в пробурённое отверстие |

 

*Увеличенные размеры и глубина заложения обусловлены ветровой нагрузкой открытого места.*

 

## **2.5. Принцип работы**

Посетитель видит четыре типа крыльев и четыре среды, пытается сопоставить их самостоятельно, затем проверяет ответ и смотрит в небо — переносит навык на реальных птиц. Нижний блок «птица — самолёт» показывает, что тот же принцип компромисса работает в авиации.

 

  

 

## **2.8. Что требует уточнения**

• Точная геопривязка

• Технология исполнения силуэтов (гравировка с заливкой vs чистая гравировка)

• Окончательные размеры панели

• Ветровая зона и глубина промерзания

• Цвет стойки

• Адрес QR-страницы

• Окончательная компоновка зон «задача» и «ответ»

 

# **3. Тексты стенда**

 **3.1. Стенд — уровень A (основной текст)**

**ЗАГОЛОВОК**

*Почему у сокола и канюка такие разные крылья?*

 

**ХУК**

*Над этим полем прямо сейчас, скорее всего, кто-то летит. Канюк, ястреб, сокол — их можно различить ещё до того, как рассмотришь окраску. Достаточно посмотреть на форму крыла.*

 

**УРОВЕНЬ A**

*Каждая птица решает разную задачу: одним нужно часами держаться в воздушных потоках, другим — стремительно атаковать в открытом небе, третьим — резко менять направление между стволами деревьев. Форма крыла — это не украшение и не случайность. Это ответ на конкретный вопрос: как лететь там, где ты живёшь и охотишься?*

 

**ДЕЙСТВИЕ ПОСЕТИТЕЛЯ**

*На панели — четыре силуэта крыльев и четыре среды. Попробуйте сопоставить их сами, не подглядывая в ответ. Потом проверьте — и посмотрите в небо: здесь, над Иннополисом, хищников часто видно.*

 

## **3.2. Стенд — уровень B (механизм)**

**УРОВЕНЬ B**

*Широкое крыло с «пальцами» на концах уменьшает завихрения и позволяет часами парить в восходящих потоках — так летает канюк. Длинное и острое крыло снижает сопротивление воздуха и разгоняет до высокой скорости — это сокол. Короткое и округлое выигрывает в тесном пространстве: быстрый поворот важнее скорости — это перепелятник в лесу. Но ни одно крыло не умеет всего сразу: то, что помогает парить, мешает манёврировать. Природа не выдаёт универсальных решений — это физика, а не случайность. Тот же компромисс решают инженеры: у планера крыло длинное и узкое, как у альбатроса; у истребителя короткое и стреловидное, как у сокола.*

 

**ПРИВЯЗКА К МЕСТУ**

*Открытое пространство — лучшая точка наблюдения. Над полем и поймой хищники парят на виду, и форму крыла хорошо читать именно здесь, в реальном времени.*

 

## **3.3. QR — уровень C (углубление)**

**ЗАГОЛОВОК QR**

*Можно ли угадать форму крыла из физики — не зная ни одной птицы?*

 

**УРОВЕНЬ C**

*Попробуем. Нам нужно крыло, которое тратит мало энергии на километр пути. Что на это влияет? Два параметра: размах — насколько крыло длинное, и удлинение — насколько оно узкое относительно своей длины. Чем больше оба, тем меньше индуктивное сопротивление — потери на завихрения у концов крыла. Запишем это как принцип: расход энергии на путь убывает с ростом размаха и удлинения. Это вывод из задачи: чтобы уменьшить потери, нужно уменьшить завихрения; чтобы уменьшить завихрения, нужно длинное и узкое крыло. Но такое крыло плохо управляется в тесноте — отсюда и компромисс. Каждый вид птиц занял свою точку в этом пространстве параметров: альбатрос — рекорд по удлинению, перепелятник — рекорд по манёвренности. Они не конкурируют: они решают разные задачи.*

 

**КАК ЭТО ЗНАЕТ НАУКА**

*Аэродинамику крыла изучают двумя путями: в аэродинамической трубе (продувают модели и измеряют силы) и в поле (GPS-трекеры на птицах фиксируют траектории, высоту, скорость и другие параметры). Данные по диким птицам и по моделям крыльев дают одну и ту же зависимость — это и есть подтверждение модели.*

 

**ОТКРЫТЫЙ ВОПРОС**

*Если форму крыла полностью диктует физика, почему у птиц одного вида крылья бывают немного разными? Индивидуальная изменчивость, возраст, линька — или что-то ещё?*

 

# **4. Компоновка панели**

Финальные пропорции уточняются на этапе дизайна. Ниже — описание логики расположения элементов.

 

|  |  |
| :-: | :-: |
| \*\*Заголовок\*\* | «Почему у сокола и канюка такие разные крылья?» — крупно, в верхней части панели |
| \*\*Блок «задача»\*\* | Слева — 4 силуэта крыльев без подписей: сокол, канюк, перепелятник, чайка. Справа — 4 пиктограммы сред: открытое небо, поле с термиками, лес, побережье. По центру — зона для стрелок / сопоставления |
| \*\*Блок «ответ»\*\* | Под зоной задачи: крыло → среда → птица → тип охоты (скорость / парение / манёвр / планирование) |
| \*\*Блок «птица — самолёт»\*\* | Нижняя часть панели. Три пары силуэтов: альбатрос / планер, сокол / истребитель, перепелятник / штурмовик с изменяемой стреловидностью. Подпись: «та же физика, разные материалы» |
| \*\*QR\*\* | Нижний угол; ведёт на страницу тропы с уровнем C |
| \*\*Основной текст A+B\*\* | Правая или нижняя колонка рядом с зоной задачи — в зависимости от итогового формата панели |

 

  
  

# WP007 — Геологический срез  

# **Экспонат WP007 — Геологический срез** 

## **Табличная запись объекта**

|  |  |
| :-: | :-: |
| \*\*Поле\*\* | \*\*Содержание\*\* |
| Геометка | https://yandex.ru/maps/-/CPe7VRmo |
| Рабочее название | Геологический срез |
| Тип станции | Стенд-наблюдатель (двусторонний) + опциональная рамка на реальном обнажении |
| Тематическая ось | Чтение времени по слоям (принцип суперпозиции + связь слоя со средой) |
| Действие посетителя | Рассматривание колонки снизу вверх; при наличии обнажения — сопоставление рамки с реальным разрезом |
| Работа зимой | Ограничена: стенд может быть под снегом, обнажение — под снежным покровом |
| Безопасность | Нет (стенд в безопасной зоне; рамка на склоне — только при устойчивом обнажении) |

## **Конструкция**

**Стенд.** Двусторонняя панель \~120×80 см на двух стойках, забетонированных ниже глубины промерзания. Материал — HPL (рекомендуется для уличных условий средней полосы) или металл с порошковой окраской и антивандальным покрытием печати. Высота нижней кромки панели — около 90 см от уровня земли.

**Рамка на обнажении (опционально).** Металлическая рамка \~80×50 см, закреплённая анкерами на склоне, с подписями слоёв. **Устанавливается только при соблюдении двух условий одновременно:**

1.  Локальная геология в точке подтверждена (есть геологический разрез).
2.  Склон устойчив: нет активной эрозии, ползучести, регулярных осыпей. Проверяется многосезонным наблюдением или экспертной оценкой.

  

Если хотя бы одно условие не выполнено, рамка **не ставится**. Стенд в этом случае работает как схема типичного разреза района — и это явно обозначено в тексте (см. блок «Почему это здесь»).

## **Содержание стенда (тексты)**

### **Сторона 1 — основной стенд**

**Заголовок**

Где здесь миллион лет?

**Хук**

Цветные полосы на этой колонке — не раскраска. Каждая из них — отпечаток времени, когда над головой у этой земли было что-то совсем другое: то море, то берег, то ледник.

**Уровень A — что наблюдаем и базово понимаем**

Посмотри на колонку снизу вверх. Внизу лежит то, что осело раньше, сверху — то, что легло позже. Так устроен любой слой осадков: снег на земле, ил на дне озера — что упало первым, то и окажется в самом низу (с оговоркой: если ничто их не потревожит). Этот простой порядок — главный инструмент геолога: разрез читается как лента времени, снизу вверх.

**Уровень B — почему это происходит и как работает механизм**

Но слои хранят не только возраст. Материал каждого слоя — подпись среды, в которой он появился. Известняк внизу сложен из раковин и скелетов морских существ: значит, здесь было мелководное море. Выше — более грубые, песчанистые отложения: море отступало, появлялись береговые и прибрежные зоны. Ещё выше — рыхлые породы, которые принесли сюда ледники, реки и ветер. А тонкий тёмный слой наверху — современная почва, самый молодой участок разреза. Фактор (среда осадконакопления) → процесс (из чего и с какой скоростью копится осадок) → следствие (цвет, зернистость и состав слоя, которые мы сейчас видим).

**Действие посетителя**

Проведи пальцем по колонке снизу вверх — так движется стрелка времени. Если рядом видно настоящее обнажение грунта, найди границы между слоями глазами: переход цвета или крупности зерна — это и есть смена эпохи.

### **Сторона 2 — перенос принципа**

**Заголовок**

Один метод — пять наук

**Хук**

Геологический разрез — не единственная летопись природы. Тот же приём чтения времени работает ещё в четырёх материалах, которые окружают нас каждый день.

**Основной текст**

Везде, где что-то копится со временем и не перемешивается, возникает запись: старое внизу, молодое сверху. Геолог читает её в каменных слоях, дендрохронолог — в годичных кольцах дерева (только там «низ» — это сердцевина, а «верх» — кора). Гляциолог бурит лёд и достаёт керн, в котором каждый год — тонкая светлая полоса, и чем глубже, тем древнее воздух в пузырьках. Лимнолог опускает трубку в донные илы озера и поднимает столбик ила, где в каждом миллиметре — пыльца деревьев, живших тысячу лет назад. Археолог в раскопе снимает слой за слоем — и чем ниже, тем раньше жили люди, оставившие этот мусор и эти угли от костра.

Пять разных наук, пять разных материалов — один метод. Они же служат друг другу проверкой: если все пять архивов говорят о похолодании в одно и то же столетие, значит, это не ошибка — значит, климат действительно менялся.

## **Содержание QR-кода**

**Заголовок QR**

Как по слоям узнают возраст и климат?

**Уровень C — углубление**

Принцип суперпозиции — «молодое лежит на старом» — сформулировал датский учёный Николаус Стено ещё в 1669 году, задолго до того, как геология стала наукой. У принципа есть границы применимости: если потом слои перевернул тектонический сдвиг, порядок нарушается — и по таким «сбоям» геологи как раз и узнают о древних катастрофах. Скорости накопления разные: морские илы копятся долями миллиметра в год, то есть метр слоя — это десятки или сотни тысяч лет; речные и ледниковые осадки — быстрее и неравномернее. Возраст слоя уточняют по окаменелостям (какие виды жили в этом периоде), по радиоактивному распаду в минералах, по остаточной намагниченности пород, фиксирующей развороты магнитного поля Земли. А климат прошлого реконструируют по составу ракушек и пыльцы

**Как это узнаёт наука**

Геолог работает как криминалист: сравнивает разрезы в разных точках, измеряет состав и возраст образцов в лаборатории, и сопоставляет получаемую картину с другими архивами времени — годичными кольцами деревьев, льдом Гренландии и Антарктиды, илами на дне озёр. Если четыре независимых архива говорят одно и то же — значит, реконструкция надёжная.

  

  
  

# WP018 - «Фрактальная рамка»  

# **WP018 - «Фрактальная рамка»**

**Локация:**[ точка на карте](https://yandex.ru/maps/-/CPe7Z-kb) **Тип станции:** наблюдательная рамка с сеткой + табличка с QR **Тип научного рассуждения:** повторяющийся закон

## **Конструкция**

Столбик высотой 1,3 м с неподвижно закреплённой на вершине рамкой \~30×25 см, ориентированной на систему оврагов. В рамке — прозрачная пластина (акрил) с нанесённой сеткой разных масштабов по принципу box-counting: ячейки 4×4 см, 8×8 см и 16×16 см, объединённые в одном поле или на сменных пластинах. Рядом со столбиком — табличка с текстом стенда и QR-кодом.

## **Что делает посетитель**

Смотрит через рамку на овражную систему. Сравнивает, какие ячейки сетки лучше «ловят» контур крупных ветвей балки и маленьких отвершков. Замечает, что форма повторяется на разных масштабах.

## **Текст стенда**

### **Овраг в овраге в овраге**

Посмотрите через рамку на склон. Маленькая промоина сбоку по форме похожа на большую ветвь балки, только уменьшенную. Если убрать траву, кусты и крыши вдалеке, по одной форме уже не поймёшь — метровая это промоина или стометровая балка.

Сетка на рамке нужна, чтобы ловить эту похожесть. Крупная ячейка накрывает большие ветви рельефа, мелкая — маленькие отвершки. Контуры в обоих случаях получаются узнаваемо одинаковыми. Овраги растут не произвольно: они наследуют форму у того процесса, которым сделаны.

**Почему так?**

Любой овраг начинается одинаково. Струйка воды находит понижение, промывает в нём русло, русло собирает следующие струйки, углубляется и ветвится. Тот же самый процесс, но в сто раз крупнее, создаёт большую балку. Когда один и тот же механизм работает и на сантиметрах, и на сотнях метров, получается похожая форма на всех масштабах. Это свойство называют **самоподобием**.

То же самое видно у речной сети, кроны дерева, молнии, капиллярной сети в листе — везде локальное правило повторяется много раз и порождает узнаваемый ветвящийся узор. Один принцип — разные явления.

**Важная оговорка.** Самоподобие здесь работает в ограниченном диапазоне. Меньше комка земли — уже не овраги, а просто грунт. Крупнее самой балки — уже другой тип рельефа. У природных «фракталов» всегда есть верхняя и нижняя граница.

**Попробуйте:** наведите рамку на крупную ветвь балки. Потом — на маленький отвершек ближе к себе. Сравните, какие ячейки сетки лучше ловят контур в каждом случае.

## **Текст QR-кода**

### **Как измерить форму, у которой нет «правильного» масштаба**

Если накрывать контур овражной сети сеткой с ячейками всё меньшего размера и каждый раз считать, сколько ячеек его пересекают, — число будет расти по степенному закону. Показатель этой степени и есть **фрактальная размерность**. Для гладкой линии он равен 1, для сплошной плоскости — 2. Для овражной или речной сети получается число между ними — обычно 1,1–1,3: сеть занимает плоскость плотнее линии, но не заполняет её целиком. Это обобщение обычного представления о размерности: не все природные формы укладываются в целые числа 1, 2, 3.

Важное ограничение: оценка имеет смысл только в том диапазоне масштабов, где реально работает один и тот же механизм размывания. Выше и ниже этого диапазона закон ломается, и красивая ветвящаяся форма ещё не делает объект строгим фракталом.

**Как это узнаёт наука.** Рельеф снимают с воздуха или со спутника и переводят в цифровую модель высот (DEM). По перепадам высот алгоритмически восстанавливают линии стока и овражную сеть. Затем на полученную сеть накладывают квадратные сетки разного шага, считают число пересечённых ячеек и строят зависимость в логарифмических координатах. Наклон прямой на этом графике и есть оценка размерности.

**Открытый вопрос.** Фрактальная размерность речных сетей у разных рек мира оказалась на удивление близкой, несмотря на разный климат, породу и уклон. Это значит, что механизм эрозии задаёт форму сильнее, чем местные условия — но почему именно такое значение, а не другое, всё ещё обсуждается.

**Похожие структуры, которые стоит поискать глазами:** береговые линии, речные дельты, кроны деревьев, ветвление молний, капиллярная сеть листа.

  
  
  

# WP009 - береза с наростами  

# WP009 - «Дерево помнит всё»

**Название объекта:** Берёза с наростами → «Дерево помнит всё» **Тип / масштаб:** Столбик с табличкой + демонстрационный модуль

  - **Что это за объект:** Столбик высотой 1,2 м с двусторонней табличкой из нержавеющей стали с гравировкой. Рядом — демонстрационный модуль 0,9–1,1 м с литыми тактильными копиями или защищёнными образцами: гладкая кора, нарост/кап/сувель, мёртвая древесина с ходами насекомых, кора с лишайниками. Образцы за поликарбонатным экраном. Объект не зависит от живого дерева рядом.
  - **Что делает посетитель:** Рассматривает и трогает образцы, сравнивает разные состояния древесины и коры.
  - **Какое явление объясняется:** Дерево как носитель биографии событий — наросты как след повреждений и аномалий роста, мёртвая древесина как среда для жизни других организмов, лишайники как отклик на условия среды.
  - **Какой механизм лежит в основе:** Сквозной принцип — каждое событие в жизни дерева (рана, грибок, отмирание ткани, состав воздуха) оставляет считываемый след в структуре древесины и на поверхности коры.
  - **Какие ограничения важны:** Не указаны. Объект всесезонный.

### **Стенд**

**Заголовок:** Дерево помнит всё

**Хук:** Перед вами несколько кусков дерева. Они выглядят по-разному — и это не случайность. Каждый из них хранит историю того, что с деревом происходило.

**Уровень A :** Посмотрите на образцы: гладкая кора, бугристый нарост, потемневшая мёртвая древесина, кора, покрытая лишайниками. Это не разные породы — это разные главы одной биографии.

Нарост на дереве — след события: рана, грибок, аномалия роста. Дерево не стирает следы — оно их заращивает, и они остаются в структуре древесины навсегда. Чем дольше живёт дерево, тем больше записей оно хранит.

**Уровень B — почему это происходит и как работает механизм:** Когда дерево повреждено — кора прорвана, ветка обломана, грибок проник в ткань — камбий (слой клеток под корой, который строит новую древесину) начинает наращивать ткань вокруг места повреждения. Так появляется нарост: не болезнь сама по себе, а ответ на неё. Тип нароста зависит от причины — именно поэтому кап, сувель и след трутовика выглядят по-разному.

Когда часть дерева или целое дерево отмирает — история не заканчивается. Мёртвая древесина становится городом: жуки-короеды прокладывают в ней ходы, грибы разрушают целлюлозу и лигнин, дятлы охотятся за личинками. Один упавший ствол кормит лес десятки лет.

Лишайники на коре — ещё один слой записи, только о среде, а не о событиях. Одни виды лишайников чувствительны к загрязнению воздуха и исчезают первыми; другие устойчивее. Густота и разнообразие лишайников на коре отражают то, какой воздух окружает это дерево годами.

**Действие посетителя:** Потрогайте образцы. Сравните поверхность гладкой коры и нароста. Найдите ходы насекомых в образце мёртвой древесины. Посчитайте, сколько разных форм лишайников видно на коре.

**Почему это здесь:** Берёза — одно из самых распространённых деревьев этой полосы, и именно она часто несёт хорошо заметные наросты. Рядом с тропой можно встретить деревья, у которых все эти «главы биографии» читаются прямо на стволе.

### **QR**

**Заголовок QR:** Кап, сувель, чага, трутовик — как их различить и почему это важно?

**Уровень C — углубление:** Не все наросты одинаковы, и причина у каждого своя. Кап — это аномальное разрастание почек-спящих узлов, которые в норме не просыпаются; структура древесины внутри завихрена и очень красива. Сувель — нарост из-за механического повреждения: морозная трещина, удар, поломка ветки. Чага — это не сам нарост, а мицелий гриба-паразита (трутовика скошенного), который разрастается внутри и снаружи берёзового ствола; снаружи она выглядит как чёрный бесформенный нарост. Трутовики — отдельная группа грибов, у многих видов плодовые тела вырастают прямо из коры полочками. Отличить их можно по форме, поверхности и — у трутовиков — по наличию пор на нижней стороне.

Мёртвая древесина — один из важнейших субстратов лесной экосистемы. Исследования европейских лесов показывают: с мёртвой древесиной связано более четверти всех лесных видов насекомых и грибов. Именно поэтому «санитарные рубки», при которых убирают упавшие и сухостойные деревья, снижают биоразнообразие леса.

**Как это узнаёт наука:** Лихеноиндикация — метод оценки качества воздуха по составу и численности видов лишайников — применяется как мониторинговый инструмент: исследователи регулярно описывают лишайниковые сообщества на контрольных деревьях и отслеживают изменения во времени. Природа нароста определяется анатомически: срез древесины под микроскопом показывает характер клеточного роста и следы грибного мицелия.

**Открытый вопрос / продолжение темы:** Дерево хранит биографию событий в структуре древесины — а дендрохронология читает в этой же структуре историю климата: засухи, холодные лета, вулканические зимы. Если одно дерево может рассказать столько — что можно прочитать в годичных кольцах тысячелетней лиственницы?

### **Подпись к схеме / модели**

Рекомендуется схема-сравнение: четыре образца в ряд с подписями «гладкая кора», «нарост (кап/сувель)», «след трутовика», «мёртвая древесина». Подпись: *Каждый образец — это след события в жизни дерева.*

  
  

# WP022 — «Почему правый берег обрывистый»  

# **WP022 — «Почему правый берег обрывистый»**

## **1. Название**

**«Почему правый берег обрывистый»**

*(Рабочее название в таблице — «Сила Кориолиса и меандры рек» — заменено. Меандрирование выведено из объяснительной линии как отдельный механизм; экспонат посвящён одному явлению — системной асимметрии берегов.)*

## **2. Тип объекта**

Информационный стенд на обзорной площадке. Без движущихся частей, без интерактивных элементов, требующих обслуживания.

## **3. Конструкция**

**Несущая часть.** Столбик-стойка из оцинкованной стали, диаметр 76 мм, высота над грунтом 1,2 м. Бетонный фундамент заглубленного типа.

**Панель.** Наклонная панель формата A2, материал — HPL (компакт-ламинат для наружного применения, класс светостойкости не ниже 4). Угол наклона к горизонту — 15°. Торцы панели защищены алюминиевым профилем по периметру.

**Ориентация.** Панель развёрнута так, чтобы посетитель при чтении стоял лицом к реке. В верхней части панели — графическая стрелка «Смотри сюда →», указывающая за панель на реальный берег.

## **4. Лицевая сторона панели — композиция**

Панель разделена на четыре функциональные зоны тонкими гравированными линиями.

**Зона 1 — верхний колонтитул.** Заголовок «Этот берег выглядит так, потому что Земля вращается». Справа от заголовка — графическая стрелка «Смотри сюда →», направленная за панель. Под заголовком — короткая инструкция мелким шрифтом: *«Сначала посмотри за панель. Потом возвращайся.»*

**Зона 2 — спутниковый снимок.** Аэрофотография или спутниковый снимок поймы Волги/Свияги с привязкой к текущему месту. Красными стрелками обозначены направления подмыва берегов. Подпись: *«Правый берег подмыт по всей длине реки.»*

**Зона 3 — план-схема реки.** Схематический участок русла с векторами течения. Правый и левый берег обозначены явно (с учётом направления течения, а не наблюдателя). Крутой берег показан штриховкой обрыва, пологий — точечной заливкой пляжа.

**Зона 4 — круговая диаграмма «Вращение Земли».** Северное полушарие сверху, стрелка вращения против часовой стрелки (вид с полюса). Рядом — иллюстрация: движущееся тело отклоняется вправо от направления движения. Подпись: *«В Северном полушарии — вправо. В Южном — влево. На экваторе — никуда.»*

**Зона 5 — текстовый блок.** Основной текст стенда (см. раздел 5).

**Зона 6 — задание для посетителя.** В нижней части панели, выделено рамкой.

На лицевой стороне нет QR-блока-иллюстрации со стаканом чая и нет последовательности «прямое русло → меандр → петля → старица». Эти материалы перенесены в QR (см. раздел 6) с явным указанием, что они относятся к другим механизмам.

## **5. Текст стенда**

### **5.1. Уровень A — крупный текст, верхняя часть**

**Этот берег выглядит так, потому что Земля вращается**

Посмотри за панель. Правый берег — обрывистый, левый — пологий.

Так устроена не только эта излучина. По всей Волге, от истоков до Каспия, правый берег круче левого. То же — на Днепре, Оби, Енисее, Лене. У всех крупных рек Северного полушария подмыт правый берег. В Южном полушарии — наоборот, левый.

Общая для всех этих рек причина может быть только одна. И она действительно одна: Земля вращается.

### **5.2. Уровень B — основной объясняющий блок**

**Почему вода «знает», в какую сторону ей сносить берег**

Когда ты стоишь на Земле, ты вращаешься вместе с ней — один оборот в сутки. Всё, что движется по её поверхности относительно тебя, ведёт себя так, будто его слегка уводит вправо. Это не настоящая сила, а следствие того, что мы смотрим на движение из вращающейся системы отсчёта. Название у этого эффекта — сила Кориолиса.

Вода Волги движется. Каждая её молекула чуть-чуть отклоняется вправо. За один проход воды это отклонение ничтожно — миллионные доли от её собственной скорости. Но течение не прерывается тысячи лет, и давление на правый берег всегда направлено в одну сторону. Слабая, но постоянная сила побеждает сильные случайные толчки. Так вырезается обрыв.

**Где закон работает, а где выключен.** Сила Кориолиса пропорциональна sin(широты). На экваторе синус равен нулю — и реки там не имеют выраженной асимметрии берегов. В средних широтах эффект максимален. У закона есть область, где он работает, и область, где он выключен, и границу можно проверить по карте.

**Как это узнали.** Асимметрию берегов обобщил Карл Бэр в 1857 году — сравнением десятков рек. Механика вращающихся систем отсчёта, объясняющая, почему так происходит, была построена раньше (Кориолис, 1835). Связь между теорией и картой установили сопоставлением: один закон — десятки рек — одно направление подмыва.

### **5.3. Задание для посетителя (нижняя рамка)**

На противоположном берегу найди глазами участок с обрывом и участок с намытым пляжем.

Встань лицом по течению. Какой из них правый берег, а какой — левый?

## **6. QR-код — углубление (уровень C)**

QR размещён в правом нижнем углу панели, с подписью *«Подробнее: масштаб эффекта, чай в чашке, меандры»*. По ссылке — веб-страница со следующим содержанием.

### **6.1. Масштаб решает**

Сила Кориолиса, действующая на единицу массы движущейся воды:

a = 2 · v · Ω · sin(φ)

  

где v — скорость потока, Ω ≈ 7,3·10⁻⁵ с⁻¹ — угловая скорость вращения Земли, φ — географическая широта.

Для Волги в районе Казани: v ≈ 1 м/с, φ ≈ 55°, sin(55°) ≈ 0,82. Подставляем:

a ≈ 2 · 1 · 7,3·10⁻⁵ · 0,82 ≈ 1,2·10⁻⁴ м/с²

  

Это в десять тысяч раз меньше ускорения свободного падения. За секунду — ничтожно. За тысячу лет систематического давления в одну сторону — километры смещённого русла.

Для чашки или раковины то же ускорение даёт эффект, в миллион раз меньший, чем перемешивание ложкой или случайная несимметрия дна. В масштабе кухни Кориолис не наблюдается.

### **6.2. Чай в чашке — это не Кориолис**

Есть распространённое заблуждение: если раскрутить чай в чашке и подождать, чаинки соберутся в центре дна — и это, дескать, действие силы Кориолиса.

Это не так. Парадокс чайных листьев разобрал Эйнштейн в 1926 году. Механизм другой: трение воды о дно замедляет нижний слой, там падает центробежная сила, баланс нарушается, и возникает винтовое течение — вдоль дна к центру, вверх по оси вращения, наружу у поверхности. Чаинки несёт этим вторичным течением.

Кориолис и парадокс чайных листьев связаны с вращающейся жидкостью, но объясняются разными причинами. В первом случае вращается система отсчёта (вся Земля), во втором — сама жидкость в покоящейся системе отсчёта. Путать их — частая ошибка.

### **6.3. Меандры — отдельный механизм**

Асимметрия берегов и изгибы русла — два разных явления.

Меандр (плавный изгиб реки, постепенно переходящий в петлю и старицу) возникает в любом повороте независимо от полушария. В повороте центробежная сила создаёт поперечный наклон поверхности воды; в сечении русла появляется винтовая циркуляция; внешний берег поворота размывается, внутренний — намывается. Петля со временем замыкается, отрезая старицу. Всё это работает и на экваторе, где Кориолис выключен.

Кориолис накладывается сверху: у крупных рек Северного полушария он делает правый берег обрывистее при прочих равных — независимо от того, где в данный момент находится поворот. Так что на одной и той же реке одновременно действуют два механизма, и смотреть на них нужно раздельно.

### **6.4. Где ещё работает та же рамка**

Сила Кориолиса пропорциональна скорости движения и синусу широты. Эта зависимость задаёт чёткую иерархию:

  - **Чашка, раковина:** эффект в миллионы раз слабее случайных возмущений. Не наблюдается.
  - **Река:** эффект слабый, но направленный в одну сторону. За тысячелетия накапливается в километры смещённого берега.
  - **Океан:** Гольфстрим, Куросио, кольцевые течения — Кориолис определяет их направление.
  - **Атмосфера:** циклоны в Северном полушарии закручиваются против часовой стрелки, в Южном — по часовой. Пассаты устойчиво дуют с востока на запад в тропиках. Здесь Кориолис уже доминирует.

Один закон, разные масштабы — разный вклад в итоговую картину.

### **6.5. Открытый вопрос**

Размер обрыва правого берега и скорость его отступания зависят не только от Кориолиса, но и от состава грунта, уклона русла, расхода воды, ледохода и направления преобладающих ветров. На разных участках одной реки вклад Кориолиса в суммарный размыв разный. Точно разделить вклады — задача современной гидрологии, и она не решается «в общем виде» для всей реки сразу: каждый участок считают отдельно.

## **7. Локация**

Обзорная площадка с видом на пойму. Расстояние от стойки до уреза воды — не менее 30 метров. В поле зрения посетителя должны одновременно попадать:

  - участок правого берега с выраженным обрывом,
  - участок левого берега с пологим или намывным пляжем,
  - по возможности — изгиб русла, чтобы посетитель мог корректно определить «право/лево по течению».

Ориентация стойки подбирается так, чтобы посетитель, читающий панель, автоматически оказывался лицом к воде.

## **8. Педагогическая схема**

**Действие посетителя.** Подходит к стойке, видит стрелку «Смотри сюда →», поднимает взгляд за панель на реальные берега. Возвращается к панели, читает заголовок. Читает уровень A, соотносит с тем, что только что видел. Читает уровень B. Выполняет задание: определяет правый и левый берег по течению, находит обрыв и пляж. По желанию — сканирует QR.

**Тип научного рассуждения.** Рамка закона. Сила Кориолиса — универсальный закон, но его наблюдаемое действие полностью определяется масштабом и широтой. Экспонат показывает, как один и тот же закон может быть ничтожным в чашке, направляющим для реки и доминирующим для атмосферы. Это и делает закон законом, а не магической силой: у него есть область применимости с проверяемыми границами.

**Метод познания, демонстрируемый экспонатом.** Сравнение многих однотипных случаев (десятки рек одного полушария) позволяет выделить систематическую составляющую на фоне случайного шума. Это тот же метод, которым работает метеорология, климатология и популяционная биология.

## **9. Круглогодичность**

Экспонат читается и работает круглый год. Зимой берега видны лучше — листва не мешает, контраст обрыва и заснеженного пляжа нагляднее. Ледостав не скрывает рельеф берега.

## **10. Обслуживание**

Не требуется. HPL-панель сохраняет читаемость 10+ лет без выцветания. Гравированные линии не стираются. Граффити смывается стандартными растворителями без повреждения печати. Стойка оцинкована, бетонный фундамент — без сезонного обслуживания.

## **11. Сводка изменений относительно исходного черновика**

|  |  |  |
| :-: | :-: | :-: |
| \*\*Элемент черновика\*\* | \*\*Решение\*\* | \*\*Причина\*\* |
| Название «Сила Кориолиса и меандры рек» | Заменено на «Почему правый берег обрывистый» | Меандры выведены из основной линии как другой механизм |
| Мини-задача «Про стакан чая» на панели | Убрана с панели, перенесена в QR как \*разграничение\* | Парадокс чайных листьев — не Кориолис; в исходной формулировке создавал ложную связь |
| Последовательность «прямое русло → меандр → петля → старица» как следствие Кориолиса | Убрана с панели, перенесена в QR как отдельный раздел | Меандрирование объясняется вторичной циркуляцией в повороте, не Кориолисом |
| Формулировка рамки закона (sin(широты), экватор) | Добавлена в уровень B | Превращает факт в проверяемое утверждение |
| Перенос принципа на циклоны, океан, атмосферу | Добавлен в QR (масштабный ряд) | Показывает, что один закон работает от реки до планеты |
| Количественная оценка a ≈ 1,2·10⁻⁴ м/с² | Добавлена в QR | Отвечает на вопрос «почему в чашке нет, а в реке есть» — через масштаб |
| Принцип «смотри за панель, потом на панель» | Сохранён | Ключевой методологический элемент экспоната |
| Финальное задание «найди правый и левый берег» | Сохранено, уточнена формулировка («встань лицом по течению») | Делает определение однозначным |
| Конструкция (стойка, HPL, 15°, обзорная площадка) | Сохранена без изменений | Оценена как оптимальная |

  
  

# WP023: «Откуда текут реки и как они меняют русло  

# **WP023: «Откуда текут реки и как они меняют русло?»**

## **Конструкция**

Столбик из стальной трубы диаметром 76 мм, высота 1,2 м, с бетонированием основания. Вертикальная или наклонная (15°) двусторонняя панель A2 из HPL толщиной 6 мм. Сторона 1 — бассейны и водоразделы Восточно-Европейской равнины. Сторона 2 — динамика русел в пойме. На нижнем крае панели — металлическая плашка с QR-кодом.

## **Графика**

**Сторона 1.** Схематическая карта Восточно-Европейской равнины с основными реками (Волга, Дон, Днепр, Северная Двина, Печора). Выделен главный водораздел равнины — линия, отделяющая сток в Каспий от стока в Балтику, Чёрное и Северное моря. Мелкие стрелки показывают направление склона по обе стороны линии. Отдельная врезка: схема перехвата одной рекой верховий соседней — то, как граница бассейнов сдвигается во времени.

**Сторона 2.** Серия из четырёх планов одного гипотетического участка реки с интервалом около 100 лет: прямое русло → начало меандра → выраженная петля → отсечённая старица. Показаны стрелки скорости потока на внешнем и внутреннем берегах поворота, зоны размыва и намыва. Врезка: фрагмент карты Менде XIX века и современный спутниковый снимок одного и того же отрезка Волги или Свияги, наложенные с прозрачностью. Видно, насколько сдвинулось русло за полтора века.

## **Текст панели**

### **Сторона 1. Куда течёт капля дождя**

На карте рядом — Волга, Дон, Северная Двина. Между ними тонкие линии, которые никто не проводил по земле. Это водоразделы.

Капля, упавшая на такую линию, может уйти в Каспий или в Азовское море. В полуметре восточнее — точно в Волгу. В полуметре западнее — точно в Дон. Решение принимает рельеф, а не сама вода.

Вода умеет одно: течь вниз и не теряться по дороге. Из этого следует вся гидрография континента. Склон смотрит на юго-восток — сюда потечёт всё, что выпадет, и это бассейн Волги. Склон смотрит на юго-запад — бассейн Дона. Водораздел — просто линия, где склон меняет направление.

Водоразделы не вечны. Река, которая пропилила своё русло глубже соседки, постепенно забирает к себе её склоны — и граница бассейнов смещается. Чем глубже пропилила, тем сильнее притягивает. Сам механизм себя подкармливает. Так одна река «перехватывает» верховья другой; в плейстоцене приток Волги таким образом забрал себе верховья Камы, и гидрография Восточной Европы стала той, что мы видим сейчас.

**Действие.** Найди на карте главный водораздел Восточно-Европейской равнины. Проведи пальцем. По одну сторону от него — всё, что течёт в Каспий. По другую — всё, что в Чёрное, Балтийское и Северное моря. Одна линия делит полконтинента.

### **Сторона 2. Почему прямых рек не бывает**

Открой спутниковую карту поймы. В радиусе двух километров от этого столбика найди в полях светлые дугообразные полосы. Это старые русла — то, как река текла 50, 100, 300 лет назад.

Сфотографируй хотя бы одну дугу. Теперь посмотри, где река течёт сейчас, и представь, как её прежняя петля медленно отсекалась от основного русла.

На повороте реки вода у внешнего берега проходит больший путь, чем у внутреннего, за одно и то же время. Значит, течёт быстрее. Быстрая вода размывает берег, медленная — намывает. Поворот становится круче. Крутой поворот — ещё большая разница скоростей. Петля растёт сама, пока её горловина не перехлестнётся и река не спрямит себе путь. Отрезанная подкова становится старицей.

Это обратная связь: маленькое отклонение от прямой усиливает само себя. Тот же механизм делает дюны из ровного песка и ветвит молнии по небу — везде, где система случайно сдвигается с симметричного состояния и начинает этот сдвиг раскачивать.

Наука видит это на картах. Сравни карту Менде XIX века и современный спутниковый снимок одного и того же отрезка Волги или Свияги — в врезке ниже. Русло сдвинулось: где-то на десятки, где-то на сотни метров. Скорость жизни реки стала измеримой.

**Действие.** Найденную старицу сравни с формой современного русла. Попробуй восстановить, какой петлёй была старица до отсечения. По её кривизне можно прикинуть, сколько примерно лет прошло с момента, как она перестала быть частью реки.

## **Содержимое QR-кода**

Меандрирование работает не везде. На каменистом ложе Карелии реки почти прямые: гранит не размывается. На крутом склоне — тоже прямые: вода несётся вниз, не успевая войти в поворот. Меандры требуют равнины, мягкого грунта и средней скорости. Пойма Волги и Свияги — ровно такой случай.

Количественно устойчивость русла описывается через число Фруда (отношение скорости потока к скорости распространения волн на поверхности) и характеристики донного грунта. Обычно встречается соотношение: длина одного витка меандра ≈ 11 ширинам русла — для очень разных рек, от ручья до Миссисипи. Это следствие одной и той же физики размыва, не зависящей от масштаба.

Открытый вопрос: что задаёт этот коэффициент 11? Строгого вывода пока нет — есть численные модели и статистика по сотням рек. Меандрирование остаётся одной из задач, где простой принцип даёт поведение, которое до конца не просчитывается.

Перехват рек (речное пиратство) — тот же механизм обратной связи в масштабе бассейнов. Граница водоразделов — это не линия, а медленно движущийся фронт, и реки ведут за эту линию непрерывную конкуренцию. Самый изученный российский случай — перехват верховий Камы в плейстоцене, изменивший сток всей Восточно-Европейской равнины.

## **Тип научного рассуждения**

Обратная связь. Маленькое отклонение от симметричного состояния усиливает само себя, пока не приводит к качественному изменению системы. На Стороне 1 это перехват рек: глубже пропилила — сильнее притягивает — ещё глубже. На Стороне 2 это меандрирование: круче поворот — больше разница скоростей — ещё круче. Одна первопричина (вода течёт вниз и сохраняется) плюс механизм обратной связи объясняют обе стороны панели.

## **Деятельность посетителя**

Работа с картой на панели, работа со спутниковой картой на смартфоне, поиск следов древних русел в окружающем ландшафте в радиусе 2 км, фотофиксация найденного, мысленная реконструкция прежнего течения реки.

## **Сезонность и устойчивость**

Тематика не зависит от сезона. Панель HPL, металлический столбик и бетонированное основание рассчитаны на круглогодичную эксплуатацию без обслуживания в течение 10+ лет.

## **Безопасность**

Специальные меры не требуются. Столбик не имеет острых краёв, панель располагается на высоте, безопасной для всех возрастных групп.

  
  

# WP024 — «Родники и грунтовые воды»  

# **WP024 — «Родники и грунтовые воды»**

## **Назначение и научная идея**

Наблюдательная станция у действующего родника. Демонстрирует один принцип — **движение жидкости в пористой среде под действием гравитации ограничивается непроницаемым слоем** — и показывает, что тот же механизм работает в разных масштабах: от цветочного горшка до нефтяных месторождений.

Тип научного рассуждения: **повторяющийся закон**. Посетитель видит не изолированный факт «тут бьёт родник», а универсальный механизм, объясняющий одновременно три наблюдения: почему вода выходит именно здесь, почему она круглый год одной температуры, почему она может быть «старше» посетителя.

## **Размещение**

Столбик устанавливается в 2–3 метрах от точки выхода родника, на устойчивом грунте выше уровня весеннего разлива. Ориентация наклонной панели — так, чтобы посетитель, читающий её, видел сам родник и срез склона периферическим зрением. Подход к воде — с естественной, уже натоптанной стороны; при необходимости — две-три каменные ступени.

## **Конструкция**

**Несущий столбик.** Оцинкованная сталь, квадратное или круглое сечение, высота 1,2 м над уровнем земли. Заглубление — не менее 0,8 м в бетонный стакан (с учётом глубины промерзания региона).

**Информационная панель.** Формат A2, наклон 30–45°, материал — HPL (компакт-ламинат) толщиной 6–8 мм с водостойкой UV-печатью. Панель сажается в стальную рамку столбика на потайной крепёж (головки болтов срезаны или залиты, либо используется пломбирующий крепёж). Между панелью и рамкой — прокладка из EPDM-резины.

**Содержание панели (лицевая сторона):**

1.  *Верхняя часть* — вертикальный разрез грунта в натуральных цветах: тёмно-коричневый почвенный слой, жёлтый песок, серая глина (водоупор), синяя водоносная зона. Стрелки показывают путь воды: вертикальное просачивание через песок, горизонтальное движение вдоль кровли глины, выход на склоне.  
      
2.  *Средняя часть* — схема «путь капли»: дождь → просачивание → накопление над водоупором → родник. Четыре пиктограммы, соединённые стрелками.  
      
3.  *Нижняя правая часть* — короткая врезка «тот же закон в других масштабах»: три пиктограммы (горшок без дренажа, лужа на асфальте, нефтяная ловушка) с подписью в одну строку под каждой.  
      
4.  *Текстовые блоки* — заголовок, хук, уровень A, уровень B, блок «действие», привязка к месту (см. раздел «Тексты»).  
      
5.  *QR-код* — гравированный непосредственно в HPL или в отдельной металлической плашке, закреплённой заподлицо с панелью. Наклейки не используются. Ведёт на страницу с углублённым текстом (уровень C) и картой глубин залегания грунтовых вод в регионе.  
      

**Сравнительная шкала уровня воды.** Вместо встроенной в панель металлической линейки — отдельный стальной кол (Ø 20 мм, нержавейка или окрашенная сталь) с нанесёнными лазером делениями через 5 см, забитый в берег у самой воды. Кол вынесен за пределы панели: при вандализме он не повреждает основную конструкцию, и его проще заменить. Задание — сравнить текущий уровень воды в роднике с уровнем ручья ниже по склону, а не «приходить через месяц».

**Мини-эксперимент (рекомендуемое дополнение).** Рядом со столбиком — две прозрачные вертикальные трубки из поликарбоната Ø 100 мм, высотой 40 см, закреплённые на общей стальной раме. Одна заполнена крупным песком, другая — утрамбованной глиной. У обеих — мелкая сетка на нижнем срезе и поддон. Рядом на цепочке — металлический ковшик. Посетитель зачерпывает воду из родника и льёт поочерёдно в каждую трубку. В песке вода уходит за секунды, в глине — стоит минутами. Это делает принцип тактильным, а не только визуальным. Трубки заменяются раз в 2–3 года.

## **Тексты**

### **Лицевая сторона панели**

**Заголовок:** Почему эта вода всегда холодная?

**Хук:** Опусти руку в воду. Она около +8 °C — в июле, в октябре, в феврале. Температура воздуха над ней меняется за год на 50 градусов. Температура родника — почти нет.

**Уровень A — что происходит.** Родник бьёт из склона там, где разрез грунта на панели показывает границу песка и глины. Дождевая вода просачивается через почву и песок — они пропускают её, как сито. Глина воду не пропускает. Вода накапливается над глиной и движется вбок, пока склон не срежет водоупорный слой. В этом месте она выходит на поверхность.

**Уровень B — почему это работает так.** Вся механика сводится к двум вещам: гравитация тянет воду вниз, а разные породы пропускают её по-разному. Из этого следует всё остальное. Родник появляется не где угодно, а ровно там, где склон вскрывает глиняный слой. Температура воды почти не меняется, потому что на глубине 2–3 метра земля почти не чувствует суточных и сезонных колебаний — её тепловая инерция сглаживает их. Вода перенимает температуру грунта, через который прошла.

Тот же механизм работает везде, где жидкость встречает непроницаемый слой: лужа стоит на асфальте, но уходит на земле; горшок с цветком без дренажных отверстий губит корни — над дном скапливается вода; нефть в толще пород собирается под непроницаемой кровлей. Один закон — от горшка до нефтяного месторождения.

**Действие:** Опусти руку в воду родника — запомни температуру. Посмотри на панель и проследи путь капли от поверхности до места, где ты стоишь. Если рядом есть трубки с песком и глиной — зачерпни воды и проверь, за какое время она проходит через каждую.

**Сравнение:** По колышку у воды сверь уровень родника с уровнем ручья ниже по склону. Родник выше? Ниже? Почему?

**Почему это здесь:** Это редкий случай, когда разрез грунта не нужно воображать — склон уже сделал его за геолога. Родник бьёт ровно там, где водоупорный слой выходит к поверхности.

### **QR-код (уровень C)**

**Заголовок:** Сколько лет этой воде?

**Текст.** Скорость движения воды в разных породах отличается на порядки: в крупном песке — около метра в сутки, в глине — порядка миллиметра в год. Если область питания родника находится в нескольких сотнях метров, а вода двигалась в основном по песку, её возраст — от недель до нескольких лет. В артезианских водоносных горизонтах, где жидкость движется сквозь плотные породы, возраст воды достигает тысяч и десятков тысяч лет.

Постоянная температура +8 °C соответствует средней годовой температуре почвы для средней полосы России; южнее она выше, севернее — ниже. На глубине более 10 метров колебания температуры уже не превышают долей градуса.

Модель имеет пределы: если водоупор прерывистый или область питания близко к поверхности, уровень и температура могут заметно меняться после сильных дождей и в засуху.

**Как это узнаёт наука.** Возраст грунтовой воды определяют по изотопам — тритию (для молодой воды, до \~60 лет) и углероду-14 (для древней, до десятков тысяч лет). Проницаемость пород измеряют в лаборатории, пропуская воду через образец и фиксируя расход; на местности — откачивая воду из скважины и наблюдая, как опускается уровень в соседних.

**Открытый вопрос.** Как изменится этот родник через 50 лет, если климат станет суше, а застройка уменьшит площадь, с которой вода просачивается в землю? И как по одному роднику можно судить о том, что происходит с водой под целым районом?

**Ссылки на странице QR:**

  - Карта глубин залегания грунтовых вод в регионе.
  - Схема «мини-эксперимент дома»: две пластиковые бутылки без дна, песок и глина.
  - Связь с другими экспонатами тропы (если применимо).

## **Дидактика**

**Что посетитель делает:** наблюдает родник, сопоставляет с разрезом на панели, опускает руку в воду (тактильное ощущение постоянной температуры), при наличии — проводит мини-эксперимент с песком и глиной, сверяет уровни воды по колышку.

**Что посетитель понимает:** не просто «откуда берётся родник», а почему гравитация и проницаемость пород вместе объясняют три разных наблюдения сразу. Понимает, что один и тот же принцип работает в масштабах от горшка до нефтяного месторождения.

**Тип рассуждения, который демонстрируется:** повторяющийся закон.

## **Эксплуатация**

**Сезонность.** Станция работает круглый год. Родник действующий, HPL и оцинкованная сталь устойчивы к морозу и УФ.

**Срок службы материалов:**

  - HPL-панель с UV-печатью: 7–10 лет без заметной деградации изображения.
  - Оцинкованный столбик: 10+ лет.
  - Нержавеющий колышек-уровнемер: 10+ лет.
  - Поликарбонатные трубки мини-эксперимента: 2–3 года (планируются к регулярной замене).
  - Сетки на нижних срезах трубок: 1–2 года, замена при обслуживании.

**Обслуживание:**

  - Ежегодный осмотр весной: проверка креплений столбика после пучения грунта, состояние панели, сохранность QR-плашки, чистота трубок мини-эксперимента (песок и глина со временем заиливаются).
  - Промывка или замена засыпки трубок — раз в 2–3 года.
  - Текст и QR рассчитаны на то, чтобы работать без текущего обновления: в них нет сезонных данных и ссылок на внешние сервисы, которые могут исчезнуть.

**Безопасность.** Подход к воде — по устойчивому грунту или каменным ступеням. Исключены элементы, за которые можно поскользнуться или уколоться. Колышек-уровнемер не выступает выше уровня земли более чем на 40 см. Трубки мини-эксперимента закреплены жёстко, без выступающих кромок.

**Вандалоустойчивость.** Все крепления — потайные или пломбируемые. QR-код гравирован, не наклеен. Колышек вынесен за пределы панели: его повреждение не выводит из строя основной экспонат. Трубки заменяемые, их потеря не критична.

## **Что не входит в конструкцию**

Электронный датчик температуры с автономным питанием — исключён. На уличной тропе без регулярного обслуживания батарея и электроника становятся слабым звеном. Постоянство температуры родника посетитель проверяет рукой — это и есть основной датчик. Для количественной проверки достаточно обычного термометра у посетителя или персонала при проведении занятий.

Задание «приди сюда через месяц и сравни уровень» — заменено на однократное сравнение уровня родника и уровня ручья ниже по склону. Причина: возвращающихся посетителей мало, журнал наблюдений без присмотра не ведётся, а разовое сравнение показывает тот же принцип (вода течёт вниз, но источник — выше ручья) и доступно прямо сейчас.

## **Итог**

Экспонат строится вокруг одного принципа, показанного с трёх сторон (место выхода родника, его температура, возможный возраст воды) и дополнительно — через мини-эксперимент с породами разной проницаемости. Конструкция рассчитана на самостоятельное существование на открытой тропе без постоянного обслуживания: основные элементы — HPL, оцинкованная сталь, нержавейка; расходные элементы (трубки мини-эксперимента) вынесены в отдельный блок с понятным циклом замены.

  
  

# WP027 — «Почему сложно идти прямо  

# **WP027 — «Почему сложно идти прямо?»**

## **Конструкция**

Столбик из оцинкованной стали высотой 1,2 м с наклонной панелью формата A2 из HPL. На расстоянии 5 м от стойки в обе стороны — пара вешек из нержавеющей трубы высотой 0,5 м с табличками «Старт» и «Финиш», обозначающих коридор длиной 15 м. Поверхность коридора — утрамбованный грунт или резиновое покрытие.

## **Содержимое панели**

Панель разделена на три смысловых блока. В правом верхнем углу — небольшая схема из трёх пиктограмм под общей подписью «Все уходят по кругу по одной причине»: человек с закрытыми глазами, лодка в тумане, самолёт в облаке. Ниже — геометрическая схема: траектория в виде дуги, обозначение малого угла отклонения Δφ на каждом шаге, радиус круга R. Внизу панели, напечатанный вверх ногами, — ответ на вопрос про метель.

## **Сценарий взаимодействия**

Парный эксперимент. Один участник встаёт на «Старт», закрывает глаза и идёт к «Финишу» — 15 шагов. Второй смотрит со стороны и запоминает, куда и насколько отклонился первый. Затем меняются ролями. Отклонение на 15 шагах обычно составляет 0,3–0,7 м и у разных людей направлено в разные стороны.

## **Тексты панели**

### **Уровень A (верх панели, крупно)**

**Ты не ходишь прямо. Никто не ходит.**

Встань на «Старт». Закрой глаза. Иди 15 шагов к «Финишу». Напарник смотрит со стороны — он увидит, как ты уходишь вправо или влево. Потом поменяйтесь.

На 15 шагах отклонение — полметра. На сотне шагов — десяток метров. Человек, заблудившийся в метели или в тумане, за час возвращается к своим же следам.

### **Уровень B (основной блок панели)**

**Почему именно круг?**

У каждого шага есть маленький перекос: левая нога идёт чуть иначе, чем правая. На одном шаге — доли градуса, незаметно. Но шаги складываются. Если на каждом шаге ты сворачиваешь на 1°, через 360 шагов направление развернётся полностью. Траектория замкнётся.

Пока глаза открыты, это не работает: ты видишь дерево впереди и поправляешь курс. Зрение — это обратная связь, она гасит ошибку на каждом шаге. Закрой глаза, убери связь — и малый перекос превращается в круг.

Принцип тот же у лодки без руля в тумане (несимметричные гребки), у самолёта в облаке без приборов («смертельная спираль» развивается за минуты), у машины с отпущенным рулём (развал колёс уводит к обочине). Муравьи, потерявшие феромонный след, иногда замыкаются в петлю и ходят по ней до изнеможения — явление называют «муравьиный круг смерти».

Прямая линия — это не свойство движения. Это результат постоянной коррекции по внешнему ориентиру.

**Как об этом узнали.** Норвежский физиолог Арне Лёрен в 1928 году просил людей с завязанными глазами идти по прямой через поле. Все траектории оказались дугами, но с очень разным радиусом — от пяти метров до сотни. В 2009 году группа Яна Соумана повторила опыт с GPS-трекерами — в лесу и в Сахаре. Результат тот же: без солнца и ориентиров испытуемые выписывали петли диаметром около двадцати метров.

### **Низ панели, напечатано вверх ногами**

*Почему в метель человек часами ходит по кругу?*

Снег скрывает ориентиры — зрение выключено так же, как если бы глаза были закрыты. Малая асимметрия шага, не погашенная коррекцией, за час разворачивает траекторию на полный оборот.

### **Уровень C (QR-код)**

**Откуда берётся круг.** Если на каждом шаге длиной L направление поворачивает на малый угол Δφ, траектория превращается в дугу окружности радиусом R ≈ L / Δφ. Полный круг замыкается через N = 2π / Δφ шагов. При шаге 0,7 м и отклонении 1° на шаг это около 360 шагов — примерно 250 метров пути, круг диаметром около 80 метров. Всё — из геометрии, без биомеханики и без предположений о том, почему есть асимметрия.

Откуда сама асимметрия — отдельный вопрос. Длина ног различается на миллиметры, сила толчка левой и правой стороны — на проценты, вестибулярный аппарат тоже не идеально симметричен. Все эти источники дают свой вклад, и их знак у разных людей разный. Поэтому в экспериментах Соумана испытуемые уходили и вправо, и влево — и один и тот же человек в разных опытах мог сворачивать в разные стороны. Популярное объяснение «правши уходят влево» данных не выдерживает: устойчивой связи с ведущей рукой не нашли.

**Более общий принцип.** Человек в пустыне — частный случай универсального правила: любая динамическая система с систематической ошибкой и без обратной связи либо расходится, либо замыкается в цикл. Автопилот, гироскоп, компас, GPS — всё это внешние ориентиры, которые замыкают контур обратной связи и позволяют идти прямо. Человечество научилось ходить прямо на большие расстояния только когда вынесло ориентир за пределы собственного тела: Полярная звезда, солнце, магнитная стрелка, спутник.

**Открытый вопрос.** Животные, мигрирующие на тысячи километров (крачки, лососи, монархи), не имеют ни компаса, ни карты в нашем смысле. Чем именно они замыкают контур обратной связи — магнитным полем Земли, поляризацией света, запахами, звёздами — до конца не ясно. Для некоторых видов ответ найден, для многих — нет.

## **Педагогический замысел**

**Тип научного рассуждения:** обратная связь. Экспонат показывает, что прямолинейное движение — не свойство системы, а результат непрерывной коррекции по внешнему ориентиру. Это универсальный принцип: человек, лодка, самолёт, автомобиль, муравей подчиняются одному и тому же правилу.

**Телесный опыт как основа.** Посетитель сам убеждается в своём отклонении, наблюдая за напарником и будучи наблюдаемым. Знание приходит через собственное тело, а не через чтение.

**Всесезонность.** При условии безопасной поверхности (утрамбованный снег без льда) эксперимент работает круглый год. Панель HPL не боится солнца, влаги и температурных циклов.

**Обслуживание.** Не требуется. Конструкция рассчитана на 3+ года эксплуатации без вмешательства.

  
  

# WP021 — Свияжские холмы: дно древнего ледника  

# **WP021 — Свияжские холмы: дно древнего ледника**

## **1. Название**

«Здесь лежал лёд толщиной в километр» (рабочее название экспоната — «Свияжские холмы»).

## **2. Локация и принцип размещения**

Обзорная точка на тропе с открывающимся видом на холмистый ландшафт Свияжских гряд. Точка выбирается так, чтобы с неё было видно асимметрию склонов и ориентацию гряд — то есть те признаки рельефа, про которые говорит текст панели. Без этой видимости экспонат теряет смысл: посетитель должен сопоставлять прочитанное с тем, что видит глазами.

## **3. Конструкция**

Одиночный столбик-стойка из оцинкованной стали диаметром 76 мм с порошковой окраской в тёмно-серый цвет, заглублённый в бетонный фундамент на 0,6 м; видимая высота стойки — 1,2 м. На стойке кронштейном закреплена наклонная (угол 15°) панель формата A2 (594 × 420 мм) из HPL-пластика толщиной 6 мм с УФ-стойким покрытием.

В 1 метре от стойки, в направлении лучшего обзора — **каменный круг или плитка заподлицо с грунтом** диаметром около 40 см. Это «точка осмотра»: посетитель встаёт на неё и поворачивается на 360°. Плитка заподлицо заменяет металлический штырь из первоначального проекта — штырь уязвим для газонокосилок и вандализма, плитка нейтральна и служит десятилетиями без обслуживания.

В нижнем правом углу панели — металлическая плашка из нержавеющей стали 70 × 70 мм с лазерно-гравированным QR-кодом. Крепёж — нержавеющие болты М8 с антивандальными головками Torx. Вес конструкции — не более 18 кг; обслуживание — ежегодный визуальный осмотр.

**Без подсветки и солнечной панели.** QR читается днём без дополнительного освещения; аккумулятор и контроллер заряда — обслуживаемые узлы с ресурсом 3–5 лет, которые не стоят добавленной функциональности в формате уличной тропы.

## **4. Компоновка панели**

Панель делится на три смысловые зоны — уже, чем в исходном черновике, чтобы дать воздух главной мысли.

**Верхняя зона (≈40% площади):** заголовок «Здесь лежал лёд толщиной в километр» и хук из двух-трёх предложений. Рядом — небольшая визуальная подсказка: силуэт ледника, наложенный полупрозрачно на силуэт современного ландшафта, с отметкой уровня «≈1 км» для масштаба.

**Средняя зона (≈40% площади):** основной текст — уровни A и B, разделённые графически, но читаемые как одна история. Слева от текста — схема-разрез: вертикальный срез почвы с обозначением слоёв (современный грунт → моренные отложения → сглаженные коренные породы). На схеме стрелкой показано направление движения льда. Подпись к схеме:

Перемешанный материал без сортировки — признак того, что его тащил лёд, а не вода: река разделила бы камни по размеру, ледник — нет.

**Нижняя зона (≈20% площади):** блок «Действие», короткая привязка к месту и QR-код с подписью «Почему ледники приходят и уходят».

Из компоновки убрана третья зона исходного проекта — орбитальная схема с циклами Миланковича. Эта тема уводит в сторону от главной мысли стенда и помещается за QR. Если Миланкович важен как отдельный сюжет тропы — он заслуживает собственного экспоната, не врезки на чужой панели.

## **5. Текст панели (уровни A и B)**

### **Заголовок**

**Здесь лежал лёд толщиной в километр**

### **Хук**

Холмы, которые ты видишь, не всегда были холмами. 20 000 лет назад над этим местом стоял ледник — выше Останкинской башни втрое. Он ушёл и оставил рельеф.

### **Уровень A — что наблюдаем**

Осмотрись. Гряды вытянуты в одном направлении, склоны с северо-запада круче, с юго-востока — пологие. Это направление, в котором двигался лёд. Холмы здесь — не холмы в обычном смысле, а валы обломков, которые ледник толкал перед собой и бросил, когда растаял.

### **Уровень B — механизм и принцип**

Лёд на малых временах кажется твёрдым. На больших — течёт под собственным весом, как очень медленная река. Нижняя поверхность ледника волочёт камни, песок, глину и работает как напильник: стёсывает породу внизу и собирает материал в валы у края, где лёд тает быстрее, чем успевает принести новый.

Скорость такого движения — около метра в год. За 10 000 лет ледник переносит материал на десятки километров. Тот же приём — маленькая скорость, большое время, крупный эффект — работает у реки, подмывающей берег на миллиметры в год; у мантии Земли, растаскивающей материки на сантиметры; у горных ледников сегодня. Геологам не нужны катастрофы, чтобы объяснить крупные формы рельефа — хватает обычной скорости, умноженной на достаточное время.

Это знают из сравнения: те же гряды с такой же асимметрией и ориентацией находят по всему северу Евразии и Северной Америки — от Карелии до Канадского щита. Форма повторяется там, где когда-то был край ледника, и отсутствует южнее. По направлению гряд восстанавливают, откуда лёд шёл.

### **Действие посетителя**

Встань на каменный круг у столбика и медленно повернись на 360°. Попробуй глазами найти направление, в котором вытянуты гряды и крутые склоны сходятся — оттуда двигался лёд.

### **Почему это здесь**

Свияжские холмы — южная окраина последнего оледенения. Рельеф здесь уже несёт следы ледника, но ледник сюда едва дошёл, поэтому формы мягкие, без драматичных впадин и скальных выглаживаний Карелии. Хорошая точка, чтобы увидеть ледник в его самом аккуратном проявлении.

## **6. Содержание QR-кода (уровень C)**

### **Заголовок**

**Почему ледники приходят и уходят**

### **Текст**

Ледниковые эпохи — не случайность. Форма орбиты Земли, наклон её оси и направление оси вращения медленно меняются под гравитационным влиянием других планет, главным образом Юпитера. Три независимых цикла — эксцентриситет (\~100 тыс. лет), наклон (\~41 тыс. лет), прецессия (\~23 тыс. лет) — складываются и меняют, сколько солнечного тепла получает северное полушарие летом. Когда лета прохладнее, снег не успевает стаять за тёплый сезон и за тысячи лет накапливается в ледник. Когда лета теплее — ледник отступает. Это называется теорией Миланковича.

Модель работает не везде одинаково. На скорость оледенений влияют ещё распределение суши и океанов, содержание CO₂ в атмосфере, течения. Без этих усилителей одних орбитальных колебаний для полного цикла оледенения не хватит.

### **Как это узнаёт наука**

Циклы Миланковича проверяют по донным осадкам океана и по слоям ледниковых кернов Гренландии и Антарктиды. В них по соотношению изотопов кислорода читают температуру прошлого — на миллионы лет назад. Ритм в этих записях совпадает с ритмом, рассчитанным из уравнений небесной механики.

### **Открытый вопрос**

Сейчас орбитальные параметры говорят, что следующее оледенение — примерно через 50 000 лет. Но выбросы CO₂ за последние 150 лет уже вывели концентрацию парниковых газов за рамки, в которых орбитальная модель давала надёжные прогнозы. Что победит на ближайшей сотне тысяч лет — астрономия или химия атмосферы — пока открытый вопрос.

### **Дополнительные материалы на странице (опционально)**

  - Краткая анимация: современная карта региона с постепенно наступающим и отступающим ледником. Без звукового сопровождения, с управлением по скроллу.
  - Ссылка на тематически соседние экспонаты тропы, если они есть.

Цифровой ресурс реализуется как одна статическая HTML-страница, размещённая на поддомене основного сайта тропы. Без сложных интерактивов, Flash, JavaScript-зависимостей, которые устаревают за 2–3 года. Содержание самодостаточно как текст — анимация приятна, но не критична.

## **7. Педагогический замысел**

Экспонат построен вокруг **одного типа научного рассуждения — повторяющегося закона**. Главная мысль: маленькая скорость, умноженная на большое время, даёт крупные формы рельефа. Этот принцип переносится с ледника на реку, на мантию, на материки — и именно поэтому геология может объяснять горы, каньоны и океаны, не прибегая к катастрофам.

Сезонность: актуально в любое время года. Столбик и панель выше снежного покрова; содержание текста не зависит от сезона — наоборот, зимой рельеф читается отчётливее, потому что снег сглаживает мелкие детали и проявляет общий контур гряд.

Целевая аудитория: школьники средней и старшей школы, взрослые без специальной подготовки. Текст уровней A и B читается за 2–3 минуты; QR — ещё 3–4 минуты для заинтересовавшихся.

## **8. Электропитание**

Не требуется.

## **9. Обслуживание**

Ежегодный визуальный осмотр: состояние HPL-покрытия, целостность болтов, читаемость QR-кода. HPL с УФ-защитой рассчитан на 10+ лет уличной эксплуатации без замены. Раз в 3–5 лет — проверка, что страница по QR-коду открывается и отображается корректно.

  
  

# WP028  

# **WP028 Как измерить высоту дерева по тени?**

### **Тип станции**

Малый объект с панелью — измерительная станция на открытой площадке.

### **Тип научного рассуждения**

**Рамка.** Формула работает не всегда, а при одном условии: источник света должен быть далёк. Измени условие — подобие исчезнет, метод сломается. Посетитель узнаёт не только приём измерения, но и границу его применимости.

## **Конструкция**

**Штырь-эталон.** Вертикально вмонтирован в грунт металлический штырь из нержавеющей стали диаметром 20 мм, высотой ровно 1,000 м. По всей длине — выгравированная шкала с делениями через 10 см. Штырь рассчитан на силовое воздействие: повиснуть на нём невозможно, сломать — тоже.

**Панель-стенд.** Рядом — невысокий столбик-постамент из оцинкованной стали высотой 0,8 м с наклонной панелью формата A3–A2 из HPL. На панели:

  - заголовок и основной текст экспоната (уровни A и B);
  - геометрическая схема «Подобие треугольников» — два прямоугольных треугольника с общей вершиной у Солнца;
  - таблица для записи результатов, выгравированная прямо в HPL;
  - поле под маркер — матовая зона, стираемая сухой тряпкой;
  - маркер на металлической цепочке;
  - QR-код со ссылкой на углублённое объяснение (уровень C) и полем для сверки результата.

**Площадка.** В радиусе 3 м от штыря поверхность выровнена и покрыта светлым твёрдым материалом для читаемости тени. На краю площадки — мерная шкала, вдавленная в покрытие, для тех, кто не хочет мерить шагами.

## **Тексты**

### **Заголовок стенда**

**Как измерить то, до чего не дотянуться?**

### **Хук**

Дерево рядом — и его высоту можно узнать, не забираясь на него и не принося лестницы. Нужен только ясный день, этот метровый штырь и две измеренные тени.

### **Уровень A — действие и базовое понимание**

Штырь рядом с тобой — ровно 1 метр, со шкалой через 10 см. Измерь длину его тени — шагами или по шкале на краю площадки. Теперь измерь длину тени ближайшего дерева. Раздели вторую длину на первую — во столько раз дерево выше метра. Запиши число в поле на панели.

### **Уровень B — механизм и рамка закона**

Тень штыря и тень дерева получаются от одного и того же Солнца, в одну и ту же минуту, под одним и тем же углом. Угол одинаковый потому, что Солнце очень далеко — его лучи приходят к Земле параллельными линиями, как строчки на тетрадном листе. Раз углы равны, отношение «высота / тень» одинаково для штыря и для дерева.

Получается измерительный приём: берём предмет, который легко измерить (штырь), и через его тень узнаём высоту предмета, который измерить трудно. Но у приёма есть граница: он работает только потому, что Солнце далеко. Заменим его на близкий источник — лучи разойдутся веером, углы перестанут совпадать, формула перестанет работать.

### **Подпись под схемой**

Угол при Солнце один и тот же — потому что Солнце далеко. Поэтому высоты и тени пропорциональны.

### **Таблица результатов (выгравирована в HPL)**

|  |  |  |
| :-: | :-: | :-: |
|   | \*\*Длина тени\*\* | \*\*Высота\*\* |
| Штырь | … м | \*\*1,00 м\*\* |
| Дерево | … м | … м |
| Столб ЛЭП (по желанию) | … м | … м |

**Формула:** высота дерева = (тень дерева ÷ тень штыря) × 1 м.

### **Действие посетителя**

Измерь тень штыря, измерь тень ближайшего дерева, посчитай частное и умножь на 1 метр. Запиши результат маркером в поле на панели. Дополнительный уровень: попробуй прикинуть высоту столба линии электропередачи, не подходя к нему.

### **Привязка к месту**

Площадка выровнена и покрыта светлым материалом, чтобы тень читалась без напряжения. Деревья рядом — измеряемая натура; солнечная сторона — рабочее время стенда.

## **QR — уровень C (углубление)**

### **Заголовок**

**Почему это работает — и где ломается?**

### **Текст**

Почти ровно 150 миллионов километров — столько от Земли до Солнца. На таком расстоянии солнечный диск виден с Земли под углом около половины градуса, а лучи, попадающие на соседние предметы на площадке, расходятся так слабо, что расхождение не измеришь никакой линейкой. Для практики это означает: лучи параллельны. Отсюда — подобие треугольников «высота — тень» для любых двух предметов рядом, и отсюда же — работоспособность всего метода.

Проверить границу можно прямо: встань ночью под уличным фонарём, поставь рядом два предмета разной высоты и измерь их тени. Отношение «тень / высота» для них окажется разным — потому что фонарь близко, его лучи расходятся веером, и углы, под которыми предметы освещены, уже не равны. То же Солнце, уменьшенное и приближённое, ломает формулу. Условие «источник света далеко» — не техническая оговорка, а часть закона.

### **Как это узнаёт наука**

Так в VI веке до н. э. Фалес измерил высоту египетской пирамиды — поставил палку, дождался момента, когда её тень сравнялась с её длиной, и в ту же минуту измерил тень пирамиды. Через три века Эратосфен таким же способом, сравнивая длины теней в Сиене и Александрии в один полдень, получил радиус Земли — с ошибкой в несколько процентов от современного значения. В обоих случаях метод — один и тот же, а инструмент — палка и тень.

### **Открытый вопрос**

Если Солнце всё же не точечное, а имеет размер, — на какой дистанции приближения «лучи параллельны» перестанет хватать? И как тот же принцип «измеряю недоступное через подобие» работает у астрономов, которые определяют расстояние до звёзд, не имея под рукой ни одной палки?

### **Сверка ответа**

Точный ответ для этого дерева: **H = L\_дер × (1,00 м / L\_шт)**, где L\_дер и L\_шт — измеренные в один момент длины теней. Расхождение до 10–15% — нормально: тень размыта полутенью, неровности грунта сдвигают край.

## **Режим работы и ограничения**

  - **Погода.** Работает только при прямом солнце. Сплошная облачность — тени нет, метод выключен. Это методологически важно: граница закона наблюдаема прямо — нет источника, нет подобия.
  - **Время суток и сезон.** Лучшее время — утро и вечер (длинные тени, высокая точность); полуденные тени короткие, шаг измерения грубее. Зимой работает, но сугробы могут закрыть площадку — нужна расчистка.
  - **Возраст посетителей.** 7+. Дети младше — со взрослым (ведут измерение шагами).
  - **Безопасность.** Штырь не имеет острых рёбер; верхний торец скруглён. Маркер на цепочке — расходник.

## **Обслуживание**

  - Маркер — замена по мере высыхания (расходник).
  - Поле под запись — протирка сухой тряпкой, 1 раз в неделю в сезон.
  - Светлое покрытие площадки — чистка от листвы и грязи весной и осенью.
  - Штырь и шкала — нержавейка, не требуют обслуживания.
  - HPL-панель — срок службы 10+ лет на улице.

## **Сводка по качеству (из верификации)**

|  |  |
| :-: | :-: |
| \*\*Ось\*\* | \*\*Оценка\*\* |
| Философская глубина | A — первопричина явлена (параллельность лучей), рамка закона показана, перенос на Эратосфена и параллакс есть |
| Методологическая честность | A — структура «тело → наблюдение → формула», формула приходит как следствие безразмерного отношения |
| Практическая реализуемость | A — конструкция робастная, вандалоустойчивая, воспроизводимая в сельской школе |

  
  

# WP031 «Сфера и форма миров»  

# **WP031 «Сфера и форма миров»**

## **Тип конструкции**

Столбик с двусторонней панелью.

## **Конструкция**

Столбик из стальной трубы высотой 1,2 м с вертикальной двусторонней панелью A2 из HPL. Панель крепится в верхней части столбика, доступна для чтения с обеих сторон. Высота размещения рассчитана на читателя ростом от 140 см.

## **Содержание панели**

### **Сторона 1 — «Почему планеты круглые, а астероиды — нет?»**

Визуальное сравнение трёх тел одинакового объёма: куб, цилиндр, шар — с численными значениями площадей поверхностей (у куба площадь примерно на 24% больше, чем у шара; у вытянутого цилиндра — ещё больше).

Текст объясняет различие формы через соревнование двух сил: гравитации, сжимающей тело к центру, и прочности породы, сопротивляющейся сжатию. Для малых тел (радиус до \~200–300 км) прочность побеждает, и форма остаётся случайной — отсюда «картофельные» астероиды вроде Фобоса, Иды, Эроса. При большем радиусе вес верхних слоёв раздавливает нижние, порода течёт, и тело скатывается к форме с наименьшей поверхностью — к шару. Порог перехода — диаметр примерно 400–600 км.

### **Сторона 2 — «Один закон — три обличья»**

Схема с тремя объектами: мыльный пузырь, капля воды в невесомости (кадр с МКС), эритроцит.

Текст показывает, что один и тот же математический принцип — минимум поверхности при фиксированном объёме — действует в трёх разных физиках: гравитация у планеты, поверхностное натяжение у капли и пузыря, упругость мембраны у клетки крови. Физика разная, результат одинаковый. Это тип научного рассуждения, который стоит отдельного внимания: совпадение формы в несвязанных системах — признак общей математической структуры за разными явлениями.

### **QR-код**

Ведёт на страницу с углублённым материалом:

  - Вывод порога 400–600 км из сравнения гравитационного давления и прочности породы (оценка через размерностный анализ: R ≈ √(σ/Gρ²), где σ — прочность, ρ — плотность, G — гравитационная постоянная). Показывает, почему ледяные тела округляются раньше каменных.
  - Исторический контекст изопериметрической теоремы: плоский случай из античности, доказательство Штейнера для двух измерений, Шварц и Минковский для трёх измерений. Разъяснение, что мыльный пузырь — физическая демонстрация теоремы, а не её доказательство.
  - Границы применимости: быстрое вращение сплющивает шар (Земля, Юпитер, Хаумеа); состав тела меняет порог округления (Мимас — шар при 400 км благодаря льду, каменная Веста при 525 км так и не стала шаром).
  - Открытый вопрос: связь минимизации поверхности с минимизацией гравитационной потенциальной энергии.

## **Образовательная ценность**

Экспонат демонстрирует повторяющийся закон: один геометрический принцип (сфера — форма с минимальной поверхностью при заданном объёме) проявляется в несвязанных системах — астрономии, гидродинамике, биологии. Показывает также тип рассуждения «ограничение»: у законов природы есть пороги (диаметр 400–600 км), которые выводятся из сравнения конкурирующих факторов, а не постулируются.

Связка математики и астрономии удерживает фокус на одном принципе. Тема многомерных пространств и концентрации меры в нейросетях на эту панель не включена — она относится к другому математическому сюжету и требует отдельного экспоната.

## **Сезонность**

Круглогодично. Панель не зависит от времени года, может читаться в любой сезон.

## **Требования безопасности**

Не требуется. HPL-панель устойчива к вандализму, царапинам, ультрафиолету и перепадам температур; стальная труба бетонируется в грунт с гидроизоляцией цоколя.

## **Обслуживание**

Минимальное. Ресурс HPL при уличной эксплуатации — 10–15 лет без обновления. Проверка антикоррозийного покрытия столбика — раз в 2–3 года.

  
  

# 33 — «Фрактальная геометрия природы»  

#  **33 — «Фрактальная геометрия природы»**

**Код:** WP033TBD **Тип объекта:** Информационный стенд (столбик с двусторонней панелью) **Тип научного рассуждения:** Повторяющийся закон

## **Конструкция**

Столбик из стальной трубы высотой 1,2 м с крупной вертикальной двусторонней панелью формата A2–A1 из HPL. Монтаж на бетонное основание или анкерные болты. Без изменений относительно исходного проекта — конструкция оценена как образцово робастная (HPL на улице служит 10+ лет без выцветания, сталь требует покраски раз в 3–5 лет, вандалоустойчивость высокая).

## **Размещение**

Рядом с живым деревом, крона которого попадает в поле зрения посетителя, стоящего у стенда. Стрелка на панели указывает «посчитай уровни ветвления у этого дерева». Привязка к конкретному объекту делает стенд из абстрактной иллюстрации — интерактивом за нулевую стоимость.

## **Графика панели**

**Сторона 1 — «Разные системы, одно правило».** Три крупные фотографии, объединённые общим принципом максимизации поверхности обмена: лёгкие человека, крона дерева (желательно той же породы, что растёт рядом со стендом), кровеносная сеть листа. Под каждой фотографией — подписанная фрактальная размерность D. Небольшая врезка в углу: три шага построения снежинки Коха как математический образец самоподобия. Остальные фотографии из первоначального списка (облако, дельта реки, броколли, береговая линия, снежинка в природе) — переведены в QR-галерею.

**Сторона 2 — «Дерево Линденмайера».** L-система, три уровня итерации. Показывает, как из одного короткого правила переписывания вырастает сложная ветвящаяся форма. Подпись объясняет связь между рисунком и живым деревом рядом.

## **Текст панели**

### **Заголовок**

**Дерево, лёгкое, река — одна и та же форма**

### **Хук**

Посмотри на крону ближайшего дерева. Ствол делится на крупные ветви, те — на средние, те — на мелкие, мелкие — на веточки с листьями. Та же схема лежит в основании твоих лёгких и у разветвлений реки на дельту.

### **Уровень A — что наблюдаем**

Ветвление устроено так, что каждая часть похожа на целое. У целого дерева видно несколько крупных ветвей; у одной ветви — несколько таких же, только поменьше; и дальше до веточек. Такая структура называется фракталом. Лёгкие взрослого человека при объёме около 5 литров имеют внутреннюю поверхность порядка 70 м² — это возможно именно потому, что дыхательные пути ветвятся примерно 23 раза подряд.

### **Уровень B — механизм и тип рассуждения**

Когда живой или физической системе нужна большая поверхность контакта при малом объёме, ветвление решает задачу точнее, чем любая гладкая форма. Сплошной мешок с тем же объёмом имел бы поверхность в сотни раз меньше. Поэтому один и тот же приём — повторяющееся деление на более мелкие ветви — возникает в системах, которые ничего не знают друг о друге: в лёгких млекопитающих, в сосудах листа, в корневой системе, в речных дельтах, в молниях. Правило ветвления не принадлежит биологии или географии — оно диктуется геометрией и общим требованием «много поверхности в малом объёме». Измерять такие структуры можно фрактальной размерностью: чем меньше линейка, тем длиннее получается контур, и по скорости этого роста вычисляют число между 1 и 2 (для береговой линии Британии — около 1,25).

### **Действие посетителя**

Посмотри на дерево рядом со стендом и посчитай, сколько уровней ветвления до самых тонких веточек. Запиши число маркером на панели. У большинства деревьев получается 7–10.

### **Почему это здесь**

Живое дерево рядом — прямой пример фрактала. Рисунок на панели и то, что видно над головой, — одно и то же правило в разных масштабах.

### **Переход дальше**

Не требуется, объект работает как самостоятельная станция.

## **QR-код (уровень C — углубление)**

### **Заголовок QR**

**Почему у фрактала всегда конечное число уровней**

### **Точная модель**

Фрактальная размерность D вводится через измерение. Если покрыть контур линейкой длины ε, понадобится N(ε) отрезков, и между ними выполняется соотношение N(ε) ∝ ε^(−D). Для гладкой линии D = 1, для плотно заполненной плоскости D = 2. Береговая линия Британии при уменьшении линейки удлиняется не как прямая и не как площадь, а между ними: D ≈ 1,25. Природные фракталы, в отличие от математических, всегда ограничены сверху и снизу: лёгкие перестают ветвиться на 23-м уровне, потому что дальше альвеола становится меньше расстояния, на которое молекула кислорода успевает пройти диффузией; дерево — на 7–10-м уровне, потому что более тонкая ветка не удержит лист или не получит достаточно света. Масштаб работы закона задаётся физикой конкретной среды, а не самой формулой ветвления.

### **Как это узнаёт наука**

Фрактальную размерность измеряют методом покрытия (box-counting): на снимок или контур накладывают сетку с разным шагом и считают, сколько клеток содержит объект. Логарифмический наклон этой зависимости и есть D. Так измеряют размерность сосудистой сети по рентгеновским снимкам, трещин — по фотографиям горных пород, облаков — по спутниковым данным.

### **Галерея «фракталы везде»**

Краткие пояснения к фотографиям, которые не поместились на стенде: речная дельта, облако, береговая линия, молния, броколли Романеско, снежинка, трещины в глине при высыхании. Каждый снимок сопровождается фрактальной размерностью и одной фразой о том, какая физика задаёт здесь ветвление.

### **Закон Клайбера**

Если геометрия сосудистой сети фрактальна, то и скорость обмена через неё должна подчиняться степенному закону. Из этой идеи выводится закон Клайбера: метаболизм животного растёт как m^0,75 от массы. Слон на килограмм веса потребляет примерно в 20 раз меньше пищи, чем мышь. Почему именно показатель 3/4, а не 2/3 (как было бы для гладкой сферы) — обсуждается до сих пор.

### **Открытый вопрос**

Можно ли из одного принципа «много поверхности в малом объёме» вывести количественные характеристики конкретной системы — число уровней ветвления, углы между ветвями, соотношение диаметров родительской и дочерней ветвей? Частично — да (есть модели Уэста–Брауна–Энквиста и закон Мюррея для сосудов). Полного вывода «от первых принципов» для живых ветвящихся систем пока нет.

## **Практическое задание (базовое)**

Посчитать уровни ветвления у ближайшего дерева и записать число маркером на специально отведённом поле панели. Поле представляет собой табличку с колонками «дата — порода дерева — число уровней», которая постепенно заполняется посетителями. HPL допускает маркер, смываемый спиртом раз в сезон.

## **Практическое задание (расширенное, опционально)**

Нарисовать дерево по правилу самоподобного ветвления: начать со ствола, разделить его на две ветви под углом \~30°, каждую из этих ветвей разделить так же, повторить 5–7 раз. Сравнить получившийся рисунок с настоящим деревом.

## **Сезонность**

Работает круглый год. Зимой крона без листьев даёт более чистую демонстрацию ветвления — видна сама структура, без зелёного шума. Летом добавляется наглядность сосудов в листе, которую можно увидеть на просвет.

## **Что убрано и почему**

Из исходного проекта убраны (перенесены в QR или опущены):

  - 5 из 8 фотографий фракталов — чтобы не превращать стенд в каталог и дать место одному принципу вместо россыпи примеров.
  - Формула M ∝ m^0,75 на главной панели — требует отдельного объяснения, перенесена в QR как закон Клайбера.
  - Упоминание 1,25D без вывода — заменено на объяснение через измерение линейкой, так что число перестаёт быть магическим.

## **Что усилено**

  - Явлена первопричина: ветвление как решение задачи «много поверхности в малом объёме».
  - Показаны рамки закона: реальные фракталы конечны, и границы задаёт физика среды.
  - Стенд привязан к живому дереву рядом, а не только к фотографиям.
  - Графика сокращена до трёх примеров, объединённых одним принципом.

  
  

# 37 «Уникальные свойства воды»  

# **37 «Уникальные свойства воды»**

## **Тип станции**

Столбик с двусторонней наклонной панелью. Обзорная точка у водоёма.

## **Конструкция**

**Опора.** Столбик из оцинкованной стали, высота 1,2 м. Бетонирование стойки в грунт (не грунтовый анкер) для жёсткой фиксации двусторонней панели.

**Панель.** Наклонная двусторонняя, формат A2, материал HPL (пластик высокого давления, уличное исполнение). Ресурс без выцветания — 10+ лет. Проверить размер шрифта вживую на образце перед тиражированием: при наклонной установке мелкие элементы графика читаются хуже, чем на плоском стенде.

**Интерактив.** Термометр-зонд с цифровым дисплеем на размеченной верёвке (метки через 0,5 м), закреплён у стойки стальным тросиком. Посетитель опускает зонд в воду на разную глубину и читает температуру. Летом виден перепад в термоклине (2–3°C на 1–2 метра), зимой подо льдом — около +4°C у дна. Работает круглый год, не требует спуска к воде.

**Размещение.** Обзорная точка с прямым видом на водоём, безопасный подход к кромке для работы с зондом.

## **Содержание панели**

### **Сторона 1 — основной вопрос**

**Заголовок:** «Почему рыбы не замерзают зимой?»

**Текст:**

Посмотри на воду перед собой. Зимой она замерзает сверху, а не со дна. Если бы промерзала полностью — жизнь в пресных водоёмах была бы невозможна.

У большинства жидкостей плотность растёт при охлаждении вплоть до замерзания: холодное тонет, тёплое всплывает. Вода ведёт себя иначе. Максимальная плотность у неё — при **+4°C**, а не у точки замерзания. При дальнейшем охлаждении вода становится легче и остаётся на поверхности. Там она и замерзает. Лёд тоже легче воды — поэтому плавает.

Лёд работает как теплоизолятор: подо льдом сохраняется около +4°C всю зиму. В этом слое и зимуют рыбы, раки, личинки насекомых.

**Визуальные элементы:**

  - Крупный график «Плотность воды от −10°C до +20°C», максимум выделен при +4°C
  - Схема водоёма зимой в разрезе: лёд сверху, температурный градиент от 0°C к +4°C у дна, силуэты зимующей фауны
  - Стрелка-подсказка к термометру-зонду: «Проверь сам — опусти зонд»

### **Сторона 2 — общая причина**

**Заголовок:** «Одна причина — несколько странностей»

**Текст:**

Плавающий лёд — не единственная особенность воды. У неё необычно высокая теплоёмкость: водоём прогревается медленнее воздуха и дольше остывает осенью. Поверхностное натяжение держит водомерку. Температура кипения — 100°C, а у похожего по строению сероводорода H₂S — около −60°C.

Все эти особенности объясняются одним свойством молекулы. Атом кислорода сильнее притягивает электроны, чем атомы водорода, поэтому у молекулы воды один конец слегка отрицательный, другой — слегка положительный. Соседние молекулы сцепляются этими полюсами. Такую связь называют **водородной**.

**Лёд легче воды.** При замерзании каждая молекула держит четырёх соседей на фиксированном расстоянии и под определёнными углами. Получается рыхлая решётка с пустотами — объём льда больше объёма той же воды примерно на 9%.

**Много энергии уходит на нагрев.** Чтобы молекулы двигались быстрее, нужно сначала растягивать и рвать водородные связи между ними. Поэтому водоём медленно прогревается и медленно остывает — он сглаживает перепады температуры вокруг.

**Поверхность держит водомерку.** Молекулы в толще воды окружены соседями со всех сторон, а на поверхности — только снизу и с боков. Недостающие связи стягивают поверхность, как натянутую плёнку.

Молекулярное объяснение подтвердилось к 1930-м годам, когда удалось измерить углы между связями во льду рентгеновскими методами. Рыхлая гексагональная решётка видна напрямую.

**Визуальные элементы:**

  - Схема молекулы воды с обозначением полюсов (δ+ у водородов, δ− у кислорода) и пунктирных водородных связей к соседним молекулам
  - Три небольшие иллюстрации к трём блокам: рыхлая гексагональная решётка льда; столбик воды с молекулами на поверхности и в толще; водомерка на воде
  - QR-код «Для тех, кто хочет глубже» в нижнем углу

## **Содержание QR-кода (уровень C)**

### **Почему именно +4°C**

Максимум плотности — результат двух встречных процессов. При охлаждении жидкости от +20°C молекулы движутся медленнее и упаковываются плотнее (обычное тепловое сжатие). Одновременно в жидкой воде постепенно растёт доля молекул, связанных в рыхлые льдоподобные группы — это начинается задолго до замерзания. До +4°C сжатие преобладает, после +4°C — разрыхление. На этой границе плотность максимальна.

### **Оцените сами**

Если бы лёд тонул, озеро глубиной 3 м при средней зимней температуре воздуха −15°C промёрзло бы до дна за несколько недель. Летом верхний метр оттаивал бы, нижние слои оставались льдом постоянно. В таких условиях исчезли бы рыбы, большая часть водных насекомых, водоросли. Пресная жизнь существует благодаря тому, что одна молекула при замерзании занимает больше места, чем в жидком состоянии.

### **Та же связь — в других местах**

Две цепи ДНК удерживаются вместе водородными связями между азотистыми основаниями. Белки сворачиваются в рабочую форму за счёт тех же связей внутри молекулы. Сок поднимается по стволу дерева на десятки метров благодаря сцеплению молекул воды друг с другом и со стенками сосудов.

### **Термоклин летом**

Опусти зонд на разную глубину в июле-августе. В верхнем прогретом слое — температура воздуха минус 2–3°C. На глубине 1,5–2 м часто видно резкий перепад: за полметра температура падает на 3–5°C. Это термоклин — граница между прогретой солнцем поверхностью и холодной придонной водой. Выше термоклина живут окунь, плотва, уклейка. Ниже — налим, ёрш, раки. Граница сезонная: осенью она размывается, и вся толща снова перемешивается.

### **Ночное наблюдение летом**

В тихий безветренный вечер приди в низину у реки. Белая дымка над поймой — это туман. Холодный воздух стекает в понижение, температура падает, водяной пар не удерживается в воздухе и конденсируется в капли. Это тот же фазовый переход, о котором говорит стенд, — но в обратную сторону: пар → вода.

### **Где модель перестаёт работать**

Простое «дипольное» объяснение даёт интуицию, но не всё. Точная структура жидкой воды обсуждается до сих пор: есть гипотезы о сосуществовании двух локальных форм (более и менее плотной), о постоянно перестраивающейся сетке связей, о поведении переохлаждённой воды ниже −40°C. Это активная область физической химии — «обычная вода» остаётся одним из самых сложных веществ для моделирования.

## **Круглогодичность**

Работает в любой сезон. Зимой фокус на плавающем льде и зонде подо льдом (+4°C у дна). Летом — на термоклине и поверхностном натяжении (водомерки видны на воде). Осенью — на медленном остывании водоёма. Весной — на ледоходе и перемешивании слоёв.

## **Безопасность**

Подход к воде — по имеющейся тропе. Зонд на тросике: в воду лезть не нужно. Отдельной разметки и ограждений не требуется, если обзорная точка сама по себе безопасна (нет обрыва, скользкого спуска).

## **Что изменено относительно исходного черновика**

  - Убрана врезка «Тройная точка» с фазовой диаграммой — не работает на основной сюжет, уместнее в отдельном экспонате.
  - Убраны «Три аномалии воды» как пронумерованный список — заменены на единую причинную линию от водородной связи к трём следствиям.
  - Интерактив «опустить руку в воду» заменён на термометр-зонд: работает круглый год, безопаснее, показывает термоклин и придонные +4°C.
  - Добавлена схема молекулы воды с полюсами и водородными связями — ключевой визуальный элемент стороны 2, без которого «общая причина» не считывается.
  - Перенос на биологию (ДНК, белки, подъём сока) вынесен в QR — на стенде не помещается без потери фокуса.

  
  

# WP045— «Под парусом и против ветра»  

# **WP045— «Под парусом и против ветра»**

## **Тип и расположение**

Столбик с наклонной двусторонней панелью на набережной Волги, с видом на акваторию.

## **Конструкция**

Столбик из оцинкованной стали высотой 1,2 м. Наклонная двусторонняя панель A2 (420×594 мм) из HPL с двусторонней УФ-стойкой печатью, угол наклона 30° к вертикали.

На стороне 1 под векторной диаграммой — встроенная маркерная зона (матовое стираемое покрытие HPL, 15×20 см) для задания с галсами. Маркер прилагается в держателе на столбике либо предполагается личный. Протирка зоны — раз в сезон в рамках регламентного обслуживания тропы.

Активных механических элементов нет. Все поверхности выдерживают уличные условия без обслуживания 5+ лет.

## **Тема**

Парус как крыло. Векторное разложение силы ветра. Движение против ветра как следствие работы двух сред одновременно — воздуха (тяга паруса) и воды (сопротивление киля). Лавировка галсами как следствие физических ограничений, а не произвольный приём.

## **Тип научного рассуждения**

**Повторяющийся закон.** Один принцип — «сила в одной среде, сопротивление в другой, движение в направлении, которое ни одна из сред не задаёт напрямую» — работает у паруса, крыла самолёта, кайта, колёс автомобиля в повороте. Экспонат показывает не устройство парусника, а способ, которым природа получает движение в «запрещённом» направлении.

## **Содержание панели**

### **Сторона 1 — физика**

**Заголовок:** «Ветер с севера. Лодка идёт на север.»

**Уровень A (хук и вопрос):** Короткий абзац о том, что парусник на Волге может идти почти навстречу ветру. Ветер толкает назад — откуда движение вперёд?

**Уровень B (механизм):** Парус работает как крыло, поставленное вертикально. Ветер обтекает его, и сила тяги направлена почти перпендикулярно плоскости паруса — то есть почти вбок от лодки, с небольшой составляющей вперёд. Боковое движение гасит киль — вертикальная пластина под днищем, упирающаяся в воду. Из двух составляющих силы остаётся только продольная — лодка идёт вперёд.

Ключевая идея, явно сформулированная на панели: *сила действует в одной среде (воздух), сопротивление приходит из другой (вода), движение получается в направлении, которое ни одна из сред не задаёт напрямую.*

**Векторная диаграмма (центр панели):** Лодка вид сверху. Вектор ветра, вектор тяги паруса (перпендикулярно плоскости паруса), реакция киля, результирующая скорости. Без формул — только стрелки с подписями.

**Блок «Тот же принцип»** (правый нижний угол, иконки с короткими подписями):

  - Крыло самолёта: воздух обтекает его, сила почти перпендикулярна крылу — из неё берётся подъёмная составляющая.
  - Кайт серфера: парус в небе тянет доску через воду.
  - Руль автомобиля в повороте: колёса толкают машину вбок, инерция несёт вперёд, получается дуга.

**Блок «Почему нельзя прямо против ветра»:** Угол между курсом и ветром меньше 45° сделать не получается. Киль не идеален, парус не идеален — часть боковой силы всё равно сносит лодку. Поэтому идут галсами: за три длины пути — одна длина продвижения навстречу ветру.

**Маркерная зона с заданием:** «Ветер с севера ⬆️. Нужно попасть севернее старта. Нарисуй маршрут галсами. Сколько поворотов получилось?» Стартовая точка и направление ветра нанесены заранее, остальное — маркером.

**Стрелка направления преобладающего ветра** на данной точке набережной — небольшая врезка в углу панели с учётом местной розы ветров.

### **Сторона 2 — геометрия лавировки**

**Заголовок:** «Как идти туда, куда не пускает ветер»

**Три схемы курсов** (не хронология, а три положения одной лодки относительно ветра):

1.  **Бейдевинд** — острый угол к ветру (\~45°). Парус почти вдоль лодки. Тяга мала, но есть, и направлена вперёд.
2.  **Галфвинд** — ветер сбоку. Парус под углом 45° к лодке. Самый эффективный курс.
3.  **Фордевинд** — ветер в спину. Парус поперёк. Лодка идёт медленнее ветра.

**Схема галсов:** зигзаг против ветра. Старт, два поворота, финиш. Угол к ветру 45°, соотношение пути к продвижению 3:1.

**Блок «Откуда это узнали»:** Разложение силы на составляющие — модель XVII века. Гюйгенс, Бернулли. Проверка — замеры тяги паруса в аэродинамической трубе; сегодня те же замеры делают для крыльев самолётов. Одна и та же модель описывает и парус XV века, и крыло современного лайнера.

**QR-код** в правом нижнем углу с подписью «Как парусник обгоняет ветер».

## **Содержание QR (уровень C)**

**Блок 1. Кажущийся ветер.** Современная гоночная яхта идёт в 2–2,5 раза быстрее ветра. Объяснение через векторное сложение настоящего ветра и встречного потока от собственного движения лодки. Чем быстрее лодка — тем сильнее кажущийся ветер — тем больше тяга паруса. Самоусиление, ограниченное сопротивлением воды. Аналогия с вертолётным ротором: лопасть движется навстречу воздуху быстрее, чем воздух движется относительно корпуса.

**Блок 2. Геометрия: почему 45°.** Проекция силы тяги паруса на ось лодки зависит от двух углов — угла паруса к лодке и угла лодки к ветру. При идеальных парусе и киле проекция положительна при любом угле, кроме 0° и 180°. На практике из-за конечного сопротивления киля и конечной подъёмной силы паруса при слишком остром угле потери превышают тягу. У типичной яхты предел \~45°, у гоночной с жёстким парусом-крылом — до 20°.

**Блок 3. История одного принципа.** Египетский квадратный парус (3000 г. до н. э.) работал только по ветру — как экран. Арабское дау с косым парусом (VII век) позволило идти под углом к ветру, что открыло муссонную торговлю в Индийском океане: один корабль возвращался домой против ветра, которое его туда принесло. Современные жёсткие паруса-крылья AC75 — вертикальное крыло самолёта. 5000 лет паруса — одно уточнение модели: парус не ловит ветер, а обтекается им.

**Блок 4. Открытый вопрос.** Если парус — крыло, может ли парус взлетать? При достаточной скорости — да. Подводные крылья гоночных яхт поднимают корпус над водой, и сопротивление резко падает. Именно так AC75 достигает скорости в 2,5 раза выше ветровой.

## **Целевая аудитория**

Средний школьный возраст и старше. Взрослым без физического образования — тоже; текст не требует предварительных знаний, кроме понятия стрелки-вектора.

## **Всесезонность**

Панель работает независимо от наблюдения парусников. Основное объяснение строится на диаграмме и тексте; парусники на Волге — иллюстрация, не опора объяснения.

## **Безопасность**

Не требуется. Нет движущихся частей, острых углов, электрики.

## **Что изменилось относительно исходного проекта**

1.  Убран исторический блок с лицевой панели (Египет, арабы, муссоны, America's Cup) — перенесён в QR.
2.  Убран рекорд 80 км/ч с лицевой панели — требует отдельного объяснения, которое без кажущегося ветра невозможно; перенесён в QR.
3.  Добавлен блок «Тот же принцип» с тремя переносами (самолёт, кайт, руль) — ключевое добавление, делающее экспонат не замкнутым на парусе, а демонстрирующим общий закон.
4.  Явно сформулирована главная идея: сила в одной среде + сопротивление в другой = движение в направлении, которое ни одна из сред не задаёт напрямую.
5.  Маркерная зона для задания с галсами выделена конструктивно как отдельный элемент панели — это единственный интерактивный компонент, и он должен быть функциональным, а не условным.
6.  Сторона 2 переработана из хронологии в три курса + схему лавировки + блок о том, как наука пришла к этой модели.

  
  

# WP036 — «Потолок у неба: солнце и ветер»  

# **WP036 — «Потолок у неба: солнце и ветер»**

## **Табличная строка (обновлённая)**

**ID:** 36 **Код:** WP036 **Название:** Потолок у неба: солнце и ветер **Тип станции:** Средняя станция (площадка 3×2 м)

## **Описание конструкции**

Комплекс из трёх работающих установок на единой площадке 3×2 м. Все три работают напрямую, без накопления энергии, — посетитель видит преобразование в реальном времени.

**Элемент 1. Солнечный модуль с моторчиком.** Один поликристаллический модуль 20–30 Вт на стальной раме под углом 45°, закреплённой на бетонном основании. Модуль напрямую подключён к малому электромотору с насаженным ярким пропеллером-флажком (диаметр 15 см). Свет падает — пропеллер крутится; закрыли модуль — остановился. Никаких контроллеров, никаких аккумуляторов: чистая цепь «свет → ток → движение». Рядом на цепочке — два съёмных цветных светофильтра (красный и синий, из толстого поликарбоната) для опытов со спектром.

**Элемент 2. Ряд из пяти вертушек.** Пять одинаковых по конструкции вертушек разного диаметра (10, 20, 30, 45, 60 см), выстроенные в ряд на металлических штырях высотой 1.5 м. Лопасти — толстая фанера 10 мм с антисептической пропиткой, ось на закрытых промышленных подшипниках (такие же, как в дверных доводчиках). Каждая вертушка вращается свободно при своём пороге скорости ветра: маленькая — от 1 м/с, большая — от 3 м/с. Рядом стоит простой флюгер-анемометр с тремя градациями (слабый/средний/сильный ветер) на основе отклоняемой пластины.

**Элемент 3. Информационный стенд A1 из HPL** на стальной раме. Содержит основной текст (уровни A и B), два пояснительных рисунка (спектр Солнца с «рабочей» и «потерянной» областями; поток воздуха через ротор), QR-код на углублённый уровень C.

Электроники, накопителей, USB-розетки, дисплея — нет. Единственный электрический компонент — моторчик с пропеллером в Элементе 1, защищённый прозрачным поликарбонатным колпаком от дождя (колпак не мешает посетителю видеть вращение). Сломается — стоит заменить его 1500 рублей, а не ремонтировать контроллер MPPT.

## **Опыт посетителя**

Первое, что замечает посетитель: пропеллер за солнечной панелью крутится ровно столько, сколько светит солнце. Закрыл модуль ладонью — остановился мгновенно. Отвёл ладонь — закрутился снова. Нет никаких кнопок, никаких батарей, никакого «включения» — ток появляется и исчезает вместе со светом.

Второе наблюдение — у вертушек. В слабый ветер крутится только самая маленькая. С усилением ветра подключается вторая, потом третья. Но главное странно: переход между «крутится одна» и «крутятся все пять» происходит не плавно, а скачком. Небольшое усиление ветра резко меняет картину. Это и есть куб скорости — главное открытие экспоната, которое посетитель *чувствует*, а не читает.

Третье — опыт со светофильтрами на солнечной установке. Красный фильтр — пропеллер крутится медленнее. Убрали — быстрее. Разные фотоны по-разному работают в кремнии.

Практическое действие: накрыть солнечный модуль красным фильтром, посмотреть на вертушки, ответить на вопрос стенда — «куда в обоих случаях девается энергия, которую мы не забрали». Ответ — в тепло.

## **Текст стенда (уровни A и B)**

### **Потолок у неба. Почему энергии всегда меньше, чем кажется**

Закрой ладонью солнечную панель. Пропеллер за ней останавливается. Убери — закрутился снова. Теперь посмотри на ряд вертушек. Сейчас крутится одна маленькая. Если подует сильнее — закрутятся все пять. Но переход случится не плавно: небольшое усиление ветра резко меняет картину.

Солнечная панель превращает свет в ток, пока на неё падают фотоны. Вертушка превращает движение воздуха в ток, пока дует ветер. В обоих случаях установка отбирает часть энергии у потока, идущего мимо. Отобрать можно не всё — и причина не в качестве сборки, а в том, как устроен сам процесс отбора.

В кремнии электроны сидят в одной «комнате», и чтобы стать током, им нужно перепрыгнуть в другую. Между комнатами — порог энергии около 1.1 электронвольта. Фотоны слабее порога не работают: их энергия не достаёт до следующей комнаты, они проходят сквозь кремний и уходят в тепло. Фотоны сильнее порога выбивают электрон, но излишек их энергии тоже уходит в тепло. Поэтому для солнечного спектра существует предел: кремниевая панель принципиально не может преобразовать больше примерно 33% падающей энергии, какой бы чистой ни была технология. Это предел Шокли-Квайссера, рассчитанный в 1961 году из спектра Солнца и ширины запрещённой зоны кремния.

Ветрогенератор устроен иначе, но ограничение такого же рода. Мощность ветра — это кубы скорости: удвойте скорость ветра, и мощность вырастет не в 2, а в 8 раз. Формула P = ρAv³ выводится без физики, из одной только размерности: ватты — это кг·м²/с³, и единственная комбинация плотности ρ, площади A и скорости v с такой размерностью — именно ρAv³. Но забрать всю эту мощность нельзя: если ротор остановит воздух полностью, за ним воздуху некуда деваться, и новый воздух через лопасти не пойдёт. Чтобы ветер продолжал идти, часть скорости надо оставить. Из закона сохранения импульса в 1919 году Альберт Бетц вывел точный потолок: не больше 59.3% энергии потока, никогда.

Странное совпадение: ветер — это тоже Солнце. Солнце неравномерно греет Землю, от этого воздух движется. Дрова — это Солнце, накопленное деревом за 50 лет. Нефть — Солнце, копившееся миллионы лет в водорослях. Вся доступная нам энергия на Земле, кроме ядерной и недр, — одна звезда в разных обличьях. Мы просто ловим её на разных этапах пути, и у каждого этапа свой потолок.

**Действие.** Накрой солнечную панель красным фильтром — пропеллер замедляется. Почему: энергия красного фотона у нижней границы порога кремния, часть фотонов уже «не достают». Теперь посмотри на вертушки и посчитай, сколько крутится сейчас. Куда девается энергия, которую мы не забрали? В тепло — и для солнца, и для ветра.

## **QR-код (уровень C)**

### **Почему нельзя просто покрыть всю Россию солнечными панелями**

Предел Шокли-Квайссера — это расчёт по спектру Солнца у поверхности Земли и запрещённой зоне кремния 1.12 эВ. Около 23% солнечного спектра идёт фотонами со слишком малой энергией — они не преобразуются вовсе. Ещё около 33% — избыток энергии фотонов над щелью, неизбежно уходящий в тепло. Остаётся теоретический максимум \~33%. Реальные коммерческие кремниевые панели дают 18–22%. Обойти сам предел можно, только уйдя от одной щели: тандемные (многослойные) элементы с разными ширинами щели для разных частей спектра дают 35–45% в лаборатории. Предел Бетца выводится иначе — из баланса скоростей до и после ротора: если обозначить отношение скорости за ротором к скорости до как a, то отбираемая доля максимальна при a = 1/3 и равна 16/27 ≈ 0.593. Современные большие ветрогенераторы достигают 45–50%, приближаясь к пределу.

Отдельный слой — сезонность и геометрия. Зимой в средних широтах солнца падает в 4–5 раз меньше, чем летом; ночью — ноль; ветер в среднем даёт 20–30% номинала за год. Поэтому ответ на вопрос про Россию физический, а не экономический: даже полное покрытие панелями не снимет ни ограничений самой технологии, ни ограничений оси вращения Земли.

Предел Шокли-Квайссера получен расчётом по спектру Солнца и статистической модели полупроводника — это теоретическая оценка, проверяемая затем на реальных образцах. Предел Бетца — классический вывод из уравнений сохранения массы, импульса и энергии для потока через диск ротора. Оба предела подтверждены десятилетиями измерений на промышленных установках.

Предел Бетца выведен для идеального ротора — без трения, без закручивания потока, без конечного числа лопастей. Реальные ограничения жёстче. Но у природы есть и более общее ограничение — второе начало термодинамики и теорема Карно, из которой оба предела можно вывести как частные случаи. Стоит ли за всеми «потолками КПД» один и тот же закон?

## **Что посетитель понимает после экспоната**

Наблюдение реальных, работающих прямо сейчас установок — солнце и ветер не имитируются, а действительно крутят моторчик и вертушки. Телесная вовлечённость: накрыть панель рукой, накрыть фильтром, посчитать крутящиеся вертушки, почувствовать куб скорости через скачкообразное подключение новых вертушек при усилении ветра. Прикидочные расчёты не в киловатт-часах и зарядках смартфона (это экспонат про физический потолок, а не про потребление), а в понимании двух пределов: 33% у кремния и 59% у ветра.

Главная мысль, которую посетитель уносит: природа даёт не столько, сколько мы хотим, а сколько позволяет сама физика процесса. Инженерия может приближаться к пределу — но никогда его не превзойдёт. Это тип научного рассуждения — *ограничение*: не случайность, не технологическая недоработка, а закон.

## **Круглогодичность**

Работает круглый год. Солнечная установка — всегда, моторчик крутится даже в пасмурный день (чувствительность выше, чем у большой промышленной станции с её порогом включения). Зимой посетитель увидит наглядно: тот же пропеллер крутится медленнее, чем летом, — это часть экспоната, а не его неисправность.

Вертушки не имеют минимального порога запуска (подшипники качения, не магнитный генератор), поэтому крутятся всегда, когда есть ветер. Зимой площадка расчищается от снега вокруг штырей; сами вертушки сметают снег с себя при первом же движении.

## **Автономность**

Полная. Нет ни подключения к сети, ни накопителей, ни шкафа с электроникой. Солнечная панель питает один моторчик напрямую. Вертушки механические, электричества не требуют вовсе. Единственное обслуживание — раз в год смазать подшипники вертушек (5 минут работы) и проверить крепление моторчика.

## **Что изменилось по сравнению с предыдущей версией**

|  |  |  |
| :-: | :-: | :-: |
| \*\*Было\*\* | \*\*Стало\*\* | \*\*Зачем\*\* |
| Площадка 6×4 м, вертикально-осевой ветрогенератор Дарье-Савониус, 2 солнечных модуля 160–200 Вт, шкаф с MPPT, аккумулятором 24В/50Ач, инвертором, USB-розеткой, цифровым дисплеем | Площадка 3×2 м, один модуль 20–30 Вт напрямую на моторчик, 5 вертушек разных диаметров, стенд A1 | Предыдущая версия стоила \\\~150–300 тыс. ₽, не выживала на улице без охраны, тонула в инфраструктуре. Новая — на порядок дешевле, масштабируется, показывает \*больше\* физики |
| «Фотоны выбивают электроны в кристалле кремния» | Щель по энергии 1.1 эВ + предел Шокли-Квайссера 33% | Была бытовая картинка без принципа; стала первопричина с ограничением |
| «Вращение лопастей → механическая → электрическая энергия» + «номинальная мощность 50–100 Вт при ветре 8 м/с» | Вывод P \\\~ ρAv³ из размерности + предел Бетца 59% | Был детсадовский ярлык; стал главный принцип (куб скорости), который посетитель \*чувствует\* через вертушки |
| Статистика ВИЭ 7%→30%, 10 зарядок смартфона, калькулятор крыши | Убрано полностью | Это материал для буклета «зелёной энергии», а не для научной тропы |
| Стенд замкнут на двух устройствах | Добавлен мост: ветер, дрова, нефть = всё это Солнце | Один принцип вместо двух разрозненных явлений |
| Главный вопрос «почему нельзя покрыть Россию панелями» был задан без ответа | Ответ дан честно, через физические пределы | Экспонат выполняет обещание, которое сам себе даёт |

  
  

# WP046 — «Погода, климат и вечерний туман»  

# **WP046 — «Погода, климат и вечерний туман»**

## **Локация и назначение**

Обзорная площадка с видом на низины и долину реки. Место выбрано под конкретное наблюдение: с этой точки в тихие летние вечера хорошо видно, как туман заполняет долину снизу вверх. С более плоского рельефа этот эффект не считывается, поэтому экспонат привязан именно к площадке.

Стенд работает круглый год: зимой наблюдаемый эффект менее выражен, но тема погоды и климата остаётся актуальной, а летний период даёт регулярное наблюдательное задание.

## **Конструкция**

Столбик из оцинкованной стали высотой 1,2 м, забетонированный в основание площадки. Наверху — наклонная двусторонняя панель формата A2 из HPL (высокого давления ламинат), крепление к столбику через прочный кронштейн.

HPL выбран как материал, устойчивый к УФ, влаге, перепадам температур и граффити (оттирается растворителем). Расчётный срок службы панели без потери читаемости — 10+ лет. Оцинкованная сталь не требует обслуживания в течение 15 лет. Конструкция выдерживает, если на неё опираются или виснут — ломать в ней нечего.

Никакой тонкой калибровки и подвижных частей. Производится и устанавливается силами любого сварщика и типографии, работающей с архитектурными материалами.

## **Содержательная структура**

На двух сторонах панели размещены две связанные, но самостоятельные темы:

**Сторона 1** — обращена к виду на долину. Объясняет конкретное физическое явление, которое посетитель может увидеть прямо сейчас: радиационно-стекающий туман.

**Сторона 2** — абстрактная, посвящена различию погоды и климата как двум временным масштабам одного и того же явления.

Связь между сторонами: один вечерний туман — это погода. Частота таких туманов за 30 лет — климат этой долины. Одно место, одно явление, две рамки наблюдения.

Темы глобальной атмосферной циркуляции (ячейки Хадли, Феррела, полярные) из экспоната исключены: они не связаны ни с конкретным пейзажем, ни с механизмом тумана, и перегружают панель.

## **Сторона 1: «Почему долина наполняется туманом снизу?»**

### **Текст стенда**

**Заголовок:** Почему долина наполняется туманом снизу?

**Основной текст (A-уровень):** Приди сюда летним вечером около девяти. Если день был ясным и безветренным, внизу происходит что-то странное: туман не опускается с неба и не поднимается из реки, а заполняет долину снизу вверх — как вода в ванне.

Холодный воздух плотнее тёплого. Ночью земля быстро отдаёт дневное тепло в небо, воздух у её поверхности остывает и становится тяжелее. Он стекает по склонам вниз и собирается в самых низких точках долины. Именно там — у воды, на лугах — и появляется туман. Вершины холмов при этом остаются ясными.

**Механизм (B-уровень):** В холодном воздухе помещается меньше водяного пара, чем в тёплом. Когда стекающий воздух остывает до температуры, при которой его влажность становится предельной (её называют точкой росы), лишний пар выходит в виде мельчайших капель — это и есть туман.

Тот же закон плотности работает везде, где есть разница температур: холодная вода опускается на дно стакана со льдом, зимний сквозняк ползёт по полу, в глубоких карстовых воронках лёд сохраняется до августа. Одно правило, разные масштабы.

Учёные измеряют эту асимметрию термометрами на разной высоте: в тихие ясные ночи в нижней точке долины бывает на 5–10 градусов холоднее, чем на соседнем гребне.

**Задание посетителю:** Встань у края площадки и проследи взглядом, докуда поднимается туман. Граница видна резко — это верхняя кромка холодного слоя.

### **Схема на стороне 1**

Поперечный разрез долины. Элементы:

  - Контур склонов, спускающихся к реке
  - Стрелки вниз по склонам — стекание холодного воздуха
  - Горизонтальная линия в нижней части долины — верхняя граница холодного слоя (совпадает с верхней кромкой тумана)
  - Подпись «точка росы» на уровне этой границы
  - Серое облако тумана в нижней части — соответствует реальной низине, видимой с площадки
  - Маленький значок термометра на дне и на гребне с указанием, что там холоднее

## **Сторона 2: «Погода — это то, что ты надел сегодня. Климат — это то, что лежит в шкафу»**

### **Текст стенда**

**Заголовок:** Погода — это то, что ты надел сегодня. Климат — это то, что лежит в шкафу.

**Основной текст (A-уровень):** Сегодня идёт мокрый снег. Климат здесь — умеренно-континентальный с тёплым летом. Оба утверждения верны одновременно.

Погода и климат — это одна и та же атмосфера, но рассмотренная через разные временные окна. Погода — то, что происходит сейчас. Климат — среднее из того, что происходит здесь много лет подряд.

Один вечерний туман внизу — погода. Сколько таких туманов бывает в июле за тридцать лет — климат этой долины.

**Механизм (B-уровень):** Всемирная метеорологическая организация считает климатической нормой среднее за 30 лет. Меньше — слишком шумно: один холодный год перекосит картину. Больше — сам климат успевает измениться, и среднее теряет смысл.

То есть климат — не отдельная сущность. Это погода, посмотренная через подходящее по длине окно. Поменяешь окно — изменится имя. Один и тот же ряд температур в разных рамках даёт разные ответы, и оба правильные.

**Задание посетителю:** Попробуй тот же приём на себе. Запиши, во что ты одет сегодня — это твоя погода. Что лежит в твоём шкафу на все сезоны — это твой климат.

### **Схема на стороне 2**

Одна горизонтальная временная шкала с колебаниями температуры (волнистая линия — «погода»). Поверх неё наложены две прямоугольные рамки:

  - **Узкая рамка** (одни сутки) с подписью «ПОГОДА». Внутри неё колебания большие.
  - **Широкая рамка** (30 лет) с подписью «КЛИМАТ». Внутри неё колебания сглажены в ровное среднее, показанное горизонтальной линией.

Под шкалой три масштабные метки: «сегодня», «1 год», «30 лет». Минимум элементов — только то, что иллюстрирует главную мысль.

## **QR-код (углубление, уровень C)**

QR-коды размещены по одному на каждой стороне, ведут на страницы углубления.

### **QR стороны 1: «Когда тумана не будет»**

Радиационный туман требует трёх условий одновременно:

1.  **Ясное небо.** Облака возвращают земное излучение обратно к поверхности — земля не остынет. Под облачностью ночь в долине будет заметно теплее.
2.  **Слабый ветер**, не более 1–2 м/с. Ветер перемешивает слои воздуха; расслоения по плотности не получится, и холодный воздух не соберётся в низине.
3.  **Достаточная влажность.** Если воздух сухой, при остывании он не дойдёт до точки росы, и конденсации не будет — холодно будет, а тумана нет.

Убери любое из трёх условий — тумана не будет. Это способ проверить, понял ли ты механизм: посмотри прогноз на вечер и предскажи, увидишь ли туман.

**Как это измеряют.** Температурный профиль долины получают цепочкой термометров на разной высоте — от дна до гребня. В тихую ясную ночь они показывают инверсию: внизу холоднее, чем вверху. Это противоположно обычной ситуации, когда с высотой температура падает.

**Открытый вопрос.** Почему в горных долинах весенние заморозки убивают сады на дне долины, а на склонах — нет? Тот же механизм: холодный воздух стекает вниз и скапливается в низине. Фермеры давно поняли: фруктовые сады выше риска.

### **QR стороны 2: «Почему именно 30 лет?»**

30-летнее окно — не произвольное число. Оно выбрано в 1935 году Международной метеорологической организацией (предшественницей ВМО) и с тех пор пересматривалось дважды. Логика такая:

  - Окно должно быть достаточно длинным, чтобы отдельные аномальные годы сгладились. Сильное извержение вулкана охлаждает планету на 2–3 года; Эль-Ниньо длится 1–2 года; один сухой сезон — несколько месяцев. За 30 лет такие эпизоды размываются.
  - Окно должно быть достаточно коротким, чтобы за это время сам климат не успел заметно сдвинуться.

Сейчас второе условие начинает нарушаться. Тридцатилетние средние 1961–1990 и 1991–2020 для большинства регионов уже заметно отличаются — именно поэтому климатологи переходят на «скользящие» нормы и отдельно говорят о «референсном» и «текущем» климате.

**Как это узнают.** Ежедневные температуры с метеостанций усредняют по декадам, месяцам, сезонам, годам — и смотрят, как это среднее смещается от одного тридцатилетия к следующему. Для данных за пределами эпохи инструментальных наблюдений (до \~1850 года) используют косвенные источники: годичные кольца деревьев, керны льда, озёрные отложения, кораллы.

**Открытый вопрос.** Если климат — это погода в длинном окне, то в каком окне живёт лес? Одно дерево чувствует погоду года. Популяция деревьев — климат века. Геологические слои под ногами — климат тысячелетий. У каждого процесса в природе есть своё «правильное» окно наблюдения.

## **Проверка перед установкой**

**Содержательная:**

  - Каждая сторона самодостаточна без QR.
  - Одна сторона — одна тема, одна линия объяснения.
  - Механизм явно описан как цепочка причина → следствие, а не как набор фактов.
  - Показан перенос принципа на другие масштабы (сторона 1) и другие процессы (сторона 2 через QR).
  - Показаны рамки — когда явление не работает (условия образования тумана, ограничения 30-летнего окна).

**Техническая:**

  - Текст крупного кегля читается с расстояния 0,5–1 м.
  - QR-коды размером не менее 3×3 см, контрастные, размещены в нижней части панели.
  - Наклон панели 30° от вертикали — оптимальный для чтения стоящим взрослым и не даёт бликов от солнца в большинстве положений.
  - Ребёнок ростом от 120 см читает A-уровень без трудностей (нижняя половина панели).

**Эксплуатационная:**

  - Регулярное обслуживание: раз в год протереть панель, проверить крепление кронштейна, проверить читаемость QR-кодов.
  - Полная замена панели — через 10–12 лет или раньше, если появились царапины, выцветание.

  
**  
**

# WP047 «Грозы, молнии и прогноз на неделю»**  
**

# **WP047 «Грозы, молнии и прогноз на неделю»**

## **Конструкция**

Столбик из оцинкованной стали высотой 1,2 м с вертикальной двусторонней панелью A2 из HPL. Всесезонный, антивандальный, не требует ограждения и обслуживания в течение 5–7 лет. QR-код — часть печати на HPL, не отдельная наклейка.

## **Педагогическая цель**

Показать, что непредсказуемость точки удара молнии и ограничение прогноза погоды десятью днями — это **одно и то же свойство атмосферы**, а не два разных факта. Тип научного рассуждения — **повторяющийся закон**: нелинейная система усиливает малые отклонения, и этот принцип работает на масштабах от облака до континента, от биения сердца до популяций. Экспонат делает связку с темой константы Фейгенбаума на соседнем стенде.

## **Расположение на панели**

**Сторона 1** — «Молния горячее Солнца»: вертикальный разрез кучево-дождевого облака с восходящими токами, разделением зарядов, каналом разряда. Шкала высот 10–15 км. Три ключевые цифры — 20 000 А, 30 000°C, 0,2 сек — не списком, а внутри схемы.

**Сторона 2** — «Почему прогноз обрывается на десятом дне»: сетка численной модели 10×10 км над Европой; график деградации точности 1 день → 95%, 7 дней → 60%, 14 дней → \~50%; стрелка, показывающая, как малое отклонение в начальных данных расходится в большое со временем.

QR-код на каждой стороне — ведёт на интерактивную карту молний lightningmaps.org и на углублённые материалы.

## **Текст экспоната**

### **Сторона 1: Молния горячее Солнца**

**Хук** Канал молнии разогревается до 30 000°C. Поверхность Солнца — 5 778°C. Разница почти в пять раз.

**Как устроена гроза** Грозовое облако — это столб высотой до 15 километров, в котором капли воды идут вверх, а ледяные кристаллы падают вниз. Они трутся и разделяют заряды: верх облака заряжается положительно, низ — отрицательно. Когда разница напряжений достигает предела пробоя воздуха, за 0,2 секунды проходит разряд. Ток в канале — около 20 000 ампер.

**Почему молния горячее Солнца** Температура зависит не от общей энергии, а от того, где и как быстро она выделилась. Солнце греется термоядерным синтезом равномерно, по большому объёму. Молния выделяет всю энергию в тонком канале за доли секунды — воздух не успевает отдать тепло, и температура поднимается на порядок выше солнечной поверхности. То же соотношение, что у жара костра и искры электросварки: костёр рядом — тепло, но не обжигает; искра на том же расстоянии — прожигает одежду.

**Почему нельзя предсказать, куда ударит** Путь разряда зависит от крошечных неоднородностей влажности, температуры, рельефа. Отклонение в доли процента меняет всю траекторию. Это то же свойство атмосферы, которое делает невозможным прогноз погоды на две недели (смотри обратную сторону панели).

**Проверить** Сравните: жар костра чувствуется рядом, но не обжигает. Искра электросварки на том же расстоянии прожигает одежду. Это та же разница, что у Солнца и молнии — одна энергия, но в разном объёме и за разное время.

### **Сторона 2: Почему прогноз обрывается на десятом дне**

**Хук** В 1950 году компьютер ENIAC впервые посчитал прогноз на сутки — за сутки работы. Сегодня европейский центр ECMWF считает в сто триллионов раз быстрее. Горизонт прогноза за 75 лет вырос с одного дня до десяти. Дальше — не растёт.

**Как считается прогноз** Атмосфера разбивается на сетку — у ECMWF ячейка около 10 километров. В каждой ячейке задаются давление, температура, ветер, влажность. Модель рассчитывает шаг за шагом, что будет через час, два, сутки. Точность на сутки вперёд — около 95%. На неделю — 60%. На две недели — около 50%, то есть на уровне подбрасывания монеты.

**Почему горизонт не растёт** Атмосфера — нелинейная система. Крошечная ошибка в начальных данных удваивается примерно каждые 2–3 дня. Через десять дней ошибка в одну десятую градуса вырастает в ошибку десятки градусов. Это не недостаток компьютеров, а свойство самих уравнений гидродинамики. Эдвард Лоренц обнаружил этот эффект случайно в 1963 году: перезапустил свою модель погоды с данными, округлёнными до трёх знаков вместо шести, и через 60 модельных дней получил совершенно другую погоду.

**Почему компьютеры не помогают** Чтобы сдвинуть горизонт прогноза на несколько дней, нужно не в два раза больше точности, а на много порядков. Экспоненту не обогнать вычислениями — это математическая граница, а не инженерная.

**Тот же закон работает везде** Чувствительность к малым отклонениям — свойство любой нелинейной системы: биения сердца, турбулентности за крылом самолёта, колебаний численности видов. Митчелл Фейгенбаум показал, что у всех таких систем есть общая универсальная константа (подробнее — на соседнем экспонате). Один закон — от капли росы до континента.

**Проверить** Сравните приложение погоды на завтра и на десятый день. Завтрашняя температура сбудется с точностью до градуса. Через десять дней — это уже не предсказание, а статистика.

## **QR-код: углублённые материалы**

### **Карта молний в реальном времени**

Ресурс lightningmaps.org показывает разряды по всему миру с задержкой 2–3 секунды. Работает сеть Blitzortung — около 500 любительских приёмников, ловящих радиоимпульсы разрядов. Метод: по времени прихода сигнала на разные приёмники триангулируется точка удара.

### **Откуда известна температура молнии**

Прямое измерение невозможно — канал существует доли секунды. Температура оценивается по спектру свечения с помощью закона Стефана-Больцмана и модели плазменного столба. Оценка 30 000°C — это температура равновесной плазмы в момент максимума тока.

### **Время Ляпунова и горизонт предсказуемости**

Характерное время удвоения ошибки в системе называется временем Ляпунова. Для атмосферы оно составляет 2–3 дня — это не эмпирическая цифра, а следствие математической структуры уравнений Навье–Стокса для сжимаемой атмосферы. Горизонт прогноза ограничен величиной 10–15 времён Ляпунова, после чего ошибка достигает масштаба самой системы.

### **Ансамблевый прогноз**

Современный ECMWF обходит проблему хаоса через ансамбль: запускает модель 51 раз с чуть разными начальными данными и считает распределение вероятностей, а не одну траекторию. Это не отменяет горизонт предсказуемости, но честно показывает, насколько уверен прогноз на каждый день. Когда говорят «вероятность дождя 70%», это доля членов ансамбля, в которых прошёл дождь.

### **Эксперимент Лоренца**

В 1963 году Эдвард Лоренц, метеоролог из MIT, моделировал погоду на компьютере LGP-30. Он захотел перезапустить расчёт с середины — ввёл данные, распечатанные на ленте с тремя знаками после запятой, хотя внутри машины они хранились с шестью. Через два модельных месяца погода разошлась с исходной до неузнаваемости. Округление в четвёртом знаке — отклонение в миллионную долю — перестроило всю систему. Это был первый численный эксперимент по теории хаоса.

### **Открытый вопрос**

Есть ли внутри хаотической атмосферы «окна предсказуемости» — структуры, для которых горизонт расширяется? Активная тема: блокирующие антициклоны живут 2–4 недели и предсказуемы на более длительный срок, чем типичная погода. Понимание, когда именно атмосфера предсказуема, а когда нет, — одна из главных задач современной  климатологии.

### **Связь с соседними экспонатами**

Константа Фейгенбаума (экспонат рядом) — универсальное число, описывающее переход любой нелинейной системы от порядка к хаосу. Атмосфера, биение сердца, популяции кроликов и этот грозовой разряд — разные лица одного закона.

## **Ключевая педагогическая связка**

Две стороны панели — не два разных экспоната, а одна мысль в двух масштабах. На стороне 1 посетитель узнаёт, что точку удара молнии нельзя предсказать — малое отклонение меняет путь разряда. На стороне 2 — что прогноз погоды нельзя построить дальше чем на две недели — малое отклонение меняет состояние атмосферы. Это одно свойство нелинейной системы, видимое на расстоянии метра и на расстоянии континента. Та же математика — в экспонате Фейгенбаума.

  
  

# Вкладка 42  

  
  

# Вкладка 43  

  
  

# Вкладка 44  

  
  

# Вкладка 45  

  
  

# Вкладка 46  

  
