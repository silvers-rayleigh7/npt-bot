...
def to_typst(doc: dict, math: bool = True) -> str:
    ...
    # Пометка о происхождении материала: учитель должен видеть, что документ собран
    # автоматически и требует его проверки — это прямой ответ на вопрос об экспертизе.
    t.append('#v(0.35cm)')
    t.append('#block(fill: rgb("#F6EFE4"), inset: 10pt, radius: 5pt, width: 100%)[')
    t.append('  #text(size: 8.5pt, fill: rgb("#7A5C2E"))[')
    t.append('    #strong[Как собран этот материал.] Методичка составлена автоматически: '
             'опорой служат вычитанные сюжеты проекта, школьная программа и проверенные '
             'научно-популярные источники, перечисленные ниже. Перед уроком просмотрите '
             'текст — ответственность за содержание занятия остаётся за учителем.')
    t.append('  ]')
    t.append(']')
