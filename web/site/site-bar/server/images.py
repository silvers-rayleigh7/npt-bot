...
def _llm_pick(topic: str, subject: str, candidates: list):
    ...
    try:
        ...
        msg = (
            ...
            f"Ниже названия изображений. Выбери номер ОДНОГО, которое лучше всего иллюстрирует "
            f"именно эту тему. Если ни одно не подходит по смыслу — ответь 0. "
            f"Важно: близкая, но другая тема — это НЕ подходит.\n"
            f"При прочих равных предпочитай: фотографию явления без надписей; "
            f"русскоязычный материал (в названии «ru», «рус») перед англоязычной схемой — "
            f"документ для русской школы.\n\n{listing}\n\n"
